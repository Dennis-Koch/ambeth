...
namespace De.Osthus.Ambeth.Merge.Model
{
    public class EntityMetaData : IEntityMetaData
    {
        ...
        public MethodInfo[] PostLoadMethods { get; private set; }

        public MethodInfo[] PrePersistMethods { get; private set; }
        ...
        public void Initialize(IEntityFactory entityFactory)
        {
            ...
            IList<MethodInfo> prePersistMethods = new List<MethodInfo>();
            IList<Type> prePersistAnnotations = new List<Type>();
            prePersistAnnotations.Add(typeof(PrePersistAttribute));
            FillMethodsAnnotatedWith(this.EntityType, prePersistMethods, ListUtil.ToArray(prePersistAnnotations));

            IList<MethodInfo> postLoadMethods = new List<MethodInfo>();
            IList<Type> postLoadAnnotations = new List<Type>();
            postLoadAnnotations.Add(typeof(PostLoadAttribute));
            FillMethodsAnnotatedWith(this.EntityType, postLoadMethods, ListUtil.ToArray(postLoadAnnotations));

            this.PrePersistMethods = ListUtil.ToArray(prePersistMethods);
            this.PostLoadMethods = ListUtil.ToArray(postLoadMethods);
        }
