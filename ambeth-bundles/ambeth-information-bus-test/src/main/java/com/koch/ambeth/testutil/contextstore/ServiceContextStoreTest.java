...
public class ServiceContextStoreTest extends AbstractIocTest {
	...
	public void setUp() throws Exception {
		...
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			public void interconnect(IServiceContextStore contextStore) {
				// By name to name
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(ServiceContextStoreTestModule.BLA_PROV_1)
						.property(SERVICE_PROPERTY);

				// By name to type
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(BlaDServiceProvider.class).property(SERVICE_PROPERTY);

				// By type to name in same context
				contextStore.injectFrom(CONTEXT_1).bean(BlaDService.class).in(CONTEXT_1)
						.intoBean(ServiceContextStoreTestModule.BLA_PROV_3).property(SERVICE_PROPERTY);

				// By type to type
				contextStore.injectFrom(CONTEXT_2).bean(BlaDService.class).in(CONTEXT_1)
						.intoBean(BlaDServiceProviderImpl.class).property(SERVICE_PROPERTY);

				// Manual
				Object sourceBean = contextStore.getContext(CONTEXT_1)
						.getService(ServiceContextStoreTestModule.BLA_DSERVICE);
				Object targetBean =
						contextStore.getContext(CONTEXT_2).getService(ServiceContextStoreTestModule.BLA_PROV_5);
	...
	public void testProviderEmpty() {
		BlaDServiceProvider provider = beanContext
				.getService(ServiceContextStoreTestModule.BLA_PROV_EMPTY, BlaDServiceProvider.class);
	...
	public void testNonExistantContext1() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1 + "_").intoBean(ServiceContextStoreTestModule.BLA_PROV_1)
						.property("Service");
			}
		}).finish();
	}
	...
	public void testNonExistantContext2() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			@Override
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2 + "_").bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(ServiceContextStoreTestModule.BLA_PROV_1).property("Service");
			}
		}).finish();
	}
	...
	public void testNonExistantBeanName1() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			@Override
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE + "_")
						.in(CONTEXT_1).intoBean(ServiceContextStoreTestModule.BLA_PROV_1).property("Service");
			}
		}).finish();
	}
	...
	public void testNonExistantBeanName2() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(ServiceContextStoreTestModule.BLA_PROV_1 + "_")
						.property("Service");
			}
		}).finish();
	}
	...
	public void testNonExistantBeanType1() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			@Override
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(BlaDServicePortType.class).in(CONTEXT_1)
	...
	public void testNonExistantBeanType2() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(BlaDServicePortType.class).property("Service");
			}
		}).finish();
	}
	...
	public void testNonExistantProperty() {
		serviceContextStore.withConfig(new IInterconnectConfig() {
			...
			public void interconnect(IServiceContextStore contextStore) {
				contextStore.injectFrom(CONTEXT_2).bean(ServiceContextStoreTestModule.BLA_DSERVICE)
						.in(CONTEXT_1).intoBean(ServiceContextStoreTestModule.BLA_PROV_1).property("Service2");
			}
		}).finish();
	}
}
