...
public class ServiceContextStore implements IServiceContextStore, IServiceContextStoreConf {
	...
	private void interconnect(IBeanGetter sourceBeanGetter, IBeanGetter targetBeanGetter,
			String propertyName) {
		...
		if (!targetSetter.getParameterTypes()[0].isAssignableFrom(sourceType)) {
			throw new IllegalStateException(
					"Setter for property '" + propertyName + "' on type '" + targetType.getName()
	...
	private Method findTargetSetter(Class<?> type, String propertyName) {
		...
		if (targetSetter == null) {
			throw new IllegalStateException("Cannot find setter for property '" + propertyName
					+ "' on type '" + type.getName() + "'");
		}
