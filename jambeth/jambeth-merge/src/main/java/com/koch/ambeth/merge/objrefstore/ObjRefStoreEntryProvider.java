...
public class ObjRefStoreEntryProvider extends IObjRefStoreEntryProvider {
	...
	protected final Tuple2KeyHashMap<Class<?>, Integer, IObjRefStoreFactory> constructorDelegateMap = new Tuple2KeyHashMap<>();

	protected final Lock writeLock = new ReentrantLock();

	@Override
	public ObjRefStore createObjRefStore(Class<?> entityType, byte idIndex, Object id) {
		IObjRefStoreFactory objRefConstructorDelegate = constructorDelegateMap.get(entityType,
	...
	protected IObjRefStoreFactory buildDelegate(Class<?> entityType, int idIndex) {
		...
		try {
			...
			objRefConstructorDelegate = accessorTypeProvider.getConstructorType(IObjRefStoreFactory.class,
					enhancedType);
