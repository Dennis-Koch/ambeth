	...
	protected EntityConfig readEntityConfig(Element entityTag) {
		...
		try {
			...
			if (attributeMap != null) {
				...
				if (attributeMap.containsKey(XmlConstants.VERSION)) {
					...
					versionRequired = !Boolean
							.parseBoolean(xmlConfigUtil.getAttribute(versionElement, XmlConstants.WITHOUT));
					if (versionRequired) {
			...
			throw new RuntimeException(
					"Error occured while processing mapping for entity: " + entityTypeName, e);
		}
	}
		...
		try {
			...
			if (localEntity) {
				...
				if (joinTableTag == null) {
					...
					String toAttributeName = xmlConfigUtil.getChildElementAttribute(joinTableTag,
							XmlConstants.INV_JOIN_ATTR, XmlConstants.NAME, null);
					...
					if (toFieldName == null && toAttributeName == null) {
						throw new IllegalArgumentException(
								"Inverse join column or attribute name has to be set");
					}
				}
			}
