...
public class OrmXmlReaderLegathy implements IOrmXmlReader {
    ...
    public Set<EntityConfig> loadFromDocument(Document doc, IOrmEntityTypeProvider ormEntityTypeProvider) {
        Set<EntityConfig> entities = new LinkedHashSet<>();
    ...
    protected EntityConfig readEntityConfig(Element entityTag) {
        ...
        try {
            ...
            IMap<String, List<Element>> entityDefs = xmlConfigUtil.childrenToElementMap(entityTag);
            ...
            if (attributeMap != null) {
                ...
                if (attributeMap.containsKey(XmlConstants.BASIC)) {
                    List<Element> basicAttrs = attributeMap.get(XmlConstants.BASIC);
                ...
                if (attributeMap.containsKey(XmlConstants.TO_ONE)) {
                    List<Element> toOneAttrs = attributeMap.get(XmlConstants.TO_ONE);
                ...
                if (attributeMap.containsKey(XmlConstants.TO_MANY)) {
                    List<Element> toManyAttrs = attributeMap.get(XmlConstants.TO_MANY);
                ...
                if (attributeMap.containsKey(XmlConstants.IGNORE)) {
                    List<Element> ignoreAttrs = attributeMap.get(XmlConstants.IGNORE);
            ...
            if (localEntity) {
                ...
                if (joinTableTag == null) {
                    ...
                    if (constraintName.isEmpty()) {
                        throw new IllegalArgumentException("Either nested element '" + XmlConstants.JOIN_TABLE + "' or attribute '" + XmlConstants.CONSTRAINT_NAME + "' required to map link");
                    }
                    ...
                    String fromFieldName = xmlConfigUtil.getChildElementAttribute(joinTableTag, XmlConstants.JOIN_COLUMN, XmlConstants.NAME, "Join column name has to be set exactly once");
