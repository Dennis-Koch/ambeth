...
public class ChangeController implements IChangeController, IChangeControllerExtendable, IMergeListener {
    protected final ClassExtendableListContainer<IChangeControllerExtension<?>> extensions = new ClassExtendableListContainer<>("change controller extension", "entity");
    ...
    protected boolean processChanges(List<Object> newEntities, List<Object> oldEntities, List<IChangeContainer> changes, IIncrementalMergeState incrementalMergeState) {
        ...
        try {
            for (int index = 0; index < size; index++) {
                ...
                processChange(newEntity, oldEntity, toBeDeleted, toBeCreated, incrementalMergeState, views, calledExtensionsSet);
            }
            ...
            if (customState != null) {
                ...
                if (incrementalMergeState.getCustomState(cleanupKey) == null) {
                    incrementalMergeState.registerMergePipelineFinishHook(new MergeProcessFinishListener());
    ...
    @Override
    public IStateRollback pushRunWithoutEDBL() {
        var oldValue = edblActiveTL.get();
        edblActiveTL.set(Boolean.FALSE);
        return () -> edblActiveTL.set(oldValue);
    }

    public class MergeProcessFinishListener implements IMergePipelineFinishHook {
        @Override
        public void mergePipelineFinished(boolean success, IIncrementalMergeState incrementalMergeState) {
            ISet<IMergePipelineListener> pipelineExtensions = incrementalMergeState.getCustomState(key);
