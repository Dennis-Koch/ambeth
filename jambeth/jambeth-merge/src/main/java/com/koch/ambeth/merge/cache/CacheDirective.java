...
public enum CacheDirective
{
	 ...
	 * 2) Does transparently try to resolve cache-misses by cascading through the cache hierarchy and calling remote CacheRetrievers<br>
	 * 3) Does return only cache-hits. So if at least a single miss happened the result does NOT correlate by index with the requested ObjRefs
	 */
	None, //
	...
	/**
	 * Customizes the default behavior in a specific aspect:<br>
	 * 2) Does transparently try to resolve cache-misses by cascading through the cache hierarchy but NOT calling remote CacheRetrievers<br>
	 */
