...
public class MergeProcess implements IMergeProcess {
	...
	public void process(final Object objectToMerge, final Object objectToDelete,
			final ProceedWithMergeHook proceedHook, final MergeFinishedCallback mergeFinishedCallback,
	...
	protected void mergePhase2(final Object objectToMerge, Object objectToDelete,
			MergeHandle mergeHandle, final ICUDResult cudResult, final ProceedWithMergeHook proceedHook,
		...
		// Store the MergeFinishedCallback from this thread on the stack and set the property null (for
		// following calls):
		if (guiThreadHelper.isInGuiThread()) {
			guiThreadHelper.invokeOutOfGui(new IBackgroundWorkerDelegate() {
				@Override
	...
	protected void removeUnpersistedDeletedObjectsFromCudResult(List<IChangeContainer> allChanges,
		...
		for (int a = allChanges.size(); a-- > 0;) {
			...
			// These are objects without an id but are marked as deleted. They will be deleted locally
		...
		// Scan all other changeContainer if they refer to the removed DeleteContainers of unpersisted
		// entities
		for (int a = allChanges.size(); a-- > 0;) {
			IChangeContainer changeContainer = allChanges.get(a);
					...
					if (changeContainer instanceof CreateContainer) {
						...
						dataChange.getInserts()
								.add(new DataChangeEntry(referenceInReturn.getRealType(),
										referenceInReturn.getIdNameIndex(), referenceInReturn.getId(),
						...
						dataChange.getDeletes().add(new DataChangeEntry(reference.getRealType(),
								reference.getIdNameIndex(), reference.getId(), reference.getVersion()));
					}
				}
