 ...
 */
public class ImplementAbstractObjectFactory
		implements IDisposableBean, IImplementAbstractObjectFactory,
		IImplementAbstractObjectFactoryExtendable, IEntityInstantiationExtension, IInitializingBean {
	@LogInstance
	private ILogger log;
	...
	protected final IMapExtendableContainer<Class<?>, Class<?>> baseTypes =
			new MapExtendableContainer<>("baseType", "keyType");

	protected final IMapExtendableContainer<Class<?>, Class<?>[]> interfaceTypes =
			new MapExtendableContainer<>("interfaceTypes", "keyType");
	...
	public Class<?> getBaseType(Class<?> keyType) {
		...
		if (baseType == null) {
			throw new IllegalArgumentException(
					"Type " + keyType.getName() + " is not registered for this extension");
		}
		return baseType;
	}
	...
	public <T> Class<? extends T> getImplementingType(Class<T> keyType) {
		if (isRegistered(keyType)) {
			return (Class<? extends T>) bytecodeEnhancer.getEnhancedType(keyType,
					ImplementAbstractObjectEnhancementHint.ImplementAbstractObjectEnhancementHint);
		}
