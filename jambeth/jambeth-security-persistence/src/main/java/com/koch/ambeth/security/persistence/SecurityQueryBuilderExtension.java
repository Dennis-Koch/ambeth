...
public class SecurityQueryBuilderExtension implements IQueryBuilderExtension {
    ...
    public IOperand applyOnWhereClause(IServiceContext queryBeanContext, IQueryBuilderIntern<?> queryBuilder, IOperand whereClause, IList<ISqlJoin> joinClauses, QueryType queryType) {
        ...
        ArrayList<IOperand> readPermissionUserIdColumns = new ArrayList<>();
        {
            ...
            ISqlJoin join = createJoin(tableOfEntity.getName(), queryBuilder, readPermissionValueColumns, readPermissionUserIdColumns, queryBeanContext, null);
            if (join != null) {
        ...
        for (int i = 0, size = joinClauses.size(); i < size; i++) {
            ...
            ISqlJoin join = createJoin(tableNameOfJoin, queryBuilder, readPermissionValueColumns, readPermissionUserIdColumns, queryBeanContext, entityJoin);
            if (join != null) {
                permissionGroupJoins.add(join);
                joinClauses.add(join);
            }
    ...
    protected ISqlJoin createJoin(String tableNameOfJoin, IQueryBuilderIntern<?> queryBuilder, List<IOperand> readPermissionValueColumns, List<IOperand> readPermissionUserIdColumns,
        ...
        ISqlJoin join = queryBuilder.joinIntern(tableName, columnOperand, readPermissionIdColumn, JoinType.LEFT);
    ...
    public void applyOnQuery(IMap<Object, Object> nameToValueMap, IList<Object> parameters, IList<String> additionalSelectColumnList) {
        ...
        if (context == null) {
            throw new IllegalStateException("Should never happen. Security activated but no verified authentication active");
        }
        IAuthorization authorization = context.getAuthorization();
        if (authorization == null) {
            IAuthentication authentication = context.getAuthentication();
            if (authentication == null) {
                throw new IllegalStateException("Security activated but no authentication available at all");
            }
