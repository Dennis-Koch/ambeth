...
public class TableResult<T> extends CommandResult {
	private SortedMap<Integer, String> indexToColumnNameMap = new TreeMap<>();
	 ...
	 *
	 * @param row instance of {@link Row}
	 */
	...
	private Map<Integer, Integer> calColWidths() {
		...
		for (Integer index : keySet) {
			map.put(index,
					(indexToColumnNameMap.get(index) == null ? 0 : indexToColumnNameMap.get(index).length()));
		}
		...
		for (int j = 0; j < rows.size(); j++) {
			for (Integer index : keySet) {
				String data =
						rows.get(j).getValue(index) == null ? "" : rows.get(j).getValue(index).toString();
