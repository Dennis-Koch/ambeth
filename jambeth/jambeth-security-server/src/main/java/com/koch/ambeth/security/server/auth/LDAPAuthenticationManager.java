...
public class LDAPAuthenticationManager extends AbstractAuthenticationManager {
    ...
    protected IAuthenticationResult buildAuthenticationResult(List<Map<String, Object>> result) {
        var map = result.get(0);
        var nameValue = map.get(userPrincipalName);
        var sid = (String) nameValue;
    ...
    public IAuthenticationResult authenticate(IAuthentication authentication) throws AuthenticationException {
        var authenticationResult = authenticationResultCache.resolveAuthenticationResult(authentication);
