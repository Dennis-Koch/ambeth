...
public class PasswordUtil implements IInitializingBean, IPasswordUtil, IPasswordValidationExtendable, IThreadLocalCleanupBean {
    protected final SmartCopyMap<String, Reference<SecretKeyFactory>> algorithmToSecretKeyFactoryMap = new SmartCopyMap<>(0.5f);
    protected final DefaultExtendableContainer<IPasswordValidationExtension> extensions = new DefaultExtendableContainer<>(IPasswordValidationExtension.class, "passwordValidadtionExtension");
    ...
    @Property(name = SecurityServerConfigurationConstants.LoginPasswordChangeRecommendation, defaultValue = "7")
    ...
    @Override
    public IStateRollback pushSuppressPasswordChangeRequired() {
        var oldValue = suppressPasswordChangeRequiredTL.get();
        if (Boolean.TRUE.equals(oldValue)) {
            return StateRollback.empty();
        }
        suppressPasswordChangeRequiredTL.set(Boolean.TRUE);
        return () -> suppressPasswordChangeRequiredTL.set(oldValue);
    }

    @Override
    public IStateRollback pushSuppressPasswordValidation() {
        var oldValue = suppressPasswordValidationTL.get();
        if (Boolean.TRUE.equals(oldValue)) {
            return StateRollback.empty();
        }
        suppressPasswordValidationTL.set(Boolean.TRUE);
        return () -> suppressPasswordValidationTL.set(oldValue);
    }

    @Override
    public IStateRollback pushSuppressAccounting() {
        var oldValue = suppressAccountingTL.get();
        if (Boolean.TRUE.equals(oldValue)) {
            return StateRollback.empty();
        }
        suppressAccountingTL.set(Boolean.TRUE);
        return () -> suppressAccountingTL.set(oldValue);
    }

    protected RuntimeException createIllegalPasswordException(String message) {
        var e = new PasswordConstraintException(message);
        if (!debugModeActive) {
    ...
    public ICheckPasswordResult checkClearTextPassword(char[] clearTextPassword, IPassword password) {
        var givenPasswordString = Base64.encodeBytes(hashClearTextPassword(clearTextPassword, password));
        var expectedPasswordString = new String(password.getValue());
        ...
        // long-term security
        var changeRecommended = isChangeRecommended(password);
        var changeRequired = isChangeRequired(password);
        var rehashRecommended = isRehashRecommended(password);
        var accountingActive = isAccountingActive(password);
        return new CheckPasswordResult(true, changeRecommended, changeRequired, rehashRecommended, accountingActive);
    }

    @Override
    public void rehashPassword(char[] clearTextPassword, IPassword existingPassword) {
        var givenPasswordString = Base64.encodeBytes(hashClearTextPassword(clearTextPassword, existingPassword));
        var expectedPasswordString = new String(existingPassword.getValue());
        if (!expectedPasswordString.equals(givenPasswordString)) {
    ...
    public void reencryptAllSalts(final char[] newLoginSaltPassword) {
        ...
        var privilegeOnPasswordType = privilegeProvider.getPrivilegeByType(IPassword.class);
        ...
        try {
            ...
            var allPasswords = queryBuilderFactory.create(IPassword.class)
                                                  .build()
                                                  .retrieve();
            var changedPasswords = new ArrayList<IPassword>(allPasswords.size());
            for (var password : allPasswords) {
                var decryptedSalt = decryptSalt(password);
                encryptSalt(password, decryptedSalt, newLoginSaltPassword);
                changedPasswords.add(password);
            }
            mergeProcess.begin()
                        .merge(changedPasswords)
                        .onSuccess(new MergeFinishedCallback() {
                            @Override
    ...
    protected boolean isChangeRecommended(IPassword password) {
        var changeAfter = password.getChangeAfter();
        if (changeAfter == null) {
            return false;
        }
    ...
    protected boolean isChangeRequired(IPassword password) {
        ...
        return changeAfter != null && Calendar.getInstance()
                                              .after(changeAfter);
    }
    ...
    public void validatePassword(char[] clearTextPassword, IUser user) {
        ...
        if (validationErrorSB != null) {
            throw createIllegalPasswordException("New password does not meet the security criteria: " + validationErrorSB.toString());
        }
        List<IPassword> passwordHistory = buildPasswordHistory(user);
        if (isPasswordUsedInHistory(clearTextPassword, passwordHistory)) {
            throw createIllegalPasswordException("New password does not meet the security criteria: Password has already been used recently");
        }
    }
    ...
    public char[] assignNewRandomPassword(IUser user, char[] oldClearTextPassword) {
        ...
        while (true) {
            newClearTextPassword = Base64.encodeBytes(createRandomPassword())
                                         .toCharArray();
    ...
    public void assignNewPassword(IPassword password, IUser user) {
        ...
        passwordMetaData.getMemberByName(IPassword.User)
                        .setValue(password, user);
        if (existingPassword != null) {
            passwordMetaData.getMemberByName(IPassword.User)
                            .setValue(existingPassword, null);
            ...
            if (passwordHistory != null) {
                ...
                passwordMetaData.getMemberByName(IPassword.HistoryUser)
                                .setValue(existingPassword, user);
            }
        }
        ...
        // of the history
        {
            ...
            Collections.sort(passwordHistoryList, new Comparator<IPassword>() {
                ...
                public int compare(IPassword o1, IPassword o2) {
                    return o1.getChangeAfter()
                             .compareTo(o2.getChangeAfter());
                }
            });
            ...
            passwordMetaData.getMemberByName(IPassword.HistoryUser)
                            .setValue(passwordToRemove, null);
        }
    }
    ...
    @Override
    public void encryptPassword(char[] newClearTextPassword, IPassword password, boolean assignNewChangeAfter) {
        ...
        encryptSalt(password, secureRandom.acquireRandomBytes(password.getSaltLength()), loginSaltPassword);
        try {
            byte[] hashedPassword = hashClearTextPassword(newClearTextPassword, password);
            password.setValue(Base64.encodeBytes(hashedPassword)
                                    .toCharArray());
        } catch (Exception e) {
            throw RuntimeExceptionUtil.mask(e);
        }
    ...
    protected void fillPassword(char[] newClearTextPassword, char[] oldClearTextPassword, IPassword password, IUser user, boolean assignNewChangeAfter) {
        ...
        if (oldClearTextPassword == null) {
            if (userIdentifierProvider == null) {
                throw new IllegalStateException("No instanceof of " + IUserIdentifierProvider.class.getName() + " found to create a new signature due to password change");
            }
            String currentSid = authenticatedUserHolder.getAuthenticatedSID();
            if (currentSid != null && currentSid.equals(sid)) {
                IAuthentication authentication = securityContextHolder.getContext()
                                                                      .getAuthentication();
        ...
        if (isNewSignature || ((IEntityMetaDataHolder) signature).get__EntityMetaData()
                                                                 .getIdMember()
        ...
        if (authorization != null && authorization.getSID()
                                                  .equals(sid)) {
            ...
            context.setAuthentication(new DefaultAuthentication(context.getAuthentication()
                                                                       .getUserName(), newClearTextPassword, PasswordType.PLAIN));
        }
    }
    ...
    protected byte[] decryptSalt(IPassword password) {
        try {
            ...
            String saltAlgorithm = password.getSaltPBEConfiguration()
            ...
            if (loginSaltPassword == null) {
                throw new IllegalStateException("Property '" + SecurityServerConfigurationConstants.LoginSaltPassword + "' is not specified but reading an encrypted salt from " + password);
            }
    ...
    protected void encryptSalt(IPassword password, byte[] salt, char[] loginSaltPassword) {
        try {
            ...
            if (loginSaltPassword == null) {
                ...
                password.setSalt(Base64.encodeBytes(salt)
                                       .toCharArray());
                return;
            }
            byte[] encryptedSalt = pbEncryptor.encrypt(saltPBEConfiguration, false, loginSaltPassword, salt);
            password.setSalt(Base64.encodeBytes(encryptedSalt)
                                   .toCharArray());
        } catch (Exception e) {
            throw RuntimeExceptionUtil.mask(e);
        }
