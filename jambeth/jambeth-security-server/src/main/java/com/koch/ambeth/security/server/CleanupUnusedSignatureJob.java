...
public class CleanupUnusedSignatureJob implements IJob, IStartingBean {
    ...
    public void afterStarted() throws Throwable {
        var qb = queryBuilderFactory.create(ISignature.class);
        q_signaturesWithoutUser =
                qb.build(qb.and(qb.let(qb.property(ISignature.User)).isNull(), qb.let(qb.property("Id")).isNotIn(qb.property("<" + IAuditEntry.class.getName() + "#" + IAuditEntry.SignatureOfUser))));
    }
    ...
    public void execute(IJobContext context) throws Exception {
        var retrieve = q_signaturesWithoutUser.retrieve();
