...
public abstract class AbstractAuthorization implements IAuthorization {
    ...
    public static ITypePropertyPrivilege unionPropertyPrivileges(ITypePropertyPrivilege left, ITypePropertyPrivilege right) {
        ...
        return TypePropertyPrivilegeImpl.create(unionFlags(left.isCreateAllowed(), right.isCreateAllowed()), unionFlags(left.isReadAllowed(), right.isReadAllowed()),
                unionFlags(left.isUpdateAllowed(), right.isUpdateAllowed()), unionFlags(left.isDeleteAllowed(), right.isDeleteAllowed()));
    }
    ...
    public AbstractAuthorization(HashMap<ISecurityScope, IServicePermission[]> servicePermissions, ISecurityScope[] securityScopes, Tuple2KeyHashMap<ISecurityScope, String, Boolean> actionPrivileges,
            Tuple2KeyHashMap<ISecurityScope, Class<?>, ITypePrivilege> entityTypePrivileges, ITypePrivilege defaultEntityTypePrivilege, IEntityMetaDataProvider entityMetaDataProvider,
        ...
        for (Entry<ISecurityScope, ArrayList<Pattern>> entry : scopeToActionPatternsMap) {
            this.scopeToActionPatternsMap.put(entry.getKey(), entry.getValue().toArray(Pattern[]::new));
        }
    }
    ...
    protected IServicePermission[] getServicePermissions(ISecurityScope[] securityScopes) {
        ...
        return servicePermissionSet.size() == 0 ? EMPTY_SERVICE_PERMISSIONS : servicePermissionSet.toArray(IServicePermission[]::new);
    }
