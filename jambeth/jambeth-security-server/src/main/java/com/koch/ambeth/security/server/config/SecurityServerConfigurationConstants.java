...
public final class SecurityServerConfigurationConstants {
    ...
    public static final String LoginPasswordChangeRecommendation = "security.login.password.changerecommendation";
     ...
     * Default = null
     */
    @ConfigurationConstantDescription("Configures the internal 'secret' which is used to decrypt encrypted salts. If you define the parameter ensure that it is NOT hard-coded in our source-code and" +
            " NOT stored at the same place as the password-entity (e.g. do NOT store it in the database). The recommended place is an external property file in the file-system of the deployed " +
            "application. Optional parameter (as long as no encrypted salts exist). Default = null")
     ...
     */
    @ConfigurationConstantDescription("Configures the algorithm which is used to encrypt salts. Value itself is not considered unless a specific salt password is defined. Optional parameter. " +
            "Default = AES/CBC/PKCS5Padding")
     ...
     * Default = AES
     */
    @ConfigurationConstantDescription("Configures the keyspec name which is used to encrypt salts. Must be compatible to the configured 'security.login.salt.algorithm.name'. Value itself is not " +
            "considered unless a specific salt password is defined. Optional parameter. Default = AES")
    ...
    public static final String SignatureProviderName = "security.signature.provider.name";

    public static final String SignatureKeyAlgorithmName = "security.signature.key.algorithm.name";

    public static final String SignatureKeyProviderName = "security.signature.key.provider.name";
     ...
     * Default = 512
     */
    @ConfigurationConstantDescription("Configures the bit-length of the generated public/private key pair. The security harness is related to the chosen SignatureAlgorithmName. For example for RSA " +
            "keys above 3072 bits are considered safe - with EC comparable safety is achieved with numbers around 512. Optional parameter. Must be >=112. Default = 512")
