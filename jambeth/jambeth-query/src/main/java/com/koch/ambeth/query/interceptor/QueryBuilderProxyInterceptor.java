...
public class QueryBuilderProxyInterceptor implements MethodInterceptor {
    ...
    public static String getPropertyPath(Object propertyProxy, ThreadLocal<String> lastPropertyPathTL) {
    ...
    public static <T> T createProxy(Class<T> type, IEntityMetaData metaData, String propertyPath, ThreadLocal<String> lastPropertyPathTL, IPropertyInfoProvider propertyInfoProvider,
            IEntityMetaDataProvider entityMetaDataProvider, IProxyFactory proxyFactory) {
        return proxyFactory.createProxy(type, INTERFACES,
                new QueryBuilderProxyInterceptor(metaData, type, propertyPath, lastPropertyPathTL, propertyInfoProvider, entityMetaDataProvider, proxyFactory));
    }
    ...
    public QueryBuilderProxyInterceptor(IEntityMetaData currMetaData, Class<?> baseEntityType, String propertyPath, ThreadLocal<String> lastPropertyPathTL, IPropertyInfoProvider propertyInfoProvider,
            IEntityMetaDataProvider entityMetaDataProvider, IProxyFactory proxyFactory) {
    ...
    public Object intercept(Object obj, Method method, Object[] args, MethodProxy proxy) throws Throwable {
        ...
        var propertyName = propertyInfoProvider.getPropertyNameFor(method);
        if (propertyName == null) {
            return null;
        }
        var member = currMetaData.getMemberByName(propertyName);
        var elementType = member.getElementType();
        var targetMetaData = entityMetaDataProvider.getMetaData(elementType, true);
        ...
        if (elementType.isEnum()) {
            var enumConstants = elementType.getEnumConstants();
            if (enumConstants.length == 0) {
                throw new IllegalStateException("Can not query for an enum without any enum constants: '" + elementType.getName() + "'");
            }
        ...
        var list = ListUtil.createCollectionOfType(member.getRealType(), 1);
        list.add(childPlan);
        return list;
    }
