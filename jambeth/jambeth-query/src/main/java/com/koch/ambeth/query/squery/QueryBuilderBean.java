...
public final class QueryBuilderBean<T> {
	...
	public QueryBuilderBean(List<OperationBean> queryBeans, Class<T> entityType,
			List<ISortDescriptor> sorts, String queryStr) {
	...
	public Object createQueryBuilder(IQueryBuilderFactory qbf, IConversionHelper conversionHelper,
			Object[] params, Class<?> expectedReturnType) {
		...
		for (ISortDescriptor isort : collectSorts(params)) {
			OrderByType direction =
					isort.getSortDirection() == SortDirection.DESCENDING ? OrderByType.DESC : OrderByType.ASC;
		...
		if (IPagingResponse.class.isAssignableFrom(expectedReturnType)) {
			...
			if (Number.class.isAssignableFrom(expectedReturnType) || expectedReturnType.isPrimitive()) {
				return conversionHelper.convertValueToType(expectedReturnType,
						queryBuilder.build(where).count());
			}
				...
				throw new RuntimeException(
						"the " + this.queryStr + " method must return Type is Long or Integer");
			}
		}
		else {
			throw new RuntimeException("the " + this.queryStr
					+ " method must return Type is IPagingResponse, List, Long, Integer or entityType");
		}
	}
	...
	private List<ISortDescriptor> collectSorts(Object[] params) {
		...
		if (sortsFromParams == null) {
			...
			result = new ArrayList<>(this.sorts);
	 ...
	 * @param method
	 * @return
	 * @throws java.lang.ArrayIndexOutOfBoundsException if params have not enough element, then throw
	 *         this Exception
	 */
	private IOperand buildOperand(IQueryBuilder<T> queryBuilder, Object[] params) {
		...
		List<IOperand> andList = new ArrayList<>();
		List<IOperand> orList = new ArrayList<>();
		int paramIndex = 0;
	...
	private boolean needValueBuildOperand(OperationBean operationBean) {
		return operationBean.getCondition() != Condition.IS_NULL
				&& operationBean.getCondition() != Condition.IS_NOT_NULL;
	}

	private void collectAndOperand(IQueryBuilder<T> queryBuilder, List<IOperand> andList,
			List<IOperand> orList) {
		if (andList.size() == 1) {
