...
public final class QueryBuilderBean<T> {
    ...
    public Object createQueryBuilder(IQueryBuilderFactory qbf, IConversionHelper conversionHelper, Object[] params, Class<?> expectedReturnType) {
        var queryBuilder = qbf.create(entityType);
        IOperand where;
        where = buildOperand(queryBuilder, params);

        // do orderBy

        for (var isort : collectSorts(params)) {
            var direction = isort.getSortDirection() == SortDirection.DESCENDING ? OrderByType.DESC : OrderByType.ASC;
        ...
        if (IPagingResponse.class.isAssignableFrom(expectedReturnType)) {
            var pagingRequest = getPageRequest(params);
            ...
            var list = queryBuilder.build(where).retrieve();
