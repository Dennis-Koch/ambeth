...
public class DataChangeEventStoreHandler implements IEventStoreHandler, IInitializingBean {
	...
	public Object preSaveInStore(Object eventObject) {
		...
		writeLock.lock();
		try {
			// important to process the deletes first: technically it may be possible that a new entity in
	...
	public Object postLoadFromStore(Object eventObject) {
		...
		List<IDataChangeEntry> inserts = insertCount > 0 ? new ArrayList<IDataChangeEntry>(insertCount)
				: Collections.<IDataChangeEntry>emptyList();
		List<IDataChangeEntry> updates = updateCount > 0 ? new ArrayList<IDataChangeEntry>(updateCount)
				: Collections.<IDataChangeEntry>emptyList();
		List<IDataChangeEntry> deletes = deleteCount > 0 ? new ArrayList<IDataChangeEntry>(deleteCount)
				: Collections.<IDataChangeEntry>emptyList();
	...
	protected IDataChangeEntry readFromCacheEntry(ObjRefStore cachedObjRefStore) {
		return new DataChangeEntry(cachedObjRefStore.getRealType(), cachedObjRefStore.getIdNameIndex(),
				cachedObjRefStore.getId(), cachedObjRefStore.getVersion());
	}
	...
	protected ObjRefStore writeToCacheEntry(IDataChangeEntry dataChangeEntry, boolean isDeleteEntry) {
		...
		if (isDeleteEntry) {
			...
			// After a specific delete the ObjRefStore will never be used by any other
