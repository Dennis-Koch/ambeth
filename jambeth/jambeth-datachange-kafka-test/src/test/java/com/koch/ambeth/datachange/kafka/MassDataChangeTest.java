...
public class MassDataChangeTest extends AbstractIocTest {
	...
	public void test() throws Throwable {
		...
		props.put(AmbethKafkaConfiguration.AMBETH_KAFKA_PROP_PREFIX + "zookeeper.connect",
				kafkaRule.zookeeperConnectionString());
		// props.put("group.id", "group");
		props.put(AmbethKafkaConfiguration.AMBETH_KAFKA_PROP_PREFIX + "bootstrap.servers",
				"localhost:" + kafkaRule.kafkaBrokerPort());
		props.put(AmbethKafkaConfiguration.AMBETH_KAFKA_PROP_PREFIX + "zookeeper.session.timeout.ms",
				"400");
		props.put(AmbethKafkaConfiguration.AMBETH_KAFKA_PROP_PREFIX + "zookeeper.sync.time.ms", "200");
		props.put(AmbethKafkaConfiguration.AMBETH_KAFKA_PROP_PREFIX + "auto.commit.interval.ms",
				"1000");
		...
		try {
			...
			try {
				...
				long withoutKafkaTime = testContextsWithout(app1.getApplicationContext(),
						app2.getApplicationContext(), NUM_ENTITIES);
	 ...
	 */
	private void fillCache(IServiceContext left, int countEntities,
			final IList<TestEntity> testEntityList) {
