...
public class TableMetaData implements ITableMetaData, IInitializingBean {
    ...
    public static final short[][] EMPTY_SHORT_ARRAY_ARRAY = new short[0][0];
    ...
    @Override
    public IFieldMetaData[][] getAlternateIdsFields() {
    ...
    public IFieldMetaData getIdField() {
        ...
        throw new IllegalStateException("Access via getIdFields() required at this point");
    }
    ...
    public IFieldMetaData getIdFieldByAlternateIdIndex(int idIndex) {
        ...
        var alternateIdFields = getAlternateIdsFields()[idIndex];
        if (alternateIdFields.length > 1) {
            throw new IllegalStateException("Must access via composite-id support");
        }
        return alternateIdFields[0];
    }

    @Override
    public IFieldMetaData[] getIdFieldsByAlternateIdIndex(int idIndex) {
        if (idIndex == ObjRef.PRIMARY_KEY_INDEX) {
            return getIdFields();
        }
        return getAlternateIdsFields()[idIndex];
    }
    ...
    public void mapField(FieldMetaData field) {
        ...
        allFields.add(field);
        var index = Integer.valueOf(fieldNameToFieldIndexDict.size());
    ...
    public IFieldMetaData mapField(String fieldName, String memberName) {
        ...
        if (member == null) {
            throw new NullPointerException("Member '" + getEntityType().getName() + "." + memberName + "' not found to map to field '" + getName() + "." + fieldName + "'");
        }
        IFieldMetaData field = getFieldByName(fieldName);
        if (field == null) {
            throw new NullPointerException("Field '" + getName() + "." + fieldName + "' not found to map to member '" + getEntityType().getName() + "." + memberName + "'");
        }
    ...
    protected void refreshAlternateIdFieldIndices() {
        if (alternateIdFields == null || alternateIdFields.length == 0 || allFields == null || allFields.isEmpty()) {
            alternateIdFieldIndices = EMPTY_SHORT_ARRAY_ARRAY;
            return;
        }
        var fieldNameToFieldIndexDict = this.fieldNameToFieldIndexDict;
        var alternateIdFields = this.alternateIdFields;
        var alternateIdFieldIndices = new short[alternateIdFields.length][];
        for (int a = alternateIdFields.length; a-- > 0; ) {
            var alternateIdField = alternateIdFields[a];
            var indices = new short[alternateIdField.length];
            for (int b = alternateIdField.length; b-- > 0; ) {
                var alternateIdFieldName = alternateIdField[b].getName();
                var index = fieldNameToFieldIndexDict.get(alternateIdFieldName);
                if (index == null) {
                    throw new IllegalArgumentException("Alternate id member '" + alternateIdFieldName + "' has to be a valid primitive member");
                }
                indices[b] = index.shortValue();
            }
            alternateIdFieldIndices[a] = indices;
        }
        this.alternateIdFieldIndices = alternateIdFieldIndices;
    }

    public void setFulltextFieldsByNames(List<String> names) {
        for (int i = names.size(); i-- > 0; ) {
            var field = getFieldByName(names.get(i));
            if (field != null) {
