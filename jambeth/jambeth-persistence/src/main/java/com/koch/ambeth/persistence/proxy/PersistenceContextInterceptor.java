...
public class PersistenceContextInterceptor extends CascadedInterceptor {
    ...
    protected MethodInterceptor resolveLogic(PersistenceContextType behaviourOfMethod) {
        ...
        typeToLogicMap.put(PersistenceContextType.REQUIRED_LAZY, (obj, method, args, proxy) -> {
            if (transactionState.isTransactionActive()) {
                return invokeTarget(obj, method, args, proxy);
            }
            return transaction.runInLazyTransaction(() -> invokeTarget(obj, method, args, proxy));
        });
        typeToLogicMap.put(PersistenceContextType.NOT_REQUIRED, (obj, method, args, proxy) -> invokeTarget(obj, method, args, proxy));
        typeToLogicMap.put(PersistenceContextType.REQUIRED, (obj, method, args, proxy) -> {
            if (transactionState.isTransactionActive()) {
                return invokeTarget(obj, method, args, proxy);
            }
