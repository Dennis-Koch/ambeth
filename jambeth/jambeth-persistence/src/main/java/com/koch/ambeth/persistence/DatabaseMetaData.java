...
public class DatabaseMetaData implements IDatabaseMetaData, IConfigurableDatabaseMetaData, IInitializingBean, IStartingBean {
    ...
    protected final HashMap<String, IPermissionGroup> nameToPermissionGroupTableDict = new HashMap<>();
    ...
    protected final Tuple2KeyHashMap<ITableMetaData, ITableMetaData, ILinkMetaData[]> tablesToLinkDict = new Tuple2KeyHashMap<>();
    ...
    public void handleTable(ITableMetaData table) {
        ...
        var fromType = table.getEntityType();
        if (fromType == null) {
        ...
        var incompleteLinkSet = incompleteLinks.remove(table.getName());
        if (incompleteLinkSet != null) {
            for (var oneIncompleteLink : incompleteLinkSet) {
        ...
        var alternateIdMembers = new ArrayList<IFieldMetaData[]>();

        // Remove not-mapped (and so not usable) alternate id fields
        for (var alternateIdMembersOfIdIndex : table.getAlternateIdsFields()) {
            var members = new ArrayList<IFieldMetaData>(alternateIdMembersOfIdIndex.length);
            for (var alternateIdMemberOfIdIndex : alternateIdMembersOfIdIndex) {
                if (alternateIdMemberOfIdIndex.getMember() != null) {
                    members.add(alternateIdMemberOfIdIndex);
                }
            }
            if (!members.isEmpty()) {
                alternateIdMembers.add(members.toArray(IFieldMetaData[]::new));
            }
        }
        ((TableMetaData) table).setAlternateIdFields(alternateIdMembers.toArray(IFieldMetaData[][]::new));
    }
            ...
            for (ITypeInfoItem member : typeInfo.getMembers()) {
                ...
                if (!elementType.isAssignableFrom(toType)) {
                    if (Collection.class.isAssignableFrom(elementType)) {
                        ...
                        if (typeNamePluralLower == null) {
                            typeNamePluralLower = StringConversionHelper.entityNameToPlural(objectCollector, typeInfoProvider.getTypeInfo(toType).getSimpleName()).toLowerCase();
                        }
                ...
                if (matchingMember != null) {
                    ...
                    throw new IllegalArgumentException("Ambiguous property-to-entity relationship. Automatic mapping for link '" + link.getName() +
                            "' not possible! Multiple properties with the same expected relation type found: " + matchingMember.toString() + " vs. " + member.toString() + " on entity '" +
                            table.getEntityType().getName() + "' both map to entity '" + member.getElementType().getName() + "'");
                }
                matchingMember = member;
            }
            if (matchingMember == null) {
                if (!(memberName == null || memberName.isEmpty())) {
                    throw new IllegalArgumentException(
                            "Property-to-entity relationship which is explicit defined for member '" + fromType.getName() + "." + memberName + "' not possible. Member not found");
                }
                continue;
            }
    ...
    public void mapPermissionGroupTable(ITableMetaData permissionGroupTable, ITableMetaData targetTable) {
        ...
        if (userField == null) {
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + tableInst.getName() + "." + PermissionGroup.userIdName + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
        if (permissionGroupField == null) {
            ...
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + tableInst.getName() + "." + PermissionGroup.permGroupIdName + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
        if (readPermissionField == null) {
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + tableInst.getName() + "." + PermissionGroup.readPermColumName + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
        if (updatePermissionField == null) {
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + tableInst.getName() + "." + PermissionGroup.updatePermColumName + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
        if (deletePermissionField == null) {
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + tableInst.getName() + "." + PermissionGroup.deletePermColumName + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
        if (permissionGroupFieldOnTarget == null) {
            if (log.isWarnEnabled()) {
                loggerHistory.warnOnce(log, this,
                        "No column found " + targetTable.getName() + "." + PermissionGroup.permGroupIdNameOfData + ": SQL based security is deactivated for table " + targetTable.getName());
            }
            return;
        }
    ...
    public ITableMetaData mapTable(String tableName, Class<?> entityType) {
        ...
        Class<?> mappedEntityType = table.getEntityType();
        if (mappedEntityType != null && !mappedEntityType.equals(entityType)) {
            throw new IllegalArgumentException("Table '" + tableName + "' already mapped to entity '" + typeInfoProvider.getTypeInfo(mappedEntityType).getSimpleName() + "'");
        }
