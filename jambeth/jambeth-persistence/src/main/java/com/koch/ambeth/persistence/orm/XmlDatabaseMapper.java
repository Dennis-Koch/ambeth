...
public class XmlDatabaseMapper extends DefaultDatabaseMapper
	...
	protected void configureTableSequence(ITableMetaData table, String sequenceName,
			int maxNameLength, ISet<String> allFullqualifiedSequences) {
	...
	protected ITableMetaData tryResolveTable(IDatabaseMetaData database, String hardName,
			String softName) {
		...
		if (table == null) {
			throw new IllegalArgumentException("No table could be autoresolved with entity name '"
					+ softName + "' in schema '" + database.getSchemaNames()[0] + "'");
		}
		return table;
	}
	...
	protected void handleVersionField(IMemberConfig versionMemberConfig, ITableMetaData table) {
		...
		if (versionField == null) {
			...
			if (versionMemberConfig instanceof MemberConfig) {
				...
				throw new IllegalStateException(
						"'versionMemberConfig' not instance of '" + MemberConfig.class.getSimpleName() + "'");
			}
			if (versionColumnName != null && !versionColumnName.isEmpty()
					&& table instanceof TableMetaData) {
	...
	protected void handleDescriminatorField(String descriminatorName, ITableMetaData table) {
		...
		if (descriminatorField == null) {
			if (descriminatorName != null && !descriminatorName.isEmpty()
					&& table instanceof TableMetaData) {
	...
	protected void addToUcPropertyMap(String memberName, IPropertyInfo propertyInfo,
			IMap<String, IPropertyInfo> ucPropertyMap, int maxNameLength) {
		...
		for (String name : producedNameCandidates) {
			...
			if (!ucPropertyMap.putIfNotExists(assumedFieldName1, propertyInfo)
					&& ucPropertyMap.get(assumedFieldName1) != propertyInfo) {
			...
			if (!ucPropertyMap.putIfNotExists(assumedFieldName2, propertyInfo)
					&& ucPropertyMap.get(assumedFieldName2) != propertyInfo) {
				// Property is not uniquely resolvable if transformed to uppercase
				// So it will be no candidate for automated field mapping!
				ucPropertyMap.put(assumedFieldName2, null);
			}
			if (!ucPropertyMap.putIfNotExists(assumedFieldName3, propertyInfo)
					&& ucPropertyMap.get(assumedFieldName3) != propertyInfo) {
				// Property is not uniquely resolvable if transformed to uppercase
				// So it will be no candidate for automated field mapping!
	...
	protected ITableMetaData getTableByType(IDatabaseMetaData database, Class<?> entityType) {
		...
		if (table == null) {
			throw new IllegalArgumentException(
					"No table could be autoresolved with entity name '" + entityType + "'");
		}
	...
	protected void mapToOne(Connection connection, DatabaseMetaData database, ITableMetaData table,
		...
		if (linkedEntityLocal) {
			...
			if (useLinkTable) {
				...
				linkName = createFkLinkToExternalEntity(connection, database, table, fromFieldName,
						toFieldName, linkedEntityType, toAttributeName, toMember, false);
			}
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("Mapping member '" + table.getEntityType().getName() + "." + memberName
					+ "' to link '" + linkName + "'");
		}
		setCascadeValuesAndMapLink(table, link, memberName, doDelete, mayDelete);
	}

	protected void mapToMany(Connection connection, DatabaseMetaData database, ITableMetaData table,
			RelationConfigLegathy relationConfig) {
		String memberName = relationConfig.getName();
		Class<?> linkedEntityType = relationConfig.getLinkedEntityType();
		boolean doDelete = relationConfig.doDelete();
		...
		if (linkedEntityMetaData == null || linkedEntityMetaData.isLocalEntity()) {
			...
			if (useLinkTable) {
				...
				String toFieldName = relationConfig.getToFieldName();
				linkName = createFkLinkToExternalEntity(connection, database, table, fromFieldName,
						toFieldName, linkedEntityType, toAttributeName, toMember, true);
			}
		}
		ILinkMetaData link = database.getLinkByName(linkName);
		if (link == null) {
	...
	protected void mapRelation(RelationConfig20 relationConfig20, ITableMetaData table,
			DatabaseMetaData database, Connection connection) {
		...
		if (!toEntityLocal) {
			...
			if (useLinkTable) {
				...
				linkName = createFkLinkToExternalEntity(connection, database, table, fromFieldName, null,
						toRealType, toMemberName, toMember, toMany);
			}
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("Mapping member '" + table.getEntityType().getName() + "." + propertyName
					+ "' to link '" + link.getName() + "' defined by '" + linkSource + "'");
		}

		setCascadeValuesAndMapLink(table, link, propertyName, entityIdentifier, doDelete, mayDelete);
	}

	protected void setCascadeValuesAndMapLink(ITableMetaData table, ILinkMetaData link,
	...
	protected boolean[] resolveCascadeDeletes(ILinkConfig linkConfig,
	...
	protected ILinkMetaData resolveLinkByTables(String propertyName, ITableMetaData table,
			IDatabaseMetaData database, ILinkMetaData link) {
		...
		if (links == null || links.length == 0) {
			throw new IllegalArgumentException(
					"No Link found for '" + table.getEntityType().getName() + "." + propertyName + "'");
		}
			...
			throw new IllegalArgumentException(
					"Unconfigured Link for '" + table.getEntityType().getName() + "." + propertyName
							+ "' not uniquely idenfifiable. Please provide the name in the orm.xml file.");
		}
	}

	private String findLinkNameLocal(ITableMetaData table, String memberName, String joinTableName,
			RelationConfigLegathy relationConfig, ITableMetaData table2, DatabaseMetaData database) {
		String linkName;
		if (joinTableName == null) {
			linkName = resolveLinkDynamically(database, memberName, relationConfig.getConstraintName(),
					table, table2);
		}
		else {
			boolean useLinkTable = !table.getName().equalsIgnoreCase(joinTableName)
					&& !table2.getName().equalsIgnoreCase(joinTableName);
			if (!useLinkTable) {
	...
	private String createFkLinkToExternalEntity(Connection connection, DatabaseMetaData database,
		...
		String linkName = database.createForeignKeyLinkName(table.getName(), fromFieldName,
				linkedEntityType.getSimpleName(), toAttributeName);
		buildAndMapLink(connection, database, linkName, table, fromField, toField, toMember);
		return linkName;
	}

	private ILinkMetaData findLinkForRelation(String propertyName, String linkName,
			ITableMetaData table, IDatabaseMetaData database) {
	...
	protected String resolveLinkDynamically(IDatabaseMetaData database, String memberName,
			String constraintName, ITableMetaData table, ITableMetaData table2) {
	...
	protected String mapDataTableWithLink(DatabaseMetaData database, ITableMetaData table,
			ITableMetaData table2, RelationConfigLegathy relationConfig, boolean reverse) {
		String joinTableName = getFqJoinTableName(table, relationConfig);
	...
	protected void mapFieldToMember(ITableMetaData table, String joinTableName, String fieldName,
			String memberName) {
		if (table.getName().equalsIgnoreCase(joinTableName)
				&& table.getFieldByName(fieldName).getMember() == null) {
			table.mapField(fieldName, memberName);
			if (log.isDebugEnabled()) {
				log.debug("Configuring member '" + table.getEntityType().getName() + "." + memberName
						+ "' for database field '" + table.getName() + "." + fieldName + "'");
			}
		}
	}

	protected void setCascadeValuesAndMapLink(ITableMetaData table, ILinkMetaData link,
			String memberName, boolean doDelete, boolean mayDelete) {
	...
	protected void addToMemberToPreBuildLink(IDatabaseMetaData database, String linkName,
			Member toMember) {
	...
	protected Member getMemberByTypeAndName(Class<?> entityType, String memberName) {
		...
		if (member == null) {
			throw new IllegalArgumentException("No entity member found for name '" + memberName
					+ "' on entity '" + entityType.getName() + "'");
		}
		return member;
	}

	protected SqlLinkMetaData buildAndMapLink(Connection connection,
			IConfigurableDatabaseMetaData confDatabase, String linkName, ITableMetaData fromTable,
		...
		directedLink
				.setStandaloneLink(fromField.isAlternateId() || fromTable.getIdField().equals(fromField));
