...
public class ServiceUtil implements IServiceUtil {
	...
	public <T> T loadObject(Class<T> entityType, IVersionItem item) {
		...
		return (T) cache.getObject(new ObjRef(entityType, ObjRef.PRIMARY_KEY_INDEX, id, version),
				CacheDirective.none());
	}

	@SuppressWarnings("unchecked")
	@Override
	public <T> void loadObjects(java.util.List<T> targetEntities, java.lang.Class<T> entityType,
			ILinkCursor cursor) {
		if (cursor == null) {
			return;
		}
