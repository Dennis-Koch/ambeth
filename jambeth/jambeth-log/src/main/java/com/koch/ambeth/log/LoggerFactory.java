...
public final class LoggerFactory {
    ...
    public static void configureLogger(String loggerName, IConfigurableLogger logger, IProperties appProps) {
        ...
        StringBuilder tempSB =
                new StringBuilder(loggerPrefix.length() + Math.max(Math.max(logLevelPropertyName.length(), logConsolePropertyName.length()), logSourcePropertyName.length()) + 1 + loggerName.length());
    ...
    public static ILogger getLogger(Class<?> source, IProperties props) {
        ...
        try {
            var loggerTypeName = props.get(LogConfigurationConstants.LoggerType);
            var loggerType = LoggerFactory.loggerType;
            if (loggerTypeName != null) {
                if (loggerTypeName instanceof Class) {
                    loggerType = (Class<? extends ILogger>) loggerTypeName;
                } else {
                    loggerType = (Class<? extends ILogger>) Thread.currentThread().getContextClassLoader().loadClass((String) loggerTypeName);
                }
            }
            var logger = loggerType.getConstructor(String.class).newInstance(source.getName());
    ...
    public static ILogger getLogger(String source) {
        try {
            var logger = LoggerFactory.loggerType.getConstructor(String.class).newInstance(source);
