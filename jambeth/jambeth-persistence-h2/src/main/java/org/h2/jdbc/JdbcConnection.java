/*
 * Copyright 2004-2014 H2 Group. Multiple-Licensed under the MPL 2.0, and the EPL 1.0
...
/*
 * ## Java 1.7 ## import java.util.concurrent.Executor; //
 */
 ...
 * </p>
 * <p>
 * Thread safety: the connection is thread-safe, because access is synchronized. However, for
 * compatibility with other databases, a connection should only be used in one thread at any time.
 * </p>
 */
public class JdbcConnection extends TraceObject implements Connection {
	...
	public JdbcConnection(JdbcConnection clone) {
		...
		commit = clone.commit;
		getGeneratedKeys = clone.getGeneratedKeys;
		getLockMode = clone.getLockMode;
		getQueryTimeout = clone.getQueryTimeout;
		getReadOnly = clone.getReadOnly;
	 ...
	 * @return the statement
	 * @throws SQLException
	 *           if the connection is closed or the result set type, concurrency, or holdability are
	 *           not supported
	 */
	@Override
	public Statement createStatement(int resultSetType, int resultSetConcurrency,
	...
	/**
	 * Prepare a statement that will automatically close when the result set is closed. This method is
	 * used to retrieve database meta data.
	 *
	 * @param sql
	 *          the SQL statement
	 * @return the prepared statement
	...
	/**
	 * Switches auto commit on or off. Enabling it commits an uncommitted transaction, if there is
	 * one.
	 *
	 * @param autoCommit
	 ...
	 * supported:
	 * <ul>
	 * <li>Connection.TRANSACTION_READ_UNCOMMITTED = SET LOCK_MODE 0: no locking (should only be used
	 * for testing).</li>
	 ...
	 * locks are released immediately (default).</li>
	 * </ul>
	 ...
	 *
	 * @param level
	 *          the new transaction isolation level: Connection.TRANSACTION_READ_UNCOMMITTED,
	 ...
	 * internally. The method getGeneratedKeys only supports one column.
	 *
	 * @param sql
	 *          the SQL statement
	...
	/**
	 * Convert JDBC escape sequences in the SQL statement. This method throws an exception if the SQL
	 * statement is null.
	 *
	 * @param sql
	 *          the SQL statement with or without JDBC escape sequences
