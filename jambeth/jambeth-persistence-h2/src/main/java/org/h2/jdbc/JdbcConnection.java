 ...
 * </p>
 * <p>
 * Thread safety: the connection is thread-safe, because access is synchronized. However, for compatibility with other databases, a connection should only be
...
public class JdbcConnection extends TraceObject implements Connection
{
	...
	/**
	 * Closes this connection. All open statements, prepared statements and result sets that where created by this connection become invalid after calling this
	 * method. If there is an uncommitted transaction, it will be rolled back.
	 */
	@Override
	...
	public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency) throws SQLException
	{
		try
		{
			...
			if (isDebugEnabled())
			{
				debugCodeAssign("PreparedStatement", TraceObject.PREPARED_STATEMENT, id, "prepareStatement(" + quote(sql) + ", " + resultSetType + ", "
	 ...
	 * except if the level is not supported. Internally, this method calls SET LOCK_MODE. The following isolation levels are supported:
	 * <ul>
	 ...
	 * </ul>
	 * This setting is not persistent. Please note that using TRANSACTION_READ_UNCOMMITTED while at the same time using multiple connections may result in
	...
	public CallableStatement prepareCall(String sql, int resultSetType, int resultSetConcurrency) throws SQLException
	{
		try
		{
			...
			if (isDebugEnabled())
			{
				debugCodeAssign("CallableStatement", TraceObject.CALLABLE_STATEMENT, id, "prepareCall(" + quote(sql) + ", " + resultSetType + ", "
						+ resultSetConcurrency + ")");
			}
			checkTypeConcurrency(resultSetType, resultSetConcurrency);
			checkClosed();
	...
	/**
	 * Creates a new prepared statement. This method just calls prepareStatement(String sql) internally. The method getGeneratedKeys only supports one column.
	 * 
	 * @param sql
	 *            the SQL statement
	 ...
	 *            whether escape sequences should be replaced
	 * @return the SQL statement without JDBC escape sequences
	 */
	...
	private static void checkHoldability(int resultSetHoldability)
	{
		...
		// ResultSet.HOLD_CURSORS_OVER_COMMIT
		if (resultSetHoldability != ResultSet.HOLD_CURSORS_OVER_COMMIT && resultSetHoldability != ResultSet.CLOSE_CURSORS_AT_COMMIT)
		{
	 ...
	 * 
	 * @param name
	 *            the name of the property (ignored)
	 * @param value
	 *            the value (ignored)
	 */
	@Override
	...
	/**
	 * Set the client properties. This method always throws a SQLClientInfoException.
	 * 
	 * @param properties
	...
	/**
	 * Get the client properties. This method always returns null.
	 * 
	 * @return always null
	 */
	@Override
	...
	{
		if (JdbcConnection.class.isAssignableFrom(iface))
		{
			return (T) this;
		}
	...
	{
		return JdbcConnection.class.isAssignableFrom(iface);
	}
	 ...
	 * ## Java 1.7 ##
	 * 
	 * @Override public void setSchema(String schema) { // not supported } //
	 */
