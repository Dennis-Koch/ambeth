...
public class ClasspathScanner implements IClasspathScanner {
    ...
    public List<Class<?>> scanClassesAnnotatedWith(Class<?>... annotationTypes) {
        ...
        List<String> targetClassNames = scanForClasses(pool);
        try {
    ...
    protected List<String> scanForClasses(ClassPool pool) {
        List<URL> urls = buildUrlsFromClasspath(pool);
        ...
        try {
            for (int a = 0, size = urls.size(); a < size; a++) {
                ...
                try {
                    path = classpathScannerServletContext != null ? classpathScannerServletContext.lookupExistingPath(path) : path;
    ...
    protected List<URL> buildUrlsFromClasspath(ClassPool pool) {
