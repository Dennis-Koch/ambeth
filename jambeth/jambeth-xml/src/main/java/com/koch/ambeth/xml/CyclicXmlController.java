...
public class CyclicXmlController extends AbstractHandler implements ICyclicXmlController, ITypeBasedHandlerExtendable, INameBasedHandlerExtendable {
    ...
    protected final MapExtendableContainer<String, INameBasedHandler> nameBasedElementReaders = new MapExtendableContainer<>("nameBasedElementReader", "elementName");
    ...
    public Object readObject(Class<?> returnType, IReader reader) {
        ...
        var idValue = reader.getAttributeValue(xmlDictionary.getIdAttribute());
        var id = idValue != null && idValue.length() > 0 ? Integer.parseInt(idValue) : 0;
        try {
            ...
            if (xmlDictionary.getObjectElement().equals(elementName)) {
                var type = classElementHandler.readFromAttribute(reader);
                obj = readObjectContent(returnType, type, id, reader);
            } else {
                var nameBasedElementReader = nameBasedElementReaders.getExtension(elementName);
            ...
            throw RuntimeExceptionUtil.mask(e, "Error occured while processing element '" + elementName + "' with id '" + id + "'");
        }
    }
    ...
    public void writeObject(Object obj, IWriter writer, boolean suppressReference) {
        ...
        var id = suppressReference ? 0 : writer.getIdOfObject(obj);
        if (id > 0) {
        ...
        for (int a = 0, size = nameBasedElementWriters.size(); a < size; a++) {
            var nameBasedElementWriter = nameBasedElementWriters.get(a);
            if (nameBasedElementWriter.writesCustom(obj, type, writer)) {
                return;
            }
        }
        id = suppressReference ? 0 : writer.acquireIdForObject(obj);
        var objectElement = xmlDictionary.getObjectElement();
