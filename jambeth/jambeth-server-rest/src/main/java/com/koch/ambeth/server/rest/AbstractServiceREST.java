...
public abstract class AbstractServiceREST {
    ...
    public StreamingOutput ping2_ambeth_xml(@Context HttpServletRequest request, @Context HttpServletResponse response) {
        return defaultStreamingRequest(request, response, this::ping2_application_xml);
    }
    ...
    protected <T> StreamingOutput defaultStreamingRequest(HttpServletRequest request, HttpServletResponse response, InputStream argsInputStream, CheckedFunction<Object[], T> operation) {
        var success = false;
        var rollback = preServiceCall(request, response);
        try {
            var args = getArguments(argsInputStream, request);
            var result = operation.apply(args);
            var streamingOutput = result instanceof StreamingOutput ? (StreamingOutput) result : createResult(result, request, response, e -> rollback.rollback(), true);
            success = true;
    ...
    protected <T> StreamingOutput defaultStreamingRequest(HttpServletRequest request, HttpServletResponse response, CheckedSupplier<T> operation) {
        var success = false;
        var rollback = preServiceCall(request, response);
        try {
    ...
    protected IStateRollback preServiceCall(final HttpServletRequest request, HttpServletResponse response) {
        if (Boolean.TRUE.equals(request.getAttribute(THREAD_LOCAL_HANDLED))) {
            return StateRollback.empty();
        }
        var log = getLog();
        var path = log.isDebugEnabled() ? request.getMethod() + " " + request.getRequestURI() : null;
        if (path == null) {
            return aspect.pushServletAspectWithThreadLocals(request);
        }
        var start = Instant.now();
        log.debug("Enter: " + path);
        return StateRollback.chain(chain -> {
            chain.append(aspect.pushServletAspectWithThreadLocals(request));
            chain.append(() -> {
                var end = Instant.now();
                var duration = Duration.between(start, end);
                log.debug("Exit (" + duration.toString() + "):  " + path);
            });
        });
    }
    ...
    protected List<String> readMultiValueFromHeader(String name, HttpServletRequest request) {
        ...
        while (headers.hasMoreElements()) {
            var tokenizer = new StringTokenizer(headers.nextElement(), ",");
    ...
    protected Object[] getArguments(InputStream is, HttpServletRequest request) {
        ...
        Object args = null;
        if (contentType == null || MediaType.APPLICATION_XML.equals(contentType) || Constants.AMBETH_MEDIA_TYPE.equals(contentType)) {
            var cyclicXmlHandler = getService(XmlModule.CYCLIC_XML_HANDLER, ICyclicXMLHandler.class);
    ...
    protected StreamingOutput createExceptionResult(Throwable e, HttpServletRequest request, final HttpServletResponse response) {
        if (e.getClass().getName().equals(MaskingRuntimeException.class.getName()) && e.getMessage() == null) {
            e = e.getCause();
        }
    ...
    protected StreamingOutput createResult(Object result, HttpServletRequest request, HttpServletResponse response) {
        return createResult(result, request, response, null, true);
    }

    @SneakyThrows
    protected StreamingOutput createSynchronousResult(Object result, HttpServletRequest request, HttpServletResponse response) {
        ...
        return output -> bos.writeTo(output);
    }

    protected StreamingOutput createResult(final Object result, HttpServletRequest request, final HttpServletResponse response, final CheckedConsumer<Throwable> streamingFinishedCallback,
        ...
        return output -> {
            ...
            if (expectedThread != executingThread) {
                throw new IllegalStateException(
                        "Thread writing the stream is not the same thread creating the " + StreamingOutput.class.getSimpleName() + ". This is a safeguard check due to an expected consistent " +
                                "thread-local state");
            }
    ...
    protected String evaluateAcceptedContentEncoding(HttpServletRequest request, HttpServletResponse response) {
    ...
    @SneakyThrows
    protected String evaluateAcceptedType(HttpServletRequest request, HttpServletResponse response) {
