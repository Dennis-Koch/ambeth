...
public class AmbethServletRequestFilter extends AmbethSimpleServletRequestFilter {
	...
	protected void doFilterIntern(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		ILogger log = beanContext.getService(ILoggerCache.class).getCachedLogger(beanContext,
				AmbethServletRequestFilter.class);
		...
		if (userName != null) {
			PasswordType passwordTypeEnum =
					passwordType != null ? PasswordType.valueOf(passwordType) : PasswordType.PLAIN;
			DefaultAuthentication authentication = new DefaultAuthentication(userName,
					userPass != null ? userPass.toCharArray() : null, passwordTypeEnum);
			session.setAttribute(ATTRIBUTE_AUTHENTICATION_HANDLE, authentication);
			setAuthentication(servletContext, authentication);
		}
		else {
			IAuthentication authentication =
					(IAuthentication) session.getAttribute(ATTRIBUTE_AUTHENTICATION_HANDLE);
			if (authentication != null) {
				...
				if (servletAuthorizationTimeToLive != null
						&& servletAuthorizationTimeToLive.longValue() > 0) {
					IAuthorization authorization =
							(IAuthorization) session.getAttribute(ATTRIBUTE_AUTHORIZATION_HANDLE);
					// FIXME:Bad because not always sid == username, use IUserProvider
					if (authorization != null
							&& !authorization.getSID().equalsIgnoreCase(authentication.getUserName())) {
						session.setAttribute(ATTRIBUTE_AUTHORIZATION_HANDLE, null);
						log.info("User authorization '" + authorization.getSID()
								+ "' has been invalidated because of authentication '"
								+ authentication.getUserName() + "' has been bound to session");
						authorization = null;
						session.invalidate();
						return;
					}
					if (authorization != null && System.currentTimeMillis()
							- authorization.getAuthorizationTime() <= servletAuthorizationTimeToLive
									.longValue()) {
	...
	protected void setHttpSession(IServiceContext beanContext, ServletRequest request,
	...
	protected void setSecurityScope(IServiceContext beanContext, ServletRequest request,
			ServletResponse response, FilterChain chain) throws IOException, ServletException {
		ISecurityScopeProvider securityScopeProvider =
				beanContext.getService(ISecurityScopeProvider.class);
