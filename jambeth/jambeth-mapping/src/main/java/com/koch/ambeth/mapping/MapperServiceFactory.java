...
public class MapperServiceFactory implements IMapperServiceFactory {
    ...
    public IMapperService create() {
        if (mapperServiceGPC == null) {
            mapperServiceGPC = garbageProxyFactory.createGarbageProxyConstructor(IMapperService.class, TypeUtil.EMPTY_TYPES);
        }
        var mapperService = beanContext.registerBean(ModelTransferMapper.class).finish();
