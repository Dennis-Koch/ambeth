...
public class AuditController implements IThreadLocalCleanupBean, IMethodCallLogger, IMergeListener, ITransactionListener, IAuditInfoController {
    ...
    @Override
    public void postMerge(ICUDResult cudResult, IObjRef[] updatedObjRefs, IIncrementalMergeState incrementalMergeState) {
    ...
    protected void auditChangeContainer(Object originalRef, IObjRef updatedObjRef, IChangeContainer changeContainer, AuditControllerState auditControllerState,
        ...
        if (versionMember != null) {
            auditedEntity.ensurePrimitive(IAuditedEntity.RefPreviousVersion).setNewValue(conversionHelper.convertValueToType(String.class, versionMember.getValue(originalRef, false)));
        }
    ...
    protected IDirectObjRef getOrCreateRef(IObjRef objRef, AuditControllerState auditControllerState, IMap<IObjRef, IDirectObjRef> objRefToRefMap) {
        ...
        auditedEntityRef.ensurePrimitive(IAuditedEntityRef.EntityType).setNewValue(objRef.getRealType().getName());
    ...
    protected void auditPUIs(Class<?> entityType, IPrimitiveUpdateItem[] puis, CreateOrUpdateContainerBuild auditedEntity, IAuditConfiguration auditConfiguration,
    ...
    protected void auditRUIs(IRelationUpdateItem[] ruis, CreateOrUpdateContainerBuild auditedEntity, IAuditConfiguration auditConfiguration, AuditControllerState auditControllerState,
            IMap<IObjRef, IDirectObjRef> objRefToRefMap) {
        if (ruis == null) {
            return;
        }
    ...
    protected void auditPropertyItem(IObjRef objRef, CreateOrUpdateContainerBuild relationProperty, AuditedEntityPropertyItemChangeType changeType, AuditControllerState auditControllerState,
            IMap<IObjRef, IDirectObjRef> objRefToRefMap) {
    ...
    public void handlePreCommit(long sessionId) throws Exception {
        var auditEntryState = auditEntryTL.get();
        if (auditEntryState == null) {
        ...
        try {
            ...
            try {
                ...
                var entities = auditEntry.findRelation(IAuditEntry.Entities);
                var services = auditEntry.findRelation(IAuditEntry.Services);
                ...
                var authorizedUser = getAuthorizedUser(additionalAuditInfo);
    ...
    @Override
    public IStateRollback pushAuditReason(String auditReason) {
    ...
    @Override
    public IStateRollback pushAuditContext(String auditContext) {
    ...
    @Override
    public IStateRollback pushClearTextPassword(char[] clearTextPassword) {
        var additionalAuditInfo = getAdditionalAuditInfo();
        var oldClearTextPassword = additionalAuditInfo.clearTextPassword;
        var oldDoClearPassword = additionalAuditInfo.doClearPassword;
    ...
    @Override
    public IStateRollback pushAuthorizedUser(final IUser user, final char[] clearTextPassword, boolean forceGivenAuthorization) {
        var additionalAuditInfo = getAdditionalAuditInfo();
        var oldAuthorizedUser = additionalAuditInfo.authorizedUser;
        ...
        var oldAuthorizedUserSID = authenticatedUserHolder.getAuthenticatedSID();
        var oldClearTextPassword = additionalAuditInfo.clearTextPassword;
        ...
        return () -> {
            var authenticatedSID = authenticatedUserHolder.getAuthenticatedSID();
    ...
    protected void signAuditEntry(CreateOrUpdateContainerBuild auditEntry, AdditionalAuditInfo additionalAuditInfo, ISignature signatureOfUser) {
        if (signatureOfUser == null && signatureActive) {
            throw new IllegalStateException("Failed to create an Audit Entry without a signature - which is mandatory because of '" + SecurityServerConfigurationConstants.SignatureActive + "'=true");
        }
        var clearTextPassword = additionalAuditInfo.clearTextPassword;
        ...
        if (clearTextPassword == null && signatureActive) {
            throw new IllegalStateException("Failed to create an Audit Entry for a signature without a given cleartext password to decrypt the private key");
        }
    ...
    protected ICUDResult buildAuditCUDResult(List<CreateOrUpdateContainerBuild> auditedChanges) {
        ...
        for (int a = 0, size = auditedChanges.size(); a < size; a++) {
            ...
            var cc = createOrUpdate.build();
            ((IDirectObjRef) cc.getReference()).setDirect(cc);
            var ruis = cc.getFullRUIs();
            if (ruis != null) {
                for (var b = ruis.length; b-- > 0; ) {
                    var rui = ruis[b];
