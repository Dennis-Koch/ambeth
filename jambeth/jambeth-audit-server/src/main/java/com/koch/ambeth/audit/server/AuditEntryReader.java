...
public class AuditEntryReader implements IAuditEntryReader {
    ...
    protected IQuery<IAuditedEntity> getQuery_AuditedEntity_WithVersion() {
        ...
        q_auditedEntity_withVersion = qb.build(qb.and(
                //
                qb.let(entityType).isEqualTo(qb.parameterValue(IAuditedEntityRef.EntityType)), //
                qb.let(entityId).isEqualTo(qb.parameterValue(IAuditedEntityRef.EntityId)), qb.let(entityVersion).isLessThanOrEqualTo(qb.parameterValue(IAuditedEntityRef.EntityVersion))));
        return q_auditedEntity_withVersion;
    }
    ...
    protected IQuery<IAuditedEntity> getQuery_AuditedEntity_NoVersion() {
        ...
        q_auditedEntity_noVersion = qb.build(qb.and(//
                qb.let(entityType).isEqualTo(qb.parameterValue(IAuditedEntityRef.EntityType)), //
                qb.let(entityId).isEqualTo(qb.parameterValue(IAuditedEntityRef.EntityId))));
        return q_auditedEntity_noVersion;
    }
    ...
    protected IQuery<IAuditEntry> getQuery_AllUserActions() {
        ...
        q_allUserActions = qb.build(qb.and(//
                qb.let(userIdentifier).isEqualTo(qb.parameterValue(IAuditEntry.UserIdentifier))));
        return q_allUserActions;
    }
    ...
    protected IQuery<IAuditEntry> getQuery_AuditEntry_EntityType_InTimeSlot() {
        ...
        q_auditEntry_entityType_InTimeSlot = qb.build(qb.and(
                //
                qb.let(entityType).isEqualTo(qb.parameterValue(IAuditedEntityRef.EntityType)), qb.let(auditEntryStart).isGreaterThanOrEqualTo(qb.parameterValue(VALUE_NAME_START)),
                qb.let(auditEntryEnd).isLessThanOrEqualTo(qb.parameterValue(VALUE_NAME_END))));
        return q_auditEntry_entityType_InTimeSlot;
    }

    protected <V> List<V> getByObjRef(IQuery<V> q_NoVersion, IQuery<V> q_WithVersion, IObjRef objRef) {
    ...
    protected <V> List<V> getByEntityType(IQuery<V> q_entityType, Class<?> entityType) {
    ...
    protected <V> List<V> get(IQuery<V> q_NoVersion, IQuery<V> q_WithVersion, Object entity) {
        ...
        var id = metaData.getIdMember().getValue(entity);
        if (version == null) {
            return q_NoVersion.param(IAuditedEntityRef.EntityType, metaData.getEntityType())//
    ...
    public List<IAuditEntry> getAllAuditEntriesOfUser(IUser user) {
        return getQuery_AllUserActions().param(IAuditEntry.UserIdentifier, userIdentifierProvider.getSID(user)).retrieve();
    }
