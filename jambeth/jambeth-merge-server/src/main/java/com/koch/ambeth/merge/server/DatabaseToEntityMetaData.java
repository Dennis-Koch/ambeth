...
public class DatabaseToEntityMetaData implements IDatabaseMappedListener, IDisposableBean {
	...
	public static final String P_PersistenceMergeServiceExtension =
			"PersistenceMergeServiceExtension";
	...
	protected final ClassExtendableContainer<IEntityMetaData> registeredMetaDatas =
	...
	protected void checkRelationMember(EntityMetaData metaData, RelationMember[] relationsNew) {
		...
		if (!Arrays.equals(relationsOld, relationsNew)) {
			throw new IllegalStateException("Relation member arrays for entity '"
					+ metaData.getEntityType().getName() + "' do not match. From object model: "
					+ Arrays.deepToString(relationsOld) + ", from database model: "
	...
	public synchronized void databaseMapped(IDatabaseMetaData database) {
		...
		firstMapping = false;
		HashSet<IFieldMetaData> alreadyHandledFields = new HashSet<>();
		List<IEntityMetaData> newMetaDatas = new ArrayList<>();
		List<IEntityMetaData> newRegisteredMetaDatas = new ArrayList<>();
		...
		for (int a = 0, size = newRegisteredMetaDatas.size(); a < size; a++) {
			...
			registeredMetaDatas.register(newRegisteredMetaData,
					newRegisteredMetaData.getEntityType());
		}
		for (int a = 0, size = newMetaDatas.size(); a < size; a++) {
			...
			mergeServiceExtensionExtendable
					.registerMergeServiceExtension(persistenceMergeServiceExtension, entityType);
	...
	public void destroy() throws Throwable {
		for (int a = handledMetaDatas.size(); a-- > 0;) {
			...
			cacheRetrieverExtendable.unregisterCacheRetriever(persistenceCacheRetriever,
					handledMetaData.getEntityType());
			mergeServiceExtensionExtendable.unregisterMergeServiceExtension(
					persistenceMergeServiceExtension, handledMetaData.getEntityType());
		}
		ILinkedMap<Class<?>, IEntityMetaData> extensions = registeredMetaDatas.getExtensions();
		for (Entry<Class<?>, IEntityMetaData> entry : extensions) {

			IEntityMetaData registeredMetaData = entry.getValue();
	...
	protected PrimitiveMember findPrimitiveMember(PrimitiveMember memberToFind,
			ISet<PrimitiveMember> members) {
	...
	protected boolean isMemberOnFieldBetter(Class<?> entityType, Member existingMember,
			IFieldMetaData newMemberField) {
		...
		throw new IllegalStateException("Inconsistent metadata configuration on member '"
				+ existingMember.getName() + "' of entity '" + entityType.getName() + "'");
	}

	protected boolean isMemberOnMetaDataBetter(Class<?> entityType, Member existingMember,
			IFieldMetaData newMemberField) {
		if (newMemberField == null) {
			return false;
		}
	...
	protected void mapTable(ITableMetaData table, List<IEntityMetaData> newMetaDatas,
			List<IEntityMetaData> newRegisteredMetaDatas,
		...
		EntityMetaData metaData =
				(EntityMetaData) entityMetaDataProvider.getMetaData(entityType, true);
		if (metaData == null) {
		...
		if (table.getCreatedOnField() != null) {
			if (isMemberOnFieldBetter(entityType, metaData.getCreatedOnMember(),
					table.getCreatedOnField())) {
				metaData.setCreatedOnMember(
						(PrimitiveMember) table.getCreatedOnField().getMember());
			}
		}
		if (table.getCreatedByField() != null) {
			if (isMemberOnFieldBetter(entityType, metaData.getCreatedByMember(),
					table.getCreatedByField())) {
				metaData.setCreatedByMember(
						(PrimitiveMember) table.getCreatedByField().getMember());
			}
		}
		if (table.getUpdatedOnField() != null) {
			if (isMemberOnFieldBetter(entityType, metaData.getUpdatedOnMember(),
					table.getUpdatedOnField())) {
				metaData.setUpdatedOnMember(
						(PrimitiveMember) table.getUpdatedOnField().getMember());
			}
		}
		if (table.getUpdatedByField() != null) {
			if (isMemberOnFieldBetter(entityType, metaData.getUpdatedByMember(),
					table.getUpdatedByField())) {
				metaData.setUpdatedByMember(
						(PrimitiveMember) table.getUpdatedByField().getMember());
			}
		}
		...
		ArrayList<Member> fulltextMembers = new ArrayList<>();
		HashSet<PrimitiveMember> primitiveMembers = new HashSet<>();
		HashSet<RelationMember> relationMembers = new HashSet<>();
		HashMap<Member, Object> memberToFieldOrLinkMap = new HashMap<>();
		...
		metaData.setCreatedByMember(
				findPrimitiveMember(metaData.getCreatedByMember(), primitiveMembers));
		metaData.setCreatedOnMember(
				findPrimitiveMember(metaData.getCreatedOnMember(), primitiveMembers));
		metaData.setUpdatedByMember(
				findPrimitiveMember(metaData.getUpdatedByMember(), primitiveMembers));
		metaData.setUpdatedOnMember(
				findPrimitiveMember(metaData.getUpdatedOnMember(), primitiveMembers));
		...
		HashSet<Member> allRefreshedMembers = new HashSet<>();
		...
		for (Entry<Member, Object> entry : memberToFieldOrLinkMap) {
			...
			if (refreshedMember == null) {
				throw new IllegalStateException(
						"Member '" + member.getName() + "' has not been refreshed");
			}
