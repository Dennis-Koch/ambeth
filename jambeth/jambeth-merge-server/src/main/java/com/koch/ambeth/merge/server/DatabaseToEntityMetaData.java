...
public class DatabaseToEntityMetaData implements IDatabaseMappedListener, IDisposableBean {
	...
	protected final ArrayList<IEntityMetaData> registeredMetaDatas = new ArrayList<>();
	...
	protected void mapTable(ITableMetaData table, List<IEntityMetaData> newMetaDatas,
			List<IEntityMetaData> newRegisteredMetaDatas, HashSet<IFieldMetaData> alreadyHandledFields) {
			...
			if (existingPrimitiveMembers != null) {
				for (PrimitiveMember primitiveMember : existingPrimitiveMembers) {
					if (primitiveMember.isTransient()) {
						// ensure that transient members are not dropped because no persisted column has been
		...
		metaData
				.setCreatedByMember(findPrimitiveMember(metaData.getCreatedByMember(), primitiveMembers));
		metaData
				.setCreatedOnMember(findPrimitiveMember(metaData.getCreatedOnMember(), primitiveMembers));
		metaData
				.setUpdatedByMember(findPrimitiveMember(metaData.getUpdatedByMember(), primitiveMembers));
		metaData
				.setUpdatedOnMember(findPrimitiveMember(metaData.getUpdatedOnMember(), primitiveMembers));
	...
	protected void checkRelationMember(EntityMetaData metaData, RelationMember[] relationsNew) {
		...
		if (!Arrays.equals(relationsOld, relationsNew)) {
			throw new IllegalStateException(
					"Relation member arrays for entity '" + metaData.getEntityType().getName()
							+ "' do not match. From object model: " + Arrays.deepToString(relationsOld)
							+ ", from database model: " + Arrays.deepToString(relationsNew));
		}
	}
