...
public class RootCacheValueVisitor extends ClassGenerator {
	...
	protected void implementPrimitives() {
		...
		for (int primitiveIndex = 0, size = primitiveMembers.length; primitiveIndex < size; primitiveIndex++) {
			...
			if (isNullable) {
				...
				FieldInstance f_nullFlag = implementField(new FieldInstance(Opcodes.ACC_PRIVATE,
						CacheMapEntryVisitor.getFieldName(member) + "$isNull", null, boolean.class));
				f_nullFlags[primitiveIndex] = f_nullFlag;
			}
			fieldType[primitiveIndex] = nativeType;
			FieldInstance f_primitive = implementField(new FieldInstance(Opcodes.ACC_PRIVATE,
					CacheMapEntryVisitor.getFieldName(member), null, nativeType));
					...
					if (java.util.Date.class.equals(originalType)) {
						...
						mg.ifThisInstanceOf(java.util.Date.class, new Script() {
							...
							public void execute(MethodGenerator mg) {
								...
								mg.invokeVirtual(new MethodInstance(ReflectUtil.getDeclaredMethod(false,
										java.util.Date.class, long.class, "getTime")));
							}
						}, new Script() {
							@Override
							public void execute(MethodGenerator mg) {
	...
	protected void implementSetRelation(RelationMember[] relationMembers,
			FieldInstance[] f_relations) {
		MethodInstance template_m_setRelation = new MethodInstance(null, RootCacheValue.class,
				void.class, "setRelation", int.class, IObjRef[].class);
	...
	protected void implementSetRelations(RelationMember[] relationMembers, FieldInstance[] fields) {
		MethodInstance template_m_setRelations = new MethodInstance(null, RootCacheValue.class,
				void.class, "setRelations", IObjRef[][].class);
