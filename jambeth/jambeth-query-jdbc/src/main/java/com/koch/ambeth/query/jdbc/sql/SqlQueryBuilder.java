...
public class SqlQueryBuilder<T> implements IInitializingBean, IQueryBuilderIntern<T> {
    ...
    protected BasicTwoPlaceOperator createTwoPlaceOperator(Class<? extends BasicTwoPlaceOperator> operatorType, IOperand leftOperand, IOperand rightOperand, Boolean caseSensitive) {
        ...
        try {
            IBeanRuntime<? extends BasicTwoPlaceOperator> operatorBC =
                    beanContext.registerBean(operatorType).propertyValue(BasicTwoPlaceOperator.P_LEFT_OPERAND, leftOperand).propertyValue(BasicTwoPlaceOperator.P_RIGHT_OPERAND, rightOperand);
    ...
    public IOperand timeUnitMultipliedInterval(IOperand timeUnit, IOperand multiplicatedInterval) {
        ...
        return beanContext.registerBean(TimeUnitMultipliedOperand.class).propertyValue("TimeUnit", timeUnit).propertyValue("MultiplicatedInterval", multiplicatedInterval).finish();
    }
                        ...
                        if (joinName.length() + 1 <= remainingJoinName.length()) {
                            ...
                            if (currentMetaData != null) {
                                if (!currentMetaData.isLocalEntity()) {
                                    ITableMetaData baseTable = databaseMetaData.getTableByType(metaData.getEntityType());
                                    IDirectedLinkMetaData linkToExternal = baseTable.getLinkByMemberName(currentPropertyName);
        ...
        if (field == null) {
            if (log.isDebugEnabled()) {
                loggerHistory.debugOnce(log, databaseMetaData,
                        "No column '" + columnName + "' found on table '" + table.getName() + "'. This may be a configuration error or usage of deprecated " + IQuery.class.getSimpleName() + " " +
                                "functionality");
            }
        }
    ...
    @Override
    public ISqlJoin joinIntern(String tableName, IOperand columnBase, IOperand columnJoined, JoinType joinType) {
        ...
        try {
            SqlJoinOperator joinClause = getBeanContext().registerBean(SqlJoinOperator.class)
                                                         .propertyValue("JoinType", joinType)
                                                         .propertyValue("TableName", tableName)
                                                         ...
                                                         .propertyValue("JoinedColumn", columnJoined)
                                                         .finish();
    ...
    public <S> IOperand subQuery(ISubQuery<S> subQuery, IOperand... selectedColumns) {
        ...
        SqlSubselectOperand subQueryOperand = getBeanContext().registerBean(SqlSubselectOperand.class).propertyValue("SelectedColumns", columns).propertyValue("SubQuery", subQuery).finish();
    ...
    public T plan() {
        ...
        return (T) QueryBuilderProxyInterceptor.createProxy(entityType, metaData, null, lastPropertyPathTL, propertyInfoProvider, entityMetaDataProvider, proxyFactory);
    }
}
