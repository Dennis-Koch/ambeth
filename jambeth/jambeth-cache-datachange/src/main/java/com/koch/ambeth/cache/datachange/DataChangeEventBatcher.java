...
public class DataChangeEventBatcher implements IEventBatcher {
    ...
    protected void batchEventsIntern(List<IQueuedEvent> batchableEvents, List<IQueuedEvent> targetBatchedEvents) {
        ...
        if (splitIndex != -1 && splitIndex < batchableEvents.size() - 1) {
            ...
            List<IDataChangeEntry> inserts = !touchedAsInsertObjRefDict.isEmpty() ? new ArrayList<IDataChangeEntry>(touchedAsInsertObjRefDict.size()) : Collections.<IDataChangeEntry>emptyList();
            List<IDataChangeEntry> updates = !touchedAsUpdateObjRefDict.isEmpty() ? new ArrayList<IDataChangeEntry>(touchedAsUpdateObjRefDict.size()) : Collections.<IDataChangeEntry>emptyList();
            List<IDataChangeEntry> deletes = !touchedAsDeleteObjRefDict.isEmpty() ? new ArrayList<IDataChangeEntry>(touchedAsDeleteObjRefDict.size()) : Collections.<IDataChangeEntry>emptyList();
