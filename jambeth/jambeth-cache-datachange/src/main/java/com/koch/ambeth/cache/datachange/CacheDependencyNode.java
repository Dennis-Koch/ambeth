...
public class CacheDependencyNode {
	...
	protected static final Set<CacheDirective> failInCacheHierarchyAndCacheValueResult =
	...
	public final HashMap<IObjRef, CacheValueAndPrivilege> objRefToCacheValueMap =
			new HashMap<>();
	...
	protected void removeNotFoundObjRefs(IObjRef[] objRefsToRemove) {
		...
		for (int a = childNodes.size(); a-- > 0;) {
			...
			childNode.privilegedHardRefResult =
					childNode.rootCache.getObjects(hardRefRequest, failInCacheHierarchyAndCacheValueResult);
		}
	}

	public ISet<IObjRef> lookForIntermediateDeletes() {
		HashSet<IObjRef> intermediateDeletes = new HashSet<>();
		// Hold cache values as hard ref to prohibit cache loss due to GC
		IList<IObjRef> hardRefRequest = hardRefObjRefsToLoad.toList();
		IList<Object> hardRefResult =
				rootCache.getObjects(hardRefRequest, cacheValueResultAndReturnMissesSet);
