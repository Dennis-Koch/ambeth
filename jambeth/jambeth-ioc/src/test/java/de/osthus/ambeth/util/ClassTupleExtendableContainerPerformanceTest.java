...
{
	/**
	 * Old ClassTupleExtendableContainer with a normal HashMap instead of the new Tuple2 optimized HashMap. This class here is just for performance comparisions
	 * 
	 * @param <V>
	 */
	public static class ClassTupleExtendableContainerOld<V> extends MapExtendableContainer<ConversionKey, V>
	{
		public static class ClassTupleEntry<V> extends HashMap<ConversionKey, Object>
		{
			public final HashMap<ConversionKey, Object> typeToDefEntryMap = new HashMap<ConversionKey, Object>(0.5f);
		...
		protected boolean checkToWeakRegisterExistingExtensions(ConversionKey conversionKey, ClassTupleEntry<V> classEntry)
		{
			...
			for (Entry<Strong2Key<V>, List<Def2Entry<V>>> entry : classEntry.definitionReverseMap)
			{
				...
				int sourceDistance = ClassExtendableContainer.getDistanceForType(conversionKey.sourceType, registeredStrongKey.sourceType);
				if (sourceDistance == ClassExtendableContainer.NO_VALID_DISTANCE)
				{
					continue;
				}
				int targetDistance = ClassExtendableContainer.getDistanceForType(registeredStrongKey.targetType, conversionKey.targetType);
		...
		protected boolean checkToWeakRegisterExistingTypes(ConversionKey key, V extension, ClassTupleEntry<V> classEntry)
		{
			...
			for (Entry<ConversionKey, Object> entry : classEntry.typeToDefEntryMap)
			{
				...
				int sourceDistance = ClassExtendableContainer.getDistanceForType(existingRequestedKey.sourceType, key.sourceType);
				if (sourceDistance == ClassExtendableContainer.NO_VALID_DISTANCE)
				{
					continue;
				}
				int targetDistance = ClassExtendableContainer.getDistanceForType(key.targetType, existingRequestedKey.targetType);
				if (targetDistance == ClassExtendableContainer.NO_VALID_DISTANCE)
				{
					continue;
				}
		...
		@SuppressWarnings("unchecked")
		protected boolean appendRegistration(ConversionKey strongTypeKey, ConversionKey key, V extension, int sourceDistance, int targetDistance,
	...
	{
		ClassTupleExtendableContainer<String> newC = new ClassTupleExtendableContainer<String>("message", "keyMessage");
		ClassTupleExtendableContainerOld<String> oldC = new ClassTupleExtendableContainerOld<String>("message", "keyMessage");
		...
		Assert.assertTrue(newSpent <= oldSpent * 0.66f); // new approach should spent at most 66% of the old approach
	}

	protected void register(ClassTupleExtendableContainer<String> newC, ClassTupleExtendableContainerOld<String> oldC, String extension, Class<?> from,
			Class<?> to)
	{
