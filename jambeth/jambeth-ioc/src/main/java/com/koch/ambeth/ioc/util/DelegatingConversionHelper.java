...
public class DelegatingConversionHelper extends IConversionHelper
	...
	public <T> T convertValueToType(Class<T> expectedType, Object value,
		...
		while (true) {
			...
			Object targetValue;
			try {
				targetValue = dedicatedConverter.convertValueToType(expectedType, sourceClass, sourceValue,
			...
			if (targetValue == null) {
				if (expectedType.isPrimitive()) {
					throw new IllegalStateException(
							"It is not allowed that an instance of " + IDedicatedConverter.class.getName()
									+ " returns null like " + dedicatedConverter + " did for conversion from '"
			...
			if (targetValue.getClass().equals(sourceValue.getClass())) {
				throw new IllegalStateException(
						"It is not allowed that an instance of " + IDedicatedConverter.class.getName()
								+ " returns a value of the same type (" + targetValue.getClass().getName()
		...
		return defaultConversionHelper.convertValueToType(expectedType, sourceValue,
				additionalInformation);
	}

	@Override
	public void registerDedicatedConverter(IDedicatedConverter dedicatedConverter,
			Class<?> sourceType, Class<?> targetType) {
		converters.register(dedicatedConverter, sourceType, targetType);
	}

	@Override
	public void unregisterDedicatedConverter(IDedicatedConverter dedicatedConverter,
			Class<?> sourceType, Class<?> targetType) {
