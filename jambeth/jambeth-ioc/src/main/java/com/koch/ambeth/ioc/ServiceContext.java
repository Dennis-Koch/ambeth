...
public class ServiceContext implements IServiceContext, IServiceContextIntern, IDisposable {
    ...
    public void addAutowiredBean(Class<?> autowireableType, Object bean) {
        ...
        if (!(bean instanceof IFactoryBean)) {
            ...
            if (!autowireableType.isAssignableFrom(bean.getClass())) {
                throw new ClassCastException("Bean instance of type " + bean.getClass().getName() + " does not match to autowired type " + autowireableType.getName());
            }
        }
    ...
    protected void checkNotDisposed() {
        if (disposed) {
            throw new IllegalStateException("This bean context is already disposed. It might be a serious error that you still have running code referencing this instance");
        }
    }
        ...
        try {
            ...
            if (disposableObjects != null) {
                ...
                for (int a = disposableObjects.size(); a-- > 0; ) {
                    ...
                    } else if (disposableObject instanceof CheckedConsumer) {
                        try {
                            ((CheckedConsumer<? super ServiceContext>) disposableObject).accept(this);
                        } catch (Throwable e) {
                            if (failOnError) {
                                throw RuntimeExceptionUtil.mask(e);
                            }
    ...
    @Override
    public IServiceContext createService(CheckedConsumer<IBeanContextFactory> registerPhaseDelegate, Class<?>... serviceModuleTypes) {
        return createService(null, IServiceContext.class, registerPhaseDelegate, serviceModuleTypes);
    }

    @Override
    public IServiceContext createService(String contextName, CheckedConsumer<IBeanContextFactory> registerPhaseDelegate, Class<?>... serviceModuleTypes) {
    ...
    @Override
    public void registerDisposeHook(CheckedConsumer<IServiceContext> waitCallback) {
    ...
    public <T> T getServiceIntern(String serviceName, Class<T> serviceType, SearchType searchType) {
        ...
        if (beanNameToAliasesMap != null) {
            ...
            if (realBeanName != null) {
                ...
                serviceName = StringBuilderUtil.concat(objectCollector.getCurrent(), factoryContentRequest ? "" : "&", parentOnlyRequest ? "*" : "", realBeanName);
            }
        }
        Object service = null;
