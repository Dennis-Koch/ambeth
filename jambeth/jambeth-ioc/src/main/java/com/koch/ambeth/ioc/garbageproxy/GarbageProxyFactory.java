...
public class GarbageProxyFactory implements IGarbageProxyFactory, IInitializingBean {
    ...
    protected Class<?> createGpType(AccessorClassLoader loader, Class<?> proxyType, Class<?>[] additionalProxyTypes, String className) {
        ...
        visitor.visit(Opcodes.V1_1, Opcodes.ACC_PUBLIC + Opcodes.ACC_SUPER, classNameInternal, null, abstractType.getInternalName(), interfaceNames.toArray(String[]::new));
        {
            ...
            for (java.lang.reflect.Method method : methods) {
                ...
                switch (asmMethod.getReturnType().getSort()) {
                    ...
                    case Type.VOID:
                        break;
                    default:
                        throw new IllegalArgumentException("Sort not supported: " + asmMethod.getReturnType().getSort());
                }
                mv.returnValue();
                mv.endMethod();
            }
