...
public class ConversionHelper extends IConversionHelper implements IThreadLocalCleanupBean {
    ...
    protected final HashMap<Class<?>, IDedicatedConverter> expectedTypeToDefaultMap = new HashMap<>(0.5f);
    @Autowired
    ...
    public ConversionHelper() {
        ...
        register(BigInteger.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(BigDecimal.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Double.class, Double.TYPE, Number.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Long.class, Long.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Integer.class, Integer.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Float.class, Float.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Short.class, Short.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Byte.class, Byte.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Boolean.class, Boolean.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Character.class, Character.TYPE, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Date.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Instant.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (java.util.Date.class.isAssignableFrom(sourceType)) {
                return Instant.ofEpochMilli(((java.util.Date) value).getTime());
            } else if (XMLGregorianCalendar.class.isAssignableFrom(sourceType)) {
                return Instant.ofEpochMilli(((XMLGregorianCalendar) value).toGregorianCalendar().getTimeInMillis());
            } else if (GregorianCalendar.class.isAssignableFrom(sourceType)) {
        ...
        register(DayOfWeek.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Duration.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (Number.class.isAssignableFrom(sourceType)) {
        ...
        register(Month.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (Number.class.isAssignableFrom(sourceType)) {
        ...
        register(MonthDay.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (CharSequence.class.isAssignableFrom(sourceType)) {
                return MonthDay.parse(((CharSequence) value).toString());
            }
            return throwNotSupported(expectedType, sourceType, value, additionalInformation);
        });
        register(Period.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (CharSequence.class.isAssignableFrom(sourceType)) {
                return Period.parse((CharSequence) value);
            }
            return throwNotSupported(expectedType, sourceType, value, additionalInformation);
        });
        register(Year.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (Number.class.isAssignableFrom(sourceType)) {
        ...
        register(XMLGregorianCalendar.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(Calendar.class, GregorianCalendar.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(java.sql.Date.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (Date.class.isAssignableFrom(sourceType)) {
        ...
        register(java.sql.Timestamp.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (Number.class.isAssignableFrom(sourceType)) {
        ...
        register(Class.class, (expectedType, sourceType, value, additionalInformation) -> {
            if (String.class.isAssignableFrom(sourceType)) {
        ...
        register(CharSequence.class, String.class, (expectedType, sourceType, value, additionalInformation) -> {
        ...
        register(char[].class, (expectedType, sourceType, value, additionalInformation) -> {
            if (CharSequence.class.isAssignableFrom(sourceType)) {
                return value.toString().toCharArray();
            }
            return throwNotSupported(expectedType, sourceType, value, additionalInformation);
        });
        register(InetAddress.class, (expectedType, sourceType, value, additionalInformation) -> {
            try {
                if (CharSequence.class.isAssignableFrom(sourceType)) {
    ...
    protected void register(Class<?> oneType, Class<?> otherType, Class<?> thirdType, IDedicatedConverter converter) {
        expectedTypeToDefaultMap.put(oneType, converter);
        expectedTypeToDefaultMap.put(otherType, converter);
    ...
    protected DateFormat getISO_8601_DateFormat() {
        var dateFormat = iso8601_DateFormatTL.get();
        if (dateFormat == null) {
    ...
    protected Object convertValueToTypeIntern(Class<?> expectedType, Object value, Object additionalInformation) {
        ...
        var defaultConverter = expectedTypeToDefaultMap.get(expectedType);
        if (defaultConverter != null) {
            return defaultConverter.convertValueToType(expectedType, sourceType, value, additionalInformation);
        }
    ...
    public <T> IPreparedConverter<T> prepareConverter(Class<T> expectedType) {
        var state = new PreparedConverterState<T>();
    ...
    protected <T> IPreparedConverter<T> prepareConverterIntern(Class<T> expectedType, Object exemplaryValue, Object exemplaryAdditionalInformation) {
        if (expectedType == null || exemplaryValue == null) {
            return (value, additionalInformation) -> (T) value;
        }
        var sourceType = exemplaryValue.getClass();
        if (expectedType.isAssignableFrom(sourceType)) {
            return (value, additionalInformation) -> (T) value;
        }
        var defaultConverter = expectedTypeToDefaultMap.get(expectedType);
        if (defaultConverter != null) {
            return (value, additionalInformation) -> (T) defaultConverter.convertValueToType(expectedType, sourceType, value, additionalInformation);
        }
        if (expectedType.isEnum()) {
            if (String.class.isAssignableFrom(sourceType)) {
                return (value, additionalInformation) -> {
                    try {
                        return (T) enumValueOf.invoke(null, expectedType, value);
                    } catch (Exception e) {
                        throw RuntimeExceptionUtil.mask(e);
                    }
                };
            } else if (Enum.class.isAssignableFrom(sourceType)) {
                var stringToEnumConverter = prepareConverterIntern(expectedType, "enumName", exemplaryAdditionalInformation);
                return (value, additionalInformation) -> stringToEnumConverter.convertValue(((Enum<?>) value).name(), additionalInformation);
            }
        }
        return (IPreparedConverter<T>) throwNotSupported(expectedType, sourceType, exemplaryValue, exemplaryAdditionalInformation);
    }

    protected Object throwNotSupported(Class<?> expectedType, Class<?> sourceType, Object value, Object additionalInformation) {
        throw new IllegalArgumentException("Cannot convert from '" + value.getClass() + "' to '" + expectedType + "'");
    }

    public static class PreparedConverterState<T> {
        IPreparedConverter<T> preparedConverter;
    }
}
