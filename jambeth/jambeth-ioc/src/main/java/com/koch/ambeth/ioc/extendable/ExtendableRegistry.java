...
public class ExtendableRegistry extends SmartCopyMap<KeyItem, Method[]>
		implements IExtendableRegistry, IInitializingBean {
	...
	@Override
	public Method[] getAddRemoveMethods(Class<?> type, String eventName, Object[] arguments,
	...
	@Override
	public Method[] getAddRemoveMethods(Class<?> extendableInterface, Object[] arguments,
			IParamHolder<Object[]> linkArguments) {
	...
	protected Class<?>[] argsToTypesAndLinkArgs(Object[] arguments,
			IParamHolder<Object[]> linkArguments) {
		if (arguments == null) {
			arguments = emptyArgs;
		}
	...
	protected Method[] getAddRemoveMethodsIntern(Class<?> extendableInterface, String propertyName,
			Class<?>[] argumentTypes) {
		...
		Method[] addRemoveMethods = get(keyItem);
		...
		try {
			...
			for (Method method : methods) {
				...
				if (propertyName != null && !methodName.equals(registerMethodName1)
						&& !methodName.equals(registerMethodName2) && !methodName.equals(unregisterMethodName1)
				...
				if (methodName.startsWith("register") || methodName.startsWith("add")) {
					...
					for (int a = paramInfos.length; a-- > 1;) {
						...
						if (argumentType != null
								&& !unboxType(paramInfo).isAssignableFrom(unboxType(argumentType))) {
							match = false;
							break;
						}
					}
			...
			if (addMethod == null || removeMethod == null) {
				throw new ExtendableException(
						"No extendable methods pair like 'add/remove' or 'register/unregister' found on interface "
	...
	protected void compareParamInfos(Class<?>[] paramInfos, Class<?>[] foundParamInfos,
