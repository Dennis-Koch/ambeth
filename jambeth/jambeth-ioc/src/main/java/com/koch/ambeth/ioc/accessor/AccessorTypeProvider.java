...
public class AccessorTypeProvider implements IAccessorTypeProvider, IInitializingBean {
			...
			try {
				...
				if (enhancedType == AbstractAccessor.class) {
					throw new IllegalStateException(
							"Must never happen. No enhancement for " + AbstractAccessor.class + " has been done");
				}
	...
	protected Class<?> getAccessorTypeIntern(Class<?> targetType, IPropertyInfo property) {
		String accessClassName = targetType.getName() + "$" + AbstractAccessor.class.getSimpleName()
				+ "$" + property.getName();
	...
	protected Class<?> createType(AccessorClassLoader loader, String accessClassName,
			Class<?> targetType, IPropertyInfo property) {
		...
		{
			String desc = Type.getMethodDescriptor(Type.VOID_TYPE, Type.getType(Class.class),
		...
		java.lang.reflect.Method r_get = ReflectUtil.getDeclaredMethod(true, targetType,
				property.getPropertyType(), "get" + property.getName(), new Class<?>[0]);
		if (r_get == null) {
			r_get = ReflectUtil.getDeclaredMethod(true, targetType, property.getPropertyType(),
					"is" + property.getName(), new Class<?>[0]);
		}
		java.lang.reflect.Method r_set = ReflectUtil.getDeclaredMethod(true, targetType, null,
				"set" + property.getName(), property.getPropertyType());
	...
	protected Class<?> createConstructorType(AccessorClassLoader loader, String constructorClassName,
			Class<?> delegateType, Class<?> targetType) {
		if (log.isDebugEnabled()) {
		...
		for (java.lang.reflect.Method r_method : r_methods) {
			if (r_method != null) {
				throw new IllegalArgumentException("No matching constructor found on "
						+ targetType.getName() + " to map on delegate method " + r_method.toString());
			}
		}
		cw.visitEnd();
		byte[] data = cw.toByteArray();
	...
	protected void implementConstructorOfConstructorType(java.lang.reflect.Method r_method,
