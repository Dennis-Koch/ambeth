...
public class MethodPropertyInfo extends AbstractPropertyInfo {
	...
	public MethodPropertyInfo(Class<?> entityType, String propertyName, Method getter,
			Method setter) {
	...
	public MethodPropertyInfo(Class<?> entityType, String propertyName, Method getter, Method setter,
		...
		this.getter = getter;
		this.setter = setter;
		writable = this.setter != null && (Modifier.isPublic(this.setter.getModifiers())
				|| Modifier.isProtected(this.setter.getModifiers()));
		readable = this.getter != null && (Modifier.isPublic(this.getter.getModifiers())
				|| Modifier.isProtected(this.getter.getModifiers()));
	...
	protected void init(IThreadLocalObjectCollector objectCollector) {
		...
		if (getter == null && setter == null) {
			...
			if (setter != null) {
				if (setter.getParameterTypes().length != 1
						|| !setter.getParameterTypes()[0].equals(propertyType)) {
					throw new RuntimeException("Misfitting property methods for property '" + name
		...
		Boolean getterIsPublic =
				getter != null ? Boolean.valueOf(Modifier.isPublic(getter.getModifiers())) : null;
		Boolean setterIsPublic =
				setter != null ? Boolean.valueOf(Modifier.isPublic(setter.getModifiers())) : null;
		if (Boolean.TRUE.equals(getterIsPublic) || Boolean.TRUE.equals(setterIsPublic)) {
			...
			Boolean getterIsProtected =
					getter != null ? Boolean.valueOf(Modifier.isProtected(getter.getModifiers())) : null;
			Boolean setterIsProtected =
					setter != null ? Boolean.valueOf(Modifier.isProtected(setter.getModifiers())) : null;
	...
	public void refreshAccessors(Class<?> realType) {
		...
		setter =
				ReflectUtil.getDeclaredMethod(true, realType, null, "set" + getName(), getPropertyType());
