...
public class RandomDataGenerator implements IInitializingBean, IJob {
	...
	public void afterPropertiesSet() throws Throwable {
		ParamChecker.assertTrue(maxThreadhold >= minThreshold,
				"Thresholds must be valid: " + maxThreadhold + " >= " + minThreshold);
	}
	...
	protected void doChange(List<TestEntity> allTestEntities, List<TestEntity2> allTest2Entities,
			int selectEntityIndex, int selectEntity2Index, ChangeOperation changeOperation,
		...
		if (changeTestEntity) {
			...
			if (allTestEntities.size() < minThreshold || ChangeOperation.INSERT.equals(changeOperation)) {
				...
				log.debug("New " + TestEntity.class.getSimpleName() + " " + newEntity.getId() + " with "
						+ TestEntity2.class.getName() + " " + testEntity2.getId());
			}
			// If there are more than maxThreadhold entities, force to deletion of one
			else if (allTestEntities.size() > maxThreadhold
					|| ChangeOperation.DELETE.equals(changeOperation)) {
			...
			// If there are less than minThreshold entities, force to insertion of one
			if (allTest2Entities.size() < minThreshold
					|| ChangeOperation.INSERT.equals(changeOperation)) {
			...
			// If there are more than maxThreadhold entities, force to deletion of one
			else if (allTest2Entities.size() > maxThreadhold
					|| ChangeOperation.DELETE.equals(changeOperation)) {
