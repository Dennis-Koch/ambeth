	...
	public IDelegate createDelegate(Class<?> delegateType, Object target, String methodName) {
		...
		for (int a = targetMethods.length; a-- > 0;) {
			...
			if (methodOnTarget == null) {
				...
				throw new IllegalArgumentException("Illegal target for delegate: Method '" + methodName
						+ "' is overloaded on type '" + targetType.getName() + "'. This is not supported");
			}
		}
		if (methodOnTarget == null) {
			throw new IllegalArgumentException(
					"Illegal target for delegate: No suitable method found with name '" + methodName
							+ "' on target type '" + targetType.getName()
							+ "' which matches parameter signature of delegate type '" + delegateMethod + "'");
		}
