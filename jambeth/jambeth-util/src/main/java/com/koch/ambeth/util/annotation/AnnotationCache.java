...
public abstract class AnnotationCache<T extends Annotation>
		extends HashMap<AnnotatedElement, AnnotationEntry<T>> {
	...
	public AnnotationEntry<T> getAnnotationEntry(AnnotatedElement annotatedElement) {
			...
			if (annotatedElement instanceof Class) {
				...
				for (int a = interfaces.length; a-- > 0;) {
					...
					if (!annotationEquals(annotation, interfaceAnnotation)) {
						throw new IllegalStateException("Ambiguous annotation on type " + type.toString()
								+ " with " + annotationType.toString()
								+ " based on multiple implementing interfaces with DIFFERENT "
								+ annotationType.toString() + " values");
					}
				}
			}
		}
		...
		entry = annotation != null ? new AnnotationEntry<>(annotation, declaringAnnotatedElement)
				: emptyAnnotationEntry;
