...
public class Tuple2KeyHashMap<Key1, Key2, V> extends AbstractTuple2KeyHashMap<Key1, Key2, V> {
	...
	public static <Key1, Key2, V> Tuple2KeyHashMap<Key1, Key2, V> create(int size, float loadFactor) {
		return new Tuple2KeyHashMap<>((int) (size / loadFactor) + 1, loadFactor);
	}
	...
	public Tuple2KeyHashMap(
			AbstractTuple2KeyHashMap<? extends Key1, ? extends Key2, ? extends V> map) {
		this((int) (map.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_LOAD_FACTOR);
		putAll(map);
	}

	@Override
	protected Tuple2KeyEntry<Key1, Key2, V> createEntry(int hash, Key1 key1, Key2 key2, V value,
			final Tuple2KeyEntry<Key1, Key2, V> nextEntry) {
		return new Tuple2KeyEntry<>(key1, key2, value, hash, nextEntry);
	}
