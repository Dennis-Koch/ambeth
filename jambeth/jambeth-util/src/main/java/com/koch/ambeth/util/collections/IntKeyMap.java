...
public class IntKeyMap<V> {
	...
	protected IntKeyMapEntry<V> createEntry(final int hash, final int key, final V value,
			final IntKeyMapEntry<V> next) {
		IntKeyMapEntry<V> e = new IntKeyMapEntry<>();
	 ...
	 * <tt>null</tt> if the map contains no mapping for this key. A return value of <tt>null</tt> does
	 * not <i>necessarily</i> indicate that the map contains no mapping for the key; it is also
	 * possible that the map explicitly maps the key to <tt>null</tt>. The <tt>containsKey</tt> method
	 ...
	 * responsibility of this method to resize the table if appropriate. Subclass overrides this to
	 * alter the behavior of put method.
	 */
	...
	public ISet<IntKeyMapEntry<V>> entrySet() {
		final LinkedHashSet<IntKeyMapEntry<V>> entrySet =
				new LinkedHashSet<>((int) (size() / DEFAULT_LOAD_FACTOR) + 1);
		entrySet(entrySet);
		return entrySet;
	}
