 ...
 * @param <V>
 */
public class Tuple5KeyHashMap<Key1, Key2, Key3, Key4, Key5, V>
		extends AbstractTuple5KeyHashMap<Key1, Key2, Key3, Key4, Key5, V> {
	public static <Key1, Key2, Key3, Key4, Key5, V> Tuple5KeyHashMap<Key1, Key2, Key3, Key4, Key5, V> create(
			int size) {
	...
	public static <Key1, Key2, Key3, Key4, Key5, V> Tuple5KeyHashMap<Key1, Key2, Key3, Key4, Key5, V> create(
		...
		return new Tuple5KeyHashMap<>((int) (size / loadFactor) + 1,
				loadFactor);
	}
	...
	public Tuple5KeyHashMap(
			AbstractTuple5KeyHashMap<? extends Key1, ? extends Key2, ? extends Key3, ? extends Key4, ? extends Key5, ? extends V> map) {
		this((int) (map.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_LOAD_FACTOR);
		putAll(map);
	}

	@Override
	protected Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> createEntry(int hash, Key1 key1,
			Key2 key2, Key3 key3, Key4 key4, Key5 key5, V value,
			final Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> nextEntry) {
		return new Tuple5KeyEntry<>(key1, key2, key3, key4, key5, value,
				hash, nextEntry);
	}
