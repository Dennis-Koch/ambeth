...
public abstract class AbstractTuple5KeyHashMap<Key1, Key2, Key3, Key4, Key5, V> implements
		IPrintable, Iterable<Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>>, Externalizable {
	...
	protected Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[] createTable(final int capacity) {
		return (Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[]) Array
				.newInstance(Tuple5KeyEntry.class, capacity);
	}
	...
	protected int extractHash(final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4,
		...
		return (key1 != null ? key1.hashCode() : 3) ^ (key2 != null ? key2.hashCode() : 5)
				^ (key3 != null ? key3.hashCode() : 7) ^ (key4 != null ? key4.hashCode() : 11)
				^ (key5 != null ? key5.hashCode() : 13);
	}
	...
	protected void addEntry(final int hash, Key1 key1, Key2 key2, Key3 key3, Key4 key4, Key5 key5,
			final V value, final int bucketIndex) {
	...
	protected boolean equalKeys(final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4,
			final Key5 key5, final Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> entry) {
		return EqualsUtil.equals(key1, entry.getKey1()) && EqualsUtil.equals(key2, entry.getKey2())
				&& EqualsUtil.equals(key3, entry.getKey3()) && EqualsUtil.equals(key4, entry.getKey4())
				&& EqualsUtil.equals(key5, entry.getKey5());
	}

	public void putAll(
			final AbstractTuple5KeyHashMap<? extends Key1, ? extends Key2, ? extends Key3, ? extends Key4, ? extends Key5, ? extends V> map) {
		for (Tuple5KeyEntry<? extends Key1, ? extends Key2, ? extends Key3, ? extends Key4, ? extends Key5, ? extends V> entry : map) {
			put(entry.getKey1(), entry.getKey2(), entry.getKey3(), entry.getKey4(), entry.getKey5(),
					entry.getValue());
		}
	}

	public V put(final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4, final Key5 key5,
			final V value) {
		final int hash = hash(extractHash(key1, key2, key3, key4, key5));
		Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[] table = this.table;
		final int i = hash & (table.length - 1);
	...
	public boolean putIfNotExists(final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4,
			final Key5 key5, final V value) {
		final int hash = hash(extractHash(key1, key2, key3, key4, key5));
		Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[] table = this.table;
		final int i = hash & (table.length - 1);
	...
	public boolean removeIfValue(final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4,
			final Key5 key5, final V value) {
		final int hash = hash(extractHash(key1, key2, key3, key4, key5));
		Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[] table = this.table;
		final int i = hash & (table.length - 1);
	...
	protected final V removeEntryForKey(final Key1 key1, final Key2 key2, final Key3 key3,
			final Key4 key4, final Key5 key5) {
		final int hash = hash(extractHash(key1, key2, key3, key4, key5));
		Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>[] table = this.table;
		final int i = hash & (table.length - 1);
		Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> entry = table[i];
	...
	protected V setValueForEntry(final Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> entry,
			final V value) {
		V oldValue = entry.getValue();
	...
	protected abstract Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> createEntry(final int hash,
			final Key1 key1, final Key2 key2, final Key3 key3, final Key4 key4, final Key5 key5,
			final V value, final Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V> nextEntry);
	...
	public Iterator<Tuple5KeyEntry<Key1, Key2, Key3, Key4, Key5, V>> iterator() {
		return new Tuple5KeyIterator<>(this, table, true);
	}
