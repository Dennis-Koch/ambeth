...
public abstract class StateRollback implements IStateRollback {
     ...
     */
    @SneakyThrows
    public static @NonNull IStateRollback chain(@Nullable CheckedConsumer<StateRollbackChain> chainBuilder) {
     ...
     *
     * @param rollbackToPrepend new rollback handle to prepend. May be null
     * @param rollbacks         rollback handles to prepend to. May be null
     * @return A non-null rollback handle presenting the merged and ordered given sum of all rollback handles
     * @see #all(IStateRollback[])
     */
    public static @NonNull IStateRollback prepend(@Nullable IStateRollback rollbackToPrepend, @Nullable IStateRollback rollbacks) {
        var mergedRollbacks = collapseRollbacks(mergeRollbacks(rollbackToPrepend, rollbacks));
        if (mergedRollbacks.length == 0) {
            return NONE;
        }
        if (mergedRollbacks.length == 1) {
            return mergedRollbacks[0];
        }
        return new GroupingStateRollback(mergedRollbacks);
    }
    ...
    protected static IStateRollback[] mergeRollbacks(IStateRollback newRollback, IStateRollback rollbacks) {
        if (newRollback == null) {
            return rollbacks != null && rollbacks != NONE ? new IStateRollback[] { rollbacks } : EMPTY_ARRAY;
        }
        if (rollbacks == null) {
            return newRollback != NONE ? new IStateRollback[] { newRollback } : EMPTY_ARRAY;
        }
        return new IStateRollback[] { rollbacks, newRollback };
    }
