...
public final class StringConversionHelper {
	private static final Pattern insertUnderscoreBeforeUppercaseLetterPattern =
			Pattern.compile("(.+[^A-ZÄÖÜ_])([A-ZÄÖÜ].*)");

	private static final Pattern insertUnderscoreBeforeNumbersPattern =
			Pattern.compile("(.+[^\\d_])(\\d+.*)");
	...
	public static String upperCaseFirst(IObjectCollector objectCollector, String s) {
		...
		return StringBuilderUtil.concat(objectCollector.getCurrent(), Character.toUpperCase(firstChar),
				s.substring(1));
	}
	...
	public static String lowerCaseFirst(IObjectCollector objectCollector, String s) {
		...
		return StringBuilderUtil.concat(objectCollector.getCurrent(), Character.toLowerCase(firstChar),
				s.substring(1));
	}
	...
	public static String entityNameToPlural(IObjectCollector objectCollector, String entityName) {
		...
		if (lastChar == 'y') {
			return StringBuilderUtil.concat(tlObjectCollector,
	...
	public static String pluralToEntityName(IObjectCollector objectCollector, String plural) {
		if (plural.endsWith("ies")) {
			return StringBuilderUtil.concat(objectCollector.getCurrent(),
					plural.substring(0, plural.length() - 3), "y");
		}
	...
	public static String insertUnderscoreBeforeUppercaseLetter(IObjectCollector objectCollector,
			String value) {
		while (true) {
			...
			value = StringBuilderUtil.concat(objectCollector.getCurrent(), matcher.group(1), "_",
					matcher.group(2));
		}
	}

	public static String insertUnderscoreBeforeNumbers(IObjectCollector objectCollector,
			String value) {
		while (true) {
