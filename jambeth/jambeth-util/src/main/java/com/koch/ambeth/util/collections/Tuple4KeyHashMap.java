 ...
 * @param <V>
 */
public class Tuple4KeyHashMap<Key1, Key2, Key3, Key4, V>
		extends AbstractTuple4KeyHashMap<Key1, Key2, Key3, Key4, V> {
	public static <Key1, Key2, Key3, Key4, V> Tuple4KeyHashMap<Key1, Key2, Key3, Key4, V> create(
			int size) {
	...
	public static <Key1, Key2, Key3, Key4, V> Tuple4KeyHashMap<Key1, Key2, Key3, Key4, V> create(
		...
		return new Tuple4KeyHashMap<>((int) (size / loadFactor) + 1,
				loadFactor);
	}
	...
	public Tuple4KeyHashMap(
			AbstractTuple4KeyHashMap<? extends Key1, ? extends Key2, ? extends Key3, ? extends Key4, ? extends V> map) {
		this((int) (map.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_LOAD_FACTOR);
		putAll(map);
	}

	@Override
	protected Tuple4KeyEntry<Key1, Key2, Key3, Key4, V> createEntry(int hash, Key1 key1, Key2 key2,
			Key3 key3, Key4 key4, V value, final Tuple4KeyEntry<Key1, Key2, Key3, Key4, V> nextEntry) {
		return new Tuple4KeyEntry<>(key1, key2, key3, key4, value, hash,
				nextEntry);
	}
