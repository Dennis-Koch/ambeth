 ...
 */
public abstract class AbstractLinkedMap<K, V> extends AbstractHashMap<K, K, V>
		implements ILinkedMap<K, V> {
	...
	public AbstractLinkedMap(final int initialCapacity, final float loadFactor,
	...
	public void clear() {
		...
		while (entry != null) {
			next = entry.getNext();
			final int i = entry.getHash() & tableLengthMinusOne;
			table[i] = null;
			entryRemoved(entry);
			entry = next;
		}
