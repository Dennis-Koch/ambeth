package com.koch.ambeth.util.codec;
...
public class Base64 {
     ...
     * is important to note that data encoded this way is <em>not</em> officially valid Base64, or at
     ...
     * @since 1.3
     */
    private static int encode3to4(byte[] source, int srcOffset, int numSigBytes, byte[] destination, int destOffset, int options) {
    ...
    public static String encodeIdentityHashCode(Object obj) {
        var bytes = new byte[4];
        intToBytes(System.identityHashCode(obj), bytes, 0);
        return encodeBytes(bytes);
    }

    public static void intToBytes(int data, byte[] targetBytes, int offset) {
        targetBytes[offset] = (byte) ((data >> 24) & 0xff);
        targetBytes[offset + 1] = (byte) ((data >> 16) & 0xff);
        targetBytes[offset + 2] = (byte) ((data >> 8) & 0xff);
        targetBytes[offset + 3] = (byte) ((data >> 0) & 0xff);
    }

    public static void longToBytes(long data, byte[] targetBytes, int offset) {
        targetBytes[offset] = (byte) ((data >> 54) & 0xff);
        targetBytes[offset + 1] = (byte) ((data >> 48) & 0xff);
        targetBytes[offset + 2] = (byte) ((data >> 40) & 0xff);
        targetBytes[offset + 3] = (byte) ((data >> 32) & 0xff);
        targetBytes[offset + 4] = (byte) ((data >> 24) & 0xff);
        targetBytes[offset + 5] = (byte) ((data >> 16) & 0xff);
        targetBytes[offset + 6] = (byte) ((data >> 8) & 0xff);
        targetBytes[offset + 7] = (byte) ((data >> 0) & 0xff);
    }
    ...
    @SneakyThrows
    public static String encodeBytesNoPadding(byte[] source) {
        return encodeBytes(source, 0, source.length, NO_PADDING);
    }
    ...
    public static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws java.io.IOException {
        ...
        int lineLength = 0;
        for (; d < len2; d += 3, e += 4) {
            var bytesWritten = encode3to4(source, d + off, 3, outBuff, e, options);

            lineLength += bytesWritten;
            if (breakLines && lineLength >= MAX_LINE_LENGTH) {
                outBuff[e + bytesWritten] = NEW_LINE;
                e++;
                lineLength = 0;
            } // end if: end of line
        ...
        if (d < len) {
            var bytesWritten = encode3to4(source, d + off, len - d, outBuff, e, options);
            e += bytesWritten;
    ...
    public static String encodeFromFile(String filename) throws java.io.IOException {
        ...
        try {
            // Set up some useful variables
            java.io.File file = new java.io.File(filename);
            byte[] buffer = new byte[Math.max((int) (file.length() * 1.4 + 1), 40)]; // Need max() for
            // math on small
            // files (v2.2.1);
            // Need +1 for a few
            // corner
