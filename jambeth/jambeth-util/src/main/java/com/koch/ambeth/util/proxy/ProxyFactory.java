...
public class ProxyFactory extends SmartCopyMap<ProxyTypeKey, Class<? extends Factory>>
	...
	public <T> T createProxy(Class<T> type, MethodInterceptor... interceptors) {
		...
		if (type.isInterface()) {
			unloadedType = FactoryMixin.weave(new ByteBuddy()
		...
		proxyType = unloadedType.load(classLoaderProvider.getClassLoader())
				.getLoaded();
	...
	public <T> T createProxy(Class<T> type, Class<?>[] interfaces,
		...
		var unloadedType = (DynamicType.Unloaded) FactoryMixin.weave(new ByteBuddy()
							.subclass(type).implement(interfaceArray))
					.make();
		proxyType = unloadedType.load(classLoaderProvider.getClassLoader())
				.getLoaded();
	...
	public Object createProxy(Class<?>[] interfaces, MethodInterceptor... interceptors) {
		...
		var unloadedType = (DynamicType.Unloaded) FactoryMixin.weave(new ByteBuddy()
						.subclass(Object.class).implement(interfaces).method(ElementMatchers.any()).intercept(MethodDelegation.to(MethodInterceptorMixin.class)))
				.make();
		proxyType = unloadedType.load(classLoaderProvider.getClassLoader())
				.getLoaded();
		var proxy = createProxyIntern(proxyType, interceptors);
