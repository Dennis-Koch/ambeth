...
/**
 * Implementierung einer &uuml;blichen HashSet mit dem Vorteil, dass sie a) direkt Collectable ist,
 * also ohne Umwege &uuml;ber den ObjectCollector verwaltet werden kann. Daraus folgt, dass eine
 * h&auml;ufige Verwendung leerer HashSets f&uuml;r kurze Zeit der Garbage f&uuml;r neue Instanzen
 * vermieden wird b) Auch die SetEntries sind Collectable. Hier speziell sogar Collectable f&uuml;r
 * maximale Performance bei intensiver Nutzung derselben Set von wenigen Threads.
 * ____________________________________________________________________ Diese Klasse kann
 * bedenkenlos, auch von mehreren Threads (mit zus&auml;tzlichem und hier nicht durchgef&uuml;hrtem
 * Sync-Aufwand nat&uuml;rlich) verwendet werden, mit einer Anmerkung: Sie ist NICHT geeignet, wenn
 * man sie in einem 'Fliessband-Pattern' einsetzt: Thread A f&uuml;gt Objekte IN die Set und Thread
 * B entfernt und bearbeitet diese wieder. Dabei w&uuml;rde im ObjectCollector von Thread B eine
 * gro&#223;e Menge an aufger&auml;umten SetEntries entstehen, die vom ObjectCollector von Thread A
 * jedoch nicht verwendet werden und dieser munter neue Instanzen generiert. Hierbei w&uuml;rde
 * fr&uuml;her oder sp&auml;ter ein sicherer OutOfMemory entstehen.
 *
 * @author kochd
...
public class HashSet<K> extends AbstractHashSet<K> {
	...
	public static <K> HashSet<K> create(int size, float loadFactor) {
		return new HashSet<>((int) (size / loadFactor) + 1);
	}
	...
	protected ISetEntry<K> createEntry(int hash, K key, ISetEntry<K> nextEntry) {
		return new SetEntry<>(hash, key, (SetEntry<K>) nextEntry);
	}
