...
public abstract class AbstractTuple3KeyHashMap<Key1, Key2, Key3, V>
		implements IPrintable, Iterable<Tuple3KeyEntry<Key1, Key2, Key3, V>>, Externalizable {
	...
	protected Tuple3KeyEntry<Key1, Key2, Key3, V>[] createTable(final int capacity) {
		return (Tuple3KeyEntry<Key1, Key2, Key3, V>[]) Array.newInstance(Tuple3KeyEntry.class,
				capacity);
	}
	...
	protected void addEntry(final int hash, final Key1 key1, final Key2 key2, final Key3 key3,
	...
	protected boolean equalKeys(final Key1 key1, final Key2 key2, final Key3 key3,
			final Tuple3KeyEntry<Key1, Key2, Key3, V> entry) {
		return EqualsUtil.equals(key1, entry.getKey1()) && EqualsUtil.equals(key2, entry.getKey2())
				&& EqualsUtil.equals(key3, entry.getKey3());
	}

	public void putAll(
			final AbstractTuple3KeyHashMap<? extends Key1, ? extends Key2, ? extends Key3, ? extends V> map) {
	...
	protected abstract Tuple3KeyEntry<Key1, Key2, Key3, V> createEntry(final int hash,
			final Key1 key1, final Key2 key2, final Key3 key3, final V value,
	...
	public Iterator<Tuple3KeyEntry<Key1, Key2, Key3, V>> iterator() {
		return new Tuple3KeyIterator<>(this, table, true);
	}
