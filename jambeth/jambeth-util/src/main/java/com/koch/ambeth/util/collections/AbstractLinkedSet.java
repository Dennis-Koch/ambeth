...
/**
 * Erweiterte Map, welche zus&auml;tzlich zu den &uuml;blichen Key/Value-Entries eine Liste aller
 * Eintr&auml;ge verwaltet. Somit die Komplexit&auml;t f&uuml;r das Iterieren &uuml;ber eine solchen
 * Map mit O(n) = n identisch mit jener einer &uuml;blichen Array-Liste. Der Tradeoff sind hierbei
 * nat&uuml;rlich de leicht aufw&auml;ndigeren put()- und remove()-Operationen, welche jedoch
 * weiterhin bzgl. der Komplexit&auml;t mit O(n) = 1 konstant bleiben.
 *
 * @author kochd
 *
...
public abstract class AbstractLinkedSet<K> extends AbstractHashSet<K> implements ILinkedSet<K> {
	...
	public SetLinkedIterator<K> iterator() {
		return new SetLinkedIterator<>(this, true);
	}

	@Override
	public Iterator<K> iterator(boolean removeAllowed) {
		return new SetLinkedIterator<>(this, removeAllowed);
	}
