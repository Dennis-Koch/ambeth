...
/**
 * Spezialisierte Form der StHashSet mit dem Unterschied, dass hierbei die Eindeutigkeit von Entries
 * nicht auf der Equal-Identit�t der jeweiligen Keys basiert, sondern auf deren Objekt-Identit�t.
 *
 * Man k�nnte somit als Beispiel, wenn auch praxisfern, 2 Keys vom Typ Integer mit dem Wert '1' in
 * diese Set hinzuf�gen, solange die beiden Integer verschiedene Integer-Instanzen darstellen.
 *
 * @author kochd
 *
...
public class IdentityHashSet<K> extends HashSet<K> {
	...
	public static <K> IdentityHashSet<K> create(int size, float loadFactor) {
		return new IdentityHashSet<>((int) (size / loadFactor) + 1, loadFactor);
	}
