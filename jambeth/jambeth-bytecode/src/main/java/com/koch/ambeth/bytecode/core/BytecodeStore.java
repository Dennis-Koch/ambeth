...
public class BytecodeStore implements IBytecodeStore {
    ...
    protected String buildHashOfBehaviors(IBytecodeEnhancer bytecodeEnhancer, IBytecodeBehavior[] behaviors) {
        try {
            ...
            ObjectOutputStream oos = new ObjectOutputStream(new DigestOutputStream(NullOutputStream.INSTANCE, digest));
    ...
    @Override
    public IMap<BytecodeStoreKey, BytecodeStoreItem> loadEnhancedTypes(IBytecodeEnhancer bytecodeEnhancer, IBytecodeBehavior[] behaviors) {
    ...
    @Override
    public void storeEnhancedType(BytecodeEnhancer bytecodeEnhancer, IBytecodeBehavior[] behaviors, Class<?> baseType, IEnhancementHint hint, Class<?> enhancedType,
        ...
        try {
            ...
            for (Class<?> enhancedTypeInPipeline : enhancedTypesPipeline) {
                ...
                try {
                    byte[] content = bytecodeClassLoader.readTypeAsBinary(enhancedTypeInPipeline, classLoader);
                    fis.write(content);
                } finally {
                    fis.close();
                }
            }
            try (OutputStream os = new FileOutputStream(nameFile, false); OutputStreamWriter fw = new OutputStreamWriter(os, Charset.forName("UTF-8"))) {
                boolean first = true;
    ...
    protected void scanForKeys(File hintInstanceDir, File propertyFile, Class<?> hintType, Map<BytecodeStoreKey, BytecodeStoreItem> contentMap) {
        try {
            ...
            contentMap.put(key, new BytecodeStoreItem(contentFiles.toArray(File[]::new), enhancedTypeNames.toArray(String[]::new)));
        } catch (Throwable e) {
            return;
        }
    }
