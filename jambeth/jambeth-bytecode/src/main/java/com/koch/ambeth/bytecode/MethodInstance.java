...
public class MethodInstance {
	public static final String getSignature(java.lang.reflect.Method method) {
		try {
			java.lang.reflect.Method getGenericSignature =
					method.getClass().getDeclaredMethod("getGenericSignature");
	...
	public static MethodInstance findByTemplate(MethodInstance methodTemplate, boolean tryOnly) {
		...
		return findByTemplate(tryOnly, methodTemplate.getReturnType(), methodTemplate.getName(),
				methodTemplate.getParameters());
	}

	public static MethodInstance findByTemplate(boolean tryOnly, Class<?> returnType,
			String methodName, Class<?>... parameters) {
		return findByTemplate(tryOnly, Type.getType(returnType), methodName,
				TypeUtil.getClassesToTypes(parameters));
	}

	public static MethodInstance findByTemplate(boolean tryOnly, Type returnType, String methodName,
			Type... parameters) {
		...
		if (!currType.isInterface()) {
			while (currType != null && currType != Object.class) {
				java.lang.reflect.Method method =
						ReflectUtil.getDeclaredMethod(true, currType, returnType, methodName, parameters);
				if (method != null) {
		...
		throw new IllegalStateException("No method found on class hierarchy: " + methodName
	...
	public MethodInstance(Type owner, Class<?> declaringTypeOfMethod, Class<?> returnType,
			String methodName, Class<?>... parameters) {
		this(owner != null ? owner : Type.getType(declaringTypeOfMethod), ReflectUtil
				.getDeclaredMethod(false, declaringTypeOfMethod, returnType, methodName, parameters));
	}
	...
	public MethodInstance(Type owner, MethodInstance superMethod) {
		this(owner, superMethod.getAccess(), superMethod.getName(), superMethod.getSignature(),
				superMethod.getDescriptor());
	}

	public MethodInstance(Class<?> owner, int access, Class<?> returnType, String name,
			String signature, Class<?>... parameters) {
		this(owner != null ? Type.getType(owner) : null, access, Type.getType(returnType), name,
	...
	public MethodInstance(Type owner, int access, Type returnType, String name, String signature,
			Type... parameters) {
		super();
		this.owner = owner;
		this.access = access;
	...
	public MethodInstance deriveAccess(int access) {
		return new MethodInstance(getOwner(), access, getReturnType(), getName(), getSignature(),
				getParameters());
	}

	public MethodInstance deriveName(String methodName) {
		return new MethodInstance(getOwner(), getAccess(), getReturnType(), methodName, getSignature(),
				getParameters());
	}
	...
	public boolean equalsSignature(MethodInstance method) {
		return getName().equals(method.getName())
				&& Arrays.equals(getParameters(), method.getParameters());
	}
