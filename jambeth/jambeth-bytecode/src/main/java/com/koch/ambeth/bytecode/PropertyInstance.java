...
public class PropertyInstance {
	...
	public static PropertyInstance findByTemplate(IPropertyInfo propertyTemplate, boolean tryOnly) {
		return findByTemplate(propertyTemplate.getName(),
				Type.getType(propertyTemplate.getPropertyType()), tryOnly);
	}

	public static PropertyInstance findByTemplate(PropertyInstance propertyTemplate,
			boolean tryOnly) {
		return findByTemplate(propertyTemplate.getName(), propertyTemplate.getPropertyType(), tryOnly);
	}

	public static PropertyInstance findByTemplate(Class<?> declaringType, String propertyName,
			Class<?> propertyType, boolean tryOnly) {
		return findByTemplate(declaringType, propertyName, Type.getType(propertyType), tryOnly);
	}

	public static PropertyInstance findByTemplate(Class<?> declaringType, String propertyName,
			Type propertyType, boolean tryOnly) {
		Method getter = ReflectUtil.getDeclaredMethod(true, declaringType, propertyType,
				"get" + propertyName, new Type[0]);
		if (getter == null) {
			getter = ReflectUtil.getDeclaredMethod(true, declaringType, propertyType, "is" + propertyName,
					new Type[0]);
		}
		Method setter = ReflectUtil.getDeclaredMethod(true, declaringType, null, "set" + propertyName,
				propertyType);
		if (getter != null) {
			...
			return new PropertyInstance(getterInstance.getOwner(), propertyName, getterInstance,
					setterInstance);
		}
		else if (setter != null) {
		...
		if (getterOrSetter != null) {
			...
			if (getterOrSetterInstance.getParameters().length == 0) {
				return new PropertyInstance(getterOrSetterInstance.getOwner(), propertyName,
						getterOrSetterInstance, null);
			}
			return new PropertyInstance(getterOrSetterInstance.getOwner(), propertyName, null,
					getterOrSetterInstance);
		}
		if (tryOnly) {
			return null;
		}
		throw new IllegalArgumentException("No property found on class hierarchy: " + propertyName
				+ ". Start type: " + declaringType.getName());
	}

	public static PropertyInstance findByTemplate(String propertyName, Class<?> propertyType,
			boolean tryOnly) {
		return findByTemplate(propertyName, propertyType != null ? Type.getType(propertyType) : null,
				tryOnly);
	}

	public static PropertyInstance findByTemplate(String propertyName, Type propertyType,
			boolean tryOnly) {
	...
	public PropertyInstance(MethodPropertyInfo property) {
		this(Type.getType(property.getDeclaringType()), property.getName(),
				property.getGetter() != null ? new MethodInstance(property.getGetter()) : null,
	...
	public PropertyInstance(Type owner, String propertyName, MethodInstance getter,
