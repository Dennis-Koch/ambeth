...
public class Tuple4KeyHashMapTest {
	...
	protected Tuple4KeyHashMap<Integer, Long, Double, Float, Object> fillForTest() {
		Tuple4KeyHashMap<Integer, Long, Double, Float, Object> map =
				new Tuple4KeyHashMap<>();
		for (int a = 100; a-- > 20;) {
			map.put(Integer.valueOf(a + 20), Long.valueOf(a + 35), Double.valueOf(a + 40),
					Float.valueOf(a + 45), a + 15);
		}
		for (int a = existingKeys1.length; a-- > 0;) {
			map.put(existingKeys1[a], existingKeys2[a], existingKeys3[a], existingKeys4[a],
					existingValues[a]);
		}
		return map;
	}
	...
	public void init() {
		Tuple4KeyHashMap<Integer, Long, Double, Float, Object> map =
				new Tuple4KeyHashMap<>(100, 0.75f);
		map.init();
	}
	...
	public void containsKey() {
		...
		Assert.assertTrue(
				map.containsKey(existingKeys1[0], existingKeys2[0], existingKeys3[0], existingKeys4[0]));
		Assert.assertFalse(map.containsKey(nonExistingKeys1[0], nonExistingKeys2[0],
				nonExistingKeys3[0], nonExistingKeys4[0]));
	}
	...
	public void get() {
		...
		for (int a = existingKeys1.length; a-- > 0;) {
			Assert.assertEquals(existingValues[a],
					map.get(existingKeys1[a], existingKeys2[a], existingKeys3[a], existingKeys4[a]));
		}
	}
	...
	public void put() {
		...
		for (int a = existingKeys1.length; a-- > 0;) {
			Assert.assertTrue(
					map.containsKey(existingKeys1[a], existingKeys2[a], existingKeys3[a], existingKeys4[a]));
		}
	}
	...
	public void putIfNotExists() {
		...
		for (int a = nonExistingKeys1.length; a-- > 0;) {
			Assert.assertTrue(map.putIfNotExists(nonExistingKeys1[a], nonExistingKeys2[a],
					nonExistingKeys3[a], nonExistingKeys4[a], 1));
		}
		Assert.assertFalse(map.putIfNotExists(nonExistingKeys1[0], nonExistingKeys2[0],
				nonExistingKeys3[0], nonExistingKeys4[0], 2));
	...
	public void remove() {
		...
		for (int a = existingKeys1.length; a-- > 0;) {
			Assert.assertEquals(existingValues[a],
					map.remove(existingKeys1[a], existingKeys2[a], existingKeys3[a], existingKeys4[a]));
		}
	...
	public void serialize() throws Throwable {
		...
		}
		{
			...
			Tuple4KeyHashMap<Integer, Long, Double, Float, Object> cloneMap =
					(Tuple4KeyHashMap<Integer, Long, Double, Float, Object>) is.readObject();
			Assert.assertEquals(map.size(), cloneMap.size());

			for (Tuple4KeyEntry<Integer, Long, Double, Float, Object> entry : map) {
				Object cloneValue =
						cloneMap.remove(entry.getKey1(), entry.getKey2(), entry.getKey3(), entry.getKey4());
