...
/**
 * Abstract test class easing usage of IoC containers in test scenarios. Isolated modules can be
 * registered with the <code>TestModule</code> annotation. The test itself will be registered as a
 * bean within the IoC container. Therefore it can consume any components for testing purpose and
 * behave like a productively bean.
 *
 * In addition to registering custom modules the environment can be constructed for specific testing
 * purpose with the <code>TestProperties</code> annotation. Multiple properties can be wrapped using
 * the <code>TestPropertiesList</code> annotation.
 *
 * All annotations can be used on test class level as well as on test method level. In ambiguous
