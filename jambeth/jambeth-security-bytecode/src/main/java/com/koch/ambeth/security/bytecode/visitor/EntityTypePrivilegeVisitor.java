...
public class EntityTypePrivilegeVisitor extends ClassGenerator {
	...
	public EntityTypePrivilegeVisitor(ClassVisitor cv, IEntityMetaData metaData, Boolean create,
	...
	public void visitEnd() {
		...
		MethodInstance template_m_getPrimitivePropertyPrivilege =
				new MethodInstance(null, ITypePrivilege.class, ITypePropertyPrivilege.class,
		...
		MethodInstance template_m_getRelationPropertyPrivilege =
				new MethodInstance(null, ITypePrivilege.class, ITypePropertyPrivilege.class,
...

		{
			MethodGenerator mg = visitMethod(
					new MethodInstance(null, ITypePrivilege.class, Boolean.class, "isCreateAllowed"));
			mg.push(create);
			mg.returnValue();
			mg.endMethod();
		}
		{
			MethodGenerator mg = visitMethod(
					new MethodInstance(null, ITypePrivilege.class, Boolean.class, "isReadAllowed"));
			mg.push(read);
			mg.returnValue();
			mg.endMethod();
		}
		{
			MethodGenerator mg = visitMethod(
					new MethodInstance(null, ITypePrivilege.class, Boolean.class, "isUpdateAllowed"));
			mg.push(update);
			mg.returnValue();
			mg.endMethod();
		}
		{
			MethodGenerator mg = visitMethod(
					new MethodInstance(null, ITypePrivilege.class, Boolean.class, "isDeleteAllowed"));
			mg.push(delete);
			mg.returnValue();
			mg.endMethod();
		}
		{
			MethodGenerator mg = visitMethod(
					new MethodInstance(null, ITypePrivilege.class, Boolean.class, "isExecuteAllowed"));
			mg.push(execute);
			mg.returnValue();
			mg.endMethod();
		}
		{
			MethodGenerator mg = visitMethod(new MethodInstance(null, ITypePrivilege.class,
					ITypePropertyPrivilege.class, "getDefaultPropertyPrivilegeIfValid"));
			mg.pushNull();
			mg.returnValue();
			mg.endMethod();
		}
	...
	protected void implementConstructor(MethodInstance template_m_setPrimitivePropertyPrivilege,
		...
		try {
			constructor = state.getCurrentType().getDeclaredConstructor(Boolean.class, Boolean.class,
		...
		ConstructorInstance c_method =
				new ConstructorInstance(constructor).deriveAccess(Opcodes.ACC_PUBLIC);
	...
	protected void implementGetSetPropertyPrivilege(Member[] members,
		...
		for (int index = 0, size = members.length; index < size; index++) {
			...
			FieldInstance field = implementField(new FieldInstance(Opcodes.ACC_PRIVATE,
					getFieldName(member), null, ITypePropertyPrivilege.class));
			fields[index] = field;
		}
