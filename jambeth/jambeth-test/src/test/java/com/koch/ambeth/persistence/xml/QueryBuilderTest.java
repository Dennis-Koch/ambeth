...
public class QueryBuilderTest extends AbstractInformationBusWithPersistenceTest {
    ...
    public void testComplexQuery() {
        var queryBuilder = queryBuilderFactory.create(Employee.class);
        String paramName = "myParam";

        var query = queryBuilder.build(queryBuilder.let(queryBuilder.property("PrimaryProject.Employees.OtherAddresses.Street")).isEqualTo(queryBuilder.parameterValue(paramName), Boolean.FALSE));
        var employees = query.param(paramName, "TestStreet").retrieve();
