...
@TestPropertiesList({
        @TestProperties(name = ServiceConfigurationConstants.mappingFile, value = "PrefetchTest_orm.xml"),
...
public class PrefetchTest extends AbstractInformationBusWithPersistenceTest {
    ...
    public void optimizingPrefetch() throws Throwable {
        ...
        List<EntityA> allEntityAs = q_allEntityAs.param("Id", prefetchTestDataSetup.rootEntityA.getId()).retrieve();
        List<EntityD> allEntityDs = q_allEntityDs.param("Id", prefetchTestDataSetup.rootEntityD.getId()).retrieve();
    ...
    protected <T> IQuery<T> createQuery(Class<T> clazz) {
        ...
        return qb.build(qb.let(qb.property("Id")).isEqualTo(qb.parameterValue("Id")));
    }
}
