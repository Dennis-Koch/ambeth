...
public class PrefetchTestModule implements IInitializingModule {
	...
	public void afterPropertiesSet(IBeanContextFactory beanContextFactory) throws Throwable {
		IBeanConfiguration prefetchTestSensorReceiver =
				beanContextFactory.registerBean(DefaultStatementPerformanceLogger.class)
						.autowireable(IStatementPerformanceReport.class);
		beanContextFactory.link(prefetchTestSensorReceiver).to(ISensorReceiverExtendable.class)
				.with(JDBCResultSet.SENSOR_NAME);
		beanContextFactory.link(prefetchTestSensorReceiver).to(ISensorReceiverExtendable.class)
				.with(LogStatementInterceptor.SENSOR_NAME);

		IBeanConfiguration prefetchTestDataSetup = beanContextFactory
				.registerBean(PrefetchTestDataSetup.class).autowireable(PrefetchTestDataSetup.class);
