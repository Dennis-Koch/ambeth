...
public class AuditTamperTest extends AbstractInformationBusWithPersistenceTest {
	public static class AuditTamperTestModule implements IInitializingModule {
		...
		public void afterPropertiesSet(IBeanContextFactory beanContextFactory) throws Throwable {
			...
			IBeanConfiguration auditTamperDataBuilder = beanContextFactory
					.registerBean(AuditTamperDataBuilder.class).autowireable(AuditTamperDataBuilder.class);
			beanContextFactory.link(auditTamperDataBuilder).to(IDatasetBuilderExtendable.class);

			IBeanConfiguration tamperedAuditEntryWriter = beanContextFactory
					.registerBean(AuditEntryWriterV1.class);
			beanContextFactory.link(tamperedAuditEntryWriter).to(IAuditEntryWriterExtendable.class)
					.with(Integer.valueOf(TAMPERED_WRITER_VERSION));

		}
	}

	public static class AuditTamperDataBuilder extends AbstractDatasetBuilder
			implements IDataSetupWithAuthorization {
	...
	public void tamperAuditEntry_POSITIVE() throws Exception {
		...
		manipulateTables(new IBackgroundWorkerParamDelegate<Statement>() {
			@Override
			public void invoke(Statement stm) throws Exception {
				// intended blank
			}
		}, "AUDIT_ENTRY");
	...
	public void tamperAuditEntry_Signature() throws Exception {
		before();
		java.security.Signature signature = privateKeyProvider.getSigningHandle(
				auditTamperDataBuilder.defaultUser, AuditTamperDataBuilder.DEFAULT_PASSWORD.toCharArray());
		signature.update("other-signature".getBytes("UTF-8"));
		simpleManipulation("AUDIT_ENTRY",
				"\"SIGNATURE\"='" + Base64.encodeBytes(signature.sign()) + "'", auditEntry);
		verify(auditEntry, false);
	}
	...
	public void tamperAuditedEntity_ChangeType() throws Exception {
		before();
		simpleManipulation("AUDITED_ENTITY",
				"\"CHANGE_TYPE\"='" + AuditedEntityChangeType.UPDATE.name() + "'", auditedEntity);
		verify(auditEntry, false);
	}
	...
	public void tamperAuditedEntity_Signature() throws Exception {
		...
		signature.update("other-signature".getBytes("UTF-8"));
		simpleManipulation("AUDITED_ENTITY",
				"\"SIGNATURE\"='" + Base64.encodeBytes(signature.sign()) + "'", auditedEntity);
		verify(auditEntry, false);
	}
	...
	public void tamperAuditedEntity_Primitives() throws Exception {
		...
		AuditedEntityPrimitiveProperty primitive = entityFactory
				.createEntity(AuditedEntityPrimitiveProperty.class);
		IEntityMetaData metaData = entityMetaDataProvider
				.getMetaData(AuditedEntityPrimitiveProperty.class);
		metaData.getMemberByName(IAuditedEntityPrimitiveProperty.Entity).setValue(primitive,
				auditedEntity);
		metaData.getMemberByName(IAuditedEntityPrimitiveProperty.Name).setValue(primitive,
				"methodName");
		metaData.getMemberByName(IAuditedEntityPrimitiveProperty.NewValue).setValue(primitive, "Hallo");
		metaData.getMemberByName(IAuditedEntityPrimitiveProperty.Order).setValue(primitive,
				auditedEntity.getPrimitives().size() + 1);
	...
	public void tamperAuditedEntity_Relations() throws Exception {
		before();
		Short oldVersion = auditedEntity.getVersion();
		AuditedEntityRelationProperty relation = entityFactory
				.createEntity(AuditedEntityRelationProperty.class);
		AuditedEntityRelationPropertyItem item = entityFactory
				.createEntity(AuditedEntityRelationPropertyItem.class);
		...
		{
			IEntityMetaData metaData = entityMetaDataProvider
					.getMetaData(AuditedEntityRelationPropertyItem.class);
			metaData.getMemberByName(IAuditedEntityRelationPropertyItem.ChangeType).setValue(item,
					AuditedEntityPropertyItemChangeType.ADD);
		...
		{
			IEntityMetaData metaData = entityMetaDataProvider
					.getMetaData(AuditedEntityRelationProperty.class);
			metaData.getMemberByName(IAuditedEntityRelationProperty.Entity).setValue(relation,
					auditedEntity);
			metaData.getMemberByName(IAuditedEntityRelationProperty.Name).setValue(relation,
					"methodName");
			metaData.getMemberByName(IAuditedEntityRelationProperty.Order).setValue(relation,
					auditedEntity.getRelations().size() + 1);
		}
	...
	public void tamperAuditedEntityRef_EntityType() throws Exception {
		before();
		simpleManipulation("AUDITED_ENTITY_REF", "\"ENTITY_TYPE\"='" + getClass().getName() + "'",
				auditedEntityRef);
		verify(auditEntry, false);
	}
	...
	protected void simpleManipulation(String tableName, String sqlUpdateFragment,
			IAbstractAuditEntity entity) throws Exception {
		manipulateTables(createSimpleUpdate(tableName, sqlUpdateFragment, entity), tableName);
	}

	protected void simpleManipulation(final String[] tableNames, final String[] sqlUpdateFragments,
			final IAbstractAuditEntity entity) throws Exception {

		IBackgroundWorkerParamDelegate<Statement> delegate = new IBackgroundWorkerParamDelegate<Statement>() {
			@Override
			public void invoke(Statement state) throws Exception {
				for (int a = 0, size = tableNames.length; a < size; a++) {
					String tableName = tableNames[a];
					if (sqlUpdateFragments[a] == null) {
						continue;
					}
					IBackgroundWorkerParamDelegate<Statement> delegate = createSimpleUpdate(tableName,
							sqlUpdateFragments[a], entity);
	...
	protected IBackgroundWorkerParamDelegate<Statement> createSimpleUpdate(final String tableName,
			final String sqlUpdateFragment, final IAbstractAuditEntity entity) {
		return new IBackgroundWorkerParamDelegate<Statement>() {
			@Override
			public void invoke(Statement stm) throws Exception {
				Assert.assertEquals(1, stm.executeUpdate("UPDATE \"" + tableName + "\" SET "
						+ sqlUpdateFragment + " WHERE \"ID\"=" + entity.getId()));
			}
		};
	}
	...
	protected void manipulateTables(Connection connection,
			IBackgroundWorkerParamDelegate<Statement> delegate, String... tableNames) throws Exception {
		Statement stm = null;
		ResultSet rs = null;
		try {
			...
			for (String tableName : tableNameSet) {
				stm.execute(
						"ALTER TABLE \"" + tableName + "\" DISABLE TRIGGER \"TR_" + tableName + "_OL\"");
			}
			delegate.invoke(stm);
			connection.commit();
			for (String tableName : tableNameSet) {
	...
	protected void manipulateTables(IBackgroundWorkerParamDelegate<Statement> delegate,
