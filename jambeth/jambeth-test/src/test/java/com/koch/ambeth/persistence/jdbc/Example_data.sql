...
INSERT INTO "LINK_MATERIAL_UNIT" ("LEFT_ID", "RIGHT_ID") VALUES (
	(SELECT "ID" FROM "D_MATERIAL" WHERE NOT EXISTS 
		(SELECT "LEFT_ID" FROM "LINK_MATERIAL_UNIT" 
 		WHERE "LINK_MATERIAL_UNIT"."LEFT_ID" = "D_MATERIAL"."ID") FETCH FIRST 1 ROWS ONLY),
	(SELECT "ID" FROM (SELECT "ID" FROM "UNIT" ORDER BY dbms_random.value) sq1 FETCH FIRST 1 ROWS ONLY)
);
