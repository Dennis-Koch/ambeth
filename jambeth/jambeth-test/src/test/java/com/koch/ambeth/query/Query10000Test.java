...
public class Query10000Test extends AbstractInformationBusWithPersistenceTest {
    ...
    public void testRetrieveFetch2000() throws Exception {
        IQuery<QueryEntity> query = qb.build(qb.let(qb.property("Name1")).like(qb.parameterValue(paramName1)));
        List<QueryEntity> result = query.param(paramName1, "Q\\_%\\_Name%").retrieve();
    ...
    public void testRetrieveFetch1000() throws Exception {
        ...
        List<QueryEntity> result = query.retrieve();
        ...
        qb = queryBuilderFactory.create(QueryEntity.class);
        IQuery<QueryEntity> query2 = qb.build(qb.and(qb.let(qb.property("Name1")).isIn(qb.value(names)), qb.let(qb.property("Version")).isEqualTo(qb.value(3))));
        List<QueryEntity> result2 = query2.retrieve();
