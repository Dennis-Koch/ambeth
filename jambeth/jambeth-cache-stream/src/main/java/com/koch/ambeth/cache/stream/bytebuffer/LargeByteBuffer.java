...
public class LargeByteBuffer implements IByteBuffer {
	...
	public LargeByteBuffer(IFileContentCache fileContentCache, FileKey fileKey, long length,
			long virtualChunkSize) {
	...
	protected ByteBuffer getByteBuffer(long offset) {
		...
		if (chunkBuffer == null) {
			...
			lastByteBufferR = new WeakReference<>(chunkBuffer);
			lastChunk = chunk;
		}
		return chunkBuffer;
	}
