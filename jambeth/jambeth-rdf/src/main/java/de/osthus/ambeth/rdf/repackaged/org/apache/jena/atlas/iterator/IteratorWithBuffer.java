...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator;
...
/**
 * Iterator that delays output by N slots so you can react to the output before it's yielded. See also PeekIterator (which predates this code). See also
 * IteratorWithHistory for an iterator that remembers what it has yielded.
 * 
 * @see de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator.PeekIterator
 * @see de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator.PushbackIterator
...
public class IteratorWithBuffer<T> implements Iterator<T>
{
	...
	/**
	 * Look at elements that will be returned by a subsequnet call of .next(). The next element is index 0, then index 1 etc. This operation is valid
	 * immediately after the constructor returns. Returns null for no such element (underlying iterator didn't yeild enough elements). Throws
	 * IndexOutOfBoundsException if an attempt i smade to go beyond the buffering window.
	 */
