...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.impl.RuleClauseCode.CompileState.RuleClauseCodeList;
...
public class LPInterpreter
{
	 ...
	 *            the query to be satisfied
	 * @param isTop
	 *            true if this is a top level call from the outside iterator, false means it is an internal generator call which means we don't need to insert
	 *            an tabled call
	 */
	...
	public LPInterpreter(LPBRuleEngine engine, TriplePattern goal, List<RuleClauseCode> clauses, boolean isTop)
	{
		...
		// Construct dummy top environemnt which is a call into the clauses for
		// this goal
	...
	public Object next()
	{
		...
		{
			if (traceOn)
				logger.info("RETURN: " + topTMFrame.lastMatch);
	...
	/**
	 * Restore the current choice point and restart execution of the LP code until either find a successful branch (in which case exit with StateFlag.ACTIVE and
	 * variables bound to the correct results) or exhaust all choice points (in which case exit with StateFlag.FAIL and no bound results). In future tabled
	 * version could also exit with StateFlag.SUSPEND in cases whether the intepreter needs to suspend to await tabled results from a parallel proof tree.
	 */
	protected StateFlag run()
	{
		...
		main: while (cpFrame != null)
		{
			...
			if (cpFrame instanceof ChoicePointFrame)
			{
				...
				if (traceOn)
					logger.info("ENTER " + clause + " : " + getArgTrace());

				// then fall through into the recreated execution context for
				// the new call
			...
			else if (cpFrame instanceof TripleMatchFrame)
			{
				...
				if (!tmFrame.nextMatch(this))
				{
					...
					if (traceOn)
						logger.info("TRIPLE match (" + tmFrame.goal + ") -> FAIL");
					continue main;
				}
				if (traceOn)
				{
				...
				// then fall through to the execution context in which the the
				// match was called
			...
			else if (cpFrame instanceof TopLevelTripleMatchFrame)
			{
				...
				{
					// Match but this is the top level so return the triple
					// directly
					if (traceOn)
						logger.info("TRIPLE match (" + tmFrame.goal + ") ->");
			...
			else if (cpFrame instanceof ConsumerChoicePointFrame)
			{
				...
				if (state == StateFlag.FAIL)
				{
					...
					cpFrame = cpFrame.getLink();
					if (traceOn)
						logger.info("FAIL " + clause);
					continue main;
				}
				else if (state == StateFlag.SUSPEND)
				{
					// Require other generators to cycle before resuming this
					// one
					preserveState(ccp);
					iContext.notifyBlockedOn(ccp);
					cpFrame = cpFrame.getLink();
					if (traceOn)
						logger.info("SUSPEND " + clause);
					continue main;
				}
				...
				while (true)
				{
					switch (code[pc++])
					{
						...
						case RuleClauseCode.CALL_PREDICATE_INDEX:
							// This code path is experimental, don't yet know if it
							// has enough
							// performance benefit to justify the cost of
							// maintaining it.
							clauses = ((RuleClauseCodeList) args[ac++]).getList();
							// Check if we can futher index the clauses
							if (!argVars[2].isVariable())
							{
								clauses = engine.getRuleStore().codeFor(new TriplePattern(argVars[0], argVars[1], argVars[2]));
							}
						...
						case RuleClauseCode.CALL_WILD_TABLED:
							...
							{
								// normal call set up
								clauses = engine.getRuleStore().codeFor(new TriplePattern(argVars[0], predicate, argVars[2]));
								if (clauses != null)
									setupClauseCall(pc, ac, clauses, false);
								setupTripleMatchCall(pc, ac);
							}
						...
						case RuleClauseCode.PROCEED:
							...
							if (recordDerivations && envFrame.getRule() != null)
							{
								if (envFrame instanceof EnvironmentFrameWithDerivation)
								{
									...
									// Also want to record this result in the
									// calling frame
						...
						case RuleClauseCode.CALL_BUILTIN:
							...
							{
								if (traceOn)
									logger.info("FAIL " + clause + ", due to " + builtin.getName());
								continue main;
							}
							break;
			...
			// End of bytecode interpreter loop, gets to here if we complete an
			// AND chain
	...
	/**
	 * Standardize a node by replacing instances of wildcard ANY by new distinct variables. This is used in constructing the arguments to a top level call from
	 * a goal pattern.
	 * 
	 * @param node
	 *            the node to be standardized
	 * @param mappedVars
	 *            known mappings from input variables to local variables
	 */
