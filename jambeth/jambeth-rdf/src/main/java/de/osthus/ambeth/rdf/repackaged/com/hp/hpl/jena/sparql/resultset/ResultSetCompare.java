...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.query.ResultSetRewindable;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.binding.BindingUtils;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.NodeIsomorphismMap;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.NodeUtils;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.NodeUtils.EqualityTest;

/**
 * Comparison of ResultSets. Note that reading ResultSets is destructive so consiuder using {@linkplain ResultSetRewindable} from
 * {@linkplain ResultSetFactory#makeRewindable}
 */
...
public class ResultSetCompare
{
	...
	/*
	 * This is from the DAWG test suite. Result set 1: --------------- | x | y | =============== | _:b0 | _:b1 | | _:b2 | _:b3 | | _:b1 | _:b0 | ---------------
	 * Result set 2: --------------- | x | y | =============== | _:b1 | _:b0 | | _:b3 | _:b2 | | _:b2 | _:b3 | ---------------
	 */
	...
	/**
	 * Compare two result sets for equivalence. Equivalance means: A row rs1 has one matching row in rs2, and vice versa. A row is only matched once. Rows match
	 * if they have the same variables with the same values. bNodes must map to a consistent other bNodes. Value comparisons of nodes.
	 * 
	...
	/**
	 * compare two result sets for equivalence. Equivalance means: A row rs1 has one matching row in rs2, and vice versa. A row is only matched once. Rows match
	 * if they have the same variables with the same values, bNodes must map to a consistent other bNodes. Term comparisons of nodes.
	 * 
	 * Destructive - rs1 and rs2 are both read, possibly to exhaustion.
	 * 
	 * @param rs1
	...
	/**
	 * Compare two result sets for equivalence. Equivalance means: Each row in rs1 matches the same index row in rs2. Rows match if they have the same variables
	 * with the same values, bNodes must map to a consistent other bNodes. Value comparisons of nodes.
	 * 
	 * Destructive - rs1 and rs2 are both read, possibly to exhaustion.
	 * 
	 * @param rs1
	 ...
	 * with the same values, bNodes must map to a consistent other bNodes. RDF term comparisons of nodes.
	 * 
	 * Destructive - rs1 and rs2 are both read, possibly to exhaustion.
	 * 
	 * @param rs1
	...
	/**
	 * compare two result sets for exact equality equivalence. Exact equalitymeans: Each row in rs1 matches the same index row in rs2. Rows match if they have
	 * the same variables with the same values, bNodes must have same labels
	 * 
	 * Destructive - rs1 and rs2 are both read, possibly to exhaustion.
	 * 
	 * @param rs1
