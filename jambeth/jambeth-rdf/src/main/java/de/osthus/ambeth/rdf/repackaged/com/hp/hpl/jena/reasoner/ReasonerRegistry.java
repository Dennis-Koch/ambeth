...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.OWLFBRuleReasonerFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.OWLMicroReasonerFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.OWLMiniReasonerFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.RDFSRuleReasonerFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner.TransitiveReasonerFactory;

import java.util.*;

/**
 * A global registry of known reasoner modules. Modules are identified by a URI and can have associated capability and descriptive information, in RDF.
 * Currently the schema/ontology for such descriptions is mostly open. However, we do ensure that each reasoner URI at least has an associated rdf:type
 * statement to indicate that it is a reasoner.
 * <p>
...
public class ReasonerRegistry
{
	...
	public void register(ReasonerFactory factory)
	{
		...
		allDescriptions.createResource(factory.getURI()).addProperty(RDF.type, ReasonerVocabulary.ReasonerClass);
	}
	...
	public void register(String reasonerUri, ReasonerFactory factory)
	{
		...
		allDescriptions.createResource(reasonerUri).addProperty(RDF.type, ReasonerVocabulary.ReasonerClass);
	}

	/**
	 * Return a composite set of RDF capability descriptions for all registered reasoners. Listing all Resources of type ReasonerClass in the returned model
	 ...
	 * 
	 * @param uri
	 *            the URI of the reasoner
	 * @return a Resource representing the reasoner whose properties (as obtainable through listProperties etc) give a capability description of the reasoner;
	 ...
	 * 
	 * @param uri
	 *            the uri of the reasoner to be created, expressed as a simple string
	 * @param configuration
	 *            an optional set of configuration information encoded in RDF this parameter can be null if no configuration information is required.
	...
	/**
	 * Return a property Node which represents the direct version of a transitively closed property. See <code>makeDirect(String)</code>; this method makes a
	 * new Node with the direct respelling of <code>node</code>s URI.
	 */
	 ...
	 * 
	 * <p>
	 * Not clear what the right thing to do here is. Should not just invent a new local name in the same namespace because that might be a controlled namespace
	 * like RDF or RDFS. Can't even just extend the namespace slightly because that would be violating the web principles of namespace ownership. On the other
	 * hand, this solution results in staggeringly clumsy names.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * Prebuilt mini configuration for the default OWL reasoner. This omits bNode introduction rules which has significant performance gain in some cases and
