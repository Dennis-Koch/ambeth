...
/**
 * An implementation of a binding environment that maintains a single array of bound values for the variables in a rule. Stack management is done externally.
 * This is intended for use in the Brule system and so also supports variable-variable bindings by use of reference chains.
 */
public class BindingVector implements BindingEnvironment
{
	...
	/**
	 * Return the current array of bindings. Useful for fast access to serveral bindings, not useful for doing updates.
	 */
	...
	/**
	 * Return the most ground version of the node. If the node is not a variable just return it, if it is a varible bound in this enviroment return the binding,
	...
	/**
	 * Bind the ith variable in the current envionment to the given value. Checks that the new binding is compatible with any current binding. Handles aliased
	 * variables.
	 * 
	 ...
	 * 
	 * @param goal
	 *            the TriplePattern to be instantiated
	...
	{
		return new TriplePattern(getGroundVersion(goal.getSubject()), getGroundVersion(goal.getPredicate()), getGroundVersion(goal.getObject()));
	}
	...
	/**
	 * Unify a goal with the head of a rule. This is a poor-man's unification, we should try swtiching to a more conventional global-variables-with-trail
	 * implementation in the future.
	 * 
	 * @param goal
	 *            the goal pattern which it being matched to a rule
	 * @param head
	 *            the head pattern of the rule which is being instantiated
	 * @param numRuleVars
	 *            the length of the environment to allocate.
	 * @return An initialized binding environment for the rule variables or null if the unification fails. If a variable in the environment becomes aliased to
	 *         another variable through the unification this is represented by having its value in the environment be the variable to which it is aliased.
	 */
	...
	/**
	 * Unify a single pair of goal/head nodes. Unification of a head var to a goal var is recorded using an Integer in the head env to point to a goal env and
	 * storing the head var in the goal env slot.
	 * 
		...
		if (hNode instanceof Node_RuleVariable)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						// Already an aliased variable, so bind both this an the
						// alias
	...
	public boolean equals(Object o)
	{
		...
		if (environment.length != other.length)
			return false;
