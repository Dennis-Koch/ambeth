...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.datatypes.xsd.impl.*;
...
final/* public */class LiteralLabelImpl implements LiteralLabel
{
	...
	/**
	 * The value form of the literal. It will be null only if the value has not been parsed or if it is an illegal value. For plain literals and xsd:string
	 * literals the value is the same as the lexicalForm.
	 */
	...
	/**
	 * The xml:lang tag. For xsd literals this is ignored and not part of equality. For plain literals it is not ignored. The lang of a literal is fixed when it
	 * is created.
	 */
	/* final */private String lang;

	/**
	 * Indicates whether this is a legal literal. The working groups requires ill-formed literals to be treated as syntactically correct so instead of only
	 * storing well-formed literals we hack around it this way. N.B. This applies to any literal, not just XML well-formed literals.
	 */
	private boolean wellformed = true;

	/**
	 * keeps the message provided by the DatatypeFormatException if parsing failed for delayed exception thrown in getValue()
	 */
	...
	/**
	 * Build a typed literal label from its lexical form. The lexical form will be parsed now and the value stored. If the form is not legal this will throw an
	 * exception.
	 * 
	 * @param lex
	...
	private void setLiteralLabel_2(Object value, String language, RDFDatatype dtype) throws DatatypeFormatException
	{
		...
		{
			// We are creating a literal from a java object, check the lexical
			// form of the object is acceptable
	...
	/**
	 * Internal function to set the object value from the lexical form. Requires datatype to be set.
	 * 
	 * @throws DatatypeFormatException
	 *             if the value is ill-formed and eager checking is on.
	 */
	...
	/**
	 * Normalize the literal. If the value is narrower than the current data type (e.g. value is xsd:date but the time is xsd:datetime) it will narrow the type.
	 * If the type is narrower than the value then it may normalize the value (e.g. set the mask of an XSDDateTime)
	 */
	...
	{
		return isXML() ? this : !lang.equals("") ? getLexicalForm() + "@" + lang.toLowerCase(Locale.ENGLISH) : wellformed ? getValue() : getLexicalForm();
	}
	...
	public Object getValue() throws DatatypeFormatException
	{
		...
		{
			throw new DatatypeFormatException(lexicalForm, dtype, exceptionMsg);
		}
	}
	...
	public boolean equals(Object other)
	{
		...
		boolean typeEqual = (dtype == null ? otherLiteral.getDatatype() == null : dtype.equals(otherLiteral.getDatatype()));
		boolean langEqual = (dtype == null ? lang.equals(otherLiteral.language()) : true);
		return typeEqual && langEqual && getLexicalForm().equals(otherLiteral.getLexicalForm());
	}
	...
	public boolean sameValueAs(LiteralLabel other)
	{
		...
		return dtype == null ? isPlainLiteralSameValueAsOther(other) : isTypedLiteralSameValueAsOther(other);
	}

	/**
	 * Need to support comparison of ill-formed literals in order for the WG tests on ill formed literals to be testable using isIsomorphic to. "same value
	 * as" on ill-formed literals is defined to mean "same lexical form and same [ignoring case] language code".
	 * <p>
	 * precondition: at least one of <i>this</i> and <i>other</i> is ill-formed.
	 */
	...
	/**
	 * Answer true iff <i>this</i> and <i>other</i> have the same lexical form and [ignoring case] language code. [Note: both typed literals and plain literals
	 * have the empty string as language code.]
	 */
	private boolean sameByFormAndLanguage(LiteralLabel other)
	{
		String lex1 = getLexicalForm();
		String lex2 = other.getLexicalForm();
		return lex1.equals(lex2) && lang.equalsIgnoreCase(other.language());
	}

	private boolean isTypedLiteralSameValueAsOther(LiteralLabel other)
	{
		return other.getDatatype() == null ? looksLikePlainString(this) && sameByFormAndLanguage(other) : dtype.isEqual(this, other);
	}

	private boolean isPlainLiteralSameValueAsOther(LiteralLabel other)
	{
		return other.getDatatype() == null || looksLikePlainString(other) ? sameByFormAndLanguage(other) : false;
	}

	/**
	 * Answer true iff <i>L</i> has type XSD string and we're treating plain literals and xsd string literals as "the same".
	 */
