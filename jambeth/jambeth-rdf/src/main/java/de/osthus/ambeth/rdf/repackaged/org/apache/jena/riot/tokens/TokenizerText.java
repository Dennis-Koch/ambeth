...
import static de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.Chars.*;
import static de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.RiotChars.*;

import java.util.NoSuchElementException;

import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.AtlasException;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.RiotParseException;
...
public final class TokenizerText implements Tokenizer
{
	...
	private Token parseToken()
	{
		...
		/*
		 * [16] integer ::= ('-' | '+') ? [0-9]+ [17] double ::= ('-' | '+') ? ( [0-9]+ '.' [0-9]* exponent | '.' ([0-9])+ exponent | ([0-9])+ exponent ) 0.e0,
		 * .0e0, 0e0 [18] decimal ::= ('-' | '+')? ( [0-9]+ '.' [0-9]* | '.' ([0-9])+ | ([0-9])+ ) 0.0 .0 0. [19] exponent ::= [eE] ('-' | '+')? [0-9]+ [] hex
		 * ::= 0x0123456789ABCDEFG
		 */
	...
	/*
	 * The token rules from SPARQL and Turtle. PNAME_NS ::= PN_PREFIX? ':' PNAME_LN ::= PNAME_NS PN_LOCAL
	 * 
	 * PN_CHARS_BASE ::= [A-Z] | [a-z] | [#x00C0-#x00D6] | [#x00D8-#x00F6] | [#x00F8-#x02FF] | [#x0370-#x037D] | [#x037F-#x1FFF] | [#x200C-#x200D] |
	 * [#x2070-#x218F] | [#x2C00-#x2FEF] | [#x3001-#xD7FF] | [#xF900-#xFDCF] | [#xFDF0-#xFFFD] | [#x10000-#xEFFFF] PN_CHARS_U ::= PN_CHARS_BASE | '_' PN_CHARS
	 * ::= PN_CHARS_U | '-' | [0-9] | #x00B7 | [#x0300-#x036F] | [#x203F-#x2040]
	 * 
	 * PN_PREFIX ::= PN_CHARS_BASE ((PN_CHARS|'.')* PN_CHARS)? PN_LOCAL ::= (PN_CHARS_U | ':' | [0-9] | PLX ) ((PN_CHARS | '.' | ':' | PLX)* (PN_CHARS | ':' |
	 * PLX) )? PLX ::= PERCENT | PN_LOCAL_ESC PERCENT ::= '%' HEX HEX HEX ::= [0-9] | [A-F] | [a-f] PN_LOCAL_ESC ::= '\' ( '_' | '~' | '.' | '-' | '!' | '$' |
	 * '&' | "'" | '(' | ')' | '*' | '+' | ',' | ';' | '=' | '/' | '?' | '#' | '@' | '%' )
	 */
	...
	/*
	 * [146] INTEGER ::= [0-9]+ [147] DECIMAL ::= [0-9]* '.' [0-9]+ [148] DOUBLE ::= [0-9]+ '.' [0-9]* EXPONENT | '.' ([0-9])+ EXPONENT | ([0-9])+ EXPONENT []
	...
	private void readNumber()
	{
		...
		/*
		 * maybeSign() digits() if dot ==> decimal, digits if e ==> double, maybeSign, digits else check not "." for decimal.
		 */
		...
		/*
		 * readPossibleSign(stringBuilder) ; readDigits may be hex readDot readDigits readExponent.
		 */
