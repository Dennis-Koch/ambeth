...
/**
 * A FileGraph is a memory-based graph that is optionally read in from a file when it is created, and is written back when it is closed. It supports (weak)
 * transactions by using checkpoint files.
 */
public class FileGraph extends GraphMem
{
	...
	/**
	 * Construct a new FileGraph who's name is given by the specified File, If create is true, this is a new file, and any existing file will be destroyed; if
	 * create is false, this is an existing file, and its current contents will be loaded. The language code for the file is guessed from its suffix.
	 * 
	 * @param f
	 *            the File naming the associated file-system file
	 * @param create
	 *            true to create a new one, false to read an existing one
	 * @param strict
	 *            true to throw exceptions for create: existing, open: not found
	 */
	...
	/**
	 * Construct a new FileGraph who's name is given by the specified File, If create is true, this is a new file, and any existing file will be destroyed; if
	 * create is false, this is an existing file, and its current contents will be loaded. The language code for the file is supplied.
	 * 
	 * @param f
	 *            the File naming the associated file-system file
	 * @param lang
	 *            the language string for the file
	 * @param create
	 *            true to create a new one, false to read an existing one
	 * @param strict
	 *            true to throw exceptions for create: existing, open: not found
	...
	/**
	 * Answer true iff the filename string given is plausibly the name of a graph, ie, may have RDF content. We appeal to FileUtils - if it can guess an RDF
	 * language name, we deliver true, otherwise false.
	 * 
	 * @param name
	 *            the leaf component of a filename
	...
	/**
	 * The graph is written out to the named file in the language guessed from the suffix, and then the parent close is invoked. The write-out goes to an
	 * intermediate file first, which is then renamed to the correct name, to try and ensure that the output is either done completely or not at all.
	 */
	...
	/**
	 * The file intermediate has the new file contents. We want to move them to the current file. renameTo doesn't have a powerful enough semantics, so we
	 * anticipate failure and attempt to bypass it ...
	 * <p>
	...
	{
		if (th == null)
			th = new FileGraphTransactionHandler(this);
		return th;
	}
