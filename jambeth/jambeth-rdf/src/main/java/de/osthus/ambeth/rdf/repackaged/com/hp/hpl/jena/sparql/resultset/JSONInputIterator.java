...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator.PeekIterator;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.iterator.QueryIteratorBase;
 ...
 * <p>
 * Creating the Iterator automatically causes it to parse a small chunk of the stream to determine the variables in the result set either by reading the header
 * or reading some portion of the results if the results appear before the header since JSON does not guarantee the order of keys within an object
 * </p>
 */
public class JSONInputIterator extends QueryIteratorBase
{
	 ...
	 * <p>
	 * If the header is encountered first then we read this, if the results are encountered first we parse the first 100 results and determine the variables
	 * present from those instead
	 * </p>
	 */
