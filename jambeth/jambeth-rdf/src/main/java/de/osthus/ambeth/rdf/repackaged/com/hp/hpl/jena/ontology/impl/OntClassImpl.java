...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.EnhGraph;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.EnhNode;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.Implementation;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.iterator.UniqueFilter;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.iterator.WrappedIterator;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.vocabulary.OWL;
...
public class OntClassImpl extends OntResourceImpl implements OntClass
{
	...
	/**
	 * A factory for generating OntClass facets from nodes in enhanced graphs. Note: should not be invoked directly by user code: use
	...
	public static Implementation factory = new Implementation()
	{
		...
		{
			// node will support being an OntClass facet if it has rdf:type
	...
	/**
	 * <p>
	 * Assert that this class is sub-class of the given class. Any existing statements for <code>subClassOf</code> will be removed.
	 * </p>
	 * 
	 * @param cls
	 *            The class that this class is a sub-class of
	 * @exception ProfileException
	 *                If the {@link Profile#SUB_CLASS_OF()} property is not supported in the current language profile.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer an iterator over all of the classes that are declared to be super-classes of this class. Each element of the iterator will be an {@link OntClass}.
	 * </p>
	 * 
	 ...
	 * <p>
	 * Answer an iterator over all of the classes that are declared to be super-classes of this class. Each element of the iterator will be an {@link OntClass}.
	 * See {@link #listSubClasses(boolean )} for a full explanation of the <em>direct</em> parameter.
	 * </p>
	 * 
	 * @param direct
	 *            If true, only answer the direcly adjacent classes in the super-class relation: i&#046;e&#046; eliminate any class for which there is a longer
	 *            route to reach that child under the super-class relation.
	...
	{
		return listDirectPropertyValues(getProfile().SUB_CLASS_OF(), "SUB_CLASS_OF", OntClass.class, getProfile().SUB_CLASS_OF(), direct, false).filterDrop(
				new SingleEqualityFilter<OntClass>(this)).filterKeep(new UniqueFilter<OntClass>());
	}
	...
	/**
	 * <p>
	 * Answer true if the given class is a super-class of this class. See {@link #listSubClasses(boolean )} for a full explanation of the <em>direct</em>
	 * parameter.
	 * </p>
	 * 
	 * @param cls
	 *            A class to test.
	 * @param direct
	 *            If true, only search the classes that are directly adjacent to this class in the class hierarchy.
	 * @return True if the given class is a super-class of this class.
	 * @exception ProfileException
	 *                If the {@link Profile#SUB_CLASS_OF()} property is not supported in the current language profile.
	 */
	...
	public boolean hasSuperClass(Resource cls, boolean direct)
	{
		...
		else
		{
			...
			{
				// otherwise, not an inf-graph or the given inf-graph does not
				// support direct directly (:-)
	 ...
	 * @exception ProfileException
	 *                If the {@link Profile#SUB_CLASS_OF()} class is not supported in the current language profile.
	 */
	@Override
	...
	/**
	 * <p>
	 * Assert that this class is super-class of the given class. Any existing statements for <code>subClassOf</code> on <code>prop</code> will be removed.
	 * </p>
	 * 
	 * @param cls
	...
	/**
	 * <p>
	 * Answer a class that is the sub-class of this class. If there is more than one such class, an arbitrary selection is made. If there is no such class,
	 * return null.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Answer an iterator over all of the classes that are declared to be sub-classes of this class. Each element of the iterator will be an {@link OntClass}.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Answer an iterator over the classes that are declared to be sub-classes of this class. Each element of the iterator will be an {@link OntClass}. The
	 * distinguishing extra parameter for this method is the flag <code>direct</code> that allows some selectivity over the classes that appear in the iterator.
	 * Consider the following scenario: <code><pre>
	 *   :B rdfs:subClassOf :A.
	 *   :C rdfs:subClassOf :A.
	 *   :D rdfs:subClassof :C.
	 * </pre></code> (so A has two sub-classes, B and C, and C has sub-class D). In a raw model, with no inference support, listing the sub-classes of A will
	 * answer B and C. In an inferencing model, <code>rdfs:subClassOf</code> is known to be transitive, so the sub-classes iterator will include D. The
	 * <code>direct</code> sub-classes are those members of the closure of the subClassOf relation, restricted to classes that cannot be reached by a longer
	 * route, i.e. the ones that are <em>directly</em> adjacent to the given root. Thus, the direct sub-classes of A are B and C only, and not D - even in an
	 * inferencing graph. Note that this is not the same as the entailments from the raw graph. Suppose we add to this example: <code><pre>
	 *   :D rdfs:subClassof :A.
	 * </pre></code> Now, in the raw graph, A has sub-class C. But the direct sub-classes of A remain B and C, since there is a longer path A-C-D that means
	 * that D is not a direct sub-class of A. The assertion in the raw graph that A has sub-class D is essentially redundant, since this can be inferred from
	 * the closure of the graph.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> This is is a change from the behaviour of Jena 1, which took a parameter <code>closed</code> to compute the closure over
	 * transitivity and equivalence of sub-classes. The closure capability in Jena2 is determined by the inference engine that is wrapped with the ontology
	 * model. The direct parameter is provided to allow, for exmaple, a level-by-level traversal of the class hierarchy, starting at some given root.
	 * </p>
	 * 
	 * @param direct
	 *            If true, only answer the direcly adjacent classes in the sub-class relation: i&#046;e&#046; eliminate any class for which there is a longer
	...
	{
		return listDirectPropertyValues(getProfile().SUB_CLASS_OF(), "SUB_CLASS_OF", OntClass.class, getProfile().SUB_CLASS_OF(), direct, true).filterDrop(
				new SingleEqualityFilter<OntClass>(this)).filterKeep(new UniqueFilter<OntClass>());
	}
	...
	/**
	 * <p>
	 * Answer true if the given class is a sub-class of this class. See {@link #listSubClasses(boolean )} for a full explanation of the <em>direct</em>
	 * parameter.
	 * </p>
	 * 
	 * @param cls
	...
	/**
	 * <p>
	 * Assert that the given class is equivalent to this class. Any existing statements for <code>equivalentClass</code> will be removed.
	 * </p>
	 * 
	 * @param cls
	 *            The class that this class is a equivalent to.
	 * @exception ProfileException
	 *                If the {@link Profile#EQUIVALENT_CLASS()} property is not supported in the current language profile.
	 */
	@Override
	...
	/**
	 * <p>
	 * Assert that this class is disjoint with the given class. Any existing statements for <code>disjointWith</code> will be removed.
	 * </p>
	 * 
	 * @param cls
	 *            The property that this class is disjoint with.
	 * @exception ProfileException
	 *                If the {@link Profile#DISJOINT_WITH()} property is not supported in the current language profile.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer an iterator over all of the classes that this class is declared to be disjoint with. Each element of the iterator will be an {@link OntClass}.
	 * </p>
	 * 
	...
	/**
	 * Equivalent to calling {@link #listDeclaredProperties(boolean)} with default value <code>direct = false</code>.
	 * 
	 */
	@Override
	...
	/**
	 * <p>
	 * Return an iterator over the properties associated with a frame-like view of this class. This captures an intuitive notion of the <em>
	 * properties of a class</em>. This can be useful in presenting an ontology class in a user interface, for example by automatically constructing a form to
	 * instantiate instances of the class. The properties in the frame-like view of the class are determined by comparing the domain of properties in this
	 * class's {@link OntModel} with the class itself. See: <a href="/documentation/notes/rdf-frames.html">Presenting RDF as frames</a> for more details.
	 * </p>
	 * <p>
	 * Note that many cases of determining whether a property is associated with a class depends on RDFS or OWL reasoning. This method may therefore return
	 * complete results only in models that have an attached reasoner.
	 * </p>
	 * 
	 * @param direct
	 *            If <code>true</code>, restrict the properties returned to those directly associated with this class. If <code>false</code>, the properties of
	 *            super-classes of this class will not be listed among the declared properties of this class.
	 * @return An iteration of the properties that are associated with this class by their domain.
	 */
	@Override
	public ExtendedIterator<OntProperty> listDeclaredProperties(boolean direct)
	{
		...
		// if the attached model does inference, it will potentially find more
		// of these
		...
		// now we iterate over the candidates and check that they match all
		// domain constraints
	...
	/**
	 * <p>
	 * Answer true if the given property is one of the declared properties of this class. For details, see {@link #listDeclaredProperties(boolean)}.
	 * </p>
	 * 
	 * @param p
	 *            A property to test
	 * @param direct
	 *            If true, only direct associations between classes and properties are considered
	 * @return True if <code>p</code> is one of the declared properties of this class
	 */
	@Override
	 ...
	 * <p>
	 * 
	 * @return An iterator over those instances that have this class as one of the classes to which they belong
	 */
	@Override
	 ...
	 * 
	 * @param direct
	 *            If true, only direct instances are counted (i.e. not instances of sub-classes of this class)
	 * @return An iterator over those instances that have this class as one of the classes to which they belong
	 */
	@Override
	public ExtendedIterator<Individual> listInstances(final boolean direct)
	{
		return getModel().listStatements(null, RDF.type, this).mapWith(new SubjectAsMapper<>(Individual.class)).filterKeep(new Filter<Individual>()
		{
			@Override
	...
	/**
	 * <p>
	 * Answer a new individual that has this class as its <code>rdf:type</code>
	 * </p>
	 * 
	 ...
	 * {@link Individual#removeOntClass} method, but invoked via the class resource rather than via the individual resource.
	 * </p>
	 * 
	 * @param individual
	...
	/**
	 * <p>
	 * Answer true if this class is one of the roots of the class hierarchy. This will be true if either (i) this class has <code>owl:Thing</code> as a direct
	 * super-class, or (ii) it has no declared super-classes (including anonymous class expressions).
	 * </p>
	 * 
	 * @return True if this class is the root of the class hierarchy in the model it is attached to
	 */
	@Override
	public boolean isHierarchyRoot()
	{
		...
		/**
		 * Note: moved the initialisation of i outside the try-catch, otherwise an exception in listSuperClasses [eg a broken Graph implementation] will avoid
		 * i's initialisation but still run i.close, generating a mysterious NullPointerException. Signed, Mr Burnt Spines.
		 */
	 ...
	 *                if the class cannot be converted to an enumerated class given the lanuage profile and the current state of the underlying model.
	 */
	@Override
	 ...
	 * @exception ConversionException
	 *                if the class cannot be converted to a union class given the lanuage profile and the current state of the underlying model.
	 */
	@Override
	 ...
	 * @exception ConversionException
	 *                if the class cannot be converted to an intersection class given the lanuage profile and the current state of the underlying model.
	 */
	@Override
	 ...
	 * @exception ConversionException
	 *                if the class cannot be converted to a complement class given the lanuage profile and the current state of the underlying model.
	 */
	@Override
	 ...
	 * @exception ConversionException
	 *                if the class cannot be converted to a restriction given the lanuage profile and the current state of the underlying model.
	 */
	@Override
	 ...
	 * 
	 * @param individuals
	 *            A list of the individuals that will comprise the permitted values of this class converted to an enumeration
	...
	/**
	 * <p>
	 * Answer true if this class has the given class as a direct super-class, without using extra help from the reasoner.
	 * </p>
	 * 
	 * @param cls
	 *            The class to test
	...
	{
		// we manually compute the maximal lower elements - this could be
		// expensive in general
		// return ResourceUtils.maximalLowerElements( listSuperClasses(),
		// getProfile().SUB_CLASS_OF(), false ).contains( cls );

		ExtendedIterator<OntClass> i = listDirectPropertyValues(getProfile().SUB_CLASS_OF(), "subClassOf", OntClass.class, getProfile().SUB_CLASS_OF(), true,
				false);
		try
		{
			while (i.hasNext())
	...
	/**
	 * <p>
	 * Answer true if this class lies with the domain of p
	 * <p>
	 * 
	 * @param p
	 * @param direct
	...
	protected boolean testDomain(Property p, boolean direct)
	{
		...
		for (StmtIterator i = getModel().listStatements(p, getProfile().DOMAIN(), (RDFNode) null); i.hasNext();)
		{
			...
			if (!(domain.equals(getProfile().THING()) || domain.equals(RDFS.Resource)))
			{
				...
				{
					// if this class is actually in the domain (as opposed to
					// one of this class's
					// super-classes), then we've detected the direct property
					// case
					seenDirect = true;
				}
				else if (!canProveSuperClass(domain))
				{
					// there is a class in the domain of p that is not a
		...
		if (direct)
		{
			// if we're looking for direct props, we must either have seen the
			// direct case
	...
	/**
	 * <p>
	 * Answer an iterator over all of the properties in this model
	 * 
	...
	protected ExtendedIterator<Statement> listAllProperties()
	{
		...
		// check reasoner capabilities - major performance improvement for inf
		// models
		if (mOnt.getReasoner() != null)
		{
			...
			{
				// we conclude that the reasoner can do the necessary work to
				// infer that
				// all owl:ObjectProperty, owl:DatatypeProperty, etc, are
				// rdf:Property resources
				return pi;
			}
		}
	...
	/**
	 * <p>
	 * Answer true if we can demonstrate that this class has the given super-class. If this model has a reasoner, this is equivalent to asking if the sub-class
	 * relation holds. Otherwise, we simulate basic reasoning by searching upwards through the class hierarchy.
	 * </p>
	 * 
	 * @param sup
	 *            A super-class to test for
	...
	protected boolean canProveSuperClass(Resource sup)
	{
		...
		if (om.getReasoner() != null)
		{
			...
			{
				// this reasoner does transitive closure on sub-classes, so we
				// just ask
