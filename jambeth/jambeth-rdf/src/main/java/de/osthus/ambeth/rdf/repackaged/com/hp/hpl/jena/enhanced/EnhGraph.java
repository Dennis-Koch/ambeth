...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.cache.*;

/**
 * TODO: remove the polymorphic aspect of EnhGraphs.
 * <p>
 * A specialisation of Polymorphic that models an extended graph - that is, one that contains{@link EnhNode Enhanced nodes} or one that itself exposes
 * additional capabilities beyond the graph API.
 * <p>
 * <span style="color:red">WARNING</span>. The polymorphic aspects of EnhGraph are <span style="color:red">not supported</span> and are not expected to be
...
// extends Polymorphic
{
	 ...
	 * 
	 * @param g
	 *            The underlying plain graph, may be null to defer binding to a given graph until later.
	 * @param p
	 *            The personality factory, that maps types to realisations
	 */
	...
	/**
	 * An enhanced graph is equal to another graph g iff the underlying graphs are equal. This is deemed to be a complete and correct interpretation of enhanced
	 * graph equality, which is why this method has been marked final.
	 * <p>
	 * Note that this equality test does not look for correspondance between the structures in the two graphs. To test whether another graph has the same nodes
	 * and edges as this one, use {@link #isIsomorphicWith}.
	 * </p>
	 * 
	 * @param o
	...
	/**
	 * Answer true if the given enhanced graph contains the same nodes and edges as this graph. The default implementation delegates this to the underlying
	 * graph objects.
	 * 
	 * @param eg
	 *            A graph to test
	 ...
	 * 
	 * @param n
	 *            A node (assumed to be in this graph)
	 * @param interf
	...
	//
	// /**
	// * Answer an enhanced graph that presents <i>this</i> in a way which
	// satisfies type
	// * t. This is a stub method that has not yet been implemented.
	// @param t A type
	// @return A polymorphic instance, possibly but not necessarily this, that
	// conforms to t.
	// */
