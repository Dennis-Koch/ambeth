...
package de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.http;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.auth.SimpleAuthenticator;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.*;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.web.HttpOp;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.Triple;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.query.*;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.ResultSetCheckCondition;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.graph.GraphFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.resultset.CSVInput;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.resultset.JSONInput;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.resultset.TSVInput;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.resultset.XMLInput;
...
public class QueryEngineHTTP implements QueryExecution
{
	 ...
	 */
	public static String[] supportedSelectContentTypes = new String[] { WebContent.contentTypeResultsXML, WebContent.contentTypeResultsJSON,
			WebContent.contentTypeTextTSV, WebContent.contentTypeTextCSV };
	/**
	 * Supported content types for ASK queries
	 */
	public static String[] supportedAskContentTypes = new String[] { WebContent.contentTypeResultsXML, WebContent.contentTypeJSON,
			WebContent.contentTypeTextTSV, WebContent.contentTypeTextCSV };
	...
	/**
	 * <p>
	 * Helper method which applies configuration from the Context to the query engine if a service context exists for the given URI
	 * </p>
	 * <p>
	 * Based off proposed patch for JENA-405 but modified to apply all relevant configuration, this is in part also based off of the private
	 * {@code configureQuery()} method of the {@link Service} class though it omits parameter merging since that will be done automatically whenever the
	 * {@link QueryEngineHTTP} instance makes a query for remote submission.
	 * </p>
	 * 
	 * @param serviceURI
	 *            Service URI
	 ...
	 * <p>
	 * Even if this returns false authentication may still be used if the default authenticator applies, this is controlled via the
	...
	/**
	 * Set user and password for basic authentication. After the request is made (one of the exec calls), the application can overwrite the password array to
	 * remove details of the secret.
	 * <p>
	...
	/**
	 * Sets the HTTP authenticator to use, if none is set then the default authenticator is used. This may be configured via the
	 * {@link HttpOp#setDefaultAuthenticator(HttpAuthenticator)} method.
	 * 
	 * @param authenticator
	 *            HTTP authenticator
