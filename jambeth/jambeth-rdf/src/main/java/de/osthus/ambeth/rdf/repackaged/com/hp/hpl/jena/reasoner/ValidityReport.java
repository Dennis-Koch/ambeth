...
/**
 * Data structure used to report the results of validation or consistency checking operations. It is an array of reports, each of which has a severity, a type
 * (string) and a description (string).
 */
public interface ValidityReport
{

	/**
	 * Returns true if no logical inconsistencies were detected. If it is false then ether will be at least one error Report included. If it is true then
	 * warnings may still be present. As of Jena 2.2 we regard classes which can't be instantiated as warnings (of type 'Inconsistent class') rather than
	 * errors.
	 */
	...
	static class Report
	{
		 ...
		 * 
		 * @param error
		 *            true if the report is an error, false if it is just a warning
		 * @param type
		...
		/**
		 * @return a reasoner dependent data structure giving more information on the problem.
		 */
