...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.ClauseEntry;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.Functor;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.Node_RuleVariable;
 ...
 * <p>
 * Wildcards are recorded by using Node_RuleVariable entries rather than nulls because they can be named. If a null is specified that is converted to a variable
 * of name "". Note that whilst some engines might simply require Node_Variables the forward engine requires variables represented using the more specialized
...
public class TriplePattern implements ClauseEntry
{
	...
	/**
	 * Constructor - builds a pattern from three nodes, use Node_RuleVariables as variables, use a variable with an empty name as a wildcard, can also use null
	 * as a wildcard.
	 */
	...
	/**
	 * Constructor - builds a pattern from a standard triple match. Node that any filter part of the triple match will not be represented within the pattern and
	 * will need to be checked for separately.
	 */
	...
	{
		return Triple.createMatch(toMatch(subject), toMatch(predicate), toMatch(object));
	}
	...
	/**
	 * Compare two patterns for compatibility - i.e. potentially unifiable. Two patterns are "compatible" in the sense we mean here if all their ground terms
	 * match. A variable in either pattern can match a ground term or a variable in the other. We are not, currently, checking for multiple occurances of the
	 * same variable. Functor-valued object literals are treated as a special case which are only checked for name/arity matching.
	 */
	...
	/**
	 * Test if a pattern is just a variant of this pattern. I.e. it is the same up to variable renaming. This takes into account multiple occurances of the same
	 * variable.
	 */
	...
	/**
	 * Check a pattern to see if it is legal, used to exclude backchaining goals that could never be satisfied. A legal pattern cannot have literals in the
	 * subject or predicate positions and is not allowed nested functors in the object.
	 */
	public boolean isLegal()
	{
		if (subject.isLiteral() || predicate.isLiteral())
			return false;
		if (Functor.isFunctor(subject))
			return false;
	...
	{
		return (subject.isVariable() || subject.equals(arg.subject)) && (predicate.isVariable() || predicate.equals(arg.predicate))
	...
	{
		return simplePrintString(subject) + " @" + simplePrintString(predicate) + " " + simplePrintString(object);
	}
	...
	{
		return simplePrintString(t.getSubject()) + " @" + simplePrintString(t.getPredicate()) + " " + simplePrintString(t.getObject());
	}
	...
	/**
	 * Convert any Node_RuleVariable wildcards to null. This loses the variable named but is used when converting a singleton pattern to a TripleMtch
	 */
	...
	public boolean equals(Object o)
	{
		...
		return o instanceof TriplePattern && nodeEqual(subject, ((TriplePattern) o).subject) && nodeEqual(predicate, ((TriplePattern) o).predicate)
	...
	public boolean sameAs(Object o)
	{
		...
		return Node_RuleVariable.sameNodeAs(subject, other.subject) && Node_RuleVariable.sameNodeAs(predicate, other.predicate)
				&& Node_RuleVariable.sameNodeAs(object, other.object);
	}
