...
/**
 * An ArrayBunch implements TripleBunch with a linear search of a short-ish array of Triples. The array can grow, but it only grows by 4 elements each time
 * (because, if it gets big enough for this linear growth to be bad, it should anyways have been replaced by a more efficient set-of-triples implementation).
 */
public class ArrayBunch implements TripleBunch
{
	...
	/**
	 * Note: linear growth is suboptimal (order n<sup>2</sup>) normally, but ArrayBunch's are meant for <i>small</i> sets and are replaced by some sort of hash-
	 * or tree- set when they get big; currently "big" means more than 9 elements, so that's only one growth spurt anyway.
	 */
	protected void grow()
	{
	...
	public ExtendedIterator<Triple> iterator(final HashCommon.NotifyEmpty container)
	{
		...
		// for (int j = 0; j < size; j += 1) System.err.println( "==    " +
		// elements[j] );
		...
		return new NiceIterator<Triple>()
		{
			...
			public void remove()
			{
				...
				// System.err.println( "++  selector currently " + i +
				// " (triple " + e[i] + ")" );
				int last = --size;
				e[i] = e[last];
				e[last] = null;
