...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.Personality;
 ...
 * 
 * <P>
 * This class implements common methods, mainly convenience methods, for model implementations. It is intended use is as a base class from which model
...
public class ModelCom extends EnhGraph implements Model, PrefixMapping, Lock
{
	...
	static
	{
		// This forces RIOT (in ARQ) to initialize but after Jena
		// readers/writers
		// have cleanly initialized from the calls of RDFReaderFImpl and
		// RDFWriterFImpl
	...
	public StmtIterator listStatements(Resource S, Property P, String O, String L)
	{
		if (O != null)
		{
			// this is not OK when L is null: returns only the statements whose
			// lang is ""
			// return listStatements( S, P, Node.createLiteral( O, L, false ) );
			if (L != null)
				return listStatements(S, P, NodeFactory.createLiteral(O, L, false));
	...
	{
		// Catch special case of a Calendar which we want to act as if it were
		// an XSDDateTime
	 ...
	 * 
	 * @param members
	 *            An array of RDFNodes that will be the members of the list
	 ...
	 * 
	 * @param m
	 *            Model who's namespace is to be updated
	 * @param ns
	...
	/**
	 * @return an iterator each of whose elements is a ReifiedStatement in this model such that it's getStatement().equals( st )
	 */
	@Override
	 ...
	 * 
	 * @param s
	 *            for which a reification is sought
	...
	public StmtIterator listStatements(final Selector selector)
	{
		...
		return selector.isSimple() ? sts : new StmtIteratorImpl(sts.filterKeep(asFilter(selector)));
	}
	...
	/**
	 * Answer an [extended] iterator which returns the triples in this graph which are selected by the (S, P, O) triple in the selector, ignoring any special
	 * tests it may do.
	 * 
	 * @param s
	 *            a Selector used to supply subject, predicate, and object
	 ...
	 * 
	 * @param t
	 *            a triple to wrap as a statement
	...
	public Statement[] asStatements(Triple[] triples)
	{
		...
		for (int i = 0; i < triples.length; i += 1)
			result[i] = asStatement(triples[i]);
		return result;
	}
	...
	/**
	 * Intersect this with another model. As an attempt at optimisation, we try and ensure we iterate over the smaller model first. Nowadays it's not clear that
	 * this is a good idea, since <code>size()</code> can be expensive on database and inference models.
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.Model#intersection(de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.Model)
	 */
	@Override
	 ...
	 * used to check for membership. [So the first one should be "small" and the second one "membership cheap".]
	 */
	 ...
	 * 
	 * @param result
	 *            the Model to add statements to and return
	 * @param it
	 *            an iterator over the candidate statements
	 * @param other
	 *            the model that must contain the statements to be added
	 ...
	 * 
	 * @param listener
	 *            A ModelChangedListener to register for model events
	 ...
	 * 
	 * @param listener
	 *            A ModelChangedListener to unregister from model events
	 * @return this model, for cascading
	 */
	@Override
	...
	/**
	 * Answer a GraphListener that, when fed graph-level update events, fires the corresponding model-level event handlers in <code>L</code>.
	 * 
	 * @see ModelListenerAdapter
	 * @param L
	 *            a model listener to be wrapped as a graph listener
