...
public class N3JenaWriterCommon implements RDFWriter
{
	...
	// BaseURI - <#>
	// final boolean doAbbreviatedBaseURIref = getBooleanValue("abbrevBaseURI",
	// true) ;
	...
	protected Map<String, String> prefixMap = new HashMap<>(); // Prefixes to
																// actually use
	...
	// A derived class may choose to intercept and implement at any of these
	// levels.
	...
	protected void processModel(Model model)
	{
		...
		// is specialized to XML-isms and Turle prefixed names aren't quite
		// qnames.
	...
	protected void writeModel(Model model)
	{
		...
		{
			// Subject:
			// First - it is something we will write out as a structure in an
			// object field?
	...
	protected void writeObjectList(Resource subject, Property property)
	{
		...
		// Write with object lists as clusters of statements with the same
		// property
		...
		for (; sIter.hasNext();)
		{
			...
			{
				// Property col allows for min gap but widePropertyLen >
				// propertyCol
	...
	protected String formatLiteral(Literal literal)
	{
		...
		if (datatype != null)
		{
			...
			{
				// Must have 'e' or 'E' (N3 and Turtle now read 2.3 as a
				// decimal).
	...
	protected String formatURI(String uriStr)
	{
		...
		// for ( Iterator<String> pIter = prefixMap.keySet().iterator() ;
		// pIter.hasNext() ; )
		// {
		// String p = pIter.next() ;
		// String u = prefixMap.get(p) ;
	...
	/*
	 * http://www.w3.org/TeamSubmission/turtle/#sec-grammar-grammar [27] qname ::= prefixName? ':' name? [30] nameStartChar ::= [A-Z] | "_" | [a-z] |
	 * [#x00C0-#x00D6] | [#x00D8-#x00F6] | [#x00F8-#x02FF] | [#x0370-#x037D] | [#x037F-#x1FFF] | [#x200C-#x200D] | [#x2070-#x218F] | [#x2C00-#x2FEF] |
	 * [#x3001-#xD7FF] | [#xF900-#xFDCF] | [#xFDF0-#xFFFD] | [#x10000-#xEFFFF] [31] nameChar ::= nameStartChar | '-' | [0-9] | #x00B7 | [#x0300-#x036F] |
	 * [#x203F-#x2040] [32] name ::= nameStartChar nameChar* [33] prefixName ::= ( nameStartChar - '_' ) nameChar*
	 */
