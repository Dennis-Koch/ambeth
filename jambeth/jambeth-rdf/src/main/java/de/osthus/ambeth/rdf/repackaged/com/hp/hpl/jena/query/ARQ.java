...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.RIOT;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.ARQConstants;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.SystemARQ;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.optimize.TransformOrderByDistinctApplication;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.main.StageBuilder;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.lib.Metadata;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.mgt.ARQMgt;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.mgt.Explain;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.mgt.Explain.InfoLevel;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.MappingRegistry;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.Symbol;
...
public class ARQ
{
	...
	/**
	 * Symbol to enable logging of execution. Must also set log4j, or other logging system, for logger "com.hp.hpl.jena.sparql.exec" e.g. log4j.properties --
	 * log4j.logger.com.hp.hpl.jena.sparql.exec=INFO See the <a href="http://jena.apache.org/documentation/query/logging.html">ARQ Logging Documentation</a>.
	 */
	...
	{
		return (Explain.InfoLevel) ARQ.getContext().get(ARQ.symLogExec);
	}
	...
	/**
	 * Enable built-in property functions - also called "magic properties". These are properties in triple patterns that need calculation, not matching. See ARQ
	 * documentation for more details. rdfs:member and http://jena.hpl.hp.com/ARQ/list#member are provided.
	 */
	...
	/**
	 * Use the SAX parser for XML result sets. The default is to use StAX for full streaming of XML results. The SAX parser takes a copy of the result set
	...
	/**
	 * Determine which regular expression system to use. The value of this context entry should be a string or symbol of one of the following: javaRegex : use
	 * java.util.regex (support features outside the strict SPARQL regex language) xercesRegex : use the internal XPath regex engine (more compliant)
	 */
	...
	/**
	 * Use this Symbol to allow passing additional query parameters to a SERVICE <IRI> call. Parameters need to be grouped by SERVICE <IRI>, a Map<String,
	 * Map<String,List<String>>> is assumed. The key of the first map is the SERVICE IRI, the value is a Map which maps the name of a query string parameters to
	 * its values.
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.http.Service
	 */
	public static final Symbol serviceParams = ARQConstants.allocSymbol("serviceParams");

	/**
	 * A Long value that specifies the number of bindings (or triples for CONSTRUCT queries) to be stored in memory by sort operations or hash tables before
	 * switching to temporary disk files. The value defaults to -1, which will always keep the bindings in memory and never write to temporary files. The amount
	 * of memory used will vary based on the size of the bindings. If you are retrieving large literal strings, then you may need to lower the value.
	 * <p/>
	 * Note that for a complex query, several sort or hash operations might be running in parallel; each one will be allowed to retain as many bindings in
	 * memory as this value specifies before it starts putting data in temporary files. Also, several running sessions could be doing such operations
	 * concurrently. Therefore, the total number of bindings held in memory could be many times this value; it is necessary to keep this fact in mind when
	 * choosing the value.
	 * <p/>
	...
	/**
	 * Context key controlling whether the main query engine flattens simple paths (e.g. <tt>?x :p/:q ?z => ?x :p ?.0 . ?.0 ?q ?z</tt>)
	 * <p>
	 * Default is "true"
	 */
	...
	/**
	 * Context key controlling whether to do filter placement within BGP and quad blocks. Modies the effect of optFilterPlacement. Default is "true" - filter
	 * placement is pushed into BGPs.
	 */
	public static final Symbol optFilterPlacementBGP = ARQConstants.allocSymbol("optFilterPlacementBGP");

	/**
	 * Context key controlling whether the main query engine moves filters to the "best" place using the more limited and conservative strategy which does not
	 * place as many filters Must be explicitly set "true" to operate. Filter placement, via {@linkplain #optFilterPlacement} must also be active (which it is
	 * by default).
	 * 
	...
	/**
	 * Context key controlling whether an ORDER BY-LIMIT query is done avoiding total sort using an heap. Default is "true" - total sort if avoided by default
	 * when ORDER BY is used with LIMIT.
	 */
	public static final Symbol optTopNSorting = ARQConstants.allocSymbol("optTopNSorting");

	/**
	 * Context key controlling whether a DISTINCT-ORDER BY query is done by replacing the distinct with a reduced. Default is "true" - the reduced operator does
	 * not need to keep a data structure with all previously seen bindings.
	 */
	public static final Symbol optDistinctToReduced = ARQConstants.allocSymbol("optDistinctToReduced");

	/**
	 * Context key controlling whether a DISTINCT-ORDER BY query is done by applying the ORDER BY after the DISTINCT when default SPARQL semantics usually mean
	 * ORDER BY applies before DISTINCT. This optimization applies only in a subset of cases unlike the more general {@link #optDistinctToReduced} optimization.
	 * <p>
	...
	/**
	 * Context key controlling whether the standard optimizer applies optimizations to equalities in FILTERs. This optimization is conservative - it does not
	 * take place if there is a potential risk of changing query semantics.
	 */
	public static final Symbol optFilterEquality = ARQConstants.allocSymbol("optFilterEquality");

	/**
	 * Context key controlling whether the standard optimizer applies optimizations to inequalities in FILTERs This optimization is conservative - it does not
	 * take place if there is a potential risk of changing query semantics
	 */
	public static final Symbol optFilterInequality = ARQConstants.allocSymbol("optFilterInequality");

	/**
	 * Context key controlling whether the standard optimizer applies optimizations to implicit joins in FILTERs. This optimization is conservative - it does
	 * not take place if there is a potential risk of changing query semantics.
	 */
	public static final Symbol optFilterImplicitJoin = ARQConstants.allocSymbol("optFilterImplicitJoin");

	/**
	 * Context key controlling whether the standard optimizer applies optimizations to implicit left joins. This optimization is conservative - it does not take
	 * place if there is a potential risk of changing query semantics.
	 */
	public static final Symbol optImplicitLeftJoin = ARQConstants.allocSymbol("optImplicitLeftJoin");

	/**
	 * Context key for a declaration that xsd:strings and simple literals are different in the storage. They are the same value in a memory store. When in
	 * doubt, xsd:strings are assumed to be the same value as simple literals
	 */
	...
	{
		setStrictMode(ARQ.getContext());
	}

	/**
	 * Set strict mode for a given Context.
	 * 
	 * Does not influence expression evaluation because NodeValues are controlled globally, not per context.
	 */
	...
	/**
	 * Ensure things have started - applications do not need call this. The method is public so any part of ARQ can call it. Note the final static initializer
	 * call
	 */
