...
/**
 * NodeToTriplesMap: a map from nodes to sets of triples. Subclasses must override at least one of useXXXInFilter methods.
 */
public class NodeToTriplesMap extends NodeToTriplesMapBase
{
	...
	public boolean add(Triple t)
	{
		...
		if (s == null)
			bunchMap.put(o, s = createSetBunch());
		if (s.baseSet().add(t))
		{
			size += 1;
			return true;
	...
	{
		return indexField.filterOn(pattern).and(f2.filterOn(pattern)).and(f3.filterOn(pattern)).filterKeep(iterateAll());
	}
	...
	public ExtendedIterator<Triple> iterator(Node index, Node n2, Node n3)
	{
		...
		return s == null ? NullIterator.<Triple> instance() : f2.filterOn(n2).and(f3.filterOn(n3)).filterKeep(s.iterator());
	}
	...
	/**
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.mem.Temp#get(java.lang.Object)
	 */
