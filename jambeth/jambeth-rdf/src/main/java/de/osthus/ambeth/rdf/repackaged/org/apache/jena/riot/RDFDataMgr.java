...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.StreamRDFLib;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.stream.StreamManager;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.writer.NQuadsWriter;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.writer.NTriplesWriter;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.query.DatasetFactory;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.DatasetGraphFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.Quad;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.graph.GraphFactory;
...
public class RDFDataMgr
{
	...
	/*
	 * Maybe: static for global (singleton) and locally tailored.
	 */
	...
	{
		read(graph, uri, hintLang, null);
	}

	/**
	 * Read triples into a Model from the given location, with hint of language and with some parameters for the reader
	 * 
	 * @see #read(Model,String,String,Lang,Context) Throws parse errors depending on the language and reader; the model may be partially updated.
	 * @param model
	 *            Destination for the RDF read.
	 * @param uri
	 *            URI to read from (includes file: and a plain file name).
	 ...
	 * 
	 * @see #read(Graph,String,String,Lang,Context) Throws parse errors depending on the language and reader; the Model model may be partially updated.
	 * @param graph
	 *            Destination for the RDF read.
	 * @param uri
	 *            URI to read from (includes file: and a plain file name).
	...
	{
		read(graph, uri, base, hintLang, null);
	}
	...
	{
		read(model.getGraph(), uri, context);
	}
	...
	/**
	 * Read triples into a Model from the given location, with hint of language and with some parameters for the reader. Throws parse errors depending on the
	...
	/**
	 * Read triples into a Model from the given location, with hint of language and the with some parameters for the reader Throws parse errors depending on the
	 * language and reader; the graph may be partially updated.
	 * 
	 * @param graph
	 *            Destination for the RDF read.
	 * @param uri
	...
	/**
	 * Read triples into a Model with bytes from an InputStream. A base URI and a syntax can be provided. The base URI defualts to "no base" in which case the
	 * data should have no relative URIs. The lang gives the syntax of the stream.
	 * 
	 * @param model
	 *            Destination for the RDF read.
	 * @param in
	...
	/**
	 * Read triples into a model with chars from an Reader. Use of java.io.Readers is not encouraged - use with a StringReader is the primary use case. For
	 * files, open a {@link java.io.FileInputStream} to ensure correct character set handling.
	 * 
	 ...
	 * @throws RiotNotFoundException
	 *             if the location is not found - the dataset is unchanged. Throws parse errors depending on the language and reader; the dataset may be
	 *             partially updated.
	 */
	...
	/**
	 * Read quads into a dataset with chars from an Reader. Use java.io.Readers is not encouraged - use with a StringReader is the primary use case. For files,
	 * open a {@link java.io.FileInputStream} to ensure correct character set handling.
	 * 
	 * @param dataset
	 *            Destination
	 * @param in
	...
	/**
	 * Read quads into a dataset with chars from a StringReader. Use java.io.Readers is not encouraged - use with a StringReader is the primary use case. For
	 * files, open a {@link java.io.FileInputStream} to ensure correct character set handling.
	 * 
	 * @param dataset
	 *            Destination
	...
	/**
	 * Determine the content type to be used, given the target URL, the content-type from Content Negotiation and a hint language. This is a pragmatic balance.
