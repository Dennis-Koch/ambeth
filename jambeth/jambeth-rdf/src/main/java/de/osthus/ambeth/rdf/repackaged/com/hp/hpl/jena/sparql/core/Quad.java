...
public class Quad
{
	...
	/**
	 * Name of the non-graph when a quad is really a triple - also parsing of triples formats (and the default graph when parsing N-Quads or TriG) Not for
	 * access to the default graph by name - use Quad.defaultGraphIRI.
	 */
	...
	{
		return isUnionGraph(graph);
	}

	/** Is it really a triple? */
	public boolean isTriple()
	{
		return equal(graph, tripleInQuad);
	}
	...
	public int hashCode()
	{
		int x = (subject.hashCode() >> 1) ^ predicate.hashCode() ^ (object.hashCode() << 1);
		if (graph != null)
			x ^= (graph.hashCode() >> 2);
		else
			x++;
	...
	public boolean equals(Object other)
	{
		...
		if (!equal(graph, quad.graph))
			return false;
		if (!subject.equals(quad.subject))
			return false;
		if (!predicate.equals(quad.predicate))
			return false;
		if (!object.equals(quad.object))
			return false;
		return true;
	}
	...
	{
		return nodeMatches(getGraph(), g) && nodeMatches(getSubject(), s) && nodeMatches(getPredicate(), p) && nodeMatches(getObject(), o);
	}
