...
/**
 * <p>
 * Some general utilities and algorithms to support developers working with the general classes in the Jena ontology API. <strong>Warning</strong> these
 * utilities are <strong>experimental</strong>. Extensive testing has not yet occurred (see <tt>com.hp.hpl.jena.ontology.impl.TestOntTools</tt> in the test area
 * for basic unit tests), and in particular performance testing has not been carried out yet. Users are advised to exercise caution before relying on these
 * utilities in production code. Please send any comments or suggestions to the <a href="http://tech.groups.yahoo.com/group/jena-dev">Jena support email
 * list</a>.
 * </p>
 */
public class OntTools
{
	 ...
	 * <code>owl:Thing</code> which is a super-class of both <code>u</code> and <code>v</code>. The algorithm is based on <a href=
	 * "http://en.wikipedia.org/wiki/Tarjan's_off-line_least_common_ancestors_algorithm" >Tarjan's off-line LCA</a>. The current implementation expects that the
	 * given model:
	 * </p>
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * Both of these conditions are true of the built-in Jena OWL reasoners, such as {@link OntModelSpec#OWL_MEM_MICRO_RULE_INF}, and external DL reasoners such
	 * as Pellet.
	 * </p>
	 * 
	 * @param m
	 *            The ontology model being queried to find the LCA, which should conform to the reasoner capabilities described above
	 * @param u
	 *            An ontology class
	 * @param v
	 *            An ontology class
	 * @return The LCA of <code>u</code> and <code>v</code>
	 * @exception JenaException
	 *                if the language profile of the given model does not define a top concept (e.g. <code>owl:Thing</code>)
	 */
	...
	/**
	 * Answer the lowest common ancestor of two classes, assuming that the given class is the root concept to start searching from. See
	 * {@link #getLCA(OntModel, OntClass, OntClass)} for details.
	 * 
	 * @param m
	 *            The ontology model being queried to find the LCA, which should conform to the reasoner capabilities described above
	 * @param root
	...
	/**
	 * <p>
	 * Answer the shortest path from the <code>start</code> resource to the <code>end</code> RDF node, such that every step on the path is accepted by the given
	 * filter. A path is a {@link List} of RDF {@link Statement}s. The subject of the first statement in the list is <code>start</code>, and the object of the
	 * last statement in the list is <code>end</code>.
	 * </p>
	 * <p>
	 * The <code>onPath</code> argument is a {@link Filter}, which accepts a statement and returns true if the statement should be considered to be on the path.
	 * To search for an unconstrained path, pass {@link Filter#any} as an argument. To search for a path whose predicates match a fixed restricted set of
	 * property names, pass an instance of {@link PredicatesFilter}.
	 * </p>
	 * <p>
	 * If there is more than one path of minimal length from <code>start</code> to <code>end</code>, this method returns an arbitrary one. The algorithm is
	 * blind breadth-first search, with loop detection.
	 * </p>
	 * 
	 * @param m
	 *            The model in which we are seeking a path
	 * @param start
	 *            The starting resource
	 * @param end
	 *            The end, or goal, node
	 * @param onPath
	 *            A filter which determines whether a given statement can be considered part of the path
	 * @return A path, consisting of a list of statements whose first subject is <code>start</code>, and whose last object is <code>end</code>, or null if no
	 *         such path exists.
	 */
	...
	/**
	 * Answer a list of the named hierarchy roots of a given {@link OntModel}. This will be similar to the results of
	 * {@link OntModel#listHierarchyRootClasses()}, with the added constraint that every member of the returned iterator will be a named class, not an anonymous
	 * class expression. The named root classes are calculated from the root classes, by recursively replacing every anonymous class with its direct
	 * sub-classes. Thus it can be seen that the values in the list consists of the shallowest fringe of named classes in the hierarchy.
	 * 
	 * @param m
	 *            An ontology model
	 * @return A list of classes whose members are the named root classes of the class hierarchy in <code>m</code>
	 */
	...
	/**
	 * Compute the LCA disjoint set at <code>cls</code>, noting that we are searching for the LCA of <code>uCls</code> and <code>vCls</code>.
	 * 
	 * @param cls
	 *            The class we are testing (this is 'u' in the Wiki article)
	 * @param uCls
	 *            One of the two classes we are searching for the LCA of. We have simplified the set P of pairs to the unity set {uCls,vCls}
	 * @param vCls
	 *            One of the two classes we are searching for the LCA of. We have simplified the set P of pairs to the unity set {uCls,vCls}
	 * @param index
	 *            A data structure mapping resources to disjoint sets (since we can't side-effect Jena resources), and which is used to record the LCA pairs
	 */
	...
	/**
	 * Check to see if we have found a solution to the problem. TODO: we could throw an exception to simulate a non-local exit here, since we've assumed that P
	 * is the unity set.
	 * 
	 * @param uCls
	 * @param vCls
	 ...
	 * 
	 * @param i
	 *            An iterator to partition
	 * @param named
	 *            A list of named classes
	 * @param anon
	 *            A list of anonymous classes
	 */
	...
	public static class DisjointSet
	{
		...
		/**
		 * Set to true when we've inspected a black set, since the result is only correct just after both of the sets for u and v have been marked black
		 */
	...
	/**
	 * A path is an application of {@link java.util.List} containing only {@link Statement} objects, and in which for all adjacent elements
	 * <code>S<sub>i-1</sub></code> and <code>S<sub>i</sub></code>, where <code>i &gt; 0</code>, it is true that:
