...
public abstract class BaseInfGraph extends GraphBase implements InfGraph
{
	 ...
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.Graph#getPrefixMapping()
	 */
	@Override
	 ...
	 * 
	 * @param data
	 *            the raw data file to be augmented with entailments
	 * @param reasoner
	 *            the engine, with associated tbox data, whose find interface can be used to extract all entailments from the data.
	 */
	...
	{
		if (bulkHandler == null)
			bulkHandler = new InfBulkUpdateHandler(this);
		return bulkHandler;
	}

	/**
	 * InfBulkUpdateHandler - a bulk update handler specialised for inference graphs by code for <code>removeAll()</code>.
	 */
	...
	/**
	 * discard any state that depends on the content of fdata, because it's just been majorly trashed, solid gone.
	 */
	...
	/**
	 * Returns a derivations graph. The rule reasoners typically create a graph containing those triples added to the base graph due to rule firings. In some
	 ...
	 * 
	 * @return the deductions graph, if relevant for this class of inference engine or null if not.
	 */
	@Override
	...
	/**
	 * Test a global boolean property of the graph. This might included properties like consistency, OWLSyntacticValidity etc. It remains to be seen what level
	 * of generality is needed here. We could replace this by a small number of specific tests for common concepts.
	 * 
	 * @param property
	 *            the URI of the property to be tested
	 * @return a Node giving the value of the global property, this may be a boolean literal, some other literal value (e.g. a size).
	 */
	@Override
	...
	/**
	 * An extension of the Graph.find interface which allows the caller to encode complex expressions in RDF and then refer to those expressions within the
	 * query triple. For example, one might encode a class expression and then ask if there are any instances of this class expression in the InfGraph.
	 * 
	 * @param subject
	 *            the subject Node of the query triple, may be a Node in the graph or a node in the parameter micro-graph or null
	 * @param property
	 *            the property to be retrieved or null
	 * @param object
	 *            the object Node of the query triple, may be a Node in the graph or a node in the parameter micro-graph.
	 * @param param
	 ...
	 * 
	 * <p>
	 * This code used to have the .filterKeep component uncommented. We think this is because of earlier history, before .matches on a literal node was
	 * implemented as sameValueAs rather than equals. If it turns out that the filter is needed, it can be commented back in, AND a corresponding filter added
	 * to find(Node x 3) -- and test cases, of course.
	 */
	@Override
	...
	/**
	 * Return a new inference graph which is a clone of the current graph together with an additional set of data premises. The default implementation loses ALL
	 * partial deductions so far. Some subclasses may be able to a more efficient job.
	 */
	...
	/**
	 * Answer true iff this graph has been through the <code>prepare()</code> step. For testing purposes.
	 * 
