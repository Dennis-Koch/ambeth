...
public final class Bits
{
	...
	/**
	 * Extract the value packed into bits start (inclusive) and finish (exclusive), the value is returned the low part of the returned long. The low bit is bit
	 * zero.
	 */
	...
	/**
	 * Place the value into the bit pattern between start and finish; leaves other bits along.
	 */
	 ...
	 * @param finishChar
	 *            Index after the last character (counted from the left, string style).
	 * @return long
	 */
	...
	/**
	 * Get the bits from start (inclusive) to finish (exclusive), leaving them aligned in the long. See alio unpack, returns the value found at that place.
	 */
	...
	private static final long mask$(int start, int finish)
	{
		...
		long mask = -1;
		// mask = mask << (LongLen-finish) >>> (LongLen-finish) ; // Clear the
		// top bits
