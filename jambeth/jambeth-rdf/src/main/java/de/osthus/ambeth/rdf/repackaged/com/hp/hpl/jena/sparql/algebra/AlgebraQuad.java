...
public class AlgebraQuad extends TransformCopy
{
	...
	/**
	 * This is the record of the transformation. The rewriteGraphName is the node to put in the graph slot of the quad. The actualGraphName is the node used in
	 * SPARQL. If they are the same (by ==), the quadrewrite is OK as is. If they are different (and that means they are variables) an assign is done after the
	 * execution of the graph pattern block.
	 */
	...
	{ // Oh scala, where art thou!
		...
		{
			return "actualGraphName=" + actualGraphName + " rewriteGraphName=" + rewriteGraphName;
		}
	}
	...
	private static class TransformQuadGraph extends TransformCopy
	{
		...
		public TransformQuadGraph(Deque<QuadSlot> tracker)
		{
		...
		{
			return tracker.peek().rewriteGraphName;
		}
		...
		public Op transform(OpGraph opGraph, Op op)
		{
			...
			/*
			 * One case to consider is when the pattern for the GRAPH statement includes uses the variable inside the GRAPH clause. In this case, we must rename
			 * away the inner variable to allow stream execution via index joins, and then put back the value via an assign. (This is what QueryIterGraph does
			 * using a streaming join for triples)
			 */
