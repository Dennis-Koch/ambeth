...
/**
 * a WrappedIterator is an ExtendedIterator wrapping around a plain (or presented as plain) Iterator. The wrapping allows the usual extended operations
 * (filtering, concatenating) to be done on an Iterator derived from some other source. <br>
 */

public class WrappedIterator<T> extends NiceIterator<T>
{
	/**
	 * set to <code>true</code> if this wrapping doesn't permit the use of .remove(). Otherwise the .remove() is delegated to the base iterator.
	 */
	protected boolean removeDenied;

	/**
	 * Answer an ExtendedIterator returning the elements of <code>it</code>. If <code>it</code> is itself an ExtendedIterator, return that; otherwise wrap
	 * <code>it</code>.
	 */
	...
	/**
	 * Answer an ExtendedIterator wrapped round <code>it</code> which does not permit <code>.remove()</code> even if <code>it</code> does.
	 */
