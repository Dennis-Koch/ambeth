...
package de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner;
...
/**
 * Datastructure used to represent a closed transitive reflexive relation. It (mostly) incrementally maintains a transitive reduction and transitive closure of
 * the relationship and so queries should be faster than dynamically computing the closed or reduced relations.
 * <p>
 * The implementation stores the reduced and closed relations as real graph (objects linked together by pointers). For each graph node we store its direct
 * predecessors and successors and its closed successors. A cost penalty is the storage turnover involved in turning the graph representation back into triples
 * to answer queries. We could avoid this by optionally also storing the manifested triples for the links.
 * </p>
 * <p>
 * Cycles are currently handled by collapsing strongly connected components. Incremental deletes would be possible but at the price of substanially more storage
 * and code complexity. We compromise by doing the easy cases incrementally but some deletes (those that break strongly connected components) will trigger a
 * fresh rebuild.
 * </p>
 * <p>
 * TODO Combine this with interval indexes (Agrawal, Borigda and Jagadish 1989) for storing the closure of the predecessor relationship. Typical graphs will be
 * nearly tree shaped so the successor closure is modest (L^2 where L is the depth of the tree branch) but the predecessor closure would be expensive. The
 * interval index would handle predecessor closure nicely.
 * </p>
 */
...
// directly access its structure. I justify this on the flimsy grounds that it
// is a
// private inner class.
...
public class TransitiveGraphCache implements Finder
{
	...
	static interface Visitor<Alpha, Beta>
	{
		...
		// If this is needed return a non-null result which is a list of pred
		// nodes to kill
	...
	static class GraphWalker extends NiceIterator<Triple> implements ExtendedIterator<Triple>
	{
		...
		/**
		 * Constructor. Creates an iterator which will walk the graph, returning triples.
		 * 
		 * @param node
		 *            the starting node for the walk
		 * @param rdfNode
		 *            the rdfNode we are try to find predecessors for
		 * @param closed
		 *            set to true of walking the whole transitive closure
		 * @param predicate
		...
		protected void walkOne()
		{
			...
			if (succIt != null)
			{
				while (succIt.hasNext())
				{
					succ = succIt.next();
					if (succ == node)
						continue; // Skip accidental reflexive cases, already
									// done
			...
			if (baseNodeIt.hasNext())
			{
				...
				next = new Triple(nodeN, predicate, nodeN); // Implicit
															// reflexive case
			}
			else
			{
	 ...
	 * 
	 * @param graph
	 *            the searchable set of triples to cache
	 * @param predicate
	 *            the predicate to cache, need not be the registered predicate due to subProperty declarations
	...
	public ExtendedIterator<Triple> find(TriplePattern pattern)
	{
		...
		if (p.isVariable() || p.equals(directPredicate) || p.equals(closedPredicate))
		{
			...
			Node pred = closedPredicate; // p.isVariable() ? closedPredicate :
											// p;
			if (s.isVariable())
			{
				if (o.isVariable())
				{
					...
					// for (Iterator i = nodeMap.values().iterator();
					// i.hasNext(); ) {
					// ExtendedIterator nexti =
					// ((GraphNode)i.next()).listTriples(closed, this);
					// if (result == null) {
					// result = nexti;
					// } else {
	...
	/**
	 * Enable/disabling caching of the Triples representing the relationships. If this is enabled then a number of triples quadratic in the graph depth will be
	 * stored. If it is disabled then all queries will turn over storage dynamically creating the result triples.
	 */
