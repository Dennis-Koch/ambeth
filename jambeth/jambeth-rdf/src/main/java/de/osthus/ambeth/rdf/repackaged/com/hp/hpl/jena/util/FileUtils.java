...
public class FileUtils
{
	...
	public static String toURL(String filename)
	{
		...
		/**
		 * Convert a File, note java.net.URI appears to do the right thing. viz: Convert to absolute path. Convert all % to %25. then convert all ' ' to %20. It
		 * quite probably does more e.g. ? # But has bug in only having one / not three at beginning
		 */
	...
	/**
	 * create a temporary file that will be deleted on exit, and do something sensible with any IO exceptions - namely, throw them up wrapped in a
	 * JenaException.
	 * 
	 * @param prefix
	 *            the prefix for File.createTempFile
	 * @param suffix
	...
	public static File tempFileName(String prefix, String suffix)
	{
		...
		if (result.exists())
			return tempFileName(prefix, suffix);
		result.deleteOnExit();
		return result;
	}

	/**
	 * Answer a File naming a freshly-created directory in the temporary directory. This directory should be deleted on exit. TODO handle threading issues,
	 * mkdir failure, and better cleanup
	 * 
	 * @param prefix
	...
	public static InputStream openResourceFileAsStream(String filename) throws FileNotFoundException
	{
		...
		if (is == null)
		{
			...
			if (is == null)
			{
				...
				if (is == null)
				{
					// Can't find it on classpath, so try relative to current
					// directory
					// Will throw security exception under and applet but
					// there's not other choice left
