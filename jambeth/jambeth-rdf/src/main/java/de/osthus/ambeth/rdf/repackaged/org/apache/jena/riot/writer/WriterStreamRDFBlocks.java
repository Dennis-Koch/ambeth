...
/**
 * An output of triples / quads that print batches of same subject / same graph, same subject. It writes something that is easier to read than N-triples,
 * N-quads but it's not full pretty printing which usually requires analysing the data before any output.
 * 
 * If fed only quads, the output is valid TriG. If fed only triples, the output is valid Turtle.
 */
...
public class WriterStreamRDFBlocks extends WriterStreamRDFBatched
{
	...
	{
		return lastGraph == Quad.defaultGraphNodeGenerated;
	}
