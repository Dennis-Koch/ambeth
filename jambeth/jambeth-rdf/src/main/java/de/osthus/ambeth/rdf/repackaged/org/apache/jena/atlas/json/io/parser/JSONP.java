...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.json.io.parser;

import static de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.tokens.TokenType.COLON;
...
import static de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.tokens.TokenType.RBRACE;
...
/**
 * Json (extended) parser Extensions: Use of ', """ and ''' for strings Bare words for strings in maps (JSON objects) Hex numbers Objects with duplicate keys
 * are a warning (keeps last)
 */
public class JSONP extends JSONParserBase
{
	...
	public void parseAny()
	{
		switch (peekToken().getType())
		{
			...
			case INTEGER:
			{
				handler.valueInteger(peekToken().getImage(), currLine, currCol);
				nextToken();
				return;
			}
			case DECIMAL:
			{
				handler.valueDecimal(peekToken().getImage(), currLine, currCol);
				nextToken();
				return;
			}
			case DOUBLE:
			{
				handler.valueDouble(peekToken().getImage(), currLine, currCol);
				nextToken();
				return;
			}
			...
			case KEYWORD:
			{
				...
				{
					handler.valueBoolean(true, currLine, currCol);
					nextToken();
					return;
				}
				if (image.equalsIgnoreCase("false"))
				{
					handler.valueBoolean(false, currLine, currCol);
					nextToken();
					return;
				}
