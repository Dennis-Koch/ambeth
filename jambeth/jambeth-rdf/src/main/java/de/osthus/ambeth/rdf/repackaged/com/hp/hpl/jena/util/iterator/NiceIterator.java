...
/**
 * NiceIterator is the standard base class implementing ExtendedIterator. It provides the static methods for <code>andThen</code>, <code>filterKeep</code> and
 * <code>filterDrop</code>; these can be reused from any other class. It defines equivalent instance methods for descendants and to satisfy ExtendedIterator.
 */
...
public class NiceIterator<T> implements ExtendedIterator<T>
{
	...
	/**
	 * Utility method for this and other (sub)classes: raise the appropriate "no more elements" exception. I note that we raised the wrong exception in at least
	 * one case ...
	 * 
	 * @param message
	...
	public static <T> ExtendedIterator<T> andThen(final Iterator<T> a, final Iterator<? extends T> b)
	{
		...
		return new NiceIterator<T>()
		{
			...
			public boolean hasNext()
			{
				while (current.hasNext() == false && index < pending.size())
					current = advance();
			...
			{
				close(current);
				for (int i = index; i < pending.size(); i += 1)
					close(pending.get(i));
				pending.clear();
				removeFrom = null;
			}
	...
	/**
	 * If <code>it</code> is a Closableiterator, close it. Abstracts away from tests [that were] scattered through the code.
	 */
	...
	/**
	 * Answer a list of the elements of <code>it</code> in order, consuming this iterator. Canonical implementation of toSet().
	 */
	...
	/**
	 * Answer a list of the elements from <code>it</code>, in order, consuming that iterator. Canonical implementation of toList().
	 */
