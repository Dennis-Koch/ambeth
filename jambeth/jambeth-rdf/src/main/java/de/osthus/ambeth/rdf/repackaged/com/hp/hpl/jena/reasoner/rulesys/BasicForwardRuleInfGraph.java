...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.BaseInfGraph;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.Derivation;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.FGraph;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.Finder;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.FinderUtil;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.impl.FRuleEngineI;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.impl.FRuleEngineIFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.impl.SafeGraph;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.iterator.NullIterator;
 ...
 * <p>
 * This implementation has a horribly inefficient rule chainer built in. Once we have this working generalize this to an interface than can call out to a rule
 * engine and build a real rule engine (e.g. Rete style).
 * </p>
 */
...
public class BasicForwardRuleInfGraph extends BaseInfGraph implements ForwardRuleInfGraphI
{
	...
	// LoggerFactory.getLogger(BasicForwardRuleInfGraph.class);

	// =======================================================================
	// Core methods

	/**
	 * Constructor. Creates a new inference graph to which a (compiled) rule set and a data graph can be attached. This separation of binding is useful to allow
	 * any configuration parameters (such as logging) to be set before the data is added. Note that until the data is added using {@link #rebind rebind} then
	 * any operations like add, remove, find will result in errors.
	 * 
	 * @param reasoner
	 *            the parent reasoner
	 * @param schema
	...
	/**
	 * Constructor. Creates a new inference graph based on the given rule set. No data graph is attached at this stage. This is to allow any configuration
	 * parameters (such as logging) to be set before the data is added. Note that until the data is added using {@link #rebind rebind} then any operations like
	 * add, remove, find will result in errors.
	 * 
	 * @param reasoner
	 *            the parent reasoner
	 * @param rules
	 *            the list of rules to use this time
	 * @param schema
	...
	/**
	 * Replace the underlying data graph for this inference graph and start any inferences over again. This is primarily using in setting up ontology imports
	 * processing to allow an imports multiunion graph to be inserted between the inference graph and the raw data, before processing.
	 * 
	 * @param data
	...
	public int graphBaseSize()
	{
		...
		// System.err.println( ">> BasicForwardRuleInfGraph::size = " + baseSize
		// + "(base) + " + dedSize + "(deductions)" );
	...
	/**
	 * Create the graph used to hold the deductions. Can be overridden by subclasses that need special purpose graph implementations here. Assumes the graph
	 * underlying fdeductions and associated SafeGraph wrapper can be reused if present thus enabling preservation of listeners.
	 */
	...
	/**
	 * Return the Graph containing all the static deductions available so far. Does not trigger a prepare action. Returns a SafeWrapper and so can be used for
	 * update (thus triggering listeners) but not for access to generalized triples
	 */
	@Override
