...
public abstract class AssemblerBase implements Assembler
{
	...
	/**
	 * Answer the string described by the value of the unique optional <code>classProperty</code> property of <code>root</code>, or null if there's no such
	...
	public static String getOptionalClassName(Resource root, Property classProperty)
	{
		...
		return classNode == null ? null : classNode.isLiteral() ? classNode.asNode().getLiteralLexicalForm() : classNode.isResource() ? mustBeJava(classNode
				.asNode().getURI()) : null;
	}

	/**
	 * Throw an exception if <code>uri</code> doesn't start with "java:", otherwise answer the string beyond the ":".
	 */
