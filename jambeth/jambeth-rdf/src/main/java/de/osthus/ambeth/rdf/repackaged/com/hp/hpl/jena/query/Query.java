...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.io.IndentedLineBuffer;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.table.TableData;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.*;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ExprAggregator;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ExprVar;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.aggregate.Aggregator;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.serializer.Serializer;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.syntax.Element;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.syntax.PatternVars;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.syntax.Template;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.FmtUtils;

/**
 * The data structure for a query as presented externally. There are two ways of creating a query - use the parser to turn a string description of the query
 * into the executable form, and the programmatic way (the parser is calling the programmatic operations driven by the quyery string). The declarative approach
...
public class Query extends Prologue implements Cloneable, Printable
{
	static
	{
		ARQ.init(); /* Ensure everything has started properly */
	}
	...
	/**
	 * Location of the source for the data. If the model is not set, then the QueryEngine will attempt to load the data from these URIs into the default
	 * (unamed) graph.
	 */
	...
	/**
	 * Location of the source for the data. If the model is not set, then the QueryEngine will attempt to load the data from these URIs as named graphs in the
	 * dataset.
	 */
	...
	/**
	 * Return true if the query has either some graph URIs or some named graph URIs in its description. This does not mean these URIs will be used - just that
	...
	{
		return havingExprs != null && havingExprs.size() > 0;
	}
	...
	{
		return valuesDataBlock != null;
	}

	public List<Var> getValuesVariables()
	{
		return valuesDataBlock == null ? null : valuesDataBlock.getVars();
	}

	/** VALUES data - null for a Node means undef */
	public List<Binding> getValuesData()
	{
		return valuesDataBlock == null ? null : valuesDataBlock.getRows();
	}
	...
	/**
	 * Are two queries equals - tests shape and details. Equality means that the queries do the same thing, including same variables, in the same places. Being
	 * unequals does <b>not</b> mean the queries do different things.
	 * 
	 * For example, reordering a group or union means that that a query is different.
	 * 
