...
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.StringUtils.printAbbrev;

import java.util.ArrayList;
import java.util.List;

import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator.AccString;
...
/**
 * Machinary. This code implements the connectiveness assumed by execution based on substitution (index joins). i.e. if <code>{ ?x :p ?v . ?x :q ?w }</code>
 * then <code>?x</code> is <code>TERM</code> at the second triple.
 */
public abstract class ReorderTransformationSubstitution implements ReorderTransformation
{
	...
	protected ReorderProc reorder(List<Triple> triples, List<PatternTriple> components)
	{
		...
		if (triples.size() != idx)
			throw new ARQException(String.format("Inconsistency: number of triples (%d) does not equal to number of indexes processed (%d)", triples.size(),
					idx));
	...
	protected enum DefaultChoice
	{
		ZERO, LAST, FIRST, NUMERIC;
	}

	/**
	 * What to do if the {@link weight} comes back as "not found". Choices are: ZERO Assume the weight is zero (the rules were complete over the data so this is
	 * a pattern that will not match the data. LAST Place after all explciitly weighted triple patterns FIRST Place before all explciitly weighted triple
	 * patterns NUMERIC Use value returned by {@link defaultWeight} The default, default choice is LAST.
	 */
	...
	private AccString<PatternTriple> formatter = new AccString<PatternTriple>()
	{
		@Override
