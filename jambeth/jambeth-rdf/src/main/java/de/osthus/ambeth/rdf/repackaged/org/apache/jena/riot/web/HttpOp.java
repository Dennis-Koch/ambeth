...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.web;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.InternalErrorException;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.HttpException;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.TypedInputStream;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.auth.HttpAuthenticator;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.auth.ServiceAuthenticator;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.RiotException;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.WebContent;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.web.HttpSC;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.query.ARQ;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.http.Params;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.http.Params.Pair;

/**
 * Simplified HTTP operations; simplification means only supporting certain uses of HTTP. The expectation is that the simplified operations in this class can be
 * used by other code to generate more application specific HTTP interactions (e.g. SPARQL queries). For more complicated requirements of HTTP, then the
 ...
 * <p>
 * For HTTP POST, the application supplies a URL, content, the accept header string, and a list of handlers to deal with different content type responses, or no
...
public class HttpOp
{
	 ...
	 * 
	 * Pattern of functions provided: 1/ The full operation (includes HttpClient, HttpContext, HttpAuthenticator) any of which can be null for "default" 2/
	 * Provide common use options without those three arguments. These all become the full operation. 3/ All calls go via exec for logging and debugging.
	 */
	...
	/**
	 * Sets the default authenticator used for authenticate requests if no specific authenticator is provided. May be set to null to turn off default
	 * authentication, when set to null users must manually configure authentication.
	 * 
	 * @param authenticator
	 *            Authenticator
	 */
	...
	/**
	 * Return the current default {@link HttpClient}. This may be null, meaning a new {@linkplain HttpClient} is created each time, if none is provided in the
	 * HttpOp function call.
	 * 
	...
	/**
	 * <p>
	 * Performance can be improved by using a shared HttpClient that uses connection pooling. However, pool management is complicated and can lead to starvation
	 * (the system locks-up, especially on Java6; it's JVM sensitive).
	 * </p>
	 * <p>
	 * The default HttpClient is not used if an HttpAuthenticator is provided since this can potentially leak authentication credentials to be leaked between
	 * requests to different services. However in some cases it may be valuable to share the client regardless in which case you can also set the
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * This defaults to false because this can potentially leak authentication credentials between requests to different services. However in some cases it may
	 * be valuable to share the client regardless e.g. when you have a specially configured HttpClient for your environment.
	 * </p>
	 * 
	 * @return True if the configured default client will be used with authentication, false otherwise
	 */
	 ...
	 * 
	 * @param useWithAuth
	 *            True if the configured default client should be used with authentication, false if it should not
	 */
	...
	/**
	 * Create an HttpClient that performs connection pooling. This can be used with {@link #setDefaultHttpClient} or provided in the HttpOp calls.
	 */
	 ...
	 *            HTTP Authenticator
	 */
	public static void execHttpPost(String url, String contentType, String content, String acceptType, HttpResponseHandler handler, HttpClient httpClient,
	 ...
	 * <p>
	 * Additional headers e.g. for authentication can be injected through an {@link HttpContext}
	 * 
	 * @param url
	 *            URL
	 * @param entity
	 ...
	 *            HTTP Authenticator
	 */
	public static void execHttpPost(String url, HttpEntity entity, String acceptHeader, HttpResponseHandler handler, HttpClient httpClient,
			HttpContext httpContext, HttpAuthenticator authenticator)
	{
		String requestURI = determineRequestURI(url);
	 ...
	 *            HTTP Authenticator
	 */
	public static void execHttpPostForm(String url, Params params, String acceptHeader, HttpResponseHandler handler, HttpClient httpClient,
			HttpContext httpContext, HttpAuthenticator authenticator)
	{
		if (handler == null)
	...
	private static void exec(String url, HttpUriRequest request, String acceptHeader, HttpResponseHandler handler, HttpClient httpClient,
			HttpContext httpContext, HttpAuthenticator authenticator)
	{
		try
		{
	 ...
	 * <p>
	 * Prefers the {@link HttpClient} provided for the request if available. Then it tries to use a Jena-wide user configurable {@link HttpClient} if available.
	 * This is used only when no authentication is required unless the user has indicated they want to use the default client with authentication regardless.
	 * </p>
	 * <p>
	 * In all other cases it creates a fresh instance of a {@link SystemDefaultHttpClient} each time.
	 * </p>
	 * 
	 * @param client
	 *            HTTP Client
	...
	/**
	 * Ensures that a context is non-null, uses a new {@link BasicHttpContext} if none is provided
	 * 
	 * @param context
	 *            HTTP Context
	 ...
	 * <p>
	 * If a null authenticator is provided this method tries to use the registered default authenticator which may be set via the
	...
	/**
	 * Calculate the request URI from a general URI. This means remove any fragment.
	 */
	...
	/**
	 * Calculate the base IRI to use from a URI. The base is without fragement and without query string.
	 */
