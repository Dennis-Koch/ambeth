...
public class GraphMem extends GraphMemBase
{
	...
	/**
	 * The GraphMemStatisticsHandler exploits the existing TripleStoreMem indexes to deliver statistics information for single-concrete-node queries and for
	 * trivial cases of two-concrete-node queries.
	 */
	protected static class GraphMemStatisticsHandler implements GraphStatisticsHandler
	{
		...
		/**
		 * Answer a good estimate of the number of triples matching (S, P, O) if cheaply possible.
		 * 
		 * <p>
		 * If there are any reifier triples, return -1. (We may be able to improve this later.)
		 * 
		 * <p>
		 * If only one of S, P, O is concrete, answers the number of triples with that value in that field.
		 * 
		 * <p>
		 * If two of S, P, P are concrete and at least one of them has no corresponding triples, answers 0.
		 * 
		 * <p>
		 * Otherwise answers -1, ie, no information available. (May change; the two degenerate cases might deserve an answer.)
		 * 
		 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.GraphStatisticsHandler#getStatistic(de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.Node,
	...
	/**
	 * Answer an ExtendedIterator over all the triples in this graph that match the triple-pattern <code>m</code>. Delegated to the store.
	 */
	@Override
	...
	/**
	 * Answer true iff this graph contains <code>t</code>. If <code>t</code> happens to be concrete, then we hand responsibility over to the store. Otherwise we
