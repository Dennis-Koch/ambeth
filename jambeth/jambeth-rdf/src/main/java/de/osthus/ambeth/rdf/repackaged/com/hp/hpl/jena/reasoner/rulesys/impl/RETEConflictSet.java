...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.Builtin;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.ForwardRuleInfGraphI;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.RuleDerivation;

/**
 * Manages a set of ready-to-fire rules. For monotonic rule sets we simply fire the rules as soon as they are triggered. For non-monotonic rule sets we stack
 * them up in a conflict set and fire them one-at-a-time, propagating all changes between times.
 * <p>
			...
			if ((isAdd && nNeg > 0) || (!isAdd && nPos > 0))
			{
				for (Iterator<CSEntry> i = conflictSet.iterator(); i.hasNext();)
				{
					...
					if (cse.env.equals(env))
					{
						...
						{
							// Redundant insert? Probably leave in for
							// side-effect cases like print
						}
					}
				}
			}
