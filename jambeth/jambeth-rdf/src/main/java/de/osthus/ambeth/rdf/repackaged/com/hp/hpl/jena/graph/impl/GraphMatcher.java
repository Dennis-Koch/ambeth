...
/**
 * An implemantation of graph isomorphism for Graph equality. The underlying algorithm is exponential but will only enter a non-deterministic polynomial part
 * when there are a lot of difficult to distinguish anonymous nodes connected to each other by statements with the same property(s). Non-pathological examples,
 * where most nodes have some properties that help distinguish them from other nodes, will experience nearly linear performance.
 */
public class GraphMatcher extends java.lang.Object
{
	...
	/**
	 * Are the two models isomorphic? The isomorphism is defined as a bijection between the anonymous variables such that the statements are identical. This is
	 * described in <a href="http://www.w3.org/TR/rdf-concepts#section-Graph-syntax"> http://www.w3.org/TR/rdf-concepts#section-Graph-syntax </a>
	 */
	...
	/**
	 * Return an isomorphism between the two models. This function is nondeterministic in that it may return a different bijection on each call, in cases where
	 * there are multiple isomorphisms between the models.
	 * 
	 * @return <code>null</code> on failure or an array of related pairs (arrays of length 2) of anonymous nodes. <code>match(m1,m2)[i][0]</code> is from
	 *         <code>m1</code>, and <code>match(m1,m2)[i][1]</code> is the corresponding node in <code>m2</code>.
	 */
	...
	/*
	 * NOTE: inner classes We use a number of non-static inner classes, these all refer to the GraphMatcher for context.
	 * 
	 * NOTE: the built-in hashCode() is not modified, so that Set's etc still work. This algorithm depends on a hash function, which I call myHashCode() This
	 * has the somewhat perplexing property of changing as we do the binding. obj.myHashCode() depends on: - obj and it's non anonymous subcomponents -
	 * ModelMatcher.myHashLevel (in the enclosing ModelMatcher) - for a bound AnonResource b in obj, it depends on a random value generated at the time that b
	 * got bound - for an unbound AnonResource, if myHashLevel>0, then it depends on the value of myHashCode() at myHashLevel-1
	 */
	...
	private Node[][] match(GraphMatcher oth)
	{
		...
		if (m.getCapabilities().sizeAccurate() && m.size() < other.m.size())
			return null;
		if (other.m.getCapabilities().sizeAccurate() && m.size() > other.m.size())
			return null;
	...
	/*
	 * Called with hashing incorrect. Returns null if situation is unworkable. Returns non-null with no outstanding obvious bindings, and with the hashing
	 * correct. The set of obligatorily bound resources is returned.
	 */
	...
	private Iterator<Bucket> scanBuckets()
	{
		...
		check(HASH_OK);
		return new FilterIterator<>(new Filter<Bucket>()
		{
			@Override
	...
	/*
	 * rehash performance notes:Uncommenting below gives an easy way of measuringrehash performance.On a 480ms job the rehash appeared to take over 200ms.(Since
	 * with the code below uncommented the sameproblem took about 1300ms).
	 */
	private int rehash(int lvl)
	{
		/*
		 * rehash0(lvl); rehash0(lvl); rehash0(lvl); rehash0(lvl);
		 */
		return rehash0(lvl);
	}
	...
	/*
	 * subjects identified by bits 0 and 1, predicate by bits 2 and 3, object by 4 and 5 If neither bit set then role is bound. If lower bit is set then role is
	 * unbound to singleton variable in triple. If higher bit is set then role is unbound with anonymous variable that is also unbound to a different role. It
	 * is an error if both bits are set.
	 * 
	 * 
	 * These funny things are read like this: e.g.
	 * 
	 * SXPYOX - the subject is a variable X, the predicate is another var Y the object is the same var X
	 */
	...
	private class AnonStatement
	{
		...
		boolean contextualEquals(AnonResource v, AnonStatement sB, AnonResource vB)
		{
			...
			if (vX != sB.varPos(vB))
				return false;
			return ((vX & S) != 0 || subj.mightBeEqual(sB.subj)) && ((vX & P) != 0 || pred.mightBeEqual(sB.pred))
	...
	private class AnonResource implements SomeResource
	{
		AnonResource bound;
		Node r;
		Set<AnonStatement> occursIn = CollectionFactory.createHashedSet(); // The
																			// AnonStatements
																			// containing
																			// me.
		int hash[] = new int[MAX_HASH_DEPTH];
		int boundHash;
		Set<AnonResource> friends = CollectionFactory.createHashedSet(); // Other
																			// vars
																			// in
																			// AnonStatements
																			// containing
		...
		boolean checkBinding(AnonResource pair)
		{
			...
			return ourStatements.removeAll(otherStatements) && ourStatements.isEmpty();
		...
		public boolean mightBeEqual(SomeResource r)
		{
			if (r != null && (r instanceof AnonResource))
			{
				...
				return a == this || bound == a || (bound == null && a.bound == null);
			}
			else
			{
				return false;
		...
		private class StatementWrapper
		{
			...
			public boolean equals(Object o)
			{
				...
				return wrapHash == w.wrapHash && statement.contextualEquals(AnonResource.this, w.statement, w.asAnonR());
			}
