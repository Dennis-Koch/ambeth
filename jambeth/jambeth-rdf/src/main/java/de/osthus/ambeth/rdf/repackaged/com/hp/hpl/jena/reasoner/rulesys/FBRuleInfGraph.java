...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.datatypes.TypeMapper;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.impl.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner.TransitiveEngine;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner.TransitiveGraphCache;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner.TransitiveReasoner;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.impl.JenaParameters;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.OneToManyMap;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.PrintUtil;
...
/**
 * An inference graph that uses a mixture of forward and backward chaining rules. The forward rules can create direct deductions from the source data and schema
 * and can also create backward rules. A query is answered by consulting the union of the raw data, the forward derived results and any relevant backward rules
 * (whose answers are tabled for future reference).
 */
public class FBRuleInfGraph extends BasicForwardRuleInfGraph implements BackwardRuleInfGraphI
{
	...
	/**
	 * Instantiate the forward rule engine to use. Subclasses can override this to switch to, say, a RETE imlementation.
	 * 
	 * @param rules
	 *            the rule set or null if there are not rules bound in yet.
	 */
	@Override
	...
	private void resetTGCCache()
	{
		...
		{
			transitiveEngine = new TransitiveEngine(new TransitiveGraphCache(ReasonerVocabulary.directSubClassOf.asNode(), RDFS.subClassOf.asNode()),
	...
	/**
	 * Search the combination of data and deductions graphs for the given triple pattern. This may different from the normal find operation in the base of
	 * hybrid reasoners where we are side-stepping the backward deduction step.
	 */
	@Override
	 ...
	 * 
	 * @param clause
	 *            the Functor representing the call
	 * @param env
	 *            the BindingEnvironment for this call
	 * @param rule
	...
	public boolean processBuiltin(ClauseEntry clause, Rule rule, BindingEnvironment env)
	{
		...
		// } else {
		// throw new ReasonerException("Illegal builtin predicate: " + clause +
		// " in rule " + rule);
		// }
	}
	 ...
	 * 
	 * @param instance
	 *            the base instance node to which the property applies
	 * @param prop
	...
	/**
	 * Add a new rule to the rule set. This should only be used by implementations of RuleProprocessHook (which are called during rule system preparation
	 * phase). If called at other times the rule won't be correctly transferred into the underlying engines.
	 */
	 ...
	 * where an application might wish greater control over when this prepration is done.
	 */
	@Override
	public synchronized void prepare()
	{
		...
		if (useTGCCaching)
		{
			...
			if (schemaGraph != null)
			{
				...
				if ((transitiveEngine.checkOccurance(TransitiveReasoner.subPropertyOf, data)
						|| transitiveEngine.checkOccurance(TransitiveReasoner.subClassOf, data) || transitiveEngine.checkOccurance(RDFS.domain.asNode(), data) || transitiveEngine
							.checkOccurance(RDFS.range.asNode(), data)))
				{

					// The data graph contains some ontology knowledge so split
					// the caches
			...
			// dataFind = FinderUtil.cascade(subClassCache, subPropertyCache,
			// dataFind);
			dataFind = FinderUtil.cascade(dataFind, transitiveEngine.getSubClassCache(), transitiveEngine.getSubPropertyCache());

			// Without the next statement then the transitive closures are not
			// seen by the forward rules
	...
	/**
	 * Cause the inference graph to reconsult the underlying graph to take into account changes. Normally changes are made through the InfGraph's add and remove
	 * calls are will be handled appropriately. However, in some cases changes are made "behind the InfGraph's back" and this forces a full reconsult of the
	 * changed data.
	 */
	@Override
	public void rebind()
	...
	/**
	 * Cause the inference graph to reconsult both the underlying graph and the reasoner ruleset, permits the forward rule set to be dynamically changed. Causes
	 * the entire rule engine to be rebuilt from the current ruleset and reinitialized against the current data. Not needed for normal cases.
	 */
	...
	/**
	 * Return the number of rules fired since this rule engine instance was created and initialized. The current implementation only counts forward rules and
	 * does not track dynamic backward rules needed for specific queries.
	 */
	@Override
	 ...
	 * @param pattern
	 *            a TriplePattern to be matched against the data
	 * @return a ExtendedIterator over all Triples in the data set that match the pattern
	 */
	@Override
	...
	public void performDelete(Triple t)
	{
		...
		if (this.isPrepared())
		{
			bEngine.deleteAllRules();
			this.setPreparedState(false);
			// Re-enable the code below when/if ref counting is added and remove
			// above
	...
	/**
	 * Return a new inference graph which is a clone of the current graph together with an additional set of data premises. Attempts to the replace the default
	 * brute force implementation by one that can reuse some of the existing deductions.
	 */
	...
	public ValidityReport validate()
	{
		...
		Triple validateOn = new Triple(NodeFactory.createAnon(), ReasonerVocabulary.RB_VALIDATION.asNode(), Functor.makeFunctorNode("on", new Node[] {}));
		// We sneak this switch directly into the engine to avoid contaminating
		// the
		// real data - this is only possible only the forward engine has been
		// prepared
		// add(validateOn);
		this.requirePrepared();
	 ...
	 * 
	 * @param prop
	 *            the property node whose range is under scrutiny
	 * @param triple
	 *            the statement whose object value is to be checked.
	 * @return null if the range is legal, otherwise a ValidityReport.Report which describes the problem.
	 */
	...
	/**
	 * Return a map from property nodes to a list of RDFDatatype objects which have been declared as the range of that property.
	 */
	...
	/**
	 * Adds a set of precomputed triples to the deductions store. These do not, themselves, fire any rules but provide additional axioms that might enable
	 * future rule firing when real data is added. Used to implement bindSchema processing in the parent Reasoner.
	 * 
	...
	/**
	 * Post-filter query results to hide unwanted triples from the glare of publicity. Unwanted triples are triples with Functor literals and triples with
	 * hidden nodes as subject or object.
	 */
