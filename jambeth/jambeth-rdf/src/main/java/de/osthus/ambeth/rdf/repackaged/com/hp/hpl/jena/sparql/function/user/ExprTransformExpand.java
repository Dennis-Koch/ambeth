...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ExprFunctionN;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ExprList;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ExprTransformCopy;
		...
		if (this.shouldExpand(f))
		{
			...
			for (int i = 0; i < args.size(); i++)
			{
				...
				else
				{
					...
					if (i > defArgs.size())
						throw new ExprBuildException("Unable to expand function dependency, the function <" + def.getUri()
								+ "> is called but uses an argument ?" + var + " which is not an argument to the outer function");
					subArgs.add(arg);
				}
			}
