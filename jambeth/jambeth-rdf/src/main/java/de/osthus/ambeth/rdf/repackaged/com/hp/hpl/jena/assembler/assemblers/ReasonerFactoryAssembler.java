...
/**
 * The ReasonerFactoryAssembler constructs a ReasonerFactory from the description. The factory class may be specified by a URL (which looks the class up in the
 * registry) or by class name; otherwise it defaults to the GenericRuleReasonerFactory.
 * <p>
 * If the class is specified by class name then an instance of that class is acquired by calling its <code>theInstance</code> method if it has one. Otherwise a
 * fresh instance is constructed by calling its zero-argument constructor (and exploding if it hasn't got one).
 * <p>
...
public class ReasonerFactoryAssembler extends AssemblerBase implements Assembler
{
	...
	protected static ReasonerFactory getReasonerFactory(Resource root)
	{
		...
		return className != null ? getReasonerFactoryByClassName(root, className) : reasonerURL == null ? GenericRuleReasonerFactory.theInstance()
	...
	/**
	 * Throw a <code>NotExpectedTypeException</code> if <code>c</code> isn't a subclass of <code>ReasonerFactory</code>.
	 */
	...
	/**
	 * Answer the string described by the value of the unique optional <code>JA.reasonerClass</code> property of <code>root</code>, or null if there's no such
	 * property. The value may be a URI, in which case it must be a <b>java:</b> URI with content the class name; or it may be a literal, in which case its
	 * lexical form is its class name; otherwise, BOOM.
	 */
	...
	/**
	 * Answer a ReasonerFactory which delivers reasoners with the given URL <code>reasonerURL</code>. If there is no such reasoner, throw an
	 * <code>UnknownreasonerException</code>.
	 */
