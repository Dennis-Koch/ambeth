...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.binding.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.main.OpExecutor;

/**
 * Execute a projection in the middle of an execution. This requires the outcome of each stage of execution (substitution evaluation assumed) to be merged with
 * the input binding. Bindings should already be compatible, otherwise this code should not be used.
 */
public class QueryIterProjectMerge extends QueryIterRepeatApply
{
	...
	static class ProjectEnsureBindingConverter implements QueryIterConvert.Converter
	{
		...
		/**
		 * Merge two bindings, the outer and inner, projecting the inner with the give variables. This is what happens in substitutIon execution, with a inner
		 * select-project.
		 */
