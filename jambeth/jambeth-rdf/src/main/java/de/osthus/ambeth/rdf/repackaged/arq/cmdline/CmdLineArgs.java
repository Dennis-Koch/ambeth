...
public class CmdLineArgs extends CommandLineBase
{
	...
	{
		return matchesIndirect(s, DefaultIndirectMarker);
	}
	...
	{
		return indirect(s, DefaultIndirectMarker);
	}
	...
	public boolean containsMultiple(String s)
	{
		return getValues(s).size() > 1;
	}
	...
	/**
	 * Get the argument associated with the argument name. Actually returns the LAST one seen
	 * 
	 * @param argName
	 *            Argument name
	 * @return Last argument that matched.
	...
	public boolean hasValueOfTrue(ArgDecl argDecl)
	{
		...
		if (x == null)
			return false;
		if (x.equalsIgnoreCase("true") || x.equalsIgnoreCase("t") || x.equalsIgnoreCase("yes") || x.equalsIgnoreCase("y"))
			return true;
		return false;
	}
	...
	public boolean hasValueOfFalse(ArgDecl argDecl)
	{
		...
		if (x == null)
			return false;
		if (x.equalsIgnoreCase("false") || x.equalsIgnoreCase("f") || x.equalsIgnoreCase("no") || x.equalsIgnoreCase("n"))
			return true;
		return false;
	}
