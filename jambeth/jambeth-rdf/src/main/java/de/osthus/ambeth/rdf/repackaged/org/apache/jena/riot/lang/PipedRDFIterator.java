...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.Closeable;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.PrefixMapFactory;

/**
 * <p>
 * A {@code PipedRDFIterator} should be connected to a {@link PipedRDFStream} implementation; the piped iterator then provides whatever RDF primitives are
 * written to the {@code PipedRDFStream}
 * </p>
 * <p>
 * Typically, data is read from a {@code PipedRDFIterator} by one thread (the consumer) and data is written to the corresponding {@code PipedRDFStream} by some
 * other thread (the producer). Attempting to use both objects from a single thread is not recommended, as it may deadlock the thread. The
 * {@code PipedRDFIterator} contains a buffer, decoupling read operations from write operations, within limits.
 * </p>
 * <p>
 ...
 * 
 * @param <T>
 *            The type of the RDF primitive, should be one of {@code Triple}, {@code Quad}, or {@code Tuple<Node>}
 * 
...
public class PipedRDFIterator<T> implements Iterator<T>, Closeable
{
	 ...
	 * <p>
	 * Buffer size must be chosen carefully in order to avoid performance problems, if you set the buffer size too low you will experience a lot of blocked
	 * calls so it will take longer to consume the data from the iterator. For best performance the buffer size should be at least 10% of the expected input
	 * size though you may need to tune this depending on how fast your consumer thread is.
	 * </p>
	 */
	 ...
	 * <p>
	 * Buffer size must be chosen carefully in order to avoid performance problems, if you set the buffer size too low you will experience a lot of blocked
	 * calls so it will take longer to consume the data from the iterator. For best performance the buffer size should be roughly 10% of the expected input size
	 * though you may need to tune this depending on how fast your consumer thread is.
	 * </p>
	 * 
	 * @param bufferSize
	 *            Buffer size
	 ...
	 * </p>
	 * <p>
	 * The fair parameter controls whether the locking policy used for the buffer is fair. When enabled this reduces throughput but also reduces the chance of
	 * thread starvation. This likely need only be set to {@code true} if there will be multiple consumers.
	 * </p>
	 * 
	 * @param bufferSize
	 *            Buffer size
	 ...
	 * </p>
	 * <p>
	 * The {@code fair} parameter controls whether the locking policy used for the buffer is fair. When enabled this reduces throughput but also reduces the
	 * chance of thread starvation. This likely need only be set to {@code true} if there will be multiple consumers.
	 * </p>
	 * <p>
	 * The {@code pollTimeout} parameter controls how long each poll attempt waits for data to be produced. This prevents the consumer thread from blocking
	 * indefinitely and allows it to detect various potential deadlock conditions e.g. dead producer thread, another consumer closed the iterator etc. and
	 * errors out accordingly. It is unlikely that you will ever need to adjust this from the default value provided by {@link #DEFAULT_POLL_TIMEOUT}.
	 * </p>
	 * <p>
	 * The {@code maxPolls} parameter controls how many poll attempts will be made by a single consumer thread within the context of a single call to
	 * {@link #hasNext()} before the iterator declares the producer to be dead and errors out accordingly. You may need to adjust this if you have a slow
	...
	/**
	 * Should be called by the producer when it begins writing to the iterator. If the producer fails to call this for whatever reason and never produces any
	 * output or calls {@code finish()} consumers may be blocked for a short period before they detect this state and error out.
	 */
	...
	/**
	 * Should be called by the producer when it has finished writing to the iterator. If the producer fails to call this for whatever reason consumers may be
	 * blocked for a short period before they detect this state and error out.
	 */
	...
	/**
	 * May be called by the consumer when it is finished reading from the iterator, if the producer thread has not finished it will receive an error the next
	 * time it tries to write to the iterator
	 */
	@Override
	public void close()
	{
