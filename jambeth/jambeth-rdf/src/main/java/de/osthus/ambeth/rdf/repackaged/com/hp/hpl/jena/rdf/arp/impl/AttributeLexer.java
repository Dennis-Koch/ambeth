...
public class AttributeLexer extends QNameLexer implements ARPErrorNumbers
{
		...
		for (index = 0; index < sz; index++)
		{
			...
			switch (lookup(taintMe))
			{
				...
				case 0:
					...
					{
						String qn = getQName();
						if ((qn.length() >= 3 && qn.substring(0, 3).equalsIgnoreCase("xml")) || xmlns.equals(getUri()))
						{
	...
	boolean isInRdfns(Taint taintMe) throws SAXParseException
	{
		...
		if (uri.equals(""))
		{
			frame.warning(taintMe, WARN_UNQUALIFIED_ATTRIBUTE, "unqualified use of rdf:" + getQName() + " is deprecated.");
			return true;
		}
		return false;
	}
	...
	void error(Taint taintMe, int r) throws SAXParseException
	{
		...
		frame.warning(taintMe, e, getQName() + " not allowed as attribute" + (e == ERR_BAD_RDF_ATTRIBUTE ? "." : " here."));
	...
	{
		frame.warning(me, ERR_BAD_RDF_ATTRIBUTE, getQName() + " has been deprecated.");
	}
	...
	private String value(Taint taintMe, String prev) throws SAXParseException
	{
		if (prev != null)
		{
			frame.warning(taintMe, ERR_SYNTAX_ERROR, "Cannot use " + getQName() + " in both qualified and unqualifed form on same element");
		}
		return att.getValue(index);
	}
