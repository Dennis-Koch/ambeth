...
public class RuleDerivation implements Derivation
{
	 ...
	 * 
	 * @param rule
	 *            the rule which created this derivation
	 * @param conclusion
	 *            the triple which the rule created
	 * @param matches
	 *            a list of matching Triples corresponding to the rule body patterns
	 * @param infGraph
	 ...
	 *            set to true to print intermediate variable bindings for each stage in the derivation
	 * @param indent
	 *            the number of indent spaces to use
	 * @param seen
	 *            a HashSet of derviations that have already been listed
	 */
	...
	/**
	 * Compare two derivations. This is a shallow comparison, two derivations are the same if they contain the same conclusion, rule and match list. They do not
	 * need to be derived from the same (or any) infGraph.
	 */
	@Override
	public boolean equals(Object other)
	{
		if (other instanceof RuleDerivation)
		{
			...
			return conclusion.equals(otherD.getConclusion()) && matches.equals(otherD.getMatches()) && rule.equals(otherD.getRule());
		}
		else
		{
			return false;
