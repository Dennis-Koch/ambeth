...
/**
 * LP version of the core backward chaining engine. For each parent inference graph (whether pure backward or hybrid) there should be one LPBRuleEngine
 * instance. The shared instance holds any common result caching, rule store and global state data. However, all the processing is done by instances of the
 * LPInterpreter - one per query.
 */
public class LPBRuleEngine
{
	...
	public synchronized void halt()
	{
		...
		// Copy because closing the LPInterpreter will detach it from this
		// engine which affects activeInterpreters
	 ...
	 * So instead we try to close the iterators and assume the rest of the context will be reset by the add call.
	 * 
	 * <p>
