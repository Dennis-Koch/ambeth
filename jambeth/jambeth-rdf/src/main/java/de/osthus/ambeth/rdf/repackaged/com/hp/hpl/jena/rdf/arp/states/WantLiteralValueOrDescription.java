...
public class WantLiteralValueOrDescription extends AbsWantLiteralValueOrDescription
{
	...
	public FrameI startElement(String uri, String localName, String rawName, Attributes atts) throws SAXParseException
	{
		...
		if (bufIsSet())
		{
			if (!isWhite(getBuf()))
			{
				...
				warning(ERR_NOT_WHITESPACE, "Cannot have both string data \"" + getBuf().toString() + "\" and XML data <" + rawName
	...
	/**
	 * flag for seen non-white. Note: buf may have non-white text in, even if this falg is false. The following holds: seenAnElement && non white text as
	 * occurred => seenNonWhiteText
	 */
	...
	public void characters(char[] ch, int start, int length) throws SAXParseException
	{
		if (seenAnElement)
		{
			...
			{
				seenNonWhiteText = true;
				warning(ERR_NOT_WHITESPACE, "Cannot have both string data: \"" + new String(ch, start, length) + "\"and XML data inside a property element. "
	...
	public void endElement() throws SAXParseException
	{
		if ((!seenAnElement) || seenNonWhiteText)
		{
			...
			if (taint.isTainted() || seenAnElement)
				literal.taint();
			((WantsObjectFrameI) getParent()).theObject(literal);
		}
		super.endElement();
	}
