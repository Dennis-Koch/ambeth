...
public class ElementLexer extends QNameLexer
{
	...
	public ElementLexer(Taint t, Frame f, String uri, String localName, String qname, int good, int bad, boolean report_1) throws SAXParseException
	{
		...
		if ((!(goodMatch || badMatch)) && (this.bad & E_RDF) == E_RDF)
		{
			if (rdfns.equals(uri))
			{
				...
				{
					if (report_1)
						frame.warning(t, WARN_RDF_NN_AS_TYPE, qname + " is being used on a typed node.");
				}
				else if (!isKnownNonMemberRDFProperty(localName))
				{
					frame.warning(t, WARN_UNKNOWN_RDF_ELEMENT, qname + " is not a recognized RDF property or type.");
	...
	{
		frame.warning(me, r == E_LI ? ERR_LI_AS_TYPE : ERR_BAD_RDF_ELEMENT, getQName() + " is not allowed as an element tag here.");
