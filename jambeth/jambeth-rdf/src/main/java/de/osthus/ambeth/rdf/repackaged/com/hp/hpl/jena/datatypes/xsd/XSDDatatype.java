...
public class XSDDatatype extends BaseDatatype
{
	...
	/**
	 * Constructor used when loading in external user defined XSD types - should only be used by the internals but public scope because the internals spread
	...
	/**
	 * Compares two instances of values of the given datatype. This ignores lang tags and defers to the equality function defined by the Xerces package - to be
	 * checked.
	 */
	@Override
	...
	/**
	 * Create and register a set of types specified in a user schema file. We use the (illegal) DAML+OIL approach that the uriref of the type is the url of the
	 * schema file with fragment ID corresponding the the name of the type.
	 * 
	 * @param uri
	 *            the absolute uri of the schema file to be loaded
	 * @param reader
	 *            the Reader stream onto the file (useful if you wish to load a cached copy of the schema file)
	 * @param encoding
	 *            the encoding of the source file (can be null)
	 * @param tm
	 ...
	 * @throws DatatypeFormatException
	 *             if there is a problem during load (not that we use Xerces in default mode for load which may provide diagnostic output direct to stderr)
	 */
	...
	/**
	 * Convert a validated xerces data value into the corresponding java data value. This function is currently the most blatently xerces-version dependent part
	 * of this subsystem. In many cases it also involves reparsing data which has already been parsed as part of the validation.
	 * 
	 * @param validatedInfo
	...
	/**
	 * Parse a validated lexical form. Subclasses which use the default parse implementation and are not convered by the explicit convertValidatedData cases
	...
	/**
	 * Test whether the given LiteralLabel is a valid instance of this datatype. This takes into accound typing information as well as lexical form - for
	 * example an xsd:string is never considered valid as an xsd:integer (even if it is lexically legal like "1").
	 */
	@Override
	...
	/**
	 * Test if the given typed value is in the right partition of the XSD type space. If this test passes then if the typed value has a legal lexical form for
	...
	public int getHashCode(byte[] bytes)
	{
		...
		return length == 0 ? 0 : (bytes[0] << 12) ^ (bytes[length / 2] << 6) ^ (bytes[length - 1]) ^ length;
	}
