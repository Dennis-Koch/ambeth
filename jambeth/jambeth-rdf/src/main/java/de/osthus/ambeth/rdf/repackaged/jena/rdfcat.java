...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.RDFWriterFImpl;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.NoWriterForLangException;
...
/**
 * <p>
 * An RDF utility that takes its name from the Unix utility <em>cat</em>, and is used to generate serialisations of the contents of zero or more input model
 * serialisations. <strong>Note</strong> In a change from previous versions, but to ensure compatability with standard argument handling practice, the input
 * language options are <em>no longer sticky</em>. In previous versions, <code>rdfcat -n A B C</code> would ensure that A, B and C were all read as N3. From
 * Jena 2.5.2 onwards, this requires: <code>rdfcat -n A -n B -n C</code>, or the use of the <code>-in</code> option.
 * </p>
 * <p>
 * Synopsis:
 * </p>
 ...
 * </pre>
 * <p>
 * The default input language is RDF/XML, but the reader will try to guess the input language based on the file extension (e.g. N3 for file with a .n3 file
 * extension.
 * </p>
 * <p>
 * The input language options set the language for the following file name only. So in the following example, input A is read as N3, inputs B, C and D are read
 * as RDF/XML, while stdin is read as N-TRIPLE:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * To change the default input language for all files that do not have a specified language encoding, use the <code>-in</code> option.
 * </p>
 * <p>
 * If the <code>include</code> option is set, the input files are scanned for <code>rdfs:seeAlso</code> and <code>owl:imports</code> statements, and the objects
 * of these statements are read as well. By default, <code>include</code> is off. If <code>include</code> is turned on, the normal behaviour is for the
 * including statements (e.g <code>owl:imports</code> to be filtered from the output models. To leave such statements in place, use the <code>--nofilter</code>
 * option.
 * </p>
 * <p>
 * rdfcat uses the Jena {@link de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.FileManager FileManager} to resolve input URI's to locations. This allows,
 * for example, <code>http:</code> URI's to be re-directed to local <code>file:</code> locations, to avoid a network transaction.
 * </p>
 * <p>
 * Examples:
 * </p>
 ...
 * </pre>
 * <p>
 * Note that, in a difference from the Unix utility <code>cat</code>, the order of input statements is not preserved. The output document is a merge of the
 * input documents, and does not preserve any statement ordering from the input serialisations. Also, duplicate triples will be suppressed.
 * </p>
 */
public class rdfcat
{
	...
	protected CommandLine m_cmdLine = new RCCommandLine().add(IN_N3).add(IN_NTRIPLE).add(IN_RDF_XML).add(OUT_LANG).add(IN_LANG).add(INCLUDE).add(NOINCLUDE)
			.add(NOFILTER).add(HELP).add(USAGE);
	...
	/**
	 * Answer the full, checked, language name expanded from <code>shortName</code>. The shortName is expanded according to the table of abbreviations [below].
	 * It is then checked against RDFWriterFImpl's writer table [this is hacky but at the moment it's the most available interface] and the NoWriter exception
	...
	/*
	 * Take the string as an input file or URI, and try to read using the current default input syntax.
	 */
	...
	protected class ReadAction implements RCAction
	{
