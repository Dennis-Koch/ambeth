...
/**
 * In some rules we need to be able to create temporary property values which are inferred from ontology constraints but not present in the ground data. This
 * structure is used to manage a pool of such temporary nodes. It is only needed in situations where the data can not be added directly to a deductions graph
 * due to the risk of concurrent access.
 */

// Implementation note: We need to map from a pair of values (instance and
// prop).
...
public class TempNodeCache
{
	...
	public static class NodePair
	{
		...
		{
			return o instanceof NodePair && first.equals(((NodePair) o).first) && second.equals(((NodePair) o).second);
		}
