...
public class ResourceUtils
{
	...
	/**
	 * <p>
	 * Answer the maximal lower elements of the given collection, given the partial ordering <code>rel</code>. See
	 * {@link #maximalLowerElements(Iterator, Property, boolean )} for details.
	 * </p>
	 * 
	 * @param resources
	 *            A collection of resources
	 * @param rel
	 *            A property defining a partial-ordering on <code>resources</code>
	 * @param inverse
	 *            If true, we invert the given property (by reversing the order of the arguments), which allows us to use eg subClassOf as a partial order
	 *            operator for both sub-class and super-class relationships
	 * @return The collection that contains only those <code>resources</code> are not greater than another resource under the partial order.
	 */
	...
	/**
	 * <p>
	 * Given a collection of resources, and a relation defining a partial order over those resources, answer the sub-collection that contains only those
	 * elements that appear in the maximal generator of the relation. Specifically, a resource <code>x</code> is excluded from the return value if there is
	 * another resource <code>y</code> in the input collection such that <code>y&nbsp;Rel&nbsp;x</code> holds.
	 * </p>
	 * 
	 * @param resources
	 ...
	 *            If true, we invert the given property (by reversing the order of the arguments), which allows us to use eg subClassOf as a partial order
	 *            operator for both sub-class and super-class relationships
	 * @return The list that contains only those <code>resources</code> are not greater than another resource under the partial order.
	 */
	public static <T extends Resource> List<T> maximalLowerElements(Iterator<T> resources, Property rel, boolean inverse)
	{
		...
		{
			T r = in.remove(0);
			boolean rCovered = testResourceCovered(in, rel, inverse, r) || testResourceCovered(out, rel, inverse, r)
					|| testResourceCovered(drop, rel, inverse, r);

			// if r is not covered by another resource, we can add it to the
			// output
	...
	/**
	 * <p>
	 * Remove from the given list l of {@link Resource Resources}, any Resource that is equivalent to the reference resource <code>ref</code> under the relation
	 * <code>p</code>. Typically, <code>p</code> will be <code>owl:subClassOf</code> or <code>owl:subPropertyOf</code> or some similar predicate. A resource R
	 * is defined to be equivalent to <code>ref</code> iff <code>R&nbsp;p&nbsp;ref</code> is true <em>and</em> <code>ref&nbsp;p&nbsp;R</code> is true.
	 * </p>
	 * <p>
	 * The equivalent resources are removed from list <code>l</code> </em>in place</em>, the return value is the list of <em>removed</em> resources.
	 * </p>
	 * 
	 * @param l
	 *            A list of resources from which the resources equivalent to ref will be removed
	 * @param p
	 ...
	 * <code>a</code> and <code>b</code> will be in the same partition iff <code>(a p b) && (b p a)</code>.
	 * </p>
	 * 
	 * @param <T>
	 * @param l
	 ...
	 * @param p
	 *            An equivalence predicate
	 * @return A list of lists which are the partitions of <code>l</code> under <code>p</code>
	 */
	...
	/**
	 * <p>
	 * Answer a new resource that occupies the same position in the graph as the current resource <code>old</code>, but that has the given URI. In the process,
	 * the existing statements referring to <code>old</code> are removed. Since Jena does not allow the identity of a resource to change, this is the closest
	 * approximation to a rename operation that works.
	 * </p>
	 * <p>
	 * Renaming a resource to its own URI is a no-op. Resources in the predicate position of statements are not renamed. Intermediate store for the triples
	 * mentioning <code>old</code> is required.
	 * </p>
	 * 
	 * <p>
	 * <b>Note</b>This implementation is a general and simple approach, and in given applications it may be possible to do this operation more efficiently.
	 * </p>
	 * 
	 * @param old
	 *            An existing resource in a given model
	 * @param uri
	 *            A new URI for resource old, or <code>null</code> to rename old to a bNode
	 * @return A new resource that occupies the same position in the graph as old, but which has the new given URI.
	 */
	public static Resource renameResource(final Resource old, final String uri)
	{
		...
		// An optimization to prevent concatenating the two find() operations
		// together every time through the outer loop
		boolean onFirstIterator = true;

		// It's possible there are triples (old wossname old) that are in
		// triples twice. It doesn't matter.
	...
	/**
	 * <p>
	 * Answer a model that contains all of the resources reachable from a given resource by any property, transitively. The returned graph is the sub-graph of
	 * the parent graph of root, whose root node is the given root. Cycles are permitted in the sub-graph.
	 * </p>
	 * 
	 * @param root
	...
	public static Model reachableClosure(Resource root)
	{
		...
		// set of resources we have passed through already (i.e. the occurs
		// check)
		...
		while (!queue.isEmpty())
		{
			...
			if (!seen.contains(r))
			{
				...
				// add the statements to the output model, and queue any new
				// resources
				for (StmtIterator i = r.listProperties(); i.hasNext();)
				{
					Statement s = i.nextStatement();

					// don't do the occurs check now in case of reflexive
					// statements
					m.add(s);
