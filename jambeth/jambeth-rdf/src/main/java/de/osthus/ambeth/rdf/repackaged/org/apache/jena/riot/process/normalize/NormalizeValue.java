...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.process.normalize;
...
class NormalizeValue
{
	...
	/*
	 * The canonical representation for double is defined by prohibiting certain options from the Lexical representation (§3.2.5.1). Specifically, the exponent
	 * must be indicated by "E". Leading zeroes and the preceding optional "+" sign are prohibited in the exponent. If the exponent is zero, it must be
	 * indicated by "E0". For the mantissa, the preceding optional "+" sign is prohibited and the decimal point is required. Leading and trailing zeroes are
	 * prohibited subject to the following: number representations must be normalized such that there is a single digit which is non-zero to the left of the
	 * decimal point and at least a single digit to the right of the decimal point unless the value being represented is zero. The canonical representation for
	 * zero is 0.0E0.
	 */
