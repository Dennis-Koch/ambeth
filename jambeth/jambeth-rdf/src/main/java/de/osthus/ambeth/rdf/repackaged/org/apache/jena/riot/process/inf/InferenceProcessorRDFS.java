...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.process.inf;
...
/**
 * Apply a fixed set of inference rules to a stream of triples. This is inference on the A-Box (the data) with respect to a fixed T-Box (the vocabulary,
 * ontology).
 * <ul>
...
abstract class InferenceProcessorRDFS
{
	...
	/*
	 * [rdfs8: (?a rdfs:subClassOf ?b), (?b rdfs:subClassOf ?c) -> (?a rdfs:subClassOf ?c)] [rdfs9: (?x rdfs:subClassOf ?y), (?a rdf:type ?x) -> (?a rdf:type
	 * ?y)]
	 */
	...
	/*
	 * [rdfs5a: (?a rdfs:subPropertyOf ?b), (?b rdfs:subPropertyOf ?c) -> (?a rdfs:subPropertyOf ?c)] [rdfs6: (?a ?p ?b), (?p rdfs:subPropertyOf ?q) -> (?a ?q
	 * ?b)]
	 */
	...
	/*
	 * [rdfs2: (?p rdfs:domain ?c) -> [(?x rdf:type ?c) <- (?x ?p ?y)] ] [rdfs9: (?x rdfs:subClassOf ?y), (?a rdf:type ?x) -> (?a rdf:type ?y)]
	 */
	...
	/*
	 * [rdfs3: (?p rdfs:range ?c) -> [(?y rdf:type ?c) <- (?x ?p ?y)] ] [rdfs9: (?x rdfs:subClassOf ?y), (?a rdf:type ?x) -> (?a rdf:type ?y)]
	 */
