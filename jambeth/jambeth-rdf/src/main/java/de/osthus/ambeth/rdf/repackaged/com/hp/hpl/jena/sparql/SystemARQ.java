...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.Sync;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.graph.GraphWrapper;
...
public class SystemARQ
{
	...
	/**
	 * Control whether additon datatypes, over and above strict, minimal SPARQL compliance, are handled. Examples incldue xsd;date and simple
	 * literal/xsd:string.
	 */
	public static boolean ValueExtensions = true;
	/**
	 * Control whether simple literals, string literals without datatype orlanguage tag, are created sameValueAs xsd:string. Normally true. Some testing for
	 * pre-RDF-1.1 assumes otherwise.
	 */
	public static boolean SameValueAsString = true;
	/**
	 * Under strict F&O, dateTimes and dates with no timezone have one magically applied. This default timezone is implementation dependent and can lead to
	 * different answers to queries depending on the timezone. Normally, ARQ uses XMLSchema dateTime comparions, which an yield "indeterminate", which in turn
	 * is an evaluation error. F&O insists on true/false and so can lead to false positves and negatives.
	 */
	...
	/**
	 * Sync an object if synchronizable (model, graph, dataset). If force is true, synchronize as much as possible (e.g. file metadata) else make a reasonable
	 * attenpt at synchronization but does not gauarantee disk state. Do nothing otherwise.
	 */
