...
/**
 * Command line argument processing based on a trigger model. An action is called whenever an argument is encountered. Example: <CODE>
 ...
 *  ...
 * }
 * </CODE> A gloabl hook is provided to inspect arguments just before the action. Tracing is enabled by setting this to a suitable function such as that
 * provided by trace(): <CODE>
 *  cl.setHook(cl.trace()) ;
 * </CODE>
 * 
 * <ul>
...
{
	/*
	 * Extra processor called before the registered one when set. Used for tracing.
	 */
	...
	protected boolean allowItemIndirect = false; // Allow @ to mean contents of
													// file
	boolean ignoreIndirectionMarker = false; // Allow comand line items to have
												// leading @ but strip it.
	 ...
	 *             Throw when something is wrong (no value found, action fails).
	 */
	...
	/**
	 * Answer true if this argument terminates argument processing for the rest of the command line. Default is to stop just before the first arg that does not
	 * start with "-", or is "-" or "--".
	 */
	 ...
	 * 
	 * @param argStr
	 *            The string image of the unrecognised argument
	 */
	 ...
	 *            Name
	 * @param hasValue
	 *            True if the command takes a (string) value
	...
	/**
	 * @param allowItemIndirect
	 *            Set whether items starting "@" have the value of named file.
	 */
	...
	public ArgHandler trace()
	{
		...
		{
			@Override
			public void action(String arg, String val) // throws
