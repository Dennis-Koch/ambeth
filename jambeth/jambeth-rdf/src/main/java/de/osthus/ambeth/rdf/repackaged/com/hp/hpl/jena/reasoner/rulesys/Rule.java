...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.RulesetNotFoundException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.WrappedIOException;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.Tokenizer;
 ...
 * </p>
 * <p>
 * The rule specification comprises a list of antecendents (body) and a list of consequents (head). If there is more than one consequent then a backchainer
 * should regard this as a shorthand for several rules, all with the same body but with a singleton head.
 * </p>
 * <p>
 * Each element in the head or body can be a TriplePattern, a Functor or a Rule. A TriplePattern is just a triple of Nodes but the Nodes can represent
 * variables, wildcards and embedded functors - as well as constant uri or literal graph nodes. A functor comprises a functor name and a list of arguments. The
 * arguments are Nodes of any type except functor nodes (there is no functor nesting). The functor name can be mapped into a registered java class that
 * implements its semantics. Functors play three roles - in heads they represent actions (procedural attachement); in bodies they represent builtin predicates;
 ...
 * </p>
 * We include a trivial, recursive descent parser but this is just there to allow rules to be embedded in code. External rule syntax based on N3 and RDF could
 * be developed. The embedded syntax supports rules such as: <blockindent> <code>[ (?C rdf:type *), guard(?C, ?P)  -> (?c rb:restriction some(?P, ?D)) ].</code> <br />
 * <code>[ (?s owl:foo ?p) -> [ (?s owl:bar ?a) -> (?s ?p ?a) ] ].</code><br />
 * <code>[name: (?s owl:foo ?p) -> (?s ?p ?a)].</code><br />
 * </blockindent> only built in namespaces are recognized as such, * is a wildcard node, ?c is a variable, name(node ... node) is a functor, (node node node) is
 * a triple pattern, [..] is an embedded rule, commas are ignore and can be freely used as separators. Functor names may not end in ':'. </p>
 */
public class Rule implements ClauseEntry
{
	 ...
	 * 
	 * @param body
	 *            a list of TriplePatterns or Functors.
	 * @param head
	 ...
	 *            a label for rule
	 * @param body
	 *            an array of TriplePatterns or Functors.
	 * @param head
	 *            an array of TriplePatterns, Functors or rules
	 */
	...
	private ClauseEntry cloneClause(ClauseEntry clause, Map<Node_RuleVariable, Node> vmap, BindingEnvironment env)
	{
		if (clause instanceof TriplePattern)
		{
			...
			return new TriplePattern(cloneNode(tp.getSubject(), vmap, env), cloneNode(tp.getPredicate(), vmap, env), cloneNode(tp.getObject(), vmap, env));
		}
		else
		{
	...
	/**
	 * Returns false for rules which can affect other rules non-monotonically (remove builtin or similar) or are affected non-monotonically (involve
	 * negation-as-failure).
	 */
	...
	/**
	 * Processes the source reader stripping off comment lines and noting prefix definitions (@prefix) and rule inclusion commands (@include). Returns a parser
	 * which is bound to the stripped source text with associated prefix and rule inclusion definitions.
	 */
	public static Parser rulesParserFromReader(BufferedReader src)
	{
		try
		{
			...
			while ((line = src.readLine()) != null)
			{
				...
				{
					// Include referenced rule file, either URL or local special
					// case
	...
	/**
	 * Inner class which provides minimalist parsing support based on tokenisation with depth 1 lookahead. No sensible error reporting on offer. No embedded
	 * spaces supported.
	 */
	public static class Parser
	{
		...
		/**
		 * Find the variable index for the given variable name and return a Node_RuleVariable with that index.
		 */
		...
		Node parseNode(String token)
		{
			if (token.startsWith("?"))
			{
				...
				// Dropped support for anon wildcards until the implementation
				// is better resolved
			}
			...
			{ // TODO rationalise [this is for
				// the RIF code]
				return NodeFactory.createAnon(new AnonId(token.substring(1)));
			}
			else if (token.indexOf(':') != -1)
			{
				String exp = prefixMapping.expandPrefix(token); // Local map
																// first
				exp = PrintUtil.expandQname(exp); // Retain global map for
													// backward compatibility
				if (exp == token)
				{
					// No expansion was possible
					String prefix = token.substring(0, token.indexOf(':'));
					if (prefix.equals("http") || prefix.equals("urn") || prefix.equals("file") || prefix.equals("ftp") || prefix.equals("mailto"))
					{
			...
			else if (token.equals("'") || token.equals("\""))
			{
				...
				if (peekToken().startsWith("^^"))
				{
					...
					if (dtURI.indexOf(':') != -1)
					{
						// Thanks to Steve Cranefield for pointing out the need
						// for prefix expansion here
						String exp = prefixMapping.expandPrefix(dtURI); // Local
																		// map
																		// first
						exp = PrintUtil.expandQname(exp); // Retain global map
															// for backward
															// compatibility
						if (exp == dtURI)
						{
							...
							{
								// Likely to be a typo in a qname or failure to
								// register
								throw new ParserException("Unrecognized qname prefix (" + prefix + ") in rule", this);
							}
						}
		 ...
		 * 
		 * @param retainVarMap
		 *            set to true to ccause the existing varMap to be left in place, which is required for nested rules.
		 */
	...
	public boolean equals(Object o)
	{
		...
		if (!(o instanceof Rule))
			return false;
		Rule other = (Rule) o;
		if (other.head.length != head.length)
			return false;
