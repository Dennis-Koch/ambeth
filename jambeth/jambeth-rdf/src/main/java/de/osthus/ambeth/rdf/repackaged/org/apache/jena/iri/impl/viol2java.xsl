...
<!-- Description: Turns violations.xml into ViolationCodes.java. -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		* Note: not all are errors, some merely reflect internal workings.
		...
		&lt;/p>
		&lt;dl>
		<xsl:apply-templates select="descendant::specification[@type='iri']"
			mode="javadoc" />
		&lt;/dl>

		&lt;p>Scheme specific checks are enabled. The syntax of the following schemes is
		fully supported:
		&lt;/p>
		&lt;dl>
		<xsl:apply-templates select="descendant::scheme[complete]"
			mode="javadoc" />
		&lt;/dl>
		...
		&lt;dl>
		<xsl:apply-templates
			select="descendant::scheme[not(complete)][*[not(self::name)]]" mode="javadoc" />
		&lt;/dl>


		&lt;p>The names of the following registered schemes are known, but they are
		otherwise unsupported:
		&lt;/p>
		&lt;dl>
		<xsl:apply-templates
			select="descendant::scheme[not(complete)][not(*[not(self::name)])]"
			mode="javadoc" />
		&lt;/dl>
		...
		&lt;/p>
		&lt;dl>
		<xsl:apply-templates select="descendant::specification[@type='other']"
			mode="javadoc" />
		&lt;/dl>
		...
		*/
		...
		<xsl:apply-templates select="descendant::specification"
			mode="static" />


		<xsl:apply-templates select="descendant::scheme"
			mode="static" />

		<xsl:apply-templates select="descendant::violation"
			mode="static" />
		}
		}
	...
	<xsl:template match="violation" mode="top">
		...
		*/
		...
		=
		<xsl:value-of select="count(preceding::violation)" />
		;
	...
	<xsl:template match="violation[description]" mode="javadoc">
		...
		<xsl:if
			test="count((spec|also)[id(@ref)/@type='iri' or id(@ref)/@type='scheme']) = 1">
			&lt;p>This violates the &lt;a href=
			"#ref-
			<xsl:value-of
				select="(spec[id(@ref)/@type='iri' or id(@ref)/@type='scheme']|also)/@ref" />
			">[
			<xsl:value-of
				select="(spec[id(@ref)/@type='iri' or id(@ref)/@type='scheme']|also)/@ref" />]&lt;/a>
			specification.&lt;/p>
		</xsl:if>
		<xsl:if
			test="count((spec|also)[id(@ref)/@type='iri' or id(@ref)/@type='scheme']) > 1">
		...
		<xsl:if
			test="not((spec|also)[id(@ref)/@type='iri' or id(@ref)/@type='scheme'])">
			&lt;p>This does not violate any of the supported IRI, URI or scheme
			specifications.&lt;/p>
		</xsl:if>

		<xsl:apply-templates select="should|minting|security|dns|must"
			mode="javadoc" />
		<xsl:if test="not(must|should|minting|security)">
			&lt;p>This violation has &lt;a
			href="http://www.apps.ietf.org/rfc/rfc2119.html#sec-1">MUST&lt;/a>
			force.&lt;/p>
		</xsl:if>

		<xsl:if test="bad">
			&lt;p>The following are examples of IRIs that have this
			violation:&lt;/p>
			&lt;ul>
		...
		<xsl:if test="good">
			&lt;p>The following are examples of IRIs that do not have this
			violation:&lt;/p>
			&lt;ul>
	...
	<xsl:template match="defn" mode="javadoc">
		&lt;dt>
		<xsl:apply-templates select="." mode="javadocDefn" />
		&lt;/dt>
		&lt;dd>
	...
	<xsl:variable name="escape-text-rules">
		&lt;;&amp;lt;:
	</xsl:variable>
	...
	<xsl:template match="should" mode="javadoc">
		&lt;p>This violation has &lt;a
		href="http://www.apps.ietf.org/rfc/rfc2119.html#sec-1">SHOULD&lt;/a>
		force.
		By default, this is treated as an error (for the relevant specs), but
		that behaviour can be modified by
	...
	<xsl:template match="security" mode="javadoc">
		&lt;p>This violation may indicate security issues, and the behaviour
		of a factory
	...
	<xsl:template match="violation/dns" mode="javadoc">
		&lt;p>This violation is relevant for IRIs using DNS as the registry of
		hostnames.
	...
	<xsl:template match="scheme/dns" mode="javadoc">
		...
		The IDNA compatibile processing of IRIs for this scheme is enabled if
		this scheme is enabled.
		To disable this behaviour, this scheme has to be disabled using // TODO
		how to disable scheme
		and DNS compatibile rules have to be disabled using {@link
		IRIFactory#dnsViolation}.
	</xsl:template>

	<xsl:template match="minting" mode="javadoc">
		&lt;p>This violation relates to creating your own IRIs, rather than
		accepting and processing other peoples', and the behaviour of a
		factory
		implementing the relevant specs can be modified by
	...
	<!-- <xsl:template match="bad|good[not(unicode)]" mode="javadoc"> &lt;li>&amp;lt;&lt;code><xsl:value-of 
		select="."/>&lt;/code>>&lt;/li> </xsl:template> -->
	...
	<xsl:template match="specification|scheme" mode="javadoc">
		...
		<xsl:if test="section or @section">
			(section
			<xsl:value-of select="section|@section" />
			)
		</xsl:if>
		&lt;/a>
		&lt;/dd>
		...
		<xsl:if
			test="*[not(self::name) and not(self::section) and not(self::complete)]">
			&lt;dd>
			<xsl:apply-templates
				select="*[not(self::bad)][not(self::good)][not(self::defn)]" mode="javadoc" />
			<xsl:if test="good">
				&lt;br/>
	...
	<xsl:template match="prohibit" mode="javadoc">
		The
		<xsl:value-of select="@component" />
		component must be omitted.&lt;br />
	</xsl:template>

	<xsl:template match="require" mode="javadoc">
		The
		<xsl:value-of select="@component" />
		component is required.&lt;br />
	</xsl:template>
	...
	<xsl:template match="pattern[text()][@reserved]" mode="javadoc">
		...
		<xsl:apply-templates mode="javadoc" select="node()" />
		&lt;/li>
		&lt;li>
		may use
		<xsl:value-of select="@reserved" />
		as sub-delimiters, and care must
		be taken when percent escaping or unescaping these delimiters to not
		disrupt
	...
	<xsl:template match="pattern[not(text())][@reserved]"
		mode="javadoc">
		The
		<xsl:value-of select="@component" />
		component
	...
	<xsl:template match="pattern[text()][not(@reserved)]"
		mode="javadoc">
		The
		<xsl:value-of select="@component" />
		component
	...
	<xsl:template match="*[../@ref or @ref]" mode="rawUrl">
		<xsl:apply-templates select="id(../@ref|@ref)"
			mode="url" />
	</xsl:template>
	...
	<!-- <xsl:template match="*" mode="url"> <xsl:message>No match.</xsl:message> 
		</xsl:template> -->
	...
	<xsl:template match="spec[@fragment or @section]|also[@fragment or @section]"
		mode="javadoc-spec-list">
	...
	<xsl:template match="violation[name/text()='PROHIBITED_COMPONENT_PRESENT']"
		mode="require-prohibit">
		<xsl:apply-templates select="//prohibit" mode="require-prohibit" />
	</xsl:template>
	<xsl:template match="violation[name/text()='REQUIRED_COMPONENT_MISSING']"
		mode="require-prohibit">
		<xsl:apply-templates select="//require" mode="require-prohibit" />
	</xsl:template>
	<xsl:template
		match="violation[name/text()='PROHIBITED_COMPONENT_PRESENT' 
                                 or name/text()='REQUIRED_COMPONENT_MISSING']"
		mode="static" priority="10">
		...
		",
		0
		<xsl:apply-templates select="(should|minting|must|dns|security)"
			mode="force" />
		,
		new InSpec[]{
	...
	<xsl:template match="violation[description]" mode="static"
		priority="1">
		new ViolationCodeInfo(
		<xsl:value-of select="name" />
		,
	...
	<xsl:template match="scheme" mode="static">
		...
		<xsl:apply-templates select="*[not(self::good or self::bad)]"
			mode="static" />
	</xsl:template>
	...
	<!-- <xsl:template match="spec[text/@component]" mode="static"> <xsl:apply-templates 
		select="text" mode="static"/> </xsl:template> -->
	...
	<xsl:template match="require[not(text)]|prohibit[not(text)]"
		mode="require-prohibit">
		new FromSpec_
		<xsl:value-of select="../@type" />
		(
	...
	<xsl:template match="text" mode="static">
		new FromSpec_
		<xsl:value-of select="id(../@ref)/@type" />
		(
		"
		<xsl:value-of select="../@ref" />
		",
	...
	<xsl:template name="translate">
		...
		<xsl:if test="not($rules='')">
			...
			<xsl:variable name="replace"
				select="substring-after(substring-before($rules,':'),';')" />
  ...
  "</xsl:text>
	...
	<xsl:template match="bad|good" mode="static">
		...
		<xsl:apply-templates select="node()"
			mode="java-string-not-normalized" />
		",
	</xsl:template>
