...
/**
 * An implementation of PrefixMapping. The mappings are stored in a pair of hash tables, one per direction. The test for a legal prefix is left to xerces's
 * XMLChar.isValidNCName() predicate.
 */
public class PrefixMappingImpl implements PrefixMapping
{
	...
	private void checkProper(String uri)
	{
		...
		// if (!isNiceURI( uri )) throw new NamespaceEndsWithNameCharException(
		// uri );
	}
	...
	/**
	 * Answer this PrefixMapping after updating it with the <code>(p, u)</code> mappings in <code>other</code> where neither <code>p</code> nor <code>u</code>
	...
	/**
	 * Add the bindings in the prefixToURI to our own. This will fail with a ClassCastException if any key or value is not a String; we make no guarantees about
	 * order or completeness if this happens. It will fail with an IllegalPrefixException if any prefix is illegal; similar provisos apply.
	 * 
	 * @param other
	...
	/**
	 * Expand a prefixed URI. There's an assumption that any URI of the form Head:Tail is subject to mapping if Head is in the prefix mapping. So, if someone
	 * takes it into their heads to define eg "http" or "ftp" we have problems.
	 */
	@Override
	 ...
	 * <p>
	 * Relies on <code>splitNamespace</code> to carve uri into namespace and localname components; this ensures that the localname is legal and we just have to
	 * (reverse-)lookup the namespace in the prefix table.
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.PrefixMapping#qnameFor(java.lang.String)
	 */
	@Override
	...
	/**
	 * Compress the URI using the prefix mapping. This version of the code looks through all the maplets and checks each candidate prefix URI for being a
	 * leading substring of the argument URI. There's probably a much more efficient algorithm available, preprocessing the prefix strings into some kind of
	 * search table, but for the moment we don't need it.
	 */
	@Override
	...
	/**
	 * Answer a prefixToURI entry in which the value is an initial substring of <code>uri</code>. If <code>partial</code> is false, then the value must equal
	 * <code>uri</code>.
	 * 
