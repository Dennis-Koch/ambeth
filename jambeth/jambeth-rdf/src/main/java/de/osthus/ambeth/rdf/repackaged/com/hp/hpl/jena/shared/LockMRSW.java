...
/**
 * Lock implemenetation using a Multiple Reader, Single Writer policy. All the locking work is done by the imported WriterPreferenceReadWriteLock. Ths class
 * adds:
 * <ul>
 * <li>The same thread that acquired a lock should release it</li>
 * <li>Lock promotion (turning read locks into write locks) is deteched as an error</li>
 * <ul>
 */
...
public class LockMRSW implements Lock
{
	...
	// WriterPreferenceReadWriteLock mrswLock = new
	...
	{
		// Don't make {enter|leave}CriticalSection synchronized - deadlock will
		// occur.
		...
		// If we have a read lock, but no write locks, then the thread is
		// attempting
		...
		// Trying to get a read lock after a write lock - get a write lock
		// instead.
	...
	synchronized private String report(LockState state)
	{
	...
	static class LockState
	{
		...
		void clean()
		{
			if (lock.activeReadLocks.get() == 0 && lock.activeWriteLocks.get() == 0)
			{
				...
				// The alternative, to delay now and do a more sophisticated
				// global GC
