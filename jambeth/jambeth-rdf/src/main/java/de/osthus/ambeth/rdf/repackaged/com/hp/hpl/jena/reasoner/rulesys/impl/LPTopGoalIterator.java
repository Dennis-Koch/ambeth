...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.rulesys.BackwardRuleInfGraphI;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.iterator.ClosableIterator;

import java.util.*;

/**
 * Wraps up the results an LP rule engine instance into a conventional iterator. Ensures that the engine is closed and detached from the inference graph if the
 * iterator hits the end of the result set.
 */
public class LPTopGoalIterator implements ClosableIterator<Triple>, LPInterpreterContext
{
	 ...
	 * 
	 * Elsewhere code takes an LPBRuleEngine then an LPTopGoalIterator Ensure we do that lock order here as well as just synchronized on the method reverses the
	 * lock ordering, leading to deadlock.
	 */
	...
	/**
	 * Notify this context that a brach was suspended awaiting futher results from the given generator.
	 */
	@Override
	...
	/**
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.iterator.ClosableIterator#close()
	 */
	@Override
	public void close()
	{
		...
		synchronized (lpEngine)
		{
			...
			synchronized (this)
			{
				if (interpreter != null)
				{
					...
					// "Entering close sync block on " + interpreter.getEngine()
					// );
					...
					lookAhead = null;
					// LogFactory.getLog( getClass() ).debug(
					// "Nulling and closing LPTopGoalIterator " + interpreter );
					interpreter.close();
					// was TEMP experiment:
					// interpreter.getEngine().detach(interpreter);
					interpreter = null;
					isReady = false;
					checkReadyNeeded = false;
					nextToRun = null;
					// choicePoints = null; // disabled to prevent async close
					// causing problems
					// LogFactory.getLog( getClass() ).debug(
					// "Leaving close sync block " );
				}
			}
		}
	}
	...
	/**
	 * Check that the iterator has either cleanly closed or the version stamp is still valid
	 */
