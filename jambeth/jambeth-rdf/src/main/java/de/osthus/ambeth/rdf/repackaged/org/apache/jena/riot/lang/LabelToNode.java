...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.MapWithScope;
...
/**
 * Allocation Nodes (Bnodes usually) based on the graph and label Various different policies. See {@link SyntaxLabels#createLabelToNode} for getting a default
 * setup; some of the others are for testing and debugging and may not generate legal RDF overall (e.g. reparsign the same file gets the same bNodes)
 */
...
public class LabelToNode extends MapWithScope<String, Node, Node>
{
	...
	/**
	 * Allocation using syntax label; output is unsafe for reading (use {@link #createUseLabelEncoded()} for output-input).
	 * 
	 * The reverse operation is provided by {@link NodeToLabel#createBNodeByLabelAsGiven()} but the pair is <em>unsafe</em> for output-input. Use encoded labels
	 * for that.
	 * 
	...
	/**
	 * Allocation using an encoded syntax label (i.e. _:B&lt;encoded&gt; format from {@link NodeFmtLib#encodeBNodeLabel}).
	 * 
	 * The reverse operation is provided by {@link NodeToLabel#createBNodeByLabelEncoded()}. This pair should be used to write out and recover blank node by
	 * internal id.
	 */
