...
/**
 * Model wrapper which provides normal access to an underlying model but also maintains a snapshot of the triples it was last known to contain. A snapshot
 * action causes the set of changes between this and the previous snapshot to be calculated and the cache updated. The snapshot process will also fire change
 * notification.
 */
...
public class MonitorModel extends ModelCom
{
	...
	/**
	 * Compute the differences between the current monitored graph and the last snapshot. The changes will also be forwarded to any listeners. Then take a new
	 * snapshot.
	 * 
	 * @param additions
	 *            a place in which the set of newly added statements should be noted, can be null
	 * @param deletions
