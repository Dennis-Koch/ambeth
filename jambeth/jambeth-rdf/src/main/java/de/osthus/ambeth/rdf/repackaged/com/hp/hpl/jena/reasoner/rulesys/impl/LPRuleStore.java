...
public class LPRuleStore extends RuleStore
{
	...
	/**
	 * A map from predicate to a list of RuleClauseCode objects for that predicate. Uses Node_RuleVariable.WILD for wildcard predicates.
	 */
	...
	/**
	 * Return an ordered list of RuleClauseCode objects to implement the given query pattern. This may use indexing to narrow the rule set more that the
	 * predicate-only case.
	 * 
	 * @param goal
	...
	/**
	 * Return true if the given goal is tabled, currently this is true if the predicate is a tabled predicate or the predicate is a wildcard and some tabled
	 * predictes exist.
	 */
	...
	/**
	 * Return true if the given predicated is tabled, currently this is true if the predicate is a tabled predicate or the predicate is a wildcard and some
	...
	/**
	 * Compile all the rules in a table. initially just indexed on predicate but want to add better indexing for the particular cases of wildcard rules and type
	 * rules.
	 */
	protected void compileAll()
	{
		...
		// Now add the wild card rules into the list for each non-wild
		// predicate)
		...
		for (Map.Entry<Node, Map<Node, List<RuleClauseCode>>> entry : indexPredicateToCodeMap.entrySet())
		{
			...
			// Now fold the rules that apply to any index entry into all the
			// indexed entries
	...
	/**
	 * Add/remove a single rule from the store. Overridden in order to reset the "isCompiled" flag.
	 * 
	 * @param rule
	 *            the rule, single headed only
	 * @param isAdd
