...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.assembler.assemblers.AssemblerGroup;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.assembler.exceptions.*;
...
/**
 * AssemblerHelp provides utility methods used by, and useful for working with, the Assembler code, including the methods that expand a model to include the
 * required inferences and to find the most specific type of a root in an assembler specification.
 */
public class AssemblerHelp
{
	...
	/**
	 * Answer a Resource .equals() to <code>root</code>, but in the expanded model.
	 */
	...
	/**
	 * Answer the full model of <code>m</code>, with all its imports included and with the necessary properties added from the JA schema. However, if the magic
	 * footprint triple (ja:this, rdf:type, ja:Expanded) is present in the model, it is returned unchanged. Imports are managed by the shared
	...
	/**
	 * Answer the full model of <code>m</code>, with all its imports included and with the necessary properties added from the JA schema. However, if the magic
	 * footprint triple (ja:this, rdf:type, ja:Expanded) is present in the model, it is returned unchanged. Imports are managed by <code>im</code>.
	 */
	public static Model fullModel(ImportManager im, Model m)
	{
		return m.contains(JA.This, RDF.type, JA.Expanded) ? m : (Model) ModelExpansion.withSchema(im.withImports(m), JA.getSchema())
				.add(JA.This, RDF.type, JA.Expanded).setNsPrefixes(PrefixMapping.Extended).setNsPrefixes(m);
	}

	/**
	 * Load all the classes which are objects of any (t, ja:loadClass, S) statements in <code>m</code>. The order in which the classes are loaded is not
	 * specified, and loading stops immediately if any class cannot be loaded.
	 * <p>
	 * Contrast with <code>loadClasses(AssemblerGroup,Model)</code>, which loads classes and assumes that those classes are assemblers to be added to the group.
	 */
	...
	/**
	 * Load all the classes which are objects of any (t, ja:assembler, S) statements in <code>m</code>. <code>group.implementWIth(t,c)</code> is called for each
	 * statement, where <code>c</code> is an instance of the class named by <code>S</code>. The order in which the classes are loaded is not specified, and
	 * loading stops immediately if any class cannot be loaded.
	 */
	...
	/**
	 * Load the class named by the object of <code>s</code>, run its <code>whenRequiredByAssembler</code> method if any, and register an
	 * <code>implementWith</code> for the subject of <code>s</code> and an instance of the class.
	 */
	...
	/**
	 * Load the class named by the object of <code>s</code> if necessary. If that class has a static method <code>whenRequiredByAssembler</code> with an
	 * <code>AssemblerGroup</code> argument, call that method passing it <code>ag</code>.
	 */
	...
	private static Class<?> loadClassNamedBy(Statement s)
	{
		try
		{
			return Class.forName(getString(s));
		}
		catch (Exception e)
		{
			throw new JenaException(e);
	...
	/**
	 * Answer the most specific type of <code>root</code> that is a subclass of ja:Object. If there are no candidate types, answer <code>givenType</code> . If
	...
	/**
	 * Answer the most specific type of <code>root</code> that is a subclass of <code>givenType</code>. If there are no candidate types, answer
	 * <code>givenType</code>. If there is more than one type, throw a NoSpecificTypeException.
	 */
	...
	/**
	 * Answer all the types of <code>root</code> which are subtypes of <code>baseType</code> and which do not have subtypes which are also types of
	 * <code>root</code>.
	 */
	...
	/**
	 * Answer the resource that is the object of the statement <code>s</code>. If the object is not a resource, throw a BadObjectException with that statement.
	 */
	...
	/**
	 * Answer the plain string object of the statement <code>s</code>. If the object is not a string literal, throw a BadObjectException with that statement.
	 */
	...
	/**
	 * Answer the String value of the literal <code>L</code>, which is the object of the Statement <code>s</code>. If the literal is not an XSD String or a
	 * plain string without a language code, throw a BadObjectException.
	 */
	...
	/**
	 * Answer a Set of the ja:Object resources in the full expansion of the assembler specification model <code>model</code>.
	 */
	...
	/**
	 * Answer a Set of the objects in the full expansion of the assembler specification <code>model</code> which have rdf:type <code>type</code>, which
	...
	/**
	 * Answer the single resource in <code>singleRoot</code> of type <code>ja:Model</code>. Otherwise throw an exception.
	 */
	...
	/**
	 * Answer the single resource in <code>singleRoot</code> of type <code>type</code>. Otherwise throw an exception.
	 */
