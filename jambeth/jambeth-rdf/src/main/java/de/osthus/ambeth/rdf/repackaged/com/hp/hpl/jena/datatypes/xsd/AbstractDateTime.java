...
/**
 * Base class for representation of XSD duration, time, date/time and related datatype instances. We are using the Xerces internal packages for the all heavy
 * lifting which represent date/times using an int array. These wrapper classes just provide more convenient access to the date values.
 * <p>
...
public class AbstractDateTime implements Comparable<AbstractDateTime>
{
	...
	/**
	 * Normal java comparison function. Treats INDETERMINATE as the same as equals. This is not strictly correct but seems like an appropriate way to handle
	 ...
	 * @param date2
	 *            normalized date representation of the second value
	 * @param strict
	...
	protected short compareDates(int[] date1, int[] date2, boolean strict)
	{
		...
		if (date1[utc] == 'Z')
		{
			...
			//
			cloneDate(date2, tempDate); // clones date1 value to global
										// temporary storage: fTempDate
			timeZone[hh] = 14;
			timeZone[mm] = 0;
			tempDate[utc] = '+';
		...
		else if (date2[utc] == 'Z')
		{
			...
			//
			cloneDate(date1, tempDate); // clones date1 value to global
										// temporary storage: tempDate
			timeZone[hh] = 14;
			timeZone[mm] = 0;
			tempDate[utc] = '-';
