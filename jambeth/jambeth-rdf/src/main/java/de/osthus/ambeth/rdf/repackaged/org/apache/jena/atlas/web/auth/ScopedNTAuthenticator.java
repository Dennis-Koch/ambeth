...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.auth;
...
/**
 * <p>
 * A credentials based authenticator where credentials are scoped to URIs. This allows for a single authenticator to present different credentials to different
 * URIs as appropriate. Works with the NTLM authentication schemes.
 * </p>
 * <p>
 * See {@link ScopedAuthenticator} for an implementation that works for the Basic and Digest authentication schemes.
 * </p>
 * 
 */
public class ScopedNTAuthenticator extends AbstractScopedAuthenticator<Pair<Tuple<String>, char[]>>
{
	 ...
	 * @param domain
	 *            Domain, this is the domain you are authenticating in which may not necessarily be the domain your workstation is in
	 */
	 ...
	 * 
	 * @param credentials
	 *            Credentials, the left of the pair should be a tuple with at least three fields where the first contains the user name, the second the
	 *            workstation and the third the domain. The right of the pair should be the password.
	 */
	...
	protected Credentials createCredentials(URI target)
	{
		...
		return new NTCredentials(credentials.getLeft().get(0), new String(credentials.getRight()), credentials.getLeft().get(1), credentials.getLeft().get(2));
	}
