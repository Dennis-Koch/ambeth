...
public class ResolvedRelativeIRI extends AbsIRIImpl
{
	...
	private void transformReferences()
	{
		...
		if (rel.has(SCHEME)
				&& (!getFactory().getSameSchemaRelativeReferences(rel.getScheme()) || !base.has(SCHEME) || !rel.getScheme().equalsIgnoreCase(base.getScheme())
	...
	{
		if (base.has(AUTHORITY) && base.getRawPath().equals(""))
		{
	...
	{
		return field == PATH ? pathErrors : field < useBaseUntilThisComponent ? base.errors(field) : rel.errors(field);
	}

	@Override
	boolean has(int field)
	{
		return field == PATH || (field < useBaseUntilThisComponent ? base.has(field) : rel.has(field));
	}

	@Override
	String get(int field)
	{
		return field == PATH ? path : field < useBaseUntilThisComponent ? base.get(field) : rel.get(field);
	}
	...
	{
		return useBaseUntilThisComponent > PATH ? base.pathRemoveDots() : path;
	}
	...
	{
		if (useBaseUntilThisComponent == SCHEME || useBaseUntilThisComponent == AUTHORITY)
