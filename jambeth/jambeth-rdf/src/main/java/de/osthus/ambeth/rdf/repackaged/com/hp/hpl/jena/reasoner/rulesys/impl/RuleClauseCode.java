...
/**
 * Object used to hold the compiled bytecode stream for a single rule clause. This uses a slightly WAM-like code stream but gluing of the clauses together into
 * disjunctions is done in the interpreter loop so a complete predicate is represented as a list of RuleClauseCode objects.
 */
public class RuleClauseCode
{
	 ...
	 * 
	 * @param ruleStore
	 *            the store of LP rules through which calls to other predicates can be resolved.
	 */
	...
	static class CompileState
	{
		...
		CompileState(Rule rule)
		{
			...
			this.rule = rule;
			// Create a scratch area for assembling the code, use a worst-case
			// size estimate
		...
		/**
		 * Emit the code for any bound/unbound tests add start of body and return the number of body clauses dealt with.
		 */
		 ...
		 *            the node to emit
		 * @param argi
		 *            the argument register to address
		 */
		void emitHeadGet(Node node, int argi)
		{
			if (node instanceof Node_RuleVariable)
			{
				...
				if (isTemp(var))
				{
					...
					if (occurrences.size() == 2 && occurrences.get(0).index <= 2 && occurrences.get(0).index == occurrences.get(1).index)
					{
		...
		void emitBody(TriplePattern goal, LPRuleStore store)
		{
			...
			else
			{
				...
				{
					// Experimental. Force tabling of any wildcard predicate
					// calls
					code[p++] = CALL_TABLED;
				}
		 ...
		 *            the node to emit
		 * @param argi
		 *            the argument register to use
		 * @param deref
		 *            if true force a dereference of the variable binding at this point for use in calling builtins
		 */
		void emitBodyPut(Node node, int argi, boolean deref)
		{
			if (argi >= MAX_ARGUMENT_VARS)
			{
				throw new LPRuleSyntaxException("Rule too complex for current implementation\n" + "Rule clauses are limited to " + MAX_ARGUMENT_VARS
						+ " argument variables\n", rule);
			...
			if (node instanceof Node_RuleVariable)
			{
				...
				{
					List<TermIndex> occurrences = varOccurrence.get(var);
					if (occurrences.size() == 2 && occurrences.get(0).index == occurrences.get(1).index)
					{
						// No movement code required, var in right place
					}
					else
		...
		void emitBody(Functor functor)
		{
			...
			{
				throw new LPRuleSyntaxException("Wrong number of arguments to functor " + functor.getName() + " : got " + functor.getArgLength()
		...
		/**
		 * Classifies the variables now and side effects the index number of the variables so they lie in two sequences - temp and permanent separately.
		 */
		void classifyVariables(Rule rule)
		{
			// Build index data structure into var use in each term in the rule
			@SuppressWarnings("unchecked")
			List<Node>[] termListArray = new List[rule.bodyLength() + 1];
			...
			// Classify vars as permanent unless they are just dummies (only
			// used once at all)
			...
			for (List<TermIndex> occurrences : varOccurrence.values())
			{
				...
				// Don't think we need to protected builtin's any more, so
				// ignore that test
				// if (inBuiltin) {
			...
			{
				throw new LPRuleSyntaxException("Rule too complex for current implementation\n" + "Rule clauses are limited to " + MAX_PERMANENT_VARS
						+ " permanent variables\n", rule);
			}

			if (tempVars.size() > MAX_TEMPORARY_VARS)
			{
				throw new LPRuleSyntaxException("Rule too complex for current implementation\n" + "Rule clauses are limited to " + MAX_TEMPORARY_VARS
						+ " temporary variables\n", rule);
			}

			// Builtins in the forward system use the var index to modify
			// variable bindings.
			// At one time I though the LP system might need to emulate this but
			// (a) it doesn't and
			// (b) renumber vars to make that possible wreaks rule equality
			// which wreaks add/remove in
			// hybrid rule sets. This code left in but commented out as a
			// reminder to not go down
			// that path again.
		...
		boolean isDummy(Node_RuleVariable var)
		{
			...
			if (occurances == null || occurances.size() <= 1)
				return true;
			return false;
		}
	...
	/**
	 * Inner class used to represent the occurance of a variable in a term. Not an abstract data type, just a structure directly accessed.
	 */
