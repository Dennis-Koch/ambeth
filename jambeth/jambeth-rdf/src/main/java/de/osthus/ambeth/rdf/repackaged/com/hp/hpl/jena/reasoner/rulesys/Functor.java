...
/**
 * A functor comprises a functor name and a list of arguments. The arguments are Nodes of any type except functor nodes (there is no functor nesting). Functors
 * play three roles in rules - in heads they represent actions (procedural attachement); in bodies they represent builtin predicates; in TriplePatterns they
 * represent embedded structured literals that are used to cache matched subgraphs such as restriction specifications.
 */
public class Functor implements ClauseEntry
{
	 ...
	 * @param args
	 *            a list of nodes defining the arguments
	 */
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof Functor)
		{
			...
			if (name.equals(f2.name) && args.length == f2.args.length)
			{
				for (int i = 0; i < args.length; i++)
				{
					if (!args[i].sameValueAs(f2.args[i]))
						return false;
				}
				return true;
			}
	...
	public boolean sameAs(Object o)
	{
		if (o instanceof Functor)
		{
			...
			if (name.equals(f2.name) && args.length == f2.args.length)
			{
				for (int i = 0; i < args.length; i++)
				{
					if (!Node_RuleVariable.sameNodeAs(args[i], f2.args[i]))
						return false;
				}
				return true;
			}
