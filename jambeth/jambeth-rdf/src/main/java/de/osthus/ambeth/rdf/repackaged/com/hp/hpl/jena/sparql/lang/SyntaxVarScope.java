...
{
	/*
	 * SPARQL 1.1 "in scope" rules These define the variables from a pattern that are in-scope These are not the usage rules.
	 * 
	 * Syntax Form In-scope variables
	 * 
	 * Basic Graph Pattern (BGP) v occurs in the BGP Path v occurs in the path Group { P1 P2 ... } v is in-scope if in-scope in one or more of P1, P2, ... GRAPH
	 * term { P } v is term or v is in-scope in P { P1 } UNION { P2 } v is in-scope in P1 or in-scope in P2 OPTIONAL {P} v is in-scope in P SERVICE term {P} v
	 * is term or v is in-scope in P (expr AS v) for BIND, SELECT and GROUP BY v is in-scope SELECT ..v .. { P } v is in-scope if v is mentioned as a project
	 * variable SELECT * { P } v is in-scope in P VALUES var (values) v is in-scope if v is in varlist VALUES varlist (values) v is in-scope if v is in varlist
	 */
