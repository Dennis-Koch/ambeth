...
/**
 * Checks a triple against the grounded matches and intra-triple matches for a single rule clause. If the match passes it creates a binding environment token
 * and passes it on the the RETE network itself. The checks and bindings are implemented using a simple byte-coded interpreter.
 */
public class RETEClauseFilter implements RETESourceNode
{
	 ...
	 * 
	 * @param instructions
	 *            the set of byte-coded instructions and argument pointers.
	 * @param args
		...
		for (int pc = 0; pc < instructions.length;)
		{
			switch (instructions[pc++])
			{
				...
				case TESTValue:
					...
					if (!getTripleValue(triple, instructions[pc++], lastFunctor).sameValueAs(args[instructions[pc++]]))
						return;
					break;
				...
				case TESTFunctorName:
					...
					if (!n.isLiteral())
						return;
					if (n.getLiteralDatatype() != Functor.FunctorDatatype.theFunctorDatatype)
						return;
	 ...
	 * 
	 * @param netCopy
	 *            a map from RETENode to cloned instance
	 * @param context
