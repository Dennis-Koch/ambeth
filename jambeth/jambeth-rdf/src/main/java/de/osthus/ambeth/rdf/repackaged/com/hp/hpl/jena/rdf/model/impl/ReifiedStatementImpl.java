...
public class ReifiedStatementImpl extends ResourceImpl implements ReifiedStatement
{
	...
	/**
	 * private constructor, relies (ugh) on super(uri, model) generating bnode if uril == null.
	 */
	...
	{
		/**
		 * convert a _node_ into a ReifiedStatement in the enhanced graph _eg_ by looking into this graph's reifier to find the binding for the node; throw a
		 * DoesNotReify exception if there's no mapping.
		 */
		@Override
		...
		/**
		 * Answer true iff the node <code>n</code> can become a reified statement, ie it is associated with a triple by <code>eg</code>'s Reifier.
		 * 
		 * @param eg
		 *            the (enhanced) graph who's Reifier might hold the triple
		 * @param n
		 *            the node who's triple is required
	...
	/**
	 * tell the underlying graph's reifier that this ReifiedStatement's node represents any Statement with this statement's triple. (May throw an exception if
	 * the node is already reified to something different.)
	 */
