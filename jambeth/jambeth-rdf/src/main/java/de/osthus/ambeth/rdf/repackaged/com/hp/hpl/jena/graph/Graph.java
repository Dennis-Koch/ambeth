...
/**
 * The interface to be satisfied by implementations maintaining collections of RDF triples. The core interface is small (add, delete, find, contains) and is
 * augmented by additional classes to handle more complicated matters such as reification, query handling, bulk update, event management, and transaction
 * handling.
 * <p>
...
public interface Graph extends GraphAdd
{
	...
	/**
	 * true if this graph's content depends on the other graph. May be pessimistic (ie return true if it's not sure). Typically true when a graph is a
	 * composition of other graphs, eg union.
	 * 
	 * @param other
	...
	/**
	 * Compare this graph with another using the method described in <a href="http://www.w3.org/TR/rdf-concepts#section-Graph-syntax">
	 * http://www.w3.org/TR/rdf-concepts#section-Graph-syntax </a>
	 * 
	 * @param g
	...
	/**
	 * Answer true iff the graph contains a triple matching (s, p, o). s/p/o may be concrete or fluid. Equivalent to find(s,p,o).hasNext, but an implementation
	 * is expected to optimise this in easy cases.
	 */
	...
	/**
	 * Answer true iff this graph is empty. "Empty" means "has as few triples as it can manage", because an inference graph may have irremovable axioms and
	 * their consequences.
	 */
	boolean isEmpty();

	/**
	 * For a concrete graph this returns the number of triples in the graph. For graphs which might infer additional triples it results an estimated lower bound
	 * of the number of triples. For example, an inference graph might return the number of triples in the raw data graph.
	 */
	int size();
