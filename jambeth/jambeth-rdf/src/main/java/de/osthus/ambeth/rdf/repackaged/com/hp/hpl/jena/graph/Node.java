...
/**
 * A Node has five subtypes: Node_Blank, Node_Anon, Node_URI, Node_Variable, and Node_ANY. Nodes are only constructed by the node factory methods, and they will
 * attempt to re-use existing nodes with the same label if they are recent enough.
 */
...
public abstract class Node
{
	...
	/**
	 * Build a literal node from its lexical form. The lexical form will be parsed now and the value stored. If the form is not legal this will throw an
	 * exception.
	 * 
	 * @param lex
	...
	/**
	 * Build a typed literal node from its lexical form. The lexical form will be parsed now and the value stored. If the form is not legal this will throw an
	 * exception.
	 * 
	 * @param lex
	 *            the lexical form of the literal
	...
	/**
	 * Visit a Node and dispatch on it to the appropriate method from the NodeVisitor <code>v</code>.
	 * 
	 * @param v
	...
	/**
	 * Answer the object which is the index value for this Node. The default is this Node itself; overridden in Node_Literal for literal indexing purposes. Only
	...
	/**
	 * The canonical NULL. It appears here so that revised definitions [eg as a bnode] that require the cache-and-maker system will work; the NodeMaker
	...
	/**
	 * Test that two nodes are semantically equivalent. In some cases this may be the same as equals, in others equals is stricter. For example, two xsd:int
	...
	/**
	 * Answer true iff this node accepts the other one as a match. The default is an equality test; it is over-ridden in subclasses to provide the appropriate
	...
	/**
	 * Answer a human-readable representation of this Node. It will not compress URIs, nor quote literals (because at the moment too many places use toString()
	 * for something machine-oriented).
	 */
	@Override
	...
	/**
	 * Answer a human-readable representation of this Node where literals are quoted according to <code>quoting</code> but URIs are not compressed.
	 */
