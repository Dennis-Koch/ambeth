...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.vocabulary.OntDocManagerVocab;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.vocabulary.RDF;

/**
 * <p>
 * Provides services for managing ontology documents, including loading imported documents, and locally caching documents from resolvable URL's to improve load
 * performance. This class now delegates some of the responsibility for resolving URI's and caching models to
 * {@link de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.util.FileManager FileManager}. By default, OntDocumentManager will use a copy of the singleton global
 * FileManager. Alternatively, a specific <code>FileManager</code> can be given to a document manager instance to use when resolving URI's and file paths. Note
 * that the default behaviour is to hold a <strong>copy</strong> of the global file manager. In order to ensure that the document manager directly uses the
 * global file manager (e.g. so that document manager sees updates to the location mappings held by the file manager), use the
 * {@link #setFileManager(FileManager)} method. For example:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * Note that in Jena 2.3, we have deprecated the capability of the document manager to store a table of known prefixes, and a table mapping document URI's to
 * ontology language types. <strong>The intention is to remove both of these capabilities from Jena 2.4 onwards</strong>. If this change would be problematic,
 * please send email to the <a href="http://groups.yahoo.com/group/jena-dev">Jena support list</a>.
 * </p>
 */
public class OntDocumentManager
{
	 ...
	 * 
	 * @param path
	 *            The search path to search for initial metadata, which will also replace the current search path for this document manager. Use null to prevent
	 *            loading of any initial ontology metadata. The path is a series of URL's, separated by a semi-colon (;).
	 */
	...
	/**
	 * <p>
	 * Initialise a document manager by with the given FileManager, and then searching the given path for ontology metadata about known ontologies cached
	 * locally.
	 * </p>
	 * 
	 * @param path
	 *            The search path to search for initial metadata
	...
	/**
	 * <p>
	 * Initialise a document manager with the given configuration model. This model is used in place of any model that might be found by searching the meta-data
	 * search path. Uses the default file manager, i.e. a copy of the global file manager.
	 * </p>
	 * 
	 * @param config
	...
	/**
	 * <p>
	 * Set the file manager used by this ODM instance to <strong>a copy</strong> of the global file manager (and, by extension, the global location mapper).
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Answer the path used to search for the ontology metadata to load. The format is a ';' separated list of URI's. The first URI on the path that is readable
	 * is taken to be the location of the local ontology metadata.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Change the search path for loading ontology metadata to the given path. If <code>replace</code> is true, any existing mappings are removed before the new
	 * path is searched. Otherwise, existing data will only be replaced if it is clobbered by keys loaded from the metadata loaded from the new path.
	 * </p>
	 * 
	 * @param path
	 *            The new metadata search path (see {@link #getMetadataSearchPath} for format)
	 * @param replace
	 *            If true, clear existing mappings first
	 */
	 ...
	 * 
	 * @param rfHandler
	 *            The new handler for failed document read attempts.
	 */
	 ...
	 * 
	 * @param config
	 *            Document manager configuration as an RDF model
	...
	/**
	 * <p>
	 * Configure this document manager according to the configuration options supplied by the given configuration model. If <code>reset</code> is true, the
	 ...
	 *            Document manager configuration as an RDF model
	 * @param reset
	 *            If true, reset the document manager to default values, before attempting to configure the document manager using the given model.
	 * @see #reset
	 */
	...
	/**
	 * <p>
	 * Reset all state in this document manager back to the default values it would have had when the object was created. Optionally reload the profile metadata
	 * from the search path. <strong>Note</strong> that the metadata search path is not changed by this reset.
	 * </p>
	 * 
	 * @param reload
	 *            If true, reload the configuration file from the search path.
	 */
	public void reset(boolean reload)
	{
		...
		{
			// we can do a general reset by throwing away the old FM and
			// creating a new one
			setFileManager();
		}
		else
	...
	/**
	 * <p>
	 * Reset all state in this document manager back to the default values it would have had when the object was created. This does <strong>not</strong> reload
	 * the configuration information from the search path. Note also that the metadata search path is one of the values that is reset back to its default value.
	 * </p>
	 * 
	 * @see #reset(boolean )
	 */
	...
	/**
	 * <p>
	 * Answer an iterator over the ontology document URI's that this document manager knows to re-direct to other URL's. <strong>Note</strong> that being in
	 * this iteration does <em>not</em> mean that a document with the given name is in the set of cached models held by this document manager.
	 * </p>
	 * 
	 * @return An Iterator ranging over the public URI strings for the known document URI's.
	 */
	 ...
	 * @param locationURL
	 *            A locally resolvable URL where an alternative copy of the ontology document can be found
	 */
	...
	/**
	 * <p>
	 * Add an entry that <code>model</code> is the appropriate model to use for the given ontology document. Will not replace any existing model that is cached
	 * for this URI (see {@link #addModel(String, Model, boolean)} for an alternative that can replace existing models).
	 * </p>
	 * 
	 * @param docURI
	 *            The public URI of the ontology document
	...
	/**
	 * <p>
	 * Add an entry that <code>model</code> is the appropriate model to use for the given ontology document
	 * </p>
	 * 
	 * @param docURI
	 *            The public URI of the ontology document
	 ...
	 *            A model containing the triples from the document
	 * @param replace
	 *            If true, replace any existing entry with this one.
	 */
	public void addModel(String docURI, Model model, boolean replace)
	{
		if (getFileManager().isCachingModels() && (replace || !getFileManager().hasCachedModel(docURI)))
		{
	 ...
	 * 
	 * @param docURI
	 *            The public URI for an ontology document
	 */
	...
	/**
	 * <p>
	 * Answer the ontology model that results from loading the document with the given URI. This may be a cached model, if this document manager's policy is to
	 * cache loaded models. If not, or if no model is cached, the document will be read into a suitable model. The model will contain the imports closure of the
	 * ontology, if that is the current policy of this document manager.
	 * </p>
	 * 
	 * @param uri
	...
	{
		// ensure consistency of document managers (to allow access to cached
		// documents)
	 ...
	 *            If true, load imported ontologies during load
	 ...
	 * 
	 * @param cacheModels
	 *            If true, models will be cached by URI
	 * @see #getCacheModels()
	 */
	 ...
	 * 
	 * @param uri
	 *            A URI to ignore when importing
	 */
	 ...
	 * 
	 * @param uri
	 *            A URI to ignore no longer when importing
	 */
	 ...
	 * according to the model's language profile. An occurs check allows cycles of importing safely. This method will do nothing if the
	 * {@linkplain #getProcessImports policy} for this manager is not to process imports. If the {@linkplain #getCacheModels cache policy} for this doc manager
	 * allows, models will be cached by URI and re-used where possible.
	 * </p>
	 * 
	 * @param model
	 *            An ontology model whose imports are to be loaded.
	 */
	public void loadImports(OntModel model)
	{
		if (m_processImports)
		{
			...
			// add the imported statements from the given model to the
			// processing queue
	 ...
	 * 
	 * @param model
	 *            A model to import into
	 * @param uri
	 *            The URI of a document to import
	 */
	...
	/**
	 * <p>
	 * Answer the URL of the most recently loaded policy URL, or null if no document manager policy has yet been loaded since the metadata search path was last
	 * set.
	 * </p>
	 * 
	 ...
	 * 
	 * @param model
	 *            The model to unload the imports from
	 * @param unloadQueue
	...
	protected void unloadImports(OntModel model, List<String> unloadQueue)
	{
		while (!unloadQueue.isEmpty())
		{
			...
			if (model.hasLoadedImport(importURI))
			{
				...
				// look up the cached model - if we can't find it, we can't
				// unload the import
				...
				if (importModel != null)
				{
					...
					// collect a list of the imports from the model that is
					...
					// check the list of imports of the model we have removed -
					// if they are not
					// imported by other imports that remain, we should remove
					// them as well
	 ...
	 * @param configPath
	 *            The path to search
	 * @return A model loaded by resolving an entry on the path, or null if no path entries succeed.
	 */
	 ...
	 * 
	 * @param metadata
	 *            A model containing metadata about ontologies.
	 */
	protected void processMetadata(Model metadata)
	{
		// there may be configuration for the location mapper in the ODM
		// metadata file
		getFileManager().getLocationMapper().processConfig(metadata);

		// first we process the general policy statements for this document
		// manager
	...
	/**
	 * <p>
	 * Load the document referenced by the given URI into the model. The cache will be used if permitted by the policy, and the imports of loaded model will be
	 ...
	 * @param readQueue
	 *            Cumulative read queue for this operation
	 */
	protected void loadImport(OntModel model, String importURI, List<String> readQueue)
	{
		if (m_processImports)
		{
			...
			if (in != model)
			{
				// queue the imports from the input model on the end of the read
				// queue
	...
	{
		// workaround - default model maker can apparently create models that
		// are closed
	 ...
	 * 
	 * @param model
	 *            A model to read statements into
	 * @param uri
	 *            A URI string
	 * @param warn
	 *            If true, warn on RDF exception
	...
	protected void setDefaultPrefixMappings()
	{
		...
		// PrefixMapping.Standard includes dc:, which OntModels traditionally
		// haven't included
	...
	/**
	 * Interface defining a handler call-back in the case that the {@link OntDocumentManager} fails in an attempt to read the contents of a URL into a model.
	 */
	public static interface ReadFailureHandler
	{
		/**
		 * Behaviour to invoke when the {@link OntDocumentManager} tries and fails to read an ontology document from a given URL.
		 * 
		 * @param url
		 *            The URL that the OntDocumentManager was trying to read
		 * @param model
		 *            The model that the OntDocumentManager is reading into
		 * @param e
	...
	public static interface ReadHook
	{
		/**
		 * <p>
		 * Behaviour that is invoked <strong>before</strong> the contents of the given source (URI or filename) are read into the given model. The return value
		 * from this method denotes a revised string to use in place of the supplied source string. Handlers are permitted to make state changes to the model
		 * and the ODM, but carefully!
		 * </p>
		 * 
		 * @param model
		 *            The model that is going to receive the contents of the source
		 * @param source
		 *            The identity of the source, as a file name or URI
		 * @param odm
		 *            The Ont Document Manager invoking this handler
		 * @return The revised name of the source (or the same string if no change to the source is required). Note that if this method returns
		...
		/**
		 * <p>
		 * Behaviour that is invoked <strong>just after</strong> the contents of the given source (URI or filename) have been read into the given model.
		 * Handlers are permitted to make state changes to the model and the ODM, but carefully!
		 * </p>
		 * 
		 * @param model
		 *            The model that is going to receive the contents of the source
