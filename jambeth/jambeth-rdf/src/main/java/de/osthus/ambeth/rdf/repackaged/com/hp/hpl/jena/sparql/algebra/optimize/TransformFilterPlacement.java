...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.CollectionUtils;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.OpVars;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.TransformCopy;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.BasicPattern;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.VarUtils;

/**
 * Rewrite an algebra expression to put filters as close to their bound variables.
 * <p>
 * Process BGP (whether triples or quads) is left as a separate step (but after this transform) because it can desirable to reorder the BGP before placing
 * filters, or afterwards.
 */
...
public class TransformFilterPlacement extends TransformCopy
{
	...
	/**
	 * Wrap the Basic Pattern with any applicable expressions from the ExprList but do not break up the BasicPattern in any way.
	 */
	...
	/**
	 * Wrap the Graph node, Basic Pattern with any applicable expressions from the ExprList but do not break up the BasicPattern in any way.
	 */
	...
	/*
	 * A Sequence is a number of joins where scoping means the LHS can be substituted into the right, i.e. there are no scoping issues. Assuming a substitution
	 * join is going to be done, filtering once as soon as the accumulated variables cover the filter is a good thing to do. It is effectively pusing on the
	 * left side only - the right side, by substitution, will never see the variables. The variable can not be reintroduced (it will have been renamed away if
	 * it's the same name, different scope, which is a different variable with the same name in the orginal query).
	 */
	...
	/**
	 * For any expression now in scope, wrap the op with a filter. Caution - the ExprList is an in-out argument which is modified. This function modifies
	 * ExprList passed in to remove any filter that is placed.
	 */
