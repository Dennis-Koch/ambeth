...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.compose.Polyadic;
...
public class OWLProfile extends AbstractProfile
{
	...
	{
		return null;
	} // qualified restrictions are not in the first version of OWL
	...
	{
		return Arrays.asList(new Resource[] { OWL.AllDifferent }).iterator();
	}
	...
	{
		return Arrays.asList(new Resource[] { OWL.versionInfo, RDFS.label, RDFS.seeAlso, RDFS.comment, RDFS.isDefinedBy }).iterator();
	}

	@Override
	public Iterator<Resource> getClassDescriptionTypes()
	{
		return Arrays.asList(new Resource[] { OWL.Class, OWL.Restriction }).iterator();
	}

	/**
	 * <p>
	 * Answer true if the given graph supports a view of this node as the given language element, according to the semantic constraints of the profile. If
	 * strict checking on the ontology model is turned off, this check is skipped.
	 * </p>
	 * 
	 * @param n
	 *            A node to test
	 * @param g
	 *            The enhanced graph containing <code>n</code>, which is assumed to be an {@link OntModel}.
	 * @param type
	 *            A class indicating the facet that we are testing against.
	 * @return True if strict checking is off, or if <code>n</code> can be viewed according to the facet resource <code>res</code>
	 */
	@Override
	...
	protected static class SupportsCheck
	{
		...
		/**
		 * Return a set of all of the nodes that are the objects of <code>rdf:type</code> triples whose subject is <code>n</code>
		 * 
		 * @param n
		 *            A subject node
		 * @param g
		...
		/**
		 * Return true if there is any intersection between the nodes in <code>nodes</code> and the nodes of the resources in <code>ref</code> .
		 * 
		 * @param nodes
		 * @param ref
		 * @return boolean
		...
		/**
		 * Return true if the node <code>n</code> in graph <code>g</code> has one of the types in <code>ref</code>
		 */
		public boolean hasType(Node n, EnhGraph eg, Resource[] ref)
		{
			// depending on the type of the underlying graph, it may or may not
			// be advantageous
			// to get all types at once, or ask many separate queries.
			// heuristically, we assume
			// that fine-grain queries to an inference graph is preferable, and
			// all-at-once for
		 ...
		 * @param g
		 *            A graph
		 * @return True if the graph is an inference graph, or is a union with an inference base graph
		 */
		public boolean isInferenceGraph(Graph g)
		{
			return (g instanceof InfGraph) || (g instanceof Polyadic && ((Polyadic) g).getBaseGraph() instanceof InfGraph);
		}
	}
	...
	private static Object[][] s_supportsCheckData = new Object[][] {
			...
			} }, { OntClass.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph eg)
				{
					...
					return hasType(n, eg, new Resource[] { OWL.Class, OWL.Restriction, RDFS.Class, RDFS.Datatype })
							...
							n.equals(OWL.Thing.asNode()) || n.equals(OWL.Nothing.asNode()) || g.contains(Node.ANY, RDFS.domain.asNode(), n)
							|| g.contains(Node.ANY, RDFS.range.asNode(), n) || g.contains(n, OWL.intersectionOf.asNode(), Node.ANY)
							|| g.contains(n, OWL.unionOf.asNode(), Node.ANY) || g.contains(n, OWL.complementOf.asNode(), Node.ANY);
				}
			...
			} }, { ObjectProperty.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return hasType(n, g, new Resource[] { OWL.ObjectProperty, OWL.TransitiveProperty, OWL.SymmetricProperty, OWL.InverseFunctionalProperty });
				}
			...
			} }, { OntProperty.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return hasType(n, g, new Resource[] { RDF.Property, OWL.ObjectProperty, OWL.DatatypeProperty, OWL.AnnotationProperty,
							OWL.TransitiveProperty, OWL.SymmetricProperty, OWL.InverseFunctionalProperty, OWL.FunctionalProperty });
				}
			...
			} }, { HasValueRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.hasValue)
			...
			} }, { AllValuesFromRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.allValuesFrom)
							&& containsSome(g, n, OWL.onProperty);
				}
			} }, { SomeValuesFromRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.someValuesFrom)
							&& containsSome(g, n, OWL.onProperty);
				}
			} }, { CardinalityRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.cardinality)
							&& containsSome(g, n, OWL.onProperty);
				}
			} }, { MinCardinalityRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.minCardinality)
							&& containsSome(g, n, OWL.onProperty);
				}
			} }, { MaxCardinalityRestriction.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return g.asGraph().contains(n, RDF.type.asNode(), OWL.Restriction.asNode()) && containsSome(g, n, OWL.maxCardinality)
							&& containsSome(g, n, OWL.onProperty);
				}
			...
			} }, { DataRange.class, new SupportsCheck()
			{
				...
				public boolean doCheck(Node n, EnhGraph g)
				{
					return n.isBlank() && g.asGraph().contains(n, RDF.type.asNode(), OWL.DataRange.asNode());
				}
			} } };
