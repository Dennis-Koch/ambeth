...
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.datatypes.xsd.XSDDatatype.*;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.ValueSpaceClassification.*;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.AnonId;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.ARQInternalErrorException;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.function.FunctionEnv;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.graph.NodeConst;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.graph.NodeTransform;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.serializer.SerializationContext;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.*;
...
public abstract class NodeValue extends ExprNode
{
	...
	/*
	 * Naming: getXXX => plain accessor asXXX => force to the required thing if necessary.
	 * 
	 * Implementation notes:
	 * 
	 * 1. There is little point delaying turning a node into its value because it has to be verified anyway (e.g. illegal literals). Because a NodeValue is
	 * being created, it is reasonably likely it is going to be used for it's value, so processing the datatype can be done at creation time where it is
	 * clearer.
	 * 
	 * 2. Conversely, delaying turning a value into a graph node is valuable because intermediates, like the result of 2+3, will not be needed as nodes unless
	 * assignment (and there is no assignment in SPARQL even if there is for ARQ). Node level operations like str() don't need a full node.
	 * 
	 * 3. nodevalue.NodeFunctions contains the SPARQL builtin implementations. nodevalue.XSDFuncOp contains the implementation of the XQuery/Xpath functions and
	 * operations. See also NodeUtils.
	 * 
	 * 4. Note that SPARQL "=" is "known to be sameValueAs". Similarly "!=" is known to be different.
	 * 
	 * 5. To add a new number type: Add sub type into nodevalue.NodeValueXXX Must implement .hashCode() and .equals() based on value. Add
	 * Functions.add/subtract/etc code and compareNumeric Add to compare code Fix TestExprNumeric Write lots of tests. Library code Maths1 and Maths2 for maths
	 * functions
	 */

	/*
	 * Effective boolean value rules. boolean: value of the boolean string: length(string) > 0 is true numeric: number != Nan && number != 0 is true ref:
	...
	public static final NodeValue nvNothing = NodeValue.makeNode(de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createAnon(new AnonId(
			"node value nothing")));
	 ...
	 * 
	 * See JENA-328. DatatypeFactory.newInstance() clashes with OSGi This is clearly crazy, but DatatypeFactory is missing a very obvious method
	 * newInstance(Classloader). The method that was added is very hard to use correctly, as we shall see...
	 */
	private static DatatypeFactory getDatatypeFactory() throws DatatypeConfigurationException
	{
		...
		File jaxpPropFile = new File(System.getProperty("java.home") + File.pathSeparator + "lib" + File.pathSeparator + "jaxp.properties");
	...
	public static NodeValue makeDateTime(XMLGregorianCalendar cal)
	{
		...
		Node node = de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createLiteral(lex, XSDdateTime);
	...
	{
		String lex = cal.toXMLFormat();
		Node node = de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createLiteral(lex, XSDdate);
		return new NodeValueDT(lex, node);
	}
	...
	{
		Node n = de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createLiteral(lexicalForm, null, dtype);
	...
	public static NodeValue makeNode(String lexicalForm, String langTag, String datatype)
	{
		...
		if (datatype != null)
		{
			...
			n = de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createLiteral(lexicalForm, null, dType);
		}
		else
			n = de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.NodeFactory.createLiteral(lexicalForm, langTag, null);
	...
	/**
	 * Compare by value if possible else compare by kind/type/lexical form Only use when you want an ordering regardless of form of NodeValue, for example in
	 * ORDER BY
	 * 
	 * @param nv1
	 * @param nv2
	...
	private static ValueSpaceClassification classifyValueSpace(NodeValue nv)
	{
		...
		if (SystemARQ.ValueExtensions && nv.getNode() != null && nv.getNode().isLiteral() && !nv.getNode().getLiteralLanguage().equals(""))
			return VSPACE_LANG;
	...
	public boolean isYearMonth()
	{
		...
		return (dur.isSet(YEARS) || dur.isSet(MONTHS)) && !dur.isSet(DAYS) && !dur.isSet(HOURS) && !dur.isSet(MINUTES) && !dur.isSet(SECONDS);
	}
	...
	private static NodeValue _setByValue(Node node)
	{
		...
		{ // DatatypeFormatException - should not happen
			...
			if ((datatype.equals(XSDdateTime) || dtXSDdateTimeStamp.equals(datatypeURI)) && XSDdateTime.isValid(lex))
			{
			...
			// XXX Pending Jena update ...
			if ((datatype.equals(XSDduration) || dtXSDdayTimeDuration.equals(datatypeURI) || dtXSDyearMonthDuration.equals(datatypeURI))
