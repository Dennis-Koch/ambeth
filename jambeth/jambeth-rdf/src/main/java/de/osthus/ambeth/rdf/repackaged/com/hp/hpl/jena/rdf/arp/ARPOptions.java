...
/**
 * The interface to set the various options on ARP. User defined implementations of this interface are not supported. This is a class rather than an interface
 * to have better backward compatibilitiy with earlier versions, however constructing instances of this class is deprecated. In addition, accessing the fields
 * of {@link ARPErrorNumbers} through this class is not supported. The inheritance of this interface will be removed.
 */
public class ARPOptions implements ARPErrorNumbers
{

	/**
	 * Do not use this constructor. An example of not using this constructor is as follows. <br/>
	 * Deprecated usage: <br/>
	 * 
	 * <pre>
	 * ARP arp = new ARP();
	...
	/**
	 * Sets or gets the error handling mode for a specific error condition. Changes that cannot be honoured are silently ignored. Illegal error numbers may
	 * result in an ArrayIndexOutOfBoundsException but are usually ignored. Most conditions are associated with one or more specific resources or literals
	 * formed during the parse. Triples involving resource or literal associated with an error condition are not produced. The precise definition of 'associated
	 * with' is deliberately undefined, and may change in future releases. This method can be used to downgrade an error condition to a warning, or to upgrade a
	 * warning to an error. Such a change modifies which triples are produced.
	 * <p>
	 * 
	 * When the condition is a violation of the RDF/XML Syntax (Revised) Recommendations, and the error mode is {@link ARPErrorNumbers#EM_IGNORE} or
	 * {@link ARPErrorNumbers#EM_WARNING}, the precise rules which ARP uses to generate triples for such ill-formed input are not defined by any standard and
	 * are subject to change with future releases. For input involving no errors, ARP creates triples in accordance with the RDF/XML Syntax Revised
	 * Recommendation.
	 * <p>
	 * 
	 ...
	 * <dd>Aborts parse and invokes ErrorHandler.errorError() for this condition. Do not produce triples. In unusual situations, a few further warnings and
	 * errors may be reported.</dd>
	 * </dl>
	 * 
	 * 
	 * @param errno
	 *            The specific error condition to change.
	 * @param mode
	...
	/**
	 * This method detects and prohibits errors according to the W3C Recommendations. For other conditions, such as
	...
	/**
	 * Sets whether the XML document is only RDF, or contains RDF embedded in other XML. The default is non-embedded mode. Embedded mode also matches RDF
	 * documents that use the rdf:RDF tag at the top-level. Non-embeded mode matches RDF documents which omit that optional tag, and consist of a single
	 * rdf:Description or typed node. To find embedded RDF it is necessary to setEmbedding(true).
	 * 
	 * @param embed
	 ...
	 * 
	 * @param eCode
	 * @return One of {@link ARPErrorNumbers#EM_IGNORE}, {@link ARPErrorNumbers#EM_WARNING}, {@link ARPErrorNumbers#EM_ERROR}, {@link ARPErrorNumbers#EM_FATAL}
	 */
	...
	/**
	 * True if the embedding flag is set. Indicates that the parser should look for rdf:RDF element, rather than treat the whole file as an RDF/XML document
	 * (possibly without rdf:RDF element).
	 */
	...
	/**
	 * Set the system-wide default IRI factory, which incorporates the checking rules. By default, Jena provides checking in compliance with the RDF spec but
	 * that is quite loose and allows strings that are not IRIs (the final IRI spec came along after the RDF spec). Example: spaces are strictly legal in RDF
	 * URIReferences but not in IRIs or URIs. Note that options to the RDF/XML parser override this.
	 */
