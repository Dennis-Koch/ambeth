...
package de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.engine.optimizer;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.sse.ItemException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.sse.ItemList;
 ...
 * </pre>
 * 
 * where <code>S</code>, <code>P</code>, <code>O</code> is a URI, variable, literal or one of the words <code>ANY</code> (matches anything), <code>VAR</code>
 * (matches a variable), <code>TERM</code> (matches a fixed URI, or literal), <code>URI</code>, <code>BNODE</code>, <code>LITERAL</code> (matches one of these
 * types).
 */
...
public final class StatsMatcher
{
	...
	private void onePattern(Item elt)
	{
		...
		else if (pat.isList() && pat.getList().size() == 3)
		{
			...
			Pattern pattern = new Pattern(((Number) (w.getNode().getLiteralValue())).doubleValue(), intern(pat.getList().get(0)), intern(pat.getList().get(1)),
	...
	{
		return match(Item.createNode(t.getSubject()), Item.createNode(t.getPredicate()), Item.createNode(t.getObject()));
	}
