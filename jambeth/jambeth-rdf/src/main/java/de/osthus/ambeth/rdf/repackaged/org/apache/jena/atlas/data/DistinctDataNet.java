...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.FileOps;

/**
 * This class is like {@link DistinctDataBag} except that you are informed if the item you just added was known to be distinct. This will normally only work
 * until the first spill. After that, the system may not be able to tell for sure, and will thus return false. When you are finished adding items, you may call
 * {@link #netIterator()} to get any distinct items that are in the spill files but were not indicated as distinct previously. This is useful for a distinct
...
public class DistinctDataNet<E> extends DistinctDataBag<E>
{
	...
	/**
	 * Returns an iterator to all additional items that are distinct but were not reported to be so at the time {@link #netAdd(Object)} was invoked.
	 * <p/>
	 * If you do not exhaust the iterator, you should call {@link de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.iterator.Iter#close(Iterator)} to be
	 * sure any open file handles are closed.
	 */
	...
	protected static class SortedDiffIterator<T> implements Iterator<T>, Closeable
	{
		...
		/**
		 * Produces the set difference of two sorted set sequences using the natural ordering of the items (null items will always be considered less than any
		 * other items).
		 * 
		 * @param first
		 *            An Iterator&lt;T&gt; whose elements that are not also in second will be returned.
		 * @param second
		 *            An Iterator&lt;T&gt; whose elements that also occur in the first sequence will cause those elements to be removed from the returned
		 *            sequence.
		 */
		 ...
		 *            sequence.
		 * @param comparator
		 *            The comparator used to compare the elements from each iterator.
		 */
