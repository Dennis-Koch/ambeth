...
/**
 * A reasoner interface that is able to invoke any of the useful rule engine combinations. The rule set can be set after the reasoner instance is created. The
 * mode can be set to forward, backward or hybrid. The OWL-specific rule augmentation can be included. Each of these settings can be controlled using the
 * configuration graph, specific methods calls or generic setParameter calls.
 */
public class GenericRuleReasoner extends FBRuleReasoner
{
	...
	/**
	 * Set the direction of rule inference desired. If set to a pure mode (FORWARD, BACKWARD) then the rules will be interpreted as operating in that direction
	 * which ever direction they were written in. In HYBRID mode then the direction of the rule itself which control whether it is used in forward or backward
	 * mode. In addition, HYBRID mode allows forward rules to generate addition backward rules.
	 */
	...
	/**
	 * Set to true to enable translation of selected parts of an OWL schema to additional rules. At present only intersction statements are handled this way.
	...
	/**
	 * Set to true to enable caching of subclass/subproperty lattices in a specialized cache rather than using the rule systems. This has substantially higher
	 * performance but it is done as a separate initialization pass and so can only work correct with some rule sets. This is only guaranteed to be implemented
	 * for the HYBRID mode.
	 */
	...
	/**
	 * Add a new preprocessing hook defining an operation that should be run when the inference graph is being prepared. This can be used to generate additional
	 * data-dependent rules or translations. This is only guaranted to be implemented for the HYBRID mode.
	 */
	...
	private void addRulesFromStrings(Resource value)
	{
		...
		while (it.hasNext())
			addRuleFromString(it.next().getString());
	}
	...
	private void addRulesFromURLs(Resource value)
	{
		...
		while (that.hasNext())
			addRules(Rule.rulesFromURL(that.next().getResource().getURI()));
	}
	...
	protected synchronized InfGraph getPreload()
	{
		...
		if (cachePreload && preload == null && mode == HYBRID)
		{
			...
			if (enableTGCCaching)
				((FBRuleInfGraph) preload).setUseTGCCache();
			preload.prepare();
		}
		return preload;
	}
