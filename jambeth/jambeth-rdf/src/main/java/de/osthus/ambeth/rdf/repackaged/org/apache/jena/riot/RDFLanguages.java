...
import static de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.WebContent.*;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.DS;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.web.MediaType;
...
public class RDFLanguages
{
	...
	/*
	 * ".owl" is not a formally registered file extension for OWL using RDF/XML. It was mentioned in OWL1 (when there was formally only one syntax for
	 * publishing RDF).
	 * 
	...
	public static final Lang RDFXML = LangBuilder.create(strLangRDFXML, contentTypeRDFXML).addAltNames("RDFXML", "RDF/XML-ABBREV", "RDFXML-ABBREV")
			.addFileExtensions("rdf", "owl", "xml").build();

	/** <a href="http://www.w3.org/TR/turtle/">Turtle</a> */
	public static final Lang TURTLE = LangBuilder.create(strLangTurtle, contentTypeTurtle).addAltNames("TTL")
			.addAltContentTypes(contentTypeTurtleAlt1, contentTypeTurtleAlt2).addFileExtensions("ttl").build();
	/** Alternative constant for {@linkplain #TURTLE} */
	public static final Lang TTL = TURTLE;

	/** N3 (treat as Turtle) */
	public static final Lang N3 = LangBuilder.create(strLangN3, contentTypeN3).addAltContentTypes(contentTypeN3, contentTypeN3Alt1, contentTypeN3Alt2)
			.addFileExtensions("n3").build();
	...
	public static final Lang NTRIPLES = LangBuilder.create(strLangNTriples, contentTypeNTriples)
			...
			.addAltContentTypes(contentTypeNTriplesAlt).addFileExtensions("nt").build();
	...
	public static final Lang JSONLD = LangBuilder.create(strLangJSONLD, "application/ld+json").addAltNames("JSONLD").addFileExtensions("jsonld").build();

	/** <a href="http://www.w3.org/TR/rdf-json/">RDF/JSON</a>. This is not <a href="http://www.w3.org/TR/json-ld/">JSON-LD</a>. */
	public static final Lang RDFJSON = LangBuilder.create(strLangRDFJSON, contentTypeRDFJSON).addAltNames("RDFJSON").addFileExtensions("rj", "json").build();

	/** <a href="http://www.w3.org/TR/trig/">TriG</a> */
	public static final Lang TRIG = LangBuilder.create(strLangTriG, contentTypeTriG).addAltContentTypes(contentTypeTriGAlt1, contentTypeTriGAlt2)
			.addFileExtensions("trig").build();

	/** <a href="http://www.w3.org/TR/n-quads">N-Quads</a> */
	public static final Lang NQUADS = LangBuilder.create(strLangNQuads, contentTypeNQuads).addAltNames("NQ", "NQuads", "NQuad", "N-Quad", "N-Quads")
			.addAltContentTypes(contentTypeNQuadsAlt1, contentTypeNQuadsAlt2).addFileExtensions("nq").build();
	...
	public static final Lang CSV = LangBuilder.create(strLangCSV, contentTypeTextCSV).addAltNames("csv").addFileExtensions("csv").build();
	...
	/**
	 * Register a language. To create a {@link Lang} object use {@link LangBuilder}. See also {@link RDFParserRegistry#registerLang} for registering a language
	 * and it's RDF parser fatory.
	 * 
	 * @see RDFParserRegistry
	 */
	...
	{
		return filenameToLang(resourceName);
	}
	...
	{
		return filenameToLang(resourceName, dftLang);
	}
	...
	/**
	 * Turn a name for a language into a {@link Lang} object. The name can be a label, or a content type.
	 */
