...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.AlreadyReifiedException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.CannotReifyException;
...
/**
 * A Reifier that only supports one style Standard (intercept, no conceal -- and intercept is a no-op anyway because all triples appear in the underlying graph
 * for storing all triples). This exists to give reification style "Standard" semantics primarly for legacy reasons.
 */
...
public class ReifierStd
{
	...
	/**
	 * Answer an iterator over the reification triples of this Reifier, or an empty iterator - if showHidden is false, only the exposed triples, otherwise only
	 * the concealed ones.
	 */
	...
	static Filter<Triple> filterReif = new Filter<Triple>()
	{
		...
		{
			return triple.getPredicate().equals(subject) || triple.getPredicate().equals(predicate) || triple.getPredicate().equals(object)
	...
	// check whether there is exactly the triple expected, and no others with
	// same S and P but different O.
	...
	{
		// Materialize the nodes to delete - avoid
