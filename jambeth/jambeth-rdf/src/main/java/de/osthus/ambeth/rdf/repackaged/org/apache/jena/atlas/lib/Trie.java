 ...
 */
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib;
...
/**
 * An implementation of a classic Trie, this is a mapping from strings to some value type optimized for fast prefix search and match. If you do not need prefix
 * search then you should typically use a standard {@link Map} instead.
 * <p>
 * The empty or null string may be used as a special key to refer to the root node of the trie.
 * </p>
 * <p>
 * A Trie cannot store null values since null is used as an internal marker to indicate that there is no value associated with a key. This is necessary because
 * the nature of the data structure means that adding a key potentially adds multiple keys many of which will not be associated with a value.
 * </p>
 * 
 * @param <T>
...
public final class Trie<T>
{
	 ...
	 * <p>
	 * Note that a null value is treated as if the key does not actually exist in the tree so trying to add a null is a no-op. If you want to remove a key use
	 * the {@link #remove(String)} method instead.
	 * </p>
	 * 
	 * @param key
	 *            Key
	 ...
	 *            Key
	 * @param requireValue
	 *            If true a key must have a non-null value associated with it to be considered to be contained in the tree, if false then the key must merely
	 *            map to a node in the trie
	 * @return True if the key exists and the value criteria is met, false otherwise
	 */
	...
	/**
	 * Performs a prefix search and returns all values mapped under the given prefix. The entirety of the prefix must be matches, if you only want part of the
	 * prefix to be matched use the {@link #partialSearch(String)} method instead.
	 * 
	 * @param prefix
	 *            Prefix
	 ...
	 * <p>
	 * The implementation is designed to be sparse such that we delay creation of things at both leafs and interior nodes until they are actually needed
	 * </p>
	 * 
	 */
