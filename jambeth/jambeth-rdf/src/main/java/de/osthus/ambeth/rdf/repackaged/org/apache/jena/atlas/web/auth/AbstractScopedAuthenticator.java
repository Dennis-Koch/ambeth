 ...
 * <p>
 * Note that credentials are not necessarily considered to be exclusively scoped to the exact URI rather they are scoped to any URI derived from the given URI.
 * For example if you declared credentials for {@code http://example.org} they would also apply to {@code http://example.org/some/path/}. When determining
 * credentials the longest match applies, so in the previous example you could define different credentials for the two URIs and URIs derived from
 * {@code http://example.org/some/path/} would prefer the credentials for that URI over those for {@code http://example.org}
 * </p>
 * <p>
