...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.json.JsonParseException;
...
class JSONParserBase
{
	...
	final protected boolean lookingAtString()
	{
		...
		if (t == null)
			return false;
		if (t.hasType(TokenType.STRING1))
			return true;
		if (t.hasType(TokenType.STRING2))
			return true;
		if (t.hasType(TokenType.LONG_STRING1))
			return true;
		if (t.hasType(TokenType.LONG_STRING2))
			return true;
		return false;
	}
	...
	final protected boolean lookingAtNumber()
	{
		...
		if (t == null)
			return false;
		if (t.hasType(TokenType.INTEGER))
			return true;
		if (t.hasType(TokenType.HEX))
			return true;
		if (t.hasType(TokenType.DECIMAL))
			return true;
		if (t.hasType(TokenType.DOUBLE))
			return true;
		return false;
	}
	...
	{
		throw new JsonParseException(String.format(msg, args), (int) tokens.getLine(), (int) tokens.getColumn());
	}
}
