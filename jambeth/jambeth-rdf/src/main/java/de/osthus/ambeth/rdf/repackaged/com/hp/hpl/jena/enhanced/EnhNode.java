...
/**
 * <p>
 * A specialisation of Polymorphic that models an extended node in a an extended graph. An extended node wraps a normal node, and adds additional convenience
 * access or user affordances, though the state remains in the graph itself.
 * </p>
 */
public class EnhNode extends Polymorphic<RDFNode> implements FrontsNode
{
	 ...
	 * @return An enhanced nodet that corresponds to t; this may be <i>this</i> Java object, or a different object.
	 */
	...
	/**
	 * An enhanced node is equal to another enhanced node n iff the underlying nodes are equal. We generalise to allow the other object to be any class
	 * implementing asNode, because we allow other implemementations of Resource than EnhNodes, at least in principle. This is deemed to be a complete and
	 * correct interpretation of enhanced node equality, which is why this method has been marked final.
	 * 
	 * @param o
	...
	/**
	 * Answer an new enhanced node object that presents <i>this</i> in a way which satisfies type <code>t</code>. The new object is linked into this object's
	 * sibling ring. If the node cannot be converted, throw an UnsupportedPolymorphismException.
	 */
	@Override
	protected <X extends RDFNode> X convertTo(Class<X> t)
	{
		...
		if (eg == null)
			throw new UnsupportedPolymorphismException(this, false, t);
		Implementation imp = getPersonality().getImplementation(t);
		if (imp == null)
			throw new UnsupportedPolymorphismException(this, true, t);
	...
	/**
	 * answer true iff this enhanced node can support the class <code>t</code>, ie it is already a value <code>t</code> or it can be reimplemented as a
	 * <code>t</code> via the graph's personality's implementation. If this node has no graph, answer false.
	 */
	@Override
	...
	/**
	 * Answer the personality object bound to this enhanced node, which we obtain from the associated enhanced graph.
	 * 
