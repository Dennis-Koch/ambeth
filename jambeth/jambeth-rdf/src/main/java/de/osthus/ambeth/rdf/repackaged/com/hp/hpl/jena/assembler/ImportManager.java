...
public class ImportManager
{
	...
	/**
	 * Answer <code>model</code> if it has no imports, or a union model with <code>model</code> as its base and its imported models as the other components. The
	 * default file manager is used to load the models.
	 */
	...
	/**
	 * Answer <code>model</code> if it has no imports, or a union model with <code>model</code> as its base and its imported models as the other components. The
	 * file manager <code>fm</code> is used to load the imported models.
	 */
	...
	private void addImportedGraphs(FileManager fm, Set<String> loading, StmtIterator oit, MultiUnion g)
	{
		while (oit.hasNext())
		{
			...
			if (loading.add(path))
				g.addGraph(graphFor(fm, loading, path));
		}
	}
