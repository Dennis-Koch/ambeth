...
/**
 * <p>
 * A HTTP authenticator which selects credentials based upon service context found in the provided {@link Context}. May also optionally use fallback credentials
 * for URIs for which authentication has not been explicitly configured. This works only with the Basic and Digest authentication schemes so you may need to use
 * an alternative authenticator if you need to use another authentication scheme.
 * </p>
 */
public class ServiceAuthenticator extends AbstractScopedAuthenticator<Context>
{
	...
	/**
	 * Creates a new authenticator using the standard ARQ context, the provided credentials are used as fallback if the context contains no registered
	...
	/**
	 * Creates a new authenticator using the given context, the provided credentials are used as fallback if the context contains no registered credentials for
	 * a target URI
	 * 
	 * @param context
	 *            Context
