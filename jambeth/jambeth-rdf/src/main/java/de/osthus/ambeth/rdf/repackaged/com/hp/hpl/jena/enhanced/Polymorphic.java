...
public abstract class Polymorphic<T>
{
	...
	/**
	 * return _true_ iff this polymorphic object supports the specified interface. Synonymous with "does the argument class have this as an instance". Actually
	 * it shouldn't be. Review.
	 */
	...
	/**
	 * Answer a polymorphic object that presents <i>this</i> in a way which satisfies type t.
	 * 
	 * @param t
	 *            A type
	...
	/**
	 * find an existing view in the ring which is an instance of _t_ and return it; otherwise return null. If _this_ is an instance, the search takes care to
	 * find it first.
	 */
	...
	/**
	 * Answer true iff this polymorphic object already has a valid view of type <code>t</code> in its ring (so .as()ing it doesn't need to construct a new
	 * object).
	 */
	...
	/**
	 * subclasses must provide a method for converting _this_, if possible, into an instance of _t_. It will only be called if _this_ doesn't already have (or
	 * be) a suitable ring-sibling.
	 */
	...
	/**
	 * add another view for this object. <code>other</code> must be freshly constructed. To be called by subclasses when they have constructed a new view for
	 * this object.
	 * 
	 * <p>
	 * The method is synchronised because addView is an update operation that may happen in a read context (because of .as()). Synchronising it ensures that
	 * simultaneous updates don't end up leaving the rings in an inconsistent state. (It's not clear whether this would actually lead to any problems; it's hard
	 * to write tests to expose these issues.)
	 * 
	 * This method is public ONLY so that it can be tested. TODO find a better way to make it testable.
	 */
