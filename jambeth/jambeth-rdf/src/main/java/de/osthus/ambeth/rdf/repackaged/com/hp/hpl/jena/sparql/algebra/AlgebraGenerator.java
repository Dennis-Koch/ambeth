...
package de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.optimize.TransformSimplify;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.path.PathLib;
...
public class AlgebraGenerator
{
	...
	/**
	 * Future extension point to allow the algebra generator to be more easily extended to understand new {@link Element} implementations that user defined
	...
	/*
	 * Extract filters, merge adjacent BGPs, do BIND. When extracting filters, BGP or PathBlocks may become adjacent so merge them into one. Return a list of
	 * elements with any filters at the end.
	 */
	...
	protected Op compileOneInGroup(Element elt, Op current, Deque<Op> acc)
	{
		...
		if (elt instanceof ElementGroup || elt instanceof ElementNamedGraph || elt instanceof ElementService || elt instanceof ElementUnion
				|| elt instanceof ElementSubQuery || elt instanceof ElementData || elt instanceof ElementTriplesBlock || elt instanceof ElementPathBlock)
		{
			Op op = compileElement(elt);
			return join(current, op);
		}
	...
	protected Op compileModifiers(Query query, Op pattern)
	{
		 ...
		 * 
		 * Limit/Offset Distinct/reduce project OrderBy Bindings having select expressions group
		 */
