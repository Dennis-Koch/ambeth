...
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NodeFunctions.checkAndGetStringLiteral;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NodeFunctions.checkTwoArgumentStringLiterals;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NumericType.OP_DECIMAL;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NumericType.OP_DOUBLE;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NumericType.OP_FLOAT;
import static de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.nodevalue.NumericType.OP_INTEGER;
...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.IRILib;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.expr.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.DateTimeStruct;
...
public class XSDFuncOp
{
	...
	/*
	 * Quote from XQuery/XPath F&O: For xs:float or xs:double values, a positive number divided by positive zero returns INF. A negative number divided by
	 * positive zero returns -INF. Division by negative zero returns -INF and INF, respectively. Positive or negative zero divided by positive or negative zero
	...
	/**
	 * Strict F&O handling of compare date(times). But that means applying the "local" timezone if there is no TZ. The data may have come from different
	 * timezones to the query.
	 */
	...
	/*
	 * Logical OR and AND is special with respect to handling errors truth table. AND they take effective boolean values, not boolean
	 * 
	 * A B | NOT A A && B A || B ------------------------------------- E E | E E E E T | E E T E F | E F E T E | F E T T T | F T T T F | F F T F E | T F E F T |
	 * T F T F F | T F F
	 */
	...
	/**
	 * Cast a NodeValue to a date/time type (xsd dateTime, date, time, g*) according to F&O <a
	 * href="http://www.w3.org/TR/xpath-functions/#casting-to-datetimes">17.1.5 Casting to date and time types</a> Throws an exception on incorrect case.
	 * 
	...
	/**
	 * Get the timezone in XSD tiezone format (e.g. "Z" or "+01:00"). Assumes the NodeValue is of suitable datatype.
	 */
	...
	private static Duration valueCanonicalDuration(NodeValue nv)
	{
		// Unclear.
		/*
		 * This semi-normalizes a duration value - the time part is normalized. Maybe > 24 hours -> set days, but not done here. Because months are variable,
		 * XSD F&O does not define
		 */
