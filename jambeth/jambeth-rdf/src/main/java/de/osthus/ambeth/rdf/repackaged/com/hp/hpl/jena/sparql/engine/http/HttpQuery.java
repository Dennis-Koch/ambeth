...
/**
 * Create an execution object for performing a query on a model over HTTP. This is the main protocol engine for HTTP query. There are higher level classes for
 * doing a query and presenting the results in an API fashion.
 * 
...
public class HttpQuery extends Params
{
	 ...
	 * <p>
	 * If the query has been made this reflects the Content-Type header returns, if it has not been made this reflects only the Accept header that will be sent
	 * (as set via the {@link #setAccept(String)} method)
	 * </p>
	 * 
	 * @return Content Type
	 */
	...
	/**
	 * Sets basic authentication. It may be preferable to use the {@link #setAuthenticator(HttpAuthenticator)} method since that provides more flexibility in
	...
	private InputStream execGet() throws QueryExceptionHTTP
	{
		...
		try
		{
			try
			{
				...
				TypedInputStream stream = HttpOp.execHttpGet(target.toString(), contentTypeResult, client, context, this.authenticator);
				if (stream == null)
