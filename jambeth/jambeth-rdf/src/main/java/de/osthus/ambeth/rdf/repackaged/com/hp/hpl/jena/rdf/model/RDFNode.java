...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.FrontsNode;

/**
 * Interface covering RDF resources and literals. Allows probing whether a node is a literal/[blank, URI]resource, moving nodes from model to model, and viewing
 * them as different Java types using the .as() polymorphism.
 */
public interface RDFNode extends FrontsNode
{
	/**
	 * Answer a String representation of the node. The form of the string depends on the type of the node and is intended for human consumption, not machine
	 * analysis.
	 */
	@Override
	...
	/**
	 * Answer true iff this RDFNode is a URI resource or an anonynous resource (ie is not a literal). Useful for one-off tests: see also visitWith() for making
	 ...
	 * RDFNode should be an instance of <code>view</code> and should have any internal invariants as specified.
	 * <p>
	...
	/**
	 * Answer true iff this RDFNode can be viewed as an instance of <code>view</code>: that is, if it has already been viewed in this way, or if it has an
	 * attached model in which it has properties that permit it to be viewed in this way. If <code>canAs</code> returns <code>true</code>, <code>as</code> on
	 * the same view should deliver an instance of that class.
	 */
	...
	/**
	 * Answer a .equals() version of this node, except that it's in the model <code>m</code>.
	 * 
	 * @param m
