...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.ontology.impl.OntModelImpl;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.transitiveReasoner.TransitiveReasonerFactory;
...
public class OntModelSpec
{
	 ...
	 */
	public static final OntModelSpec OWL_MEM_RDFS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, RDFSRuleReasonerFactory.theInstance(),
	 ...
	 */
	public static final OntModelSpec OWL_MEM_TRANS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, TransitiveReasonerFactory.theInstance(),
			ProfileRegistry.OWL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_MEM_RULE_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, OWLFBRuleReasonerFactory.theInstance(),
			ProfileRegistry.OWL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_MEM_MICRO_RULE_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, OWLMicroReasonerFactory.theInstance(),
			ProfileRegistry.OWL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_MEM_MINI_RULE_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, OWLMiniReasonerFactory.theInstance(),
			ProfileRegistry.OWL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_DL_MEM_RDFS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, RDFSRuleReasonerFactory.theInstance(),
	 ...
	 */
	public static final OntModelSpec OWL_DL_MEM_TRANS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, TransitiveReasonerFactory.theInstance(),
			ProfileRegistry.OWL_DL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_DL_MEM_RULE_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, OWLFBRuleReasonerFactory.theInstance(),
			ProfileRegistry.OWL_DL_LANG);
	 ...
	 */
	public static final OntModelSpec OWL_LITE_MEM_TRANS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null,
			TransitiveReasonerFactory.theInstance(), ProfileRegistry.OWL_LITE_LANG);

	/**
	 * A specification for OWL Lite models that are stored in memory and use the RDFS inferencer for additional entailments
	 */
	public static final OntModelSpec OWL_LITE_MEM_RDFS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, RDFSRuleReasonerFactory.theInstance(),
	 ...
	 */
	public static final OntModelSpec OWL_LITE_MEM_RULES_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null,
			OWLFBRuleReasonerFactory.theInstance(), ProfileRegistry.OWL_LITE_LANG);
	 ...
	 */
	public static final OntModelSpec RDFS_MEM_TRANS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, TransitiveReasonerFactory.theInstance(),
			ProfileRegistry.RDFS_LANG);
	 ...
	 */
	public static final OntModelSpec RDFS_MEM_RDFS_INF = new OntModelSpec(ModelFactory.createMemModelMaker(), null, RDFSRuleReasonerFactory.theInstance(),
			ProfileRegistry.RDFS_LANG);
	...
	protected String[][] defaultPrefixes = new String[][] { { "owl", OWL.getURI() }, { "rdf", RDF.getURI() }, { "rdfs", RDFS.getURI() },
			{ "xsd", XSD.getURI() } };
	 ...
	 * 
	 * @param importsMaker
	 *            The model maker, which will be used to construct stores for statements in the imported ontologies and the base ontology. Use null to get a
	 *            default (memory) model maker.
	 * @param docMgr
	 *            The document manager, or null for the default document manager.
	 * @param rFactory
	 *            The factory for the reasoner to use to infer additional triples in the model, or null for no reasoner
	 * @param languageURI
	 ...
	 * 
	 * @param baseModelName
	 *            the name of the model in the baseModelMaker
	 * @param baseMaker
	 *            the model-maker to use for the base model
	 * @param importsMaker
	 *            the model-maker to use for imported models
	 ...
	 *            the ontology language URI (must not be null)
	 */
	public OntModelSpec(String baseModelName, ModelMaker baseMaker, ModelMaker importsMaker, OntDocumentManager docMgr, ReasonerFactory rFactory,
			String languageURI)
	{
		// super( baseMaker );
	...
	/**
	 * <p>
	 * Create one model spec as a copy of another. This is useful when what is required is similar to an existing spec, but with some changes.
	 * <strong>Note:</strong> this is only a shallow copy, so the structured objects (reasoners, document managers, etc) are not themselves copied. Thus, even
	 * after calling this copy constructor, making a change to the document manager in the copy specification will also affect the one that the copy was made
	 * from. The correct idiom is to replace the object before side-effecting it, e.g: <code><pre>
	...
	{
		return getLanguage().equals(other.getLanguage()) && sameReasonerFactory(other) && getDocumentManager().equals(other.getDocumentManager())
	 ...
	 * 
	 * @return The ModelMaker that is used to get or create a model used to hold imports to an OntModel.
	 */
	...
	{
		if (importModelGetter == null)
			importModelGetter = m_importsMaker; // fabricateModelGetter();
	...
	/**
	 * Answer the OntModelSpec described using the Jena Assembler vocabulary properties of <code>root</code>. If the assembled resource is not an OntModelSpec,
	 * throw an exception reporting the constructed class.
	 */
	...
	/**
	 * Answer the OntModelSpec described using the Jena Assembler vocabulary properties of the single resource in <code>model</code> of type JA:OntModelSpec.
	 */
	 ...
	 * Specifically, OWL and RDFS languages will have RDFS level inferencing capability (chosen to give a reasonable balance between power and efficiency of
	 * computation). To get other (more powerful or less powerful) reasoning capabilities, users should create ontology models by passing an explicit
	 * <code>OntModelSpec</code> parameter to the {@link ModelFactory#createOntologyModel(OntModelSpec, Model ) model factory}.
	 * </p>
	 * 
	 * @param languageURI
	...
	/**
	 * <p>
	 * Answer the document manager for this model specification. Defaults to a standard instance of {@link OntDocumentManager}
	 * </p>
	 * 
	...
	public Reasoner getReasoner()
	{
		if (m_reasoner == null && m_rFactory != null)
		{
			...
			// create a new one on each call since reasoners aren't guaranteed
			// to be reusable
	...
	/**
	 * <p>
	 * Set the reasoner that will be used by ontology models that conform to this specification to compute entailments. <strong>Note:</strong> The reasoner is
	 * generated on demand by the reasoner factory. To prevent this spec from having a reasoner, set the reasoner factory to null, see
	 * {@link #setReasonerFactory}.
	 * </p>
	 * 
	 * @param reasoner
	 *            The new reasoner
	...
	/**
	 * <p>
	 * Set the factory object that will be used to generate the reasoner object for this model specification. <strong>Note</strong> that the reasoner itself is
	 * cached, so setting the factory after a call to {@link #getReasoner()} will have no effect.
	 * </p>
	 * 
	 * @param rFactory
