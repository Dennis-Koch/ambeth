...
public class StAX2SAX
{
	...
	public StAX2SAX(ContentHandler handler)
	{
		...
		this.lhandler = (handler instanceof LexicalHandler) ? (LexicalHandler) handler : NO_LEXICAL_HANDLER;
	...
	public void parse(XMLEventReader xmlReader) throws XMLStreamException, SAXException
	{
		while (xmlReader.hasNext())
		{
			...
			if (e.isStartDocument())
				...
				emitPi((ProcessingInstruction) e);
			else if (e.isCharacters())
				emitChars(e.asCharacters());
			else if (e.isAttribute())
				emitAttr((Attribute) e);
			else if (e.isEntityReference())
				emitEnt((EntityDeclaration) e);
			else if (e.isNamespace())
				emitNS((Namespace) e);
			else if (e instanceof Comment)
				emitComment((Comment) e);
			else if (e instanceof DTD)
				emitDTD((DTD) e);
			else
			{
				// System.err.println("Unknown / unhandled event type " + e);
				// throw new SAXException("Unknown / unhandled event type " +
				// e);
			}
		}
	}
	...
	private void emitSE(StartElement se) throws SAXException
	{
		...
		handler.startElement(se.getName().getNamespaceURI(), se.getName().getLocalPart(), qnameToS(se.getName()), convertAttrs(aIter));
		@SuppressWarnings("unchecked")
	...
	{
		handler.endElement(ee.getName().getNamespaceURI(), ee.getName().getLocalPart(), qnameToS(ee.getName()));
		@SuppressWarnings("unchecked")
	...
	private void emitChars(Characters chars) throws SAXException
	{
		if (chars.isIgnorableWhiteSpace())
			handler.ignorableWhitespace(chars.getData().toCharArray(), 0, chars.getData().length());
		else
