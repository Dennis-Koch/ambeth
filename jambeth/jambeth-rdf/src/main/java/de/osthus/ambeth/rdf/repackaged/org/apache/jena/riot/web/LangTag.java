...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.Chars;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.riot.system.RiotChars;

/**
 * Language tags: support for parsing and canonicalization of case. Grandfathered forms ("i-") are left untouched. Unsupported or syntactically illegal forms
 * are handled in canonicalization by doing nothing.
 * <ul>
...
public class LangTag
{
	...
	// Canonical forms:
	/*
	 * RFC 4646 In this format, all non-initial two-letter subtags are uppercase, all non-initial four-letter subtags are titlecase, and all other subtags are
	 * lowercase.
	 */
	/*
	 * RFC 5646 An implementation can reproduce this format without accessing the registry as follows. All subtags, including extension and private use subtags,
	 * use lowercase letters with two exceptions: two-letter and four-letter subtags that neither appear at the start of the tag nor occur after singletons.
	 * Such two-letter subtags are all uppercase (as in the tags "en-CA-x-ca" or "sgn-BE-FR") and four- letter subtags are titlecase (as in the tag
	 * "az-Latn-x-latn").
	 */
	 ...
	 * 
	 * Language-Tag = langtag / privateuse ; private use tag / grandfathered ; grandfathered registrations
	 * 
	 * langtag = (language ["-" script] ["-" region]("-" variant)("-" extension) ["-" privateuse])
	 * 
	 * language = (2*3ALPHA [ extlang ]) ; shortest ISO 639 code / 4ALPHA ; reserved for future use / 5*8ALPHA ; registered language subtag
	 * 
	 ...
	 * 
	 * singleton = %x41-57 / %x59-5A / %x61-77 / %x79-7A / DIGIT ; "a"-"w" / "y"-"z" / "A"-"W" / "Y"-"Z" / "0"-"9" ; Single letters: x/X is reserved for private
	 * use
	 * 
	 * privateuse = ("x"/"X") 1*("-" (1*8alphanum))
	 * 
	 * grandfathered = 1*3ALPHA 1*2("-" (2*8alphanum)) ; grandfathered registration ; Note: i is the only singleton ; that starts a grandfathered tag
	 * 
	...
	private static final String langtag = String.format("^(%s)(?:-(%s))?(?:-(%s))?(?:-(%s))?(?:-(%s))?$", language, script, region, variant, extension);
	...
	/**
	 * Canonicalize with the rules of RFC 4646 "In this format, all non-initial two-letter subtags are uppercase, all non-initial four-letter subtags are
	 * titlecase, and all other subtags are lowercase." In addition, leave extensions unchanged.
	 */
