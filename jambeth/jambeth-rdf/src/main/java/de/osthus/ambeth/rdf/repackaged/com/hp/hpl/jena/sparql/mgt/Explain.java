...
package de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.mgt;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.QuadPattern;
...
/**
 * Execution logging for query processing on a per query basis. This class provides an overlay on top of the system logging to provide control of log message
 * down to a per query basis. The associated logging channel must also be enabled.
 * 
 * An execution can detail the query, the algebra and every point at which the dataset is touched.
 * 
 * Caution: logging can be a significant cost for small queries and for memory-backed datasets because of formatting the output and disk or console output
 * overhead.
 * 
 * @see ARQ#logExec
...
public class Explain
{
	...
	/*
	 * The logging system provided levels: TRACE < DEBUG < INFO < WARN < ERROR < FATAL Explain logging is always at logging level INFO. Per query: SYSTEM > EXEC
	 * (Query) > DETAIL (Algebra) > DEBUG (every BGP)
	 * 
