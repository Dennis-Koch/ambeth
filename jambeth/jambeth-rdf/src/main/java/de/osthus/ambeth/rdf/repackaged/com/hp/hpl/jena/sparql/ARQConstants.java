...
public class ARQConstants
{
	...
	/*
	 * Variable names and allocated variables. NB Must agree with the variable parsing rules in SSE Allocated variables use names that are not legal in SPARQL.
	 * Examples include the "?" variable initial character.
	 * 
	 * We need to allocate so clashes never occur within scopes. Distinguished (named variables) and non-distinguished (anon variables, bNodes)
	 * 
	 * SSE also allows some convenience forms of exactly these string:
	 * 
	 * See: ParseHandlerPlain.emitVar
	 * 
	 * Naming: Named (distinguished) allocated variables start "?." Non-Distinguished, allocated variables start "??" Quad transform hidden vars: "?*"
	 * 
	 * Scopes and usages: Global: allocVarMarker "?.." VarAlloc.getVarAllocator
	 * 
	 * Query: Expressions and aggregates Parser: Used in turning blank nodes into variables in query patterns Via LabelToNodeMap ("??") Algebra Generator:
	 * PathCompiler ("??P") : Non-distinguished variables.
	 * 
	 * SSE "?" short hand for "some variable" using ?0, ?1, ?2 naming (legal SPARQL names) "??" short hand for "some new anon variable" "?." short hand for
	 * "some new named variable"
	 * 
	...
	/**
	 * Context key for the dataset description (if any). See the <a href="http://www.w3.org/TR/sparql11-protocol">SPARQL protocol</a>.
	 * <p>
	 * A dataset description specified outside the query should override a dataset description in query and also the implicit dataset of a service. The order
	 * is:
	 * <ol>
	...
	public static final Symbol registryPropertyFunctions = ARQConstants.allocSymbol("registryPropertyFunctions");

	/** The describe handler registry key */
	public static final Symbol registryDescribeHandlers = ARQConstants.allocSymbol("registryDescribeHandlers");

	/** The function library registry key */
	public static final Symbol registryFunctions = ARQConstants.allocSymbol("registryFunctions");

	/** The function library registry key */
	public static final Symbol registryProcedures = ARQConstants.allocSymbol("registryProcedures");

	/** The extension library registry key */
	public static final Symbol registryExtensions = ARQConstants.allocSymbol("registryExtensions");
}
