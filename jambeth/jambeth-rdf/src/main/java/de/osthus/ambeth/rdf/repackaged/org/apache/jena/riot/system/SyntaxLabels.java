 ...
 * 
 * For node to label (pretty labels output), we use a unique tracking scheme. Fully scalable writers use different polices and don't have short, pretty bNode
 * labels.
 */
public class SyntaxLabels
{
	/** Default setup - scope by document, relabel BNodes ids to short forms */
	static public NodeToLabel createNodeToLabel()
	{
		return NodeToLabel.createScopeByDocument();
	}

	static public LabelToNode createLabelToNode()
	{
		return LabelToNode.createScopeByDocumentHash();
	}

	/** Round-trip setup */
	static public NodeToLabel createNodeToLabelRT()
	{
		return NodeToLabel.createBNodeByLabelEncoded();
	}

	static public LabelToNode createLabelToNodeRT()
	{
