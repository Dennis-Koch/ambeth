...
/**
 * Inner class used to represent the graph node structure. Rather fat nodes (four sets)
 */
class GraphNode
{
	...
	/**
	 * Plain Siblings for simple nodes. For the lead node in a SCC will be a Leader with components containing all the nodes in the SCC. For non-lead nodes it
	 * will be a Subordinate referring to the lead node.
	 */
	...
	/**
	 * Return the lead node in the strongly connected component containing this node. It will be the node itself if it is a singleton or the lead node.
	 */
	...
	public String dump()
	{
		return rdfNode.getLocalName() + siblings.dump() + " succ=" + dumpSet(succ) + ", succClose=" + dumpSet(succClosed) + ", pred=" + dumpSet(pred);
	}
	...
	/**
	 * Return an iterator over all the indirect successors of this node. This does NOT include aliases of successors and is used for graph maintenance.
	 */
	...
	/**
	 * Assert a direct link between this node and this given target. Does not update the closed successor cache
	 */
	...
	/**
	 * Remove a direct link currently from this node to the given target. Does not update the closed successor cache.
	 */
			...
			public List<GraphNode> visit(GraphNode node, GraphNode processing, Set<GraphNode> sc, GraphNode target)
			{
				...
				for (Iterator<GraphNode> i = node.succ.iterator(); i.hasNext();)
				{
					...
					if (sc.contains(s))
					{
						...
						if (s == processing)
						{
							// Can't remove immediately w/o beaking the visitor
							// loop
							if (kill == null)
								kill = new ArrayList<>();
							kill.add(node);
	...
	/**
	 * Given a set of SCC nodes make this the lead member of the SCC and reroute all incoming and outgoing links accordingly. This eager rewrite is based on the
	 * assumption that there are few cycles so it is better to rewrite once and keep the graph easy to traverse.
	 */
	...
	/**
	 * This node is being absorbed into an SCC with the given node as the new lead node. Trace out all predecessors to this node and relocate them to point to
	 * the new lead node.
	 */
	 ...
	 * 
	 * @param closed
	 *            if set to true it returns triples in the transitive closure, if set to false it returns triples in the transitive reduction
	 * @param tgc
	...
	private List<Triple> triplesForSuccessors(Node base, boolean closed, TransitiveGraphCache tgc)
	{
		...
		result.add(new Triple(base, tgc.closedPredicate, base)); // implicit
																	// reflexive
																	// case
	...
	/**
	 * Return an iterator over all of the triples representing incoming links to this node. Currently no caching enabled.
	 */
