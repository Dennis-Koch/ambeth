...
/**
 * Rule-based reasoner interface. This is the default rule reasoner to use. It supports both forward reasoning and backward reasoning, including use of forward
 * rules to generate and instantiate backward rules.
 */
public class FBRuleReasoner implements RuleReasoner
{
	...
	/**
	 * Add the given rules to the current set and answer this Reasoner. Provided so that the Factory can deal out reasoners with specified rulesets. There may
	 * well be a better way to arrange this. TODO review & revise
	 */
	...
	/**
	 * Answer the schema graph bound into this reasoner, or null if there isn't one. (Added mainly for testing purposes, which is why it isn't part of the
	 * Reasoner API.)
	 */
	 ...
	 * 
	 * @param property
	 *            the property which we want to ask the reasoner about, given as a Node since this is part of the SPI rather than API
	...
	public void setRules(List<Rule> rules)
	{
		...
		if (schemaGraph != null)
		{
			// The change of rules invalidates the existing precomputed schema
			// graph
			// This might be recoverable but for now simply flag the error and
			// let the
	...
	public synchronized void tablePredicate(Node predicate)
	{
		...
		Rule tablePredicateRule = new Rule("", new ClauseEntry[] { new Functor("table", new Node[] { predicate }) }, new ClauseEntry[] {});
	...
	/**
	 * Set a parameter from a statement, given the property and its RDFNode value. Most parameters are, historically, set from the string value of the RDFNode,
	 * but newer parameters may have Resource values with embedded models, for which their toString() is not just suspect, but definitively wrong. Hence the
	 * introduction of this relay station.
	 * 
	 * @param parameter
	 *            the propoerty naming the value to set
	 * @param value
	 *            the RDFNode with the value of that property
	...
	{
		return (value instanceof Resource && doSetResourceParameter(parameter, (Resource) value))
				|| (value instanceof Literal && doSetParameter(parameter, ((Literal) value).getValue())) || doSetParameter(parameter, value.toString());
	}
