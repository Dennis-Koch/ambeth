...
public class OneToManyMap<From, To> implements Map<From, To>
{
	...
	/**
	 * Answer true if the map contains the given object as a value stored against any key. Note that this is quite an expensive operation in the current
	 * implementation.
	 * 
	 * @param value
	...
	/**
	 * <p>
	 * Answer true if this mapping contains the pair <code>(key,&nbsp;value)</code>.
	 * </p>
	 * 
	 * @param key
	 *            A key object
	 * @param value
	 *            A value object
	 * @return True if <code>key</code> has <code>value</code> as one of its values in this mapping
	 */
	...
	/**
	 * Get a value for this key. Since this map is explicitly designed to allow there to be more than one mapping per key, this method will return an
	 * undetermined instance of the mapping. If no mapping exists, or the selected value is null, null is returned.
	 * 
	 * @param key
	...
	/**
	 * Associates the given value with the given key. Since this map formulation allows many values for one key, previous associations with the key are not
	 * lost. Consequently, the method always returns null (since the replaced value is not defined).
	 * 
	 * @param key
	 *            The key object
	 * @param value
	 ...
	 * 
	 * @param m
	 *            The map whose contents are to be copied into this map
	 */
	@Override
	public void putAll(Map<? extends From, ? extends To> m)
	{
		...
		for (From key : m.keySet())
		{
			if (many)
			{
				// Bizare way to write it but this way makes all compilers
				// happy.
	...
	/**
	 * Remove all of the associations for the given key. If only a specific association is to be removed, use
	 * {@link #remove(java.lang.Object, java.lang.Object )} instead. Has no effect if the key is not present in the map. Since no single specific association
	 * with the key is defined, this method always returns null.
	 * 
	 * @param key
	 *            All associations with this key will be removed
	 * @return null
	 */
	@Override
	...
	/**
	 * <p>
	 * Remove the specific association between the given key and value. Has no effect if the association is not present in the map. If all values for a
	 * particular key have been removed post removing this particular association, the key will no longer appear as a key in the map.
	 * </p>
	 * 
	 * @param key
	 *            The key object
	...
	/**
	 * <p>
	 * Returns a collection view of the values contained in this map. Specifically, this will be a set, so duplicate values that appear for multiple keys are
	 * suppressed.
	 * </p>
	 * 
	...
	public static class Entry<From, To> implements Map.Entry<From, To>
	{
		 ...
		 * same mapping. More formally, two entries e1 and e2 represent the same mapping if <code><pre>
		 *      (e1.getKey()==null ?
		...
		public boolean equals(Object x)
		{
			if (x instanceof java.util.Map.Entry<?, ?>)
			{
				...
				return (e1.getKey() == null ? m_key == null : e1.getKey().equals(m_key))
						&& (e1.getValue() == null ? m_value == null : e1.getValue().equals(m_value));
			}
			else
				return false;
		}
		 ...
		 * 
		 * This ensures that e1.equals(e2) implies that e1.hashCode()==e2.hashCode() for any two Entries e1 and e2, as required by the general contract of
		 * Object.hashCode.
		 */
		@Override
		public int hashCode()
