...
/**
 * A FileGraph factory, making FileGraphs based round some supplied directory. We have to keep track of created files ourselves, because a FileGraph that has
 * been created but not closed is not visible in the filing system. (This might count as a bug at some point.)
 */
public class FileGraphMaker extends BaseGraphMaker implements FileGraph.NotifyOnClose
{
	...
	/**
	 * Construct a file graph factory whose files will appear in root. The reifier style is Minimal and the files will be retained when the maker is closed.
	 * 
	 * @param root
	...
	/**
	 * Construct a file graph factory whose files will appear in root. If deleteOnClose is true, the files created by this factory will be deleted when the
	 * factory is closed.
	 * 
	 * @param root
	 *            the directory to keep the files in
	 * @param deleteOnClose
	 *            iff true, delete created files on close
	 */
	...
	{
		File f = withRoot(name);
		return created.containsKey(f) ? created.get(f).openAgain() : remember(f, new FileGraph(this, f, false, strict));
	}
	 ...
	 * 
	 * @param fileName
	 *            a filename, possible containing _U, _C, and _S escapes
	...
	/**
	 * Answer a FilenameFilter which recognises plausibly RDF filenames; they're not directories, and FileGraph likes them. Pass the buck, pass the buck ...
	 * 
	...
	/**
	 * Answer an iterator over the names of graphs in the FileGraphMaker. This is all the names of freshly-created graphs, plus the names of any files in the
	 * fileBase that might be RDF files. "Might" is weaker than we'd like for now.
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.GraphMaker#listGraphs()
	 */
	@Override
