...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.BuiltinPersonalities;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.enhanced.EnhNode;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.graph.compose.MultiUnion;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.ontology.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.listeners.StatementListener;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.*;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.IteratorFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.ModelCom;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.Derivation;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.reasoner.ValidityReport;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.vocabulary.RDFS;
...
public class OntModelImpl extends ModelCom implements OntModel
{
	...
	/**
	 * Found from {@link owlSyntaxCheckerClassName}, must implement {@link OWLSyntaxChecker}.
	 */
	...
	/**
	 * <p>
	 * Construct a new ontology model, using the given model as a base. The document manager given in the specification object will be used to build the imports
	 * closure of the model if its policy permits.
	 * </p>
	 * 
	 * @param model
	 *            The base model that may contain existing statements for the ontology. if it is null, a fresh model is created as the base.
	 * @param spec
	 *            A specification object that allows us to specify parameters and structure for the ontology model to be constructed.
	 */
	 ...
	 * <strong>Note</strong> the default ontology model {@linkplain OntModelSpec specifications} each have a contained default document manager. Changing the
	 * document managers specified by these default specification may (in fact, probably will) affect other models built with the same specification policy.
	 * This may or may not be as desired by the programmer!
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the ontology resources in this model, i&#046;e&#046; the resources with <code>rdf:type Ontology</code> or equivalent.
	 * These resources typically contain metadata about the ontology document that contains them.
	 * </p>
	 * <p>
	 ...
	 * see {@link Profile#ONTOLOGY}.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the property resources in this model, i&#046;e&#046; the resources with <code>rdf:type Property</code> or equivalent.
	 * An <code>OntProperty</code> is equivalent to an <code>rdfs:Property</code> in a normal RDF graph; this type is provided as a common super-type for the
	 * more specific {@link ObjectProperty} and {@link DatatypeProperty} property types.
	 * </p>
	 * <p>
	 * <strong>Note</strong> This method searches for nodes in the underlying model whose <code>rdf:type</code> is <code>rdf:Property</code>. This type is
	 * <em>entailed</em> by specific property sub-types, such as <code>owl:ObjectProperty</code>. An important consequence of this is that in
	 * <em>models without an attached reasoner</em> (e.g. in the <code>OWL_MEM</code> {@link OntModelSpec}), the entailed type will not be present and this
	 * method will omit such properties from the returned iterator. <br />
	 * <strong>Solution</strong> There are two ways to address to this issue: either use a reasoning engine to ensure that type entailments are taking place
	 * correctly, or call {@link #listAllOntProperties()}. Note that <code>listAllOntProperties</code> is potentially less efficient than this method.
	 * </p>
	 * <p>
	...
	public ExtendedIterator<OntProperty> listOntProperties()
	{
		...
		// if we are in OWL_FULL, the properties should also include the
	...
	/**
	 * <p>
	 * Answer an iterator over all of the ontology properties in this model, including object properties, datatype properties, annotation properties, etc. This
	 * method takes a different approach to calculating the set of property resources to return, and is robust against the absence of a reasoner attached to the
	 * model (see note in {@link #listOntProperties()} for explanation). However, the calculation used by this method is potentially less efficient than the
	 * alternative <code>listOntProperties()</code>. Users whose models have an attached reasoner are recommended to use {@link #listOntProperties()}.
	 * </p>
	 * 
	 * @return An iterator over all available properties in a model, irrespective of whether a reasoner is available to perform <code>rdf:type</code>
	 *         entailments. Each property will appear exactly once in the iterator.
	 */
	@Override
	public ExtendedIterator<OntProperty> listAllOntProperties()
	{
		ExtendedIterator<OntProperty> i = findByTypeAs(RDF.Property, OntProperty.class).andThen(listObjectProperties()).andThen(listDatatypeProperties())
				.andThen(listAnnotationProperties()).andThen(listFunctionalProperties()).andThen(listTransitiveProperties()).andThen(listSymmetricProperties());
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the object property resources in this model, i&#046;e&#046; the resources with <code>rdf:type ObjectProperty</code>
	 * or equivalent. An object property is a property that is defined in the ontology language semantics as a one whose range comprises individuals (rather
	 * than datatyped literals).
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#OBJECT_PROPERTY}.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	 ...
	 * <code>rdf:type DatatypeProperty</code> or equivalent. An datatype property is a property that is defined in the ontology language semantics as a one
	 * whose range comprises datatyped literals (rather than individuals).
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#DATATYPE_PROPERTY}.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	 ...
	 * <code>rdf:type FunctionalProperty</code> or equivalent. A functional property is a property that is defined in the ontology language semantics as having
	 * a unique domain element for each instance of the relationship.
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#FUNCTIONAL_PROPERTY}.
	 * </p>
	 * 
	 ...
	 * <code>rdf:type TransitiveProperty</code> or equivalent.
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#TRANSITIVE_PROPERTY}.
	 * </p>
	 * 
	 ...
	 * <code>rdf:type SymmetricProperty</code> or equivalent.
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#SYMMETRIC_PROPERTY}.
	 * </p>
	 * 
	 ...
	 * <code>rdf:type InverseFunctionalProperty</code> or equivalent.
	 * </p>
	 * <p>
	 * Specifically, the resources in this iterator will those whose type corresponds to the value given in the ontology vocabulary associated with this model:
	 * see {@link Profile#INVERSE_FUNCTIONAL_PROPERTY}.
	 * </p>
	 * 
	...
	public ExtendedIterator<InverseFunctionalProperty> listInverseFunctionalProperties()
	{
		...
		return findByTypeAs(getProfile().INVERSE_FUNCTIONAL_PROPERTY(), InverseFunctionalProperty.class).filterKeep(
				new UniqueFilter<InverseFunctionalProperty>());
	}

	/**
	 * <p>
	 * Answer an iterator over the individuals in this model. Where possible, an individual is defined as an instance of the <em>top</em> class in an ontology,
	 * i.e. <code>owl:Thing</code> or <code>daml:Thing</code>. However, since this test relies on the presence of an inference capability, and is not defined in
	 * cases where there is no <em>top</em> class (such as RDFS), a secondary heuristic is used when needed: an individual is an instance of a class defined in
	 * the ontology (i.e. it is a resource with an <code>rdf:type</code>, where the <code>rdf:type</code> of that resource is a class or restriction in the
	 * ontology.
	 * </p>
	 * 
	...
	{
		// since the reasoner implements some OWL full functionality for RDF
		// compatibility, we
		// have to decide which strategy to use for identifying individuals
		// depending on whether
		// or not a powerful reasoner (i.e. owl:Thing/daml:Thing aware) is being
		// used with this model
		boolean supportsIndAsThing = false;
		if (getGraph() instanceof InfGraph)
		{
			supportsIndAsThing = ((InfGraph) getGraph()).getReasoner().getReasonerCapabilities()
		...
		{
			// no inference, or we are in RDFS land, so we pick things that have
			// rdf:type whose rdf:type is Class

			// it's tricky to make this efficient and cover all possible cases.
			// I've changed the code to
			// make use of the isIndividual() test on OntResource, at the
			// expense of some redundant queries
			// to the model, which could become expensive in the case of a DB
			// model - ijd Apr-23-09
	 ...
	 * </p>
	 * 
	 * @return An iterator over individual resources whose <code>rdf:type</code> is <code>cls</code>.
	 */
	@Override
	 ...
	 * {@link #listEnumeratedClasses enumerated} classes, {@link #listUnionClasses union} classes, {@link #listComplementClasses complement} classes,
	 * {@link #listIntersectionClasses intersection} classes, {@link #listClasses named} classes and {@link #listRestrictions property restrictions}.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	...
	/**
	 * <p>
	 * Answer an iterator over the classes in this ontology model that represent the uppermost nodes of the class hierarchy. Depending on the underlying
	 * reasoner configuration, if any, these will be calculated as the classes that have Top (i.e. <code>owl:Thing</code> or <code>daml:Thing</code>) as a
	 * direct super-class, or the classes which have no declared super-class.
	 * </p>
	 * 
	...
	public ExtendedIterator<OntClass> listHierarchyRootClasses()
	{
		...
		if (getReasoner() != null)
		{
			...
			if (conf != null && conf.contains(null, ReasonerVocabulary.supportsP, ReasonerVocabulary.directSubClassOf) && getProfile().THING() != null)
			{
				...
				// test against
				return listStatements(null, ReasonerVocabulary.directSubClassOf, getProfile().THING()).mapWith(
						new OntResourceImpl.SubjectAsMapper<>(OntClass.class));
			}
		}
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the enumerated class class-descriptions in this model, i&#046;e&#046; the class resources specified to have a
	 * property <code>oneOf</code> (or equivalent) and a list of values.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the named class-descriptions in this model, i&#046;e&#046; resources with <code>rdf:type
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the property restriction class-descriptions in this model, i&#046;e&#046; resources with <code>rdf:type
	...
	/**
	 * <p>
	 * Answer an iterator that ranges over the properties in this model that are declared to be annotation properties. Not all supported languages define
	 * annotation properties (the category of annotation properties is chiefly an OWL innovation).
	 * </p>
	 * <p>
	 * <strong>Note:</strong> the number of nodes returned by this iterator will vary according to the completeness of the deductive extension of the underlying
	 * graph. See class overview for more details.
	...
	public ExtendedIterator<AnnotationProperty> listAnnotationProperties()
	{
		...
		{
			return findByType(r).mapWith(new SubjectNodeAs<>(AnnotationProperty.class)).filterKeep(new UniqueFilter<AnnotationProperty>());
		}
	}

	/**
	 * <p>
	 * Answer a resource that represents an ontology description node in this model. If a resource with the given uri exists in the model, and can be viewed as
	 * an Ontology, return the Ontology facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the ontology node. Conventionally, this corresponds to the base URI of the document itself.
	...
	/**
	 * <p>
	 * Answer a resource that represents an Individual node in this model. If a resource with the given uri exists in the model, and can be viewed as an
	 * Individual, return the Individual facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing an generic property in this model. If a property with the given uri exists in the model, return the OntProperty facet,
	...
	/**
	 * <p>
	 * Answer a resource representing an object property in this model. If a resource with the given uri exists in the model, and can be viewed as an
	 * ObjectProperty, return the ObjectProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing a transitive property. If a resource with the given uri exists in the model, and can be viewed as a TransitiveProperty,
	 * return the TransitiveProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing a symmetric property. If a resource with the given uri exists in the model, and can be viewed as a SymmetricProperty,
	 * return the SymmetricProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the property. May not be null.
	...
	/**
	 * <p>
	 * Answer a resource representing an inverse functional property. If a resource with the given uri exists in the model, and can be viewed as a
	 * InverseFunctionalProperty, return the InverseFunctionalProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the property. May not be null.
	...
	/**
	 * <p>
	 * Answer a resource that represents datatype property in this model. . If a resource with the given uri exists in the model, and can be viewed as a
	 * DatatypeProperty, return the DatatypeProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource that represents an annotation property in this model. If a resource with the given uri exists in the model, and can be viewed as an
	 * AnnotationProperty, return the AnnotationProperty facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource that represents a class description node in this model. If a resource with the given uri exists in the model, and can be viewed as an
	 * OntClass, return the OntClass facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing the class that is the complement of another class. If a resource with the given uri exists in the model, and can be viewed
	 * as a ComplementClass, return the ComplementClass facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing the class that is the enumeration of a list of individuals. If a resource with the given uri exists in the model, and can
	 * be viewed as an EnumeratedClass, return the EnumeratedClass facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing the class that is the union of a list of class desctiptions. If a resource with the given uri exists in the model, and can
	 * be viewed as a UnionClass, return the UnionClass facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource representing the class that is the intersection of a list of class descriptions. If a resource with the given uri exists in the model,
	 * and can be viewed as a IntersectionClass, return the IntersectionClass facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource that represents a property restriction in this model. If a resource with the given uri exists in the model, and can be viewed as a
	 * Restriction, return the Restriction facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have the given resource as the value of the given property. If a resource with
	 * the given uri exists in the model, and can be viewed as a HasValueRestriction, return the HasValueRestriction facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have at least one property with a value belonging to the given class. If a
	 * resource with the given uri exists in the model, and can be viewed as a SomeValuesFromRestriction, return the SomeValuesFromRestriction facet, otherwise
	 * return null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals for which all values of the given property belong to the given class. If a resource
	 * with the given uri exists in the model, and can be viewed as an AllValuesFromResriction, return the AllValuesFromRestriction facet, otherwise return
	 * null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have exactly the given number of values for the given property. If a resource
	 * with the given uri exists in the model, and can be viewed as a CardinalityRestriction, return the CardinalityRestriction facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	 *            The URI for the restriction
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have at least the given number of values for the given property. If a resource
	 * with the given uri exists in the model, and can be viewed as a MinCardinalityRestriction, return the MinCardinalityRestriction facet, otherwise return
	 * null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have at most the given number of values for the given property. If a resource
	 * with the given uri exists in the model, and can be viewed as a MaxCardinalityRestriction, return the MaxCardinalityRestriction facet, otherwise return
	 * null.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have a property p, all values of which are members of a given class. Typically
	 * used with a cardinality constraint. If a resource with the given uri exists in the model, and can be viewed as a QualifiedRestriction, return the
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have a property p, with cardinality N, all values of which are members of a
	 * given class. If a resource with the given uri exists in the model, and can be viewed as a CardinalityQRestriction, return the CardinalityQRestriction
	 * facet, otherwise return null.
	 * </p>
	 * 
	 * @param uri
	 *            The URI for the restriction
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have a property p, with min cardinality N, all values of which are members of a
	 * given class. If a resource with the given uri exists in the model, and can be viewed as a MinCardinalityQRestriction, return the
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have a property p, with max cardinality N, all values of which are members of a
	 * given class. If a resource with the given uri exists in the model, and can be viewed as a MaxCardinalityQRestriction, return the
	...
	/**
	 * <p>
	 * Answer a resource that represents an ontology description node in this model. If a resource with the given uri exists in the model, it will be re-used.
	 * If not, a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the ontology node. Conventionally, this corresponds to the base URI of the document itself.
	...
	/**
	 * <p>
	 * Answer a resource that represents an Indvidual node in this model. A new anonymous resource will be created in the updateable sub-graph of the ontology
	 * model.
	 * </p>
	 * 
	 * @param cls
	...
	/**
	 * <p>
	 * Answer a resource that represents an Individual node in this model. If a resource with the given uri exists in the model, it will be re-used. If not, a
	 * new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param cls
	 *            Resource representing the ontology class to which the individual belongs
	...
	/**
	 * <p>
	 * Answer a resource representing an generic property in this model. Effectively this method is an alias for {@link #createProperty(String )}, except that
	 * the return type is {@link OntProperty}, which allow more convenient access to a property's position in the property hierarchy, domain, range, etc.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the property. May not be null.
	...
	/**
	 * <p>
	 * Answer a resource that represents an object property in this model. An object property is defined to have a range of individuals, rather than datatypes.
	 * If a resource with the given uri exists in the model, it will be re-used. If not, a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the object property. May not be null.
	 * @param functional
	 *            If true, the resource will also be typed as a {@link FunctionalProperty}, that is, a property that has a unique range value for any given
	 *            domain value.
	...
	/**
	 * <p>
	 * Answer a resource representing a transitive property, which is optionally also functional. <strong>Note:</strong> although it is permitted in OWL full to
	 * have functional transitive properties, it makes the language undecideable. Functional transitive properties are not permitted in OWL Lite or OWL DL.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the property. May not be null.
	...
	/**
	 * <p>
	 * Answer a resource that represents datatype property in this model. A datatype property is defined to have a range that is a concrete datatype, rather
	 * than an individual. If a resource with the given uri exists in the model, it will be re-used. If not, a new one is created in the updateable sub-graph of
	 * the ontology model.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource that represents an annotation property in this model. If a resource with the given uri exists in the model, it will be re-used. If not,
	 * a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a resource that represents an anonymous class description in this model. A new anonymous resource of <code>rdf:type C</code>, where C is the class
	 * type from the language profile.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Answer a resource that represents a class description node in this model. If a resource with the given uri exists in the model, it will be re-used. If
	 * not, a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the class node, or null for an anonymous class.
	 ...
	 * 
	 * @param uri
	 *            The URI of the new complement class, or null for an anonymous class description.
	 * @param cls
	...
	public ComplementClass createComplementClass(String uri, Resource cls)
	{
		...
		checkProfileEntry(getProfile().COMPLEMENT_OF(), "COMPLEMENT_OF");
		// if the class that this class is a complement of is not specified, use
		// owl:nothing or daml:nothing
	 ...
	 * 
	 * @param uri
	 *            The URI of the new enumeration class, or null for an anonymous class description.
	 * @param members
	 *            An optional list of resources denoting the individuals in the enumeration
	 ...
	 * 
	 * @param uri
	 *            The URI of the new union class, or null for an anonymous class description.
	 * @param members
	 ...
	 * 
	 * @param uri
	 *            The URI of the new intersection class, or null for an anonymous class description.
	 * @param members
	...
	/**
	 * <p>
	 * Answer a resource that represents an anonymous property restriction in this model. A new anonymous resource of <code>rdf:type R</code>, where R is the
	 * restriction type from the language profile.
	 * </p>
	 * 
	 * @param p
	...
	/**
	 * <p>
	 * Answer a resource that represents a property restriction in this model. If a resource with the given uri exists in the model, it will be re-used. If not,
	 * a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * 
	 * @param uri
	 *            The uri for the restriction node, or null for an anonymous restriction.
	 * @param p
	 *            The property that is restricted by this restriction
	 ...
	 * 
	 * @param uri
	 *            The optional URI for the restriction, or null for an anonymous restriction (which should be the normal case)
	 * @param prop
	 *            The property the restriction applies to
	 * @param value
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have at most the given number of values for the given property, all values of
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have at least the given number of values for the given property, all values of
	 * which belong to the given class.
	 * </p>
	 * 
	 * @param uri
	...
	/**
	 * <p>
	 * Answer a class description defined as the class of those individuals that have exactly the given number of values for the given property, all values of
	 * which belong to the given class.
	 * </p>
	 * 
	 * @param uri
	 ...
	 * 
	 * @param literals
	 *            An iterator over a set of literals that will be the members of the data range, or null to define an empty data range
	...
	/**
	 * <p>
	 * Answer a new, anonymous node representing the fact that a given set of classes are all pair-wise distinct. <code>AllDifferent</code> is a feature of OWL
	 * only, and is something of an anomoly in that it exists only to give a place to anchor the <code>distinctMembers</code> property, which is the actual
	...
	/**
	 * <p>
	 * Answer a resource that represents a generic ontology node in this model. If a resource with the given uri exists in the model, it will be re-used. If
	 * not, a new one is created in the updateable sub-graph of the ontology model.
	 * </p>
	 * <p>
	 * This is a generic method for creating any known ontology value. The selector that determines which resource to create is the same as as the argument to
	 * the {@link RDFNode#as as()} method: the Java class object of the desired abstraction. For example, to create an ontology class via this mechanism, use:
	 * <code><pre>
	 ...
	 * @param rdfType
	 *            Optional resource denoting the ontology class to which an individual or axiom belongs, if that is the type of resource being created.
	 * @param uri
	 ...
	 * 
	 * @param uri
	 *            The URI of the resource, or null for an anonymous resource (aka bNode)
	...
	/**
	 * <p>
	 * Determine which models this model imports (by looking for, for example, <code>owl:imports</code> statements, and load each of those models as an import.
	 * A check is made to determine if a model has already been imported, if so, the import is ignored. Thus this method is safe against circular sets of import
	 * statements. Note that actual implementation is delegated to the associated {@link OntDocumentManager}.
	 */
	@Override
	public void loadImports()
	{
		// load the imports closure, according to the policies in my document
		// manager
	...
	/**
	 * <p>
	 * Answer true if this model has had the given URI document imported into it. This is important to know since an import only occurs once, and we also want
	 * to be able to detect cycles of imports.
	 * </p>
	 * 
	 * @param uri
	 *            An ontology URI
	 * @return True if the document corresponding to the URI has been successfully loaded into this model
	 */
	@Override
	 ...
	 * 
	 * @param uri
	 *            A document URI that has now been imported into the model.
	 */
	@Override
	 ...
	 * 
	 * @param uri
	 *            A document URI that is no longer imported into the model.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer a list of the imported URI's in this ontology model. Detection of <code>imports</code> statments will be according to the local language profile
	 * </p>
	 * 
	 * @return The imported ontology URI's as a set. Note that since the underlying graph is not ordered, the order of values in the list in successive calls to
	 *         this method is not guaranteed to be preserved.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer a list of the imported URI's in this ontology model, and optionally in the closure of this model's imports. Detection of <code>imports</code>
	 * statments will be according to the local language profile. Note that, in order to allow this method to be called during the imports closure process, we
	 * <b>only query the base model</b>, thus side-stepping the any attached reasoner.
	 * </p>
	 * 
	 * @param closure
	 *            If true, the set of uri's returned will include not only those directly imported by this model, but those imported by the model's imports
	 *            transitively.
	 * @return The imported ontology URI's as a list. Note that since the underlying graph is not ordered, the order of values in the list in successive calls
	 *         to this method is not guaranteed to be preserved.
	 */
	@Override
	 ...
	 * 
	 * @param uri
	 *            URI to read from, may be mapped to a local source by the document manager
	 */
	@Override
	...
	{
		// we don't want to load this document again if imported by one of the
		// imports
		addLoadedImport(uri);
		...
		{
			// now we can actually do the read, check first if we should use
			// negotiation
			if (base == null && // require non-null base
					!ignoreFileURI(source) && // and that negotiation makes
												// sense (don't conneg to file:)
			...
			{
				// if we were given the base, use it ... otherwise default to
				// the base being the source
	...
	/**
	 * <p>
	 * Answer an iterator over the ontologies that this ontology imports, each of which will have been wrapped as an ontology model using the same
	 * {@link OntModelSpec} as this model. If this model has no imports, the iterator will be non-null but will not have any values.
	 * </p>
	 * 
	 * @return An iterator, each value of which will be an <code>OntModel</code> representing an imported ontology.
	 * @deprecated This method has been re-named to <code>listSubModels</code>, but note that to obtain the same behaviour as <code>listImportedModels</code>
	 *             from Jena 2.4 and earlier, callers should invoke {@link #listSubModels(boolean)} with parameter <code>true</code>.
	 * @see #listSubModels()
	...
	/**
	 * <p>
	 * Answer an iterator over the ontology models that are sub-models of this model. Sub-models are used, for example, to represent composite documents such as
	 * the imports of a model. So if ontology A imports ontologies B and C, each of B and C will be available as one of the sub-models of the model containing
	 * A. This method replaces the older {@link #listImportedModels}. Note that to fully replicate the behaviour of <code>listImportedModels</code>, the
	 * <code>withImports</code> flag must be set to true. Each model returned by this method will have been wrapped as an ontology model using the same
	 * {@link OntModelSpec} as this model. If this model has no sub-models, the returned iterator will be non-null but will not have any values.
	 * </p>
	 * 
	 * @param withImports
	 *            If true, each sub-model returned by this method will also include its import models. So if model A imports D, and D imports D, when called
	 *            with <code>withImports</code> set to true, the return value for <code>modelA.listSubModels(true)</code> will be an iterator, whose only value
	 *            is a model for D, and that model will contain a sub-model representing the import of E. If <code>withImports</code> is false, E will not be
	 *            included as a sub-model of D.
	 * @return An iterator, each value of which will be an <code>OntModel</code> representing a sub-model of this ontology.
	 */
	@Override
	 ...
	 * Answer an iterator over the ontology models that are sub-models of this model. Sub-models are used, for example, to represent composite documents such as
	 * the imports of a model. So if ontology A imports ontologies B and C, each of B and C will be available as one of the sub-models of the model containing
	 * A. <strong>Important note on behaviour change:</strong> please see the comment on {@link #listSubModels(boolean)} for explanation of the
	 * <code>withImports</code> flag. This zero-argument form of <code>listSubModels</code> sets <code>withImports</code> to false, so the returned models will
	 * not themselves contain imports. This behaviour differs from the zero-argument method {@link #listImportedModels()} in Jena 2.4 an earlier.
	 * </p>
	 * 
	 * @return An iterator, each value of which will be an <code>OntModel</code> representing a sub-model of this ontology.
	 * @see #listSubModels(boolean)
	...
	/**
	 * <p>
	 * Answer an <code>OntModel</code> representing the imported ontology with the given URI. If an ontology with that URI has not been imported, answer null.
	 * </p>
	 * 
	 * @param uri
	 *            The URI of an ontology that may have been imported into the ontology represented by this model
	 * @return A model representing the imported ontology with the given URI, or null.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer the base model of this model. The base model is the model wrapping the graph that contains the triples read from the source document for this
	 * ontology. It is therefore the model that will be updated if statements are added to a model that is built from a union of documents (via the
	...
	/**
	 * <p>
	 * Add the given model as one of the sub-models of the enclosed ontology union model. <strong>Note</strong> that if <code>model</code> is a composite model
	 * (i.e. an {@link OntModel} or {@link InfModel}), the model and all of its submodels will be added to the union of sub-models of this model. If this is
	 * <strong>not</strong> required, callers should explicitly add only the base model:
	 * </p>
	 * 
	 * <pre>
	 ...
	 *            A sub-model to add
	 * @param rebind
	 *            If true, rebind any associated inferencing engine to the new data (which may be an expensive operation)
	 */
	@Override
	...
	/**
	 * <p>
	 * Remove the given model as one of the sub-models of the enclosed ontology union model. Will cause the associated infererence engine (if any) to update, so
	 * this may be an expensive operation in some cases.
	 * </p>
	 * 
	 * @param model
	 *            A sub-model to remove
	...
	/**
	 * <p>
	 * Answer true if the given node is a member of the base model of this ontology model. This is an important distiction, because only the base model receives
	 * updates when the ontology model is updated. Thus, removing properties of a resource that is not in the base model will not actually side-effect the
	 * overall model.
	 * </p>
	 * 
	 * @param node
	...
	/**
	 * <p>
	 * Answer true if the given statement is defined in the base model of this ontology model. This is an important distiction, because only the base model
	 * receives updates when the ontology model is updated. Thus, removing a statement that is not in the base model will not actually side-effect the overall
	 * model.
	 * </p>
	 * 
	 * @param stmt
	...
	/**
	 * <p>
	 * Answer true if this model is currently in <i>strict checking mode</i>. Strict mode means that converting a common resource to a particular language
	 * element, such as an ontology class, will be subject to some simple syntactic-level checks for appropriateness.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Set the flag that controls whether adding or removing <i>imports</i> statements into the model will result in the imports closure changing dynamically.
	 * </p>
	 * 
	 * @param dynamic
	 *            If true, adding or removing an imports statement to the model will result in a change in the imports closure. If false, changes to the imports
	 *            are not monitored dynamically. Default false.
	 */
	@Override
	...
	/**
	 * Cause the inference model to reconsult the underlying data to take into account changes. Normally changes are made through the InfModel's add and remove
	 * calls are will be handled appropriately. However, in some cases changes are made "behind the InfModels's back" and this forces a full reconsult of the
	 * changed data.
	 */
	@Override
	public void rebind()
	...
	/**
	 * Perform any initial processing and caching. This call is optional. Most engines either have negligable set up work or will perform an implicit "prepare"
	 * if necessary. The call is provided for those occasions where substantial preparation work is possible (e.g. running a forward chaining rule system) and
	 * where an application might wish greater control over when this prepration is done rather than just leaving to be done at first query time.
	 */
	@Override
	public void prepare()
	{
	...
	/**
	 * Reset any internal caches. Some systems, such as the tabled backchainer, retain information after each query. A reset will wipe this information
	 * preventing unbounded memory use at the expense of more expensive future queries. A reset does not cause the raw data to be reconsulted and so is less
	...
	/**
	 * <p>
	 * Returns a derivations model. The rule reasoners typically create a graph containing those triples added to the base graph due to rule firings. In some
	 * applications it can useful to be able to access those deductions directly, without seeing the raw data which triggered them. In particular, this allows
	 ...
	 * <code>subject</code> argument, whose predicate matches the <code>predicate</code> argument and whose object matches the <code>object</code> argument. If
	 * an argument is <code>null</code> it matches anything.
	 * </p>
	 * <p>
	 * The s/p/o terms may refer to resources which are temporarily defined in the "posit" model. This allows one, for example, to query what resources are of
	 * type CE where CE is a class expression rather than a named class - put CE in the posit arg.
	 * </p>
	 * 
	 ...
	 *            The value sought
	 * @param posit
	 *            Model containing additional assertions to be considered when matching statements
	 */
	@Override
	...
	/**
	 * Switch on/off drivation logging. If this is switched on then every time an inference is a made that fact is recorded and the resulting record can be
	 ...
	 * 
	 * @return an iterator over Derivation records or null if there is no derivation information available for this triple.
	 */
	@Override
	...
	/**
	 * <p>
	 * Answer an iterator over all of the resources that have <code>rdf:type</code> type.
	 * </p>
	 * 
	 * @param type
	 *            The resource that is the value of <code>rdf:type</code> we want to match
	...
	/**
	 * <p>
	 * Answer an iterator over all of the resources that have <code>rdf:type type</code>, or optionally, one of the alternative types.
	 * </p>
	 * 
	 * @param type
	 *            The resource that is the value of <code>rdf:type</code> we want to match
	 ...
	 * @return An iterator over all triples <code>_x rdf:type t</code> where t is <code>type</code> or one of the values from <code>types</code> .
	 */
	...
	/**
	 * <p>
	 * Answer an iterator over all of the resources that have <code>rdf:type type</code>, or optionally, one of the alternative types, and present the results
	 * <code>as()</code> the given class.
	 * </p>
	 * 
	 * @param type
	 *            The resource that is the value of <code>rdf:type</code> we want to match
	...
	/**
	 * <p>
	 * Answer an iterator over all of the resources that has an <code>rdf:type</code> from the types iterator, and present the results <code>as()</code> the
	 * given class.
	 * </p>
	 * 
	 * @param types
	 *            An iterator over types to search for. An exception will be raised if this iterator does not have at least one next() element.
	 * @param asKey
	 *            The value to use to present the polymorphic results
	 * @return An iterator over all triples <code>_x rdf:type type</code>
	 */
	...
	/**
	 * <p>
	 * Answer an iterator over resources with the given rdf:type; for each value in the iterator, ensure that is is presented <code>as()</code> the polymorphic
	 * object denoted by the given class key.
	 * </p>
	 * 
	 * @param type
	 *            The rdf:type to search for
	 * @param asKey
	 *            The key to pass to as() on the subject nodes
	 * @return An iterator over subjects with the given type, presenting as the given polymorphic class.
	 */
	...
	/**
	 * <p>
	 * Answer the resource with the given uri and that optionally has the given <code>rdf:type</code>, creating the resource if necessary.
	 * </p>
	 * 
	 * @param uri
	 *            The uri to use, or null for an anonymous resource
	 * @param rdfType
	 *            The resource to assert as the <code>rdf:type</code>, or null to leave untyped
	 ...
	 * @return An {@link OntResource} attached to this model that has the same URI or anonID as the given resource
	 */
	@Override
	 ...
	 * 
	 * @param profileTerm
	 *            The entry from the profile
	 * @param desc
	 *            A label for the profile term
	 * @exception OntologyException
	 *                if profileTerm is null.
	 */
	 ...
	 * @exception LanguageConsistencyException
	 *                if any member of the list does not have <code>rdf:type <i>rdfType</i></code>
	 */
	...
	/**
	 * Answer the supplied model, unless it's null, in which case answer a new model constructed as per spec.
	 */
	...
	/**
	 * Test for whether we ignore <code>file:</code> URI's when testing for content negotiation.
	 * 
	 * @param source
	 * @return
	 */
