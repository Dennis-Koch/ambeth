...
/**
 * Builtin data type to represent XMLLiteral (i.e. items created by use of <code>rdf:parsetype='literal'</code>.
 */
public class XMLLiteralType extends BaseDatatype implements RDFDatatype
{
	...
	{
		/*
		 * To check the lexical form we construct a dummy RDF/XML document and parse it with ARP. ARP performs an exclusive canonicalization, the dummy document
		 * has exactly one triple. If the lexicalForm is valid then the resulting literal found by ARP is unchanged. All other scenarios are either impossible
		...
		arp.getHandlers().setStatementHandler(new StatementHandler()
		{
			...
			{
				/*
				 * this method is invoked exactly once while parsing the dummy document. The l argument is in exclusive canonical XML and corresponds to where
				 * the lexical form has been in the dummy document. The lexical form is valid iff it is unchanged.
				 */
				...
				// throw new
				// BrokenException("plain literal in XMLLiteral code.");
				status[1] = true;
			...
			public void statement(AResource a, AResource b, AResource l)
			{
				status[0] = true;
				// throw new
				// BrokenException("resource valued RDF/XML in XMLLiteral code.");
			}
		});
		try
		{
