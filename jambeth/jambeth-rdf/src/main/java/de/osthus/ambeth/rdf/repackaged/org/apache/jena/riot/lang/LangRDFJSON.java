	...
	private void tryParseTriples()
	{
		...
		while (moreTokens())
		{
			if (lookingAt(TokenType.RBRACE))
			{
				if (subjectExpected)
				{
					exception(peekToken(), "Expected a Property Name after a comma to represent the Subject of the next block of triples but got %s",
							peekToken());
				}
			...
			else
			{
				...
				else
				{
					exception(
							peekToken(),
							"Expected either the end of the JSON Object (the } character) or a JSON Property Name (String) to set the Subject for some Triples but got %s",
							peekToken());
				}
			}
		}
	...
	private void tryParsePredicateObjectList(Node subj)
	{
		...
		while (true)
		{
			...
			else if (!first && lookingAt(TokenType.RBRACE))
			{
				...
				{
					exception(peekToken(), "Expected a further Property Name to represent a Predicate after a comma in a Predicate Object List but got %s",
							peekToken());
				}
				nextToken();
				return;
		...
		if (lookingAt(TokenType.LBRACKET))
		{
			...
			while (true)
			{
				if (lookingAt(TokenType.LBRACE))
				{
					if (!objectExpected)
					{
						exception(peekToken(), "Expected the end of the JSON Array for the Object List as no comma was seen after the preceding } but got %s",
								peekToken());
					}
		...
		if (lookingAt(TokenType.LBRACE))
		{
			...
			while (true)
			{
				...
				else if (!first && lookingAt(TokenType.RBRACE))
				{
					if (propertyNameExpected)
					{
						exception(peekToken(), "Expected a further Property Name to represent a property of the Object of a Triple after a comma but got %s",
								peekToken());
					}
					break;
				}
			...
			switch (typeStr)
			{
				...
				case "literal":
					...
					{
						obj = profile.createTypedLiteral(value.getImage(), TypeMapper.getInstance().getSafeTypeByName(datatype.getImage()), value.getLine(),
								value.getColumn());
					}
					else
					{
