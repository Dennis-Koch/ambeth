	...
	RandCache(String name, int size)
	{
		...
		catch (IllegalArgumentException e)
		{
			if ("Illegal load factor: NaN".equals(e.getMessage()))
			{
				...
				// When we implemented XSDbase64Binary/XSDhexBinary support
				// involving use
				// of byte[] we started seeing this error here. Since the
				// default loadfactor
				// is a static final constant in HashMap this should never be
				// possible.
				// It only happens under JDK 1.4.1 not under 1.3.1 nor 1.4.2.
				// The retry, however does seem to work and hence gives us a
				// work around
				// which is completely mysterious but at least enables the unit
				// tests to pass.
				// - der 4/5/04
