 ...
 * <h3>Status</h3>
 * <p>
 * Performance testing has shown that often this gives minimal performance benefit so this optimization is not enabled by default. It may be explicitly enabled
 * by setting the {@link ARQ#optFilterInequality} symbol in your {@link Context} or the ARQ global context ({@link ARQ#getContext()} to {@code true}
 * </p>
 * 
 * <h3>Applicability</h3>
 * <p>
 * This optimizer is conservative in that it only makes the optimization where the inequality constraint is against a non-literal as otherwise substituting the
 * value changes the query semantics because it switches from value equality to the more restrictive term equality. The optimization is safe for non-literals
 * because for those value and term equality are equivalent (in fact value equality is defined to be term equality).
 * </p>
 * <p>
