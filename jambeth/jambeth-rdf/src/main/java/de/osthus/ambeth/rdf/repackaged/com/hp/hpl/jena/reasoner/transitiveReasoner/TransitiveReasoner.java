 ...
 * <p>
 * This reasoner caches a transitive closure of the subClass and subProperty graphs. The generated infGraph allows both the direct and closed versions of these
 * properties to be retrieved. The cache is built when the tbox is bound in but if the final data graph contains additional subProperty/subClass declarations
 * then the cache has to be rebuilt.
 * </p>
 * <p>
...
public class TransitiveReasoner implements Reasoner
{
	...
	/**
	 * Private constructor used by bindSchema when returning a partially bound reasoner instance.
	 */
	...
	/**
	 * Extracts all of the subClass and subProperty declarations from the given schema/tbox and caches the resultant graphs. It can only be used once, can't
	 * stack up multiple tboxes this way. This limitation could be lifted - the only difficulty is the need to reprocess all the earlier tboxes if a new
	 * subPropertyOf subPropertyOf subClassOf is discovered.
	 * 
	 * @param tbox
	...
	/**
	 * Attach the reasoner to a set of RDF ddata to process. The reasoner may already have been bound to specific rules or ontology axioms (encoded in RDF)
	...
	/**
	 * Set a configuration paramter for the reasoner. In the case of the this reasoner there are no configuration parameters and this method is simply here to
	 * meet the interfaces specification
	 * 
	 * @param parameter
