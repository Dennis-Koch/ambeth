...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.resultset.*;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.sse.builders.BuilderTable;
...
public class ResultSetFactory
{
	...
	public static SPARQLResult result(String filenameOrURI, ResultsFormat format)
	{
		...
		if (format.equals(ResultsFormat.FMT_RS_XML) || format.equals(ResultsFormat.FMT_RS_JSON) || format.equals(ResultsFormat.FMT_RS_TSV)
				|| format.equals(ResultsFormat.FMT_RS_CSV))
		{
			InputStream in = null;
			try
			{
	...
	/**
	 * Turn an existing result set into a rewindable one. May take a copy but this is not guarantted Uses up the result set passed in which is no longer valid
	 * as a ResultSet.
	 * 
	 * @param resultSet
	 * @return ResultSetRewindable
	 ...
	 * <p>
	 * Using the returned result set consumes the result set passed in, the underlying result set must be at the start in order to be made peeking. If you
	 * create such a result set you should avoid accessing the underlying result set directly as this may cause results to be missed or put the returned
	 * peekable result set into an invalid state.
	 * </p>
	 * </p> Note that rewindable results may typically also be peekable so may be more broadly applicable if you can afford the cost of loading all the results
	 * into memory. </p>
	 * 
	 * @param resultSet
	 *            Result set to wrap
