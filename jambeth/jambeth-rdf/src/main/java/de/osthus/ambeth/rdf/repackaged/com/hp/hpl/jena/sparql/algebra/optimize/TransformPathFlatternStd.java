...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.OpTriple;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.VarAlloc;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.path.*;

/**
 * The path transformation step exactly as per the SPARQL 1.1 spec. It does not produce very nice execution structures so ARQ uses a functional equivalent, but
...
public class TransformPathFlatternStd extends TransformCopy
{
	...
	static class PathTransform extends PathVisitorBase
	{
		...
		/*
		 * Reverse transformations. X !(^:uri1|...|^:urin)Y ==> ^(X !(:uri1|...|:urin) Y) Split into forward and reverse. X
		 * !(:uri1|...|:urii|^:urii+1|...|^:urim) Y ==> { X !(:uri1|...|:urii|)Y } UNION { X !(^:urii+1|...|^:urim) Y }
		 */
		@Override
