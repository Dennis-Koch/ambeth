...
/**
 * <p>
 * An authenticator capable of making Form based logins and using cookies to maintain authentication state. Different logins may be used for different services
 * as required.
 * </p>
 * <h3>Login Scope</h3>
 * <p>
 * Note that logins are not exclusively scoped to the exact URI rather they are scoped to any URI derived from the given URI. For example if you declared logins
 * for {@code http://example.org} they would also apply to {@code http://example.org/some/path/}. When determining credentials the longest match applies, so in
 * the previous example you could define different logins for the two URIs and URIs derived from {@code http://example.org/some/path/} would prefer the login
 * for that URI over those for {@code http://example.org}
 * </p>
 * 
 */
		...
		synchronized (login)
		{
			...
			try
			{
				...
				if (response.getStatusLine().getStatusCode() >= 400)
				{
					...
					throw new HttpException(response.getStatusLine().getStatusCode(), "Login attempt failed - " + response.getStatusLine().getReasonPhrase(),
							payload);
				}
