...
public class StreamRDFLib
{
	...
	{
		return new WriterStreamRDFTuples(IO.wrapUTF8(out));
	}

	public static StreamRDF writer(AWriter out)
	{
		return new WriterStreamRDFTuples(out);
	}

	public static StreamRDF writer(Writer out)
	{
		return new WriterStreamRDFTuples(IO.wrap(out));
	}
	...
	/**
	 * Output to a sink; prefix and base handled only within the parser. Unfortunately, Java needs different names for the triples and quads versions because of
	 * type erasure.
	 */
	...
	private static class ParserOutputGraph extends StreamRDFBase
	{
		...
		{
			try
			{ // Jena applies XML rules to prerfixes.
