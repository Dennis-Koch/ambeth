...
/**
 * DatasetGraph: The graph representation of an RDF Dataset. See {@link Dataset} for the Model level of an RDF dataset.
 * <p>
...
public interface DatasetGraph extends Closeable
{
	...
	/**
	 * Get the graph named by graphNode : returns null on no graph NB Whether a dataset contains a graph if there are no triples is not defined - see the
	 * specifc implementation. Some datasets are "open" - they have all graphs even if no triples,
	 * */
	...
	/**
	 * Set the default graph. Set the active graph if it was null. This replaces the contents default graph, not merge data into it. Do not assume that the same
	 * object is returned by {@link #getDefaultGraph}
	 */
	public void setDefaultGraph(Graph g);

	/**
	 * Add the given graph to the dataset. <em>Replaces</em> any existing data for the named graph; to add data, get the graph and add triples to it, or add
	 * quads to the dataset. Do not assume that the same Java object is returned by {@link #getGraph}
	 */
