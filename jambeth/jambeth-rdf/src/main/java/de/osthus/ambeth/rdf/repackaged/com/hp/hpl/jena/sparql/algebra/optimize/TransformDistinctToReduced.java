...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.OpDistinct;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.OpOrder;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.OpProject;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.algebra.op.OpReduced;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.core.Var;

/**
 * <p>
 * Transforms generic {@code DISTINCT} plus {@code ORDER BY} combinations to {@code REDUCED} plus {@code ORDER BY} which typically gives better performance and
 * memory consumption because engines have to keep less data in-memory to evaluate it.
 * </p>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Uses both {@code ORDER BY} and {@code DISTINCT} on the same level of the query</li>
 * <li>There is a fixed list of variables to project i.e. not {@code SELECT *}</li>
 * <li>{@code ORDER BY} conditions cover all the projected variables prior to the use of any other variables</li>
 * </ul>
 * <h3>Related Optimizations</h3>
 * <p>
 * See also {@link TransformOrderByDistinctApplication} which is a better optimization for these kinds of queries but only applies to a more limited range of
 * queries. Where possible that optimization is applied in preference to this one.
 * </p>
 * <p>
 * {@link TransformTopN} covers the case of {@code DISTINCT} plus {@code ORDER BY} where there is also a {@code LIMIT}. Where possible that optimization is
 * applied in preference to either this or {@link TransformOrderByDistinctApplication}.
 * </p>
 * 
 */
public class TransformDistinctToReduced extends TransformCopy
{
	...
	public Op transform(OpDistinct opDistinct, Op subOp)
	{
		...
		/*
		 * SELECT DISTINCT * {} ORDER BY giving an alegbra expression of the form: (distinct (order
		 */
