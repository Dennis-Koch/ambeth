...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.ResIterator;
...
public class Basic extends BaseXMLWriter
{
	...
	protected void writePredicate(Statement stmt, final PrintWriter writer)
	{
		...
		writer.print(space + space + "<" + startElementTag(predicate.getNameSpace(), predicate.getLocalName()));
	...
	protected void writeResourceId(Resource r, PrintWriter writer)
	{
		...
		{
			writer.print(rdfAt("about") + "=" + substitutedAttribute(relativize(r.getURI())));
		}
	}
	...
	protected void writeResourceReference(Resource r, PrintWriter writer)
	{
		...
		{
			writer.print(rdfAt("resource") + "=" + substitutedAttribute(relativize(r.getURI())));
		}
	}
