...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.PropertyImpl;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.Util;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.BrokenException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.JenaException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.shared.PropertyNotFoundException;
...
class Unparser
{
	...
	/**
	 * Creates an Unparser for the specified model. The localName is the URI (typical URL) intended for the output file. No trailing "#" should be used. This
	 * will control the use of <I>ID</I> or <I>about</I> or <I>resource</I> on various rules.
	 * 
	 * @param localName
	...
	Unparser(Abbreviated parent, String localName, Model m, PrintWriter w)
	{
		...
		try
		{
			...
			while (reified.hasNext())
			{
				...
				{
					/**
					 * This block of code assumes that really we are dealing with a reification. We may, on the contrary, be dealing with a random collection of
					 * triples that do not make sense.
					 */
	...
	/**
	 * Should be called exactly once for each Unparser. Calling it a second time will have undesired results.
	 */
	void write()
	{
		...
		wRDF();
		/*
		 * System.out.print("Coverage = "); for (int i=0;i<codeCoverage.length;i++) System.out.print(" c[" + i + "] = " + codeCoverage[i]+ ";");
		 * System.out.println();
		 */
	}
	...
	/*
	 * THE MORE INTERESTING MEMBER VARIABLES. Note there are others scattered throughout the file, but those are only used by one or two methods.
	 */
	...
	private Map<Resource, Integer> objectTable; // This is a map from Resource
												// to Integer
	...
	private Set<Statement> doneSet = new HashSet<>(); // The triples that have
														// been
	// output.

	private Set<Resource> haveReified = new HashSet<>(); // Those local
															// resources that
	// are
	...
	/*
	 * The top-down recursive descent unparser. The methods starting in w all refer to one of the rules of the grammar, which they implement. boolean valued
	 * rules first check whether they are applicable and return false if not. Otherwise they create appropriate output (using recursive descent) and return
	 * true. Note all necessary checks are made before any output or any recursive descent. The void w- methods just implement the rule, which typically does
	 * not involve any choice.
	 */
	/*
	 * [6.1] RDF ::= ['<rdf:RDF>'] obj* ['</rdf:RDF>']
	 */
	...
	/*
	 * [6.12] propertyElt ::= '<' propName idAttr? '>' value '</' propName '>' | '<' propName idAttr? parseLiteral '>' literal '</' propName '>' | '<' propName
	 * idAttr? parseResource '>' propertyElt* '</' propName '>' | '<' propName idRefAttr? bagIdAttr? propAttr* '/>' | '<' * propName idAttr? parseDamlCollection
	 * '>' obj* '</' propName '>' [daml.2] parseDamlCollection ::= ' parseType="rdf:collection"'
	 * 
	 * For RDF collections we prefer the special syntax otherwise: We prefer choice 4 where possible, except in the case where the statement is reified and the
	 * object is not anonymous in which case we use one of the others (e.g. choice 1). For embedded XML choice 2 is obligatory. For untyped, anonymous resource
	 * valued items choice 3 is used. Choice 1 is the fall back.
	 */
	private boolean wPropertyElt(WType wt, Property prop, Statement s, RDFNode val)
	{
		return wPropertyEltCompact(wt, prop, s, val) || // choice 4
				...
				wPropertyEltDatatype(wt, prop, s, val) || wPropertyEltValue(wt, prop, s, val);
		// choice 1.
	}
	...
	/*
	 * [6.12] propertyElt ::= '<' propName idAttr? '>' value '</' propName '>' [6.17.1] value ::= obj
	 */
	...
	/*
	 * [6.2] obj ::= description | container [6.3] description ::= '<rdf:Description' idAboutAttr? bagIdAttr? propAttr* '/>' | '<rdf:Description' idAboutAttr?
	 * bagIdAttr? propAttr* '>' propertyElt* '</rdf:Description>' | typedNode [6.4] container ::= sequence | bag | alternative We use: [6.2a] obj ::=
	 * description | container | typedNode [6.3a] description ::= '<rdf:Description' idAboutAttr? bagIdAttr? propAttr* '/>' | '<rdf:Description' idAboutAttr?
	 * bagIdAttr? propAttr* '>' propertyElt* '</rdf:Description>'
	 * 
	 * This method has got somewhat messy. If we are not at the topLevel we may choose to not expand a node but just use a typedNode ::= '<' typeName
	 * idAboutAttr '/>' rule. This rules also applies to Bags that we feel unconfortable with, such as a Bag arising from a BagId rule that we don't handle
	 * properly.
	 */
	...
	/*
	 * [6.3a] description ::= '<rdf:Description' idAboutAttr? bagIdAttr? propAttr* '/>' | '<rdf:Description' idAboutAttr? bagIdAttr? propAttr* '>' propertyElt*
	 * '</rdf:Description>'
	 */
	...
	/*
	 * [6.13.2] typedNode ::= '<' typeName idAboutAttr? bagIdAttr? propAttr* '>' propertyElt* '</' typeName '>'
	 */
	...
	/*
	 * [6.5] idAboutAttr ::= idAttr | aboutAttr | aboutEachAttr we use [6.5a] idAboutAttr ::= idAttr | aboutAttr
	 */
	...
	/**
	 * Returns false if the resource is not genuinely anonymous and cannot be referred to using an ID. [6.6] idAttr ::= ' ID="' IDsymbol '"'
	 */
	...
	private void wIdAttrReified(Statement s)
	{
		...
		{
			/*
			 * if ( prettyWriter.sReification ) System.err.println("???"); else System.err.println("!!!");
			 */
	 ...
	 * [6.19] Qname ::= [ NSprefix ':' ] name
	 * 
	 * private void wQnameStart(String ns, String local) { print(prettyWriter.startElementTag(ns, local)); }
	 * 
	 * private void wQnameEnd(String ns, String local) { print(prettyWriter.endElementTag(ns, local)); }
	 */
	...
	/*
	 * [6.10] propAttr ::= typeAttr | propName '="' string '"' (with embedded quotes escaped) [6.11] typeAttr ::= ' type="' URI-reference '"'
	 */
	...
	/**
	 * Quote str with either ' or " quotes to be in attribute position in XML. The real rules are found at http://www.w3.org/TR/REC-xml#AVNormalize
	 */
	...
	/**
	 * Indentation screws up if there is a tab character in s. We do not check this.
	 */
	...
	private void addTypeNameSpaces()
	{
		...
		try
		{
			while (nn.hasNext())
			{
				...
				if (split != -1)
					prettyWriter.addNameSpace(((Resource) obj).getURI().substring(0, split));
			}
		}
		finally
		{
	...
	{
		return (!r.isAnon()) && getNameSpace(r).equals(localName + "#") && XMLChar.isValidNCName(getLocalName(r));
	}

	/*
	 * Utility for turning an integer into an alphabetic string.
	 * 
	 * private static String getSuffix(int suffixId) { if (suffixId == 0) return ""; else { suffixId--; int more = (suffixId / 26);
	 * 
	...
	private Statement[] reification(Statement s)
	{
		...
		return new Statement[] { m.createStatement(r, RDF.type, RDF.Statement), m.createStatement(r, RDF.subject, s.getSubject()),
				m.createStatement(r, RDF.predicate, s.getPredicate()), m.createStatement(r, RDF.object, s.getObject()) };
	}
	...
	private boolean hasProperties(Resource r)
	{
		...
		if (avoidExplicitReification && // ( r instanceof Statement ) &&
				(!r.isAnon()) && isLocalReference(r) && res2statement.containsKey(r))
		{
			ss = new MapFilterIterator<>(new MapFilter<Statement, Statement>()
			{
				...
				public Statement accept(Statement o)
				{
					...
					return ((!p.getNameSpace().equals(rdfns)) || !((RDF.type.equals(p) && s.getObject().equals(RDF.Statement)) || RDF.object.equals(p)
							|| RDF.predicate.equals(p) || RDF.subject.equals(p))) ? o : null;
				}
			}, ss);
		}
		try
	...
	/**
	 * This class is an iterator over the set infinite, but we wait until it is used before instantiating the underlying iterator.
	 */
	private Iterator<Resource> allInfiniteLeft()
	{
		...
		{
			@Override
			public Iterator<Resource> create()
			{
	...
	private Iterator<Resource> pleasingTypeIterator()
	{
		...
		Set<Resource> bucketArray[] = new Set[pleasingTypes.length];// new
																	// Set<Resource>[pleasingTypes.length];
		...
		return WrappedIterator.createIteratorIterator(new Map1Iterator<>(mapper, Arrays.asList(bucketArray).iterator()));
	}

	/**
	 * listSubjects - generates a list of subjects for the wObjStar rule. We wish to order these elegantly. The current implementation goes for:
	 * <ul>
	 ...
	 * <li>non-anonymous resources that are the object of more than one rule that are in infinite cycles.
	 ...
	 * 
	 * 
	 * At the end, we need to close any underlying ResIterators from the model, however to avoid complications in much of this code we use general
	 * java.util.Iterator-s. We hence use a wrapper around a ResIterator to allow us to manage the closing issue.
	 */
	private Iterator<Resource> listSubjects()
	{
		...
		// Iterator<Resource> nonObjects = new FilterIterator<Resource>(new
		// Filter<Resource>() {
		...
		// }
		// }, modelListSubjects());
		Iterator<Resource> nonObjects = modelListSubjects().filterKeep(new Filter<Resource>()
		{
			@Override
			public boolean accept(Resource o)
			{
				return (!objectTable.containsKey(o)) && (!wantReification(o));
			}
		});
		...
		@SuppressWarnings("unchecked")
		Iterator<Resource> all[] = new Iterator[] { currentFile, pleasing, fakeStopPleasing, nonObjects, fakeLazyEvaluator, firstChoiceCyclic, nonAnonInfinite,
				inf, anotherFake, reifications, new NullIterator<Resource>()
				{
					@Override
					public boolean hasNext()
					{
