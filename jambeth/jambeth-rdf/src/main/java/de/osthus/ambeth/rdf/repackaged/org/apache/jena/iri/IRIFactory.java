...
package de.osthus.ambeth.rdf.repackaged.org.apache.jena.iri;

import java.io.UnsupportedEncodingException;

import de.osthus.ambeth.rdf.repackaged.org.apache.jena.iri.impl.IRIFactoryImpl;

/**
 * An IRIFactory is the entry point to this module. It allows:
 * <ul>
 * <li>The setting of conformance levels, particularly identifying which standard(s) to enforce
 * <li>The creation of IRI objects, checking them against the selected standard(s)
 * <li>The setting of optional behaviours, as specified in the various standard(s)
 * </ul>
 * <p>
 * Any configuration of the factory to implement particular standards, or to treat particular violations as warnings or errors, must be completed before using
 * the construct or create methods.
 * </p>
 * <p>
 * The easiest way to use this class is to use one of the preconfigured factories: {@link #semanticWebImplementation()} {@link #iriImplementation()}, or
 * {@link #uriImplementation()}. If none of these meets your application needs then you have to configure your own factory.
 * </p>
 * 
 * <p>
 ...
 * </ul>
 * <li>One or more of (note the effect is cumulative, all the used specifications will be enforced):
 * <ul>
...
public class IRIFactory extends IRIFactoryImpl implements IRIFactoryI
{
	...
	/**
	 * This factory implements RFC 3987 <a href="http://www.apps.ietf.org/rfc/rfc3987.html"> Internationalized Resource Identifiers (IRIs) </a>.
	 * 
	...
	/**
	 * This factory implements RFC 3986 <a href="http://www.apps.ietf.org/rfc/rfc3986.html"> Uniform Resource Identifier (URI): Generic Syntax </a>.
	 * 
	...
	/**
	 * This factory is a conservative implementation appropriate for Semantic Web applications. It implements: RDF URI Reference (essential), IRI (needed for
	 * SPARQL) and XLink locator (ensures that only legal XML characters are included, allowing RDF/XML usage). In addition,
	 * {@link ViolationCodes#NON_INITIAL_DOT_SEGMENT} is treated as an error (since any IRI with this condition cannot be serialized in RDF/XML, which resolves
	 * all IRIs against the inscope base, and hence uses the remove_dot_segments algorithm). This should ensure that any IRI that is not in error, can be used
	 * interoperably in RDF/XML, SPARQL, N3 and N-Triple.
	 * 
	...
	/**
	 * Create a new IRIFactory without any conformance settings. To check for errors, you must call one or more of {@link #useSpecificationIRI(boolean)},
	 * {@link #useSpecificationRDF(boolean)}, {@link #useSpecificationURI(boolean)}, {@link #useSpecificationXMLSchema(boolean)},
	 * {@link #useSpecificationXLink(boolean)} or {@link #useSpecificationXMLSystemID(boolean)}.
	 * 
	 */
	public IRIFactory()
	{
		/*
		 * Resets this factory to: <ul> <li>ignore all errors, <li>have no scheme specific rules, </ul>
		 */
	 ...
	 * 
	 * @param scheme
	 *            The scheme to enable this behaviour for, or "*" for all schemes
	 */
	@Override
	...
	/**
	 * Allows the unwise characters as optionally permitted by RFC 3987 (IRI). <blockquote> Systems accepting IRIs MAY also deal with the printable characters
	 * in US-ASCII that are not allowed in URIs, namely "&lt;", "&gt;", '"', space, "{", "}", "|", "\", "^", and "`", in step 2 above. If these characters are
	 * found but are not converted, then the conversion SHOULD fail. Please note that the number sign ("#"), the percent sign ("%"), and the square bracket
	 * characters ("[", "]") are not part of the above list </blockquote>
	 * <p>
	 * This method is intended to be used with factories that are using the IRI specification. The unwise characters are treated as minting warnings after this
	 * method is called. This method does not override any setting from {@link #useSpecificationXMLSchema(boolean)} concerning
	 * {@link ViolationCodes#DOUBLE_WHITESPACE}.
	 * </p>
	 */
	...
	/**
	 * The character constraints on the query component of an IRI are weaker than on other components. It is not clear how much weaker. Calling this method with
	 * <code>restrict=false</code> removes (all?) restrictions, calling this method with <code>restrict=true</code> adds restrictions, specifically disallowing
	 * private use codepoints.
	 * 
	 * @param restrict
	 *            True to make the query component checking stricter, false to make the query component checking more lenient
	 */
	 ...
	 * <p>
	 * Using this method does not conform with the IRI specification, or XLink, XML system identifiers, RDF URI references, or XML Schema anyURI. This method is
	 * conformant with the URI specification.
	 * </p>
	 * 
	...
	/**
	 * Create an IRI from the given components. Performs whatever percent escaping or punycode encoding is necessary to make this IRI legal for this factory (if
	 * possible). Omitted components are passed as null.
	 * 
	 * @param scheme
	 *            Schema
	 * @param userInfo
	 *            User Info
	 * @param host
	 *            Will be encoded using punycode, if necessary.
	 * @param port
	...
	/**
	 * Create an IRI from the given components. Performs whatever percent escaping is necessary to make this IRI legal for this factory (if possible). Omitted
	 * components are passed as null. Use {@link #create(String, String, String, int, String, String, String)} when the authority is a DNS hostname, even if
	 * both the user information and the port are unspecified; this version uses percent escaping as opposed to punycode for the authority. DNS hostnames should
	 * be escaped in punycode (if necessary).
	 * 
	 * @param scheme
	 *            Scheme
	 * @param authority
	 *            Will be percent escaped if necessary
	 * @param path
	 *            May not be null
	 * @param query
	 *            Query
	...
	/**
	 * Create an IRI from the given components. Performs whatever percent escaping is necessary to make this IRI legal for this factory (if possible). Omitted
	 * components are passed as null. Use {@link #construct(String, String, String, int, String, String, String)} when the authority is a DNS hostname, even if
	 * both the user information and the port are unspecified; this version uses percent escaping as opposed to punycode for the authority. DNS hostnames should
	 * be escaped in punycode (if necessary).
	 * 
	 * @param scheme
	 ...
	 * 
	 * @throws IRIException
	 *             If the resulting IRI has unfixable errors, e.g. non-ascii chars in the scheme name
	 * @see #setEncoding(String)
	 */
	 ...
	 * 
	 * @param code
	 *            A condition code from {@link ViolationCodes}.
	 * 
	 */
	...
	/**
	 * Is condition #<code>code</code> being treated as a warning.
	 * 
	 * @param code
	 *            A condition code from {@link ViolationCodes}.
	 */
	...
	/**
	 * Set condition #<code>code</code> to be treated as an error; or clear it as an error condition.
	 * <p>
	 * Care must be taken when using this to clear the error behaviour on a code documented in {@link ViolationCodes} as having SHOULD force: see the
	 * documentation at {@link #shouldViolation(boolean, boolean)}, concerning the necessary steps. Using this method with <code>code</code> being one that is
	 * documented as having MUST force will result in non-conformant behaviour.
	 * </p>
	 * 
	 * @param code
	 *            A condition code from {@link ViolationCodes}.
	...
	/**
	 * Set condition #<code>code</code> to be treated as a warning; or clear it as a warning condition. Setting a code to be a warning, clears it from being an
	 * error. Care must be taken if the <code>code</code> is one that is documented in {@link ViolationCodes} has having SHOULD or MUST force, since ignoring
	 * any resulting warning may result in a nonconformant application.
	 * 
	 * @param code
	 *            A condition code from {@link ViolationCodes}.
	 * @param set
	...
	/**
	 * The factory will treat violations of "SHOULD" force statements from the specifications it is enforcing as errors, or warnings, or not at all. (Default is
	 * error)
	 * 
	 * <p>
	 * From <a href="http://www.apps.ietf.org/rfc/rfc2119.html#sec-3">RFC 2119</a>
	 * <em>the full implications must be understood and carefully weighed before</em> calling this method with <code>isError=false</code>. Thus, you MUST have
	 * read and understood the specifications that you are configuring the factory to use, before switching SHOULDs to warnings or to be ignored. An easier
	 * path, is to understand a specific error code, with SHOULD force, and then use {@link #setIsError(int, boolean)}, and {@link #setIsWarning(int, boolean)}
	 * to modify the behaviour of that error code only. The prerequisite for modifiying a single error code to be ignored, or to be treated as a warning is to
	 * have understood the full implications of that condition, rather than of all the SHOULD force statements within all the specifications being used.
	 * </p>
	 * 
	 * 
	 * @param isError
	 *            If true, treat violations of SHOULDs as errors.
	 * @param isWarning
	 *            If true, treat violations of SHOULDs as warnings.
	 * @throws IllegalArgumentException
	 *             if <code>isError &amp;&amp; isWarning</code>.
	 * 
	 * @throws IllegalStateException
	 *             If this factory has already been used to create an IRI.
	 */
	...
	/**
	 * The factory will treat violations of statements from the specifications flagged as security issues, including weak heuristical suggestions, it is
	 * enforcing as errors, or warnings, or not at all. (Default is error)
	 * 
	 * @param isError
	 *            If true, treat security violations as errors.
	 * @param isWarning
	 ...
	 * (Default is error) You must also call {@link #useSchemeSpecificRules(String)} to enable particular enforcement of particular schemes. This method
	 * primarily permits the scheme specific rules to be treated as warnings.
	 * 
	 * @param isError
	 *            If true, treat scheme violations as errors.
	 * @param isWarning
	 *            If true, treat scheme violations as warnings.
	 * @throws IllegalArgumentException
	 *             if <code>isError &amp;&amp; isWarning</code>.
	 */
	...
	// }
	/**
	 * The factory will treat violations of statements from the specifications flagged as DNS issues, including Internationalized Domain Name issues, it is
	 * enforcing as errors, or warnings, or not at all. (Default is error)
	 * 
	 * 
	 * @param isError
	 *            If true, treat DNS violations as errors.
	 * @param isWarning
	...
	/**
	 * The factory will treat violations of statements from the specifications concerning creating new IRIs it is enforcing as errors, or warnings, or not at
	 * all. (Default is warning). A sample phrase indicating the intent is this one from RFC 3986: <blockquote> An implementation should accept uppercase
	 * letters as equivalent to lowercase in scheme names (e.g., allow "HTTP" as well as "http") for the sake of robustness but should only produce lowercase
	 * scheme names for consistency. </blockquote>
	 * 
	 * @param isError
	 *            If true, treat violations of minting force statements as errors.
	 * @param isWarning
	...
	//
	/* *
	 * Adds a scheme to the list of known schemes. The <code>port</code> argument either associates this scheme with a default port number, and hence with the
	 * generic syntax; or is one of the constants {@link #GENERIC_NO_PORT}, {@link #GENERIC_UNKNOWN_PORT}, {@link #OPAQUE}, {@link #UNKNOWN_SYNTAX} to indicate
	 * what restrictions if any should be used.
	 * 
	 ...
	 * <dt>
	 * Partial</dt>
	 * <dd>indicates that some but not all of the scheme specific restrictions on the IRI are implemented.</dd>
	 * <dt>
	 * component</dt>
	 * <dd>means that the scheme prohibits or requires one or more components of the IRI; and only these checks are performed.</dd>
	 * </dl>
	 * </table>
	 * <p>
	 ...
	 * 
	 * @param scheme
	 *            The scheme name or "*" to use all implemented scheme specific rules.
	 * @param asErrors
	 *            If true, then violations are treated as errors; if false violations are treated as warnings.
	 * @throws IllegalStateException
	 *             If this factory has already been used to create an IRI.
	...
	/**
	 * This <em>globally</em> sets the {@link #jenaImplementation}; use with care. This should be used before any calls to {@link #jenaImplementation} ; it does
	 * not modify the factory returned by any previous calls, but subsequent calls to {@link #jenaImplementation} will return the new value.
	 * 
	 * @param jf
	...
	/**
	 * This <em>globally</em> sets the {@link #iriImplementation}; use with care. This should be used before any calls to {@link #iriImplementation}; it does
	 * not modify the factory returned by any previous calls, but subsequent calls to {@link #iriImplementation} will return the new value.
	 * 
	 * @param iriF
	 *            The new IRI Factory
	 */
	...
	/**
	 * This <em>globally</em> sets the {@link #uriImplementation}; use with care. This should be used before any calls to {@link #uriImplementation}; it does
	 * not modify the factory returned by any previous calls, but subsequent calls to {@link #uriImplementation} will return the new value.
	 * 
	 * @param uriF
	 *            The new URI Factory
	 */
	...
	/**
	 * This <em>globally</em> sets the {@link #semanticWebImplementation}; use with care. This should be used before any calls to
	 * {@link #semanticWebImplementation}; it does not modify the factory returned by any previous calls, but subsequent calls to
	 * {@link #semanticWebImplementation} will return the new value.
	 * 
	 * @param sw
	 *            The new IRI Factory
	 */
