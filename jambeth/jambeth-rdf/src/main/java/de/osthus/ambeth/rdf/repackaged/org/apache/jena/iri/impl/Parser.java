...
public class Parser implements IRIComponents, ViolationCodes
{
	...
	static
	{
		for (int i = 0; i < lexers.length; i++)
			lexers[i] = new Lexer[] { new LexerScheme((Reader) null), new LexerUserinfo((Reader) null), new LexerHost((Reader) null),
					new LexerPort((Reader) null), new LexerPath((Reader) null), new LexerQuery((Reader) null), new LexerFragment((Reader) null),
	...
	static final int fields[] = { SCHEME, USER, HOST, PORT, PATH, QUERY, FRAGMENT, AUTHORITY, PATHQUERY };
	...
	public Parser(String uri, IRIImpl iri)
	{
		...
		if ((errors[invFields[HOST]] & ((1l << DOUBLE_DASH_IN_REG_NAME) | (1l << ACE_PREFIX) | (1l << NON_URI_CHARACTER))) != 0)
		{
			...
			try
			{
				try
				{
					IDN.toUnicode(IDNP.toASCII(h, IDN.USE_STD3_ASCII_RULES), IDN.USE_STD3_ASCII_RULES | IDN.ALLOW_UNASSIGNED);
				...
				catch (IllegalArgumentException e)
				{
					try
					{
						IDN.toUnicode(IDNP.toASCII(h, IDN.USE_STD3_ASCII_RULES | IDN.ALLOW_UNASSIGNED), IDN.USE_STD3_ASCII_RULES | IDN.ALLOW_UNASSIGNED);
		...
		hasComponents(~found & iri.scheme.getRequired(), REQUIRED_COMPONENT_MISSING);
		hasComponents(found & iri.scheme.getProhibited(), PROHIBITED_COMPONENT_PRESENT);
