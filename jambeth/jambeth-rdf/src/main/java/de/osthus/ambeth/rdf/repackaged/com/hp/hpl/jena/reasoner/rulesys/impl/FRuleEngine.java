...
public class FRuleEngine implements FRuleEngineI
{
	...
	/**
	 * Process all available data. This should be called once a deductions graph has be prepared and loaded with any precomputed deductions. It will process the
	 ...
	 * 
	 * @return true if the effects could be correctly propagated or false if not (in which case the entire engine should be restarted).
	 */
	@Override
	...
	/**
	 * Add a set of new triple to the data graph, run any rules triggered by the new data item, recursively adding any generated triples. Technically the
	 * triples having been physically added to either the base or deduction graphs and the job of this function is just to process the stack of additions firing
	 * any relevant rules.
	 * 
	 * @param context
	...
	/**
	 * Match the rest of a set of rule clauses once an initial rule trigger has fired. Carries out any actions as a side effect.
	 * 
	 * @param trigger
	 *            the index of the clause which has already be successfully matched
	 * @param context
	 *            a context containing a set of new triples to be added
		...
		else
		{
			...
			for (int i = 0; i < len; i++)
			{
				...
				if (body[i] instanceof TriplePattern)
				{
					...
					int score = scoreNodeBoundness(clause.getSubject(), env) * 3 + scoreNodeBoundness(clause.getPredicate(), env) * 2
	...
	/**
	 * Match each of a list of clauses in turn. For all bindings for which all clauses match check the remaining clause guards and fire the rule actions.
	 * 
	 * @param clauses
	 *            the list of clauses to match, start with last clause
	 * @param context
	 *            a context containing a set of new triples to be added
	 * @return true if the rule actually fires
	 */
	private boolean matchClauseList(List<ClauseEntry> clauses, BFRuleContext context)
	{
		...
		Iterator<Triple> i = infGraph.findDataMatches(env.getBinding(clause.getSubject()), env.getBinding(clause.getPredicate()), env.getBinding(objPattern));
	...
	/**
	 * Score a Node in terms of groundedness - heuristic. Treats a variable as better than a wildcard because it constrains later clauses. Treats rdf:type as
	 * worse than any other ground node because that tends to link to lots of expensive rules.
	 */
	...
	/**
	 * Test if a TriplePattern matches a Triple in the given binding environment. If it does then the binding environment is modified the reflect any additional
	 * bindings.
	 * 
	...
	{
		env.push();
		boolean matchOK = match(pattern.getPredicate(), triple.getPredicate(), env) && match(pattern.getObject(), triple.getObject(), env)
	...
	/**
	 * Test if a pattern Node matches a Triple Node in the given binding environment. If it does then the binding environment is modified the reflect any
	 * additional bindings.
	 * 
	...
	public static boolean match(Node pattern, Node node, BindingStack env)
	{
		...
		else if (Functor.isFunctor(pattern))
		{
			...
			if (patternArgs.length != nodeArgs.length)
				return false;
			// Compatible functor shapes so bind an embedded variables in the
			// pattern
			env.push();
	...
	/**
	 * Structure used in the clause index to indicate a particular clause in a rule. This is used purely as an internal data structure so we just use direct
	 * field access.
	 */
