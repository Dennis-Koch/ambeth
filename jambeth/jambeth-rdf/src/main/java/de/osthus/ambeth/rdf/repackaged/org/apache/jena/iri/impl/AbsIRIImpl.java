...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.iri.*;
 ...
 */
abstract public class AbsIRIImpl extends IRI implements ViolationCodes, IRIComponents
{
	...
	/*
	 * static private Idna idna; static { try { idna = new Idna(new Punycode(), new Nameprep()); } catch (XcodeException e) {
	 * System.err.println("Internal error in IDN setup"); e.printStackTrace(); } }
	 */
	...
	Iterator<Violation> exceptionsMask(final long mask)
	{
		...
		return foundExceptions == null ? nullIterator : new Iterator<Violation>()
		{
			...
			public boolean hasNext()
			{
				if (next == null)
				{
					while (underlying.hasNext())
					{
						...
						if (((1l << (next).getViolationCode()) & mask) != 0)
							return true;
					}
					next = null;
					return false;
	...
	// }
	/*
	 * public boolean hasException(int conformance) { return hasExceptionMask(getFactory().recsToMask(conformance)); }
	 * 
	 * public Iterator exceptions(int conformance) { return exceptionsMask(getFactory().recsToMask(conformance)); }
	 */
	...
	// TODO ToAsciiMask
	static long ToAsciiMask = ~0l;

	/*
	 * (1l << LTR_CHAR) | (1l << ILLEGAL_CHAR) | (1l << IRI_CHAR) | (1l << UNWISE_CHAR) | (1l << WHITESPACE) | (1l << NOT_XML_SCHEMA_WHITESPACE) | (1l <<
	 * NON_XML_CHARACTER) | (1l << DOUBLE_DASH_IN_REG_NAME);
	 */
	@Override
	...
	static CharSequence domainToAscii(String host) throws MalformedIDNException
	{
		...
		/*
		 * int u[] = new int[host.length()]; for (int i = 0; i < host.length(); i++) u[i] = host.charAt(i);
		 * 
		 * try { return idna.domainToAscii(u); } catch (XcodeException e) { throw new MalformedIDNException(e); }
		 */
	}
	...
	private void toAscii(StringBuffer asciiString, String field, long errs)
	{
		 ...
		 * 
		 * We interpret this as any charcater above 127, below 32 and the unwise chars
		 * 
		 * Systems accepting IRIs MAY also deal with the printable characters in US-ASCII that are not allowed in URIs, namely "<", ">", '"', space, "{", "}",
		 * "|", "\", "^", and "`", in step 2 above.
		 */
	 ...
	 * 
	 * Relativizing path components is somewhat tricky. The code is in the method PathRelative.check which is invoked from relativizePaths
	 * 
	 * There are only three static stateless objects of this class, each of which checks for a particular rule.
	 * 
	 * The child object behaves slightly differently, with a helper method overridden, and one line of code specific to that object only.
	 */

	static private final PathRelativize child = new PathRelativize(CHILD, CHILD | PARENT | GRANDPARENT, ".")
	{
		@Override
	...
	}, parent = new PathRelativize(PARENT, PARENT | GRANDPARENT, ".."), grandparent = new PathRelativize(GRANDPARENT, GRANDPARENT, "../..");

	/**
	 * 
	 * @param in
	 *            The path from this IRI
	 * @param out
	 *            If successful, out[0] contains the answer
	 * @param flags
	...
	static class PathRelativize
	{
		 ...
		 * 
		 * @param flag
		 *            If this flag is not present then this rule does not apply.
		 * @param allFlags
		 *            If none of these flags are present then this rule and subsequence rules do not apply.
		 * @param replacement
		 *            If there is a match, then use this as the relative path
		 */
		...
		/**
		 * Return true if the rule applied. The result of the rule is returned in out[0] Return false if the rule did not apply. The input for the next rule is
		 * in out[0]
		 * 
		 * @param in
		 *            Absolute path to use in match
		 * @param out
		 *            Result, as above.
		 * @param flags
	...
	/*
	 * public String relativize(String abs, int flags) { return relativize(factory.create(abs),abs,flags); }
	 */
	@Override
	...
	{
		if (!has(AUTHORITY)) // we could use the new rules for relative URIs for
								// rootless, but I don't like them
			return def;
		...
		boolean absl = net && equal(r.getRawHost(), getRawHost()) && equal(getRawUserinfo(), r.getRawUserinfo()) && equal(getPort(), r.getPort());
		boolean same = absl && equal(getRawPath(), r.getRawPath()) && equal(getRawQuery(), r.getRawQuery());

		String rslt = r.getRawFragment() == null ? "" : ("#" + r.getRawFragment());
		...
		if (r.getRawQuery() != null)
		{
			...
			if (equal(thisPath, pathToRel) && (flags & CHILD) != 0)
			{
				return rslt;
			}
		}
		...
		{
			return "//" + (r.getRawUserinfo() == null ? "" : (r.getRawUserinfo() + "@")) + r.getRawHost()
	...
	{
		return new ResolvedRelativeIRI(this, (AbsIRIImpl) getFactory().create(i));
	}
	...
	/*
	 * Bidirectional IRIs MUST be rendered in the same way as they would be if they were in a left-to-right embedding; i.e., as if they were preceded by U+202A,
	 * LEFT-TO-RIGHT EMBEDDING (LRE), and followed by U+202C, POP DIRECTIONAL FORMATTING (PDF).
	 */
	@Override
