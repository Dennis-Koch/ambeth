...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.RDFDefaultErrorHandler;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.impl.ResourceImpl;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.xmloutput.RDFXMLWriterI;

//import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.vocabulary.DAML_OIL;

/**
 * This is not part of the public API. Base class for XML serializers. All methods with side-effects should be synchronized in this class and its subclasses.
 * (i. e. XMLWriters assume that the world is not changing around them while they are writing).
 * 
 * Functionality:
 * 
 * <ul>
...
abstract public class BaseXMLWriter implements RDFXMLWriterI
{
	...
	/*
	 * There are two sorts of id's for anonymous resources. Short id's are the default, but require a mapping table. The mapping table means that serializing a
	 * large model could run out of memory. Long id's require no mapping table, but are less readable.
	 */
	...
	/*
	 * A shortAnonId is computed by maintaining a mapping table from the internal id's of anon resources. The short id is the index into the table of the
	 * internal id.
	 */
	...
	/*
	 * A longAnonId is the internal id of the anon resource expressed as a character string.
	 * 
	 * This code makes no assumptions about the characters used in the implementation of an anon id. It checks if they are valid namechar characters and escapes
	 * the id if not.
	 */
	...
	/**
	 * true means all namespaces defined in the model prefixes will be noted in xmlns declarations; false means only "required" ones will be noted. Hook for
	 * configuration.
	 */
	...
	private void primeNamespace(Model model)
	{
		...
		if (usesPrefix(model, ""))
		{
			// Doing "" prefix. Ensure it is a non-clashing, non-empty entity
			// name.
	...
	String tag(String namespace, String local, int type, boolean localIsQname)
	{
		...
		if (type != FAST && type != FASTATTR)
		{
			...
			{
				// Description, ID, nodeID, about, aboutEach, aboutEachPrefix,
				// li
		...
		{
			checkURI(namespace);
			logger.warn("Internal error: unexpected QName URI: <" + namespace + ">.  Fixing up with j.cook.up code.", new BrokenException(
					"unexpected QName URI " + namespace));
			cookUp = true;
		}
		...
		if (cookUp)
			return cookUpAttribution(type, namespace, local);
		return prefix + ":" + local;
	}
	 ...
	 * @param out
	 *            the OutputStream to receive the serialization
	 * @param base
	 ...
	 * @param out
	 *            The Writer to which the serialization should be sent.
	 * @param base
	 ...
	 * 
	 * @param errHandler
	 *            The new error handler to be used, or null for the default handler.
	 ...
	 * 
	 * @see de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.RDFWriter#setProperty(java.lang.String, java.lang.Object)
	 */
	@Override
	final synchronized public Object setProperty(String propName, Object propValue)
	{
		...
		else if (propName.equalsIgnoreCase("minimalPrefixes"))
		{
			try
			{
				return new Boolean(!writingAllModelPrefixNamespaces);
			}
			finally
			{
	...
	/**
	 * Answer the boolean value corresponding to o, which must either be a Boolean, or a String parsable as a Boolean.
	 */
	...
	/*
	 * private boolean sameDocument = true; private boolean network = false; private boolean absolute = true; private boolean relative = true; private boolean
	 * parent = true; private boolean grandparent = false;
	 */
	private int relativeFlags = IRI.SAMEDOCUMENT | IRI.ABSOLUTE | IRI.CHILD | IRI.PARENT;

	/**
	 * Answer the form of the URI after relativisation according to the relativeFlags set by properties. If the flags are 0 or the base URI is null, answer the
	 * original URI. Throw an exception if the URI is "bad" and we demandGoodURIs.
	 */
	...
	/**
	 * Answer the argument URI, but if we demandGoodURIs and it isn't good, throw a JenaException that encapsulates a MalformedURIException. There doesn't
	...
	/**
	 * Answer true iff prefix is a "legal" prefix to use, ie, is empty [for the default namespace] or an NCName that does not start with "xml" and does not
	 * match the reserved-to-Jena pattern.
	 */
