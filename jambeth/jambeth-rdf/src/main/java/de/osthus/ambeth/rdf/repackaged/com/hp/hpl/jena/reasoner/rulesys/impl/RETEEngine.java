...
public class RETEEngine implements FRuleEngineI
{
	...
	public void fastInit(Finder inserts)
	{
		...
		// Below is used during testing to ensure that all ruleset work (if less
		// efficiently) if marked as non-monotonic
		// conflictSet = new RETEConflictSet(new RETERuleContext(infGraph,
		// this), false);
		...
		if (infGraph.getRawGraph() != null)
		{
			...
			else
			{
				...
				{
					// System.out.println("FastInit: " + ent.getKey() + " = " +
					// ent.getValue());
		...
		for (Rule rule : rules)
		{
			...
			for (int i = 0; i < rule.bodyLength(); i++)
			{
				...
				if (clause instanceof TriplePattern)
				{
					...
					// Create list of variables which should be cross matched
					// between the earlier clauses and this one
	...
	/**
	 * Helper. Record a new predicate/value pattern. Ensure there is either one wildcard or a number of specific patterns
	 */
	private void recordPredicatePattern(Node predicate, Node value)
	{
		...
		{
			// TODO possibly introduce a threshold here for the number of
			// patterns allowed before generalizing
			predicatePatterns.put(predicate, value);
		}
		return;
	}
	...
	/**
	 * Create a terminal node for the RETE network. Normally this is RETETerminal but some people have experimented with alternative delete handling by creating
	 * RETETerminal subclasses so this hook simplifies use of that approach.
	 */
	 ...
	 *            the new triple
	 * @param deduction
	 *            true if the triple has been generated by the rules and so should be added to the deductions graph.
	 */
	public synchronized void addTriple(Triple triple, boolean deduction)
	{
		...
		if (deletesPending.size() > 0)
			deletesPending.remove(triple);
		if (!addsHash.contains(triple))
		{ // Experimental, not sure why it
			// wasn't done before
	...
	public synchronized void deleteTriple(Triple triple, boolean deduction)
	{
		...
		if (deduction)
		{
			...
			// deduction retractions should not remove asserted facts, so
	...
	public void runAll()
	{
		while (true)
		{
			...
			if (next == null)
			{
				// Nothing more to inject, if this is a non-mon rule set now
				// process one rule from the conflict set
	...
	/**
	 * This fires a triple into the current RETE network. This format of call is used in the unit testing but needs to be public because the tester is in
	 * another package.
	 */
	...
	protected void findAndProcessAxioms()
	{
		for (Rule r : rules)
		{
			if (r.isAxiom())
			{
				...
				/*
				 * // Old version, left during debug and final checks for (int j = 0; j < r.headLength(); j++) { Object head = r.getHeadElement(j); if (head
				 * instanceof TriplePattern) { TriplePattern h = (TriplePattern) head; Triple t = new Triple(h.getSubject(), h.getPredicate(), h.getObject());
				 * addTriple(t, true); } }
				 */
			}
		}
