...
/**
 * The ModelExpansion code expands a model <code>M</code> against a schema <code>S</code>, returning a new model which contains
 * 
 * <ul>
 * <li>the statements of M
 * <li>any statements (A rdfs:subClassOf B) from S where neither A nor B is a bnode.
 * <li>statements (A rdf:type T) if M contains (A P any) and S contains (P rdfs:domain T).
 * <li>statements (A rdf:type T) if M contains (any P A) and S contains (P rdfs:range T).
 * <li>statements (A rdf:type T) if (A rdf:type U) and (U rdfs:subClassOf T).
 * </ul>
 * 
 * This is sufficient to allow the subjects in <code>M</code> which have properties from <code>S</code> to have enough type information for AssemblerGroup
 * dispatch.
 */
public class ModelExpansion
{
	 ...
	 * <li>the rdf:type statements implied by the rdfs:domain statements of <code>schema</code> and the <code>model</code> statements using that statements
	 * property
	 ...
	 * <p>
	 * Those classes in <code>m</code> that appear in <code>subClassOf</code> statements are given as explicit superclasses all their indirect superclasses.
	 */
	...
	/**
	 * To each subclass X of <code>type</code> add as superclass all the classes between X and <code>type</code>.
	 */
	...
	/**
	 * To each subclass X of <code>parents.item</code> add as superclass all the classes between X and that item and all the items in the rest of
	 * <code>parents</code>.
	 */
	...
	/**
	 * Add to <code>toAdd</code> all the superclass statements needed to note that any indirect subclass of <code>X = parents.item</code> has as superclass all
	 * the classes between it and X and all the remaining elements of <code>parents</code>.
	 */
	...
	/**
	 * Answer the set of all classes which appear in <code>m</code> as the subject or object of a <code>rdfs:subClassOf</code> statement.
	 */
	private static Set<RDFNode> findClassesBySubClassOf(Model m)
	{
		...
		while (it.hasNext())
			addClasses(classes, it.nextStatement());
		return classes;
	}
	...
	protected static void addSupertypes(Model result)
	{
		...
		for (StmtIterator it = result.listStatements(ANY, RDF.type, ANY); it.hasNext();)
		{
			...
			for (StmtIterator subclasses = result.listStatements(c, RDFS.subClassOf, ANY); subclasses.hasNext();)
			{
				...
				// System.err.println( ">> adding super type: subject " +
				// s.getSubject() + ", type " + type );
	...
	private static void addIntersections(Model result, Model schema)
	{
		...
		while (it.hasNext())
			addIntersections(result, schema, it.nextStatement());
	}
	...
	private static void removeElementsWithoutType(Set<Resource> candidates, Resource type)
	{
		for (Iterator<Resource> it = candidates.iterator(); it.hasNext();)
		{
			...
			if (!candidate.hasProperty(RDF.type, type))
				it.remove();
		}
	}
	...
	/**
	 * A Lisp-style linked list. Used because we want non-updating cons operations.
	 */
