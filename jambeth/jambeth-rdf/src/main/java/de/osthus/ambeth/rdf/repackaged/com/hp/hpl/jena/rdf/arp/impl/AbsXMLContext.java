...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.lang.LanguageTag;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.lang.LanguageTagCodes;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.lang.LanguageTagSyntaxException;

public abstract class AbsXMLContext implements ARPErrorNumbers, LanguageTagCodes
{
	...
	abstract void checkBaseUse(XMLHandler forErrors, Taint taintMe, String relUri, IRI rslt) throws SAXParseException;
	...
	private void checkXMLLang(XMLHandler arp, Taint taintMe, String newLang) throws SAXParseException
	{
		...
		try
		{
			...
			{
				arp.warning(taintMe, WARN_BAD_XMLLANG, "Unnecessary use of language tag \"und\" prohibited by RFC3066");
			}
			if ((tagType & LT_IANA_DEPRECATED) == LT_IANA_DEPRECATED)
			{
				arp.warning(taintMe, WARN_DEPRECATED_XMLLANG, "Use of deprecated language tag \"" + newLang + "\".");
			}
			if ((tagType & LT_PRIVATE_USE) == LT_PRIVATE_USE)
			{
				arp.warning(taintMe, IGN_PRIVATE_XMLLANG, "Use of (IANA) private language tag \"" + newLang + "\".");
			}
			else if ((tagType & LT_LOCAL_USE) == LT_LOCAL_USE)
			{
				arp.warning(taintMe, IGN_PRIVATE_XMLLANG, "Use of (ISO639-2) local use language tag \"" + newLang + "\".");
			}
			else if ((tagType & LT_EXTRA) == LT_EXTRA)
			{
				arp.warning(taintMe, IGN_PRIVATE_XMLLANG, "Use of additional private subtags on language \"" + newLang + "\".");
			}
		}
