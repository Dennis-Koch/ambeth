...
/* implements RDFWriter */
{
	...
	// (RDF lists, small anon nodes) and to calculate the prefixes that will be
	// used.
	...
	protected Set<RDFNode> oneRefObjects = null; // Bnodes referred to once as
													// an object - can inline
	protected Set<Resource> oneRefDone = null; // Things done - so we can check
												// for missed items
	...
	// Find well-formed RDF lists - does not find empty lists (this is
	// intentional)
		...
		for (; listTailsIter.hasNext();)
		{
			...
			for (;;)
			{
				...
				if (N3JenaWriter.DEBUG)
					out.println("# RDF list all: " + formatResource(listElement));
				...
				{
					if (N3JenaWriter.DEBUG)
						out.println("# RDF shared tail from " + formatResource(listElement));
					break;
				}
			}
			// At head of a pretty-able list - add its elements and its head.
			if (N3JenaWriter.DEBUG)
				out.println("# Collection list head: " + formatResource(validListHead));
	...
	protected boolean checkListElement(Resource listElement)
	{
		...
		{
			if (N3JenaWriter.DEBUG)
				out.println("# RDF list element does not have required properties: " + formatResource(listElement));
			return false;
		}
		...
		if (N3JenaWriter.DEBUG)
			out.println("# RDF list element does not right number of properties: " + formatResource(listElement));
		return false;
	}

	// Find bnodes that are objects of only one statement (and hence can be
	// inlined)
	...
	{
		return rdfListsAll.contains(subj) || oneRefObjects.contains(subj);
	}
	...
	// Very hard to do a pretty thing here because the objects may be large or
	// small or a mix.
	...
	// Need to out.print in short (all on one line) and long forms (multiple
	// lines)
