...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.datatypes.xsd.impl.XSDAbstractDateTimeType;

/**
 * Represent an XSD duration value. We use a seven dimensional space with years, months, days, hours, minutes, seconds and fractional seconds. This deviates
 * from the spec which allows arbitrary position decimals for seconds.
 */
...
public class XSDDuration extends AbstractDateTime
{
	...
	{
		return BigDecimal.valueOf(data[ms], data[msscale]).add(BigDecimal.valueOf(data[s]));
	}
	...
	private final static int[][] DATETIMES = { { 1696, 9, 1, 0, 0, 0, 0, 'Z' }, { 1697, 2, 1, 0, 0, 0, 0, 'Z' }, { 1903, 3, 1, 0, 0, 0, 0, 'Z' },
			{ 1903, 7, 1, 0, 0, 0, 0, 'Z' } };
	 ...
	 *            Unnormalized duration
	 * @param strict
	 *            (min/max)Exclusive strict == true ( LESS_THAN ) or ( GREATER_THAN ) (min/max)Inclusive strict == false (LESS_EQUAL) or (GREATER_EQUAL)
	 * @return INDETERMINATE if the order relationship between date1 and date2 is indeterminate. EQUAL if the order relation between date1 and date2 is EQUAL.
	 *         If the strict parameter is true, return LESS_THAN if date1 is less than date2 and return GREATER_THAN if date1 is greater than date2. If the
	 *         strict parameter is false, return LESS_THAN if date1 is less than OR equal to date2 and return GREATER_THAN if date1 is greater than OR equal to
	 *         date2
	 */
	@Override
