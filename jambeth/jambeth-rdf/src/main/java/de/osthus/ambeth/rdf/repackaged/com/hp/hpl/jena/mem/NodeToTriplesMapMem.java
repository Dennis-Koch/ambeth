...
public class NodeToTriplesMapMem extends NodeToTriplesMapBase
{
	...
	/**
	 * Answer an iterator over all the triples in this NTM which have index node <code>o</code>.
	 */
	@Override
	public Iterator<Triple> iterator(Object o, HashCommon.NotifyEmpty container)
	{
		// System.err.println( ">> BOINK" ); // if (true) throw new
		// JenaException( "BOINK" );
		TripleBunch s = bunchMap.get(o);
	...
	/**
	 * Answer an iterator over all the triples in this NTM which match <code>pattern</code>. The index field of this NTM is guaranteed concrete in the pattern.
	 */
	@Override
	public ExtendedIterator<Triple> iterator(Node index, Node n2, Node n3)
	{
		...
		// System.err.println( ">> ntmf::iterator: " + (s == null ? (Object)
		// "None" : s.getClass()) );
		return s == null ? NullIterator.<Triple> instance() : f2.filterOn(n2).and(f3.filterOn(n3)).filterKeep(s.iterator(new NotifyMe(indexValue)));
	}
