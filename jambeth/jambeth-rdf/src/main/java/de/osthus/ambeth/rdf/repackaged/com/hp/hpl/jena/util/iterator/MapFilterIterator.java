...
/**
 * A MapFilterIterator takes a MapFilter and an [Extended]Iterator and returns a new ExtendedIterator which delivers the sequence of all non-null elements
 * MapFilter(X) for X from the base iterator.
 */
...
public class MapFilterIterator<T, X> extends NiceIterator<X> implements ExtendedIterator<X>
{
	...
	/**
	 * remove's the member from the underlying <CODE>Iterator</CODE>; <CODE>hasNext()</CODE> may not be called between calls to <CODE>next()</CODE> and
	 * <CODE>remove()</CODE>.
	 */
	@Override
