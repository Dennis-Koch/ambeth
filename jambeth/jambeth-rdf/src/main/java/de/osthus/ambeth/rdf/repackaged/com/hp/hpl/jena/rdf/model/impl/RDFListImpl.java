...
public class RDFListImpl extends ResourceImpl implements RDFList
{
	...
	public static Implementation factory = new Implementation()
	{
		...
		public boolean canWrap(Node node, EnhGraph eg)
		{
			...
			// if we are using a language profile, get the first, rest and next
			...
			// node will support being an RDFList facet if it has rdf:type
			// rdf:List, is nil, or is in the domain of a list property
			return node.equals(nil.asNode()) || g.contains(node, first.asNode(), Node.ANY) || g.contains(node, rest.asNode(), Node.ANY)
	 ...
	 * 
	 * @param n
	 *            The node that is the head of the list, currently
	 * @param g
	 ...
	 * list. If this list is the empty (nil) list, we cannot perform a side-effecting update without changing the URI of this node (from <code>rdf:nil</code) to
	 * a blank-node for the new list cell) without violating a Jena invariant. Therefore, this update operation will throw an exception if an attempt is made to
	 * add to the nil list. Safe ways to add to an empty list include {@link #with} and {@link #cons}.
	 * </p>
	 * 
	 * @param value
	 ...
	 * @return The index (from zero, the front of the list) of the first occurrence of <code>value</code> in the list not less than <code>start</code>, or
	...
	public RDFList append(RDFList list)
	{
		...
		{
			// could do this recursively, but for long lists it's better to
			// iterate
			// do the copy, then change the last tail pointer to point to the
			// arg
	...
	/**
	 * <p>
	 * Set a flag to indicate whether to strictly check the well-formedness of lists at each operation. Default false. Note that the flag that is manipulated is
	 * actually a static: it applies to all lists. However, RDFList is a Java interface, and Java does not permit static methods in interfaces.
	 * </p>
	 * 
	 * @param strict
	 ...
	 * 
	 * @param value
	 *            The value at the head of the new list cell
	 * @param tail
	...
	/**
	 * <p>
	 * Answer true if this is a valid list cell, which means either that it is nil, or it has the appropriate type and a first and next relation. Updated
	 * 17-06-2003: RDFCore last comments process has decided that the rdf:type of a list is implied by the domain constraints on rdf:first and rdf:rest, so no
	 * longer needs to be asserted directly. The test for rdf:type has therefore been removed.
	 * </p>
	 * 
	...
	protected void checkValid()
	{
		if (!equals(listNil()))
		{
			// note that the rdf:type of list cells is now implied by the RDF
			// M&S
	...
	private void checkValidProperty(Property p, RDFNode expected)
	{
		...
		if (count == 0)
		{
			...
			throw new InvalidListException("List node " + toString() + " is not valid: it should have property " + p.toString()
	 ...
	 * Note that this method assumes the pre-condition that <code>this</code> is not the empty list.
	 * </p>
	 * 
	 * @param last
	 *            If true, find the element whose tail is nil
	 * @param index
	 *            If <code>last</code> is false, find the index'th element from the head of the list
	 * @return The list cell
	 * @exception ListIndexException
	 *                if try to access an element beyond the end of the list
	 * @exception InvalidListException
	 *                if try to find the end of a badly formed list
	 */
	...
	protected RDFList copy(Iterator<? extends RDFNode> i)
	{
		...
		if (i.hasNext())
		{
			while (i.hasNext())
			{
				// create a list cell to hold the next value from the existing
				// list
	 ...
	 * 
	 * @param root
	 *            The resource representing the list cell we're setting the tail of
	 * @param tail
	 *            The new tail for this list, as a resource.
