 ...
 * 
 * This is intended to be used in places where a graph is required but iteration is the only expected operation. All graph operations are supported but many are
 * not efficient and will be slow on large collections. In these cases a memory based graph will be more efficient.
 * <p>
 * This implementation:
 * <ul>
 * <li>
 * Does not support deleting triples from the iterator</li>
 * <li>
 * Does not handle literal typing</li>
 * </ul>
 */
public class CollectionGraph extends GraphBase
{
	...
	{
		return equalNode(patternTriple.getSubject(), dataTriple.getSubject()) && equalNode(patternTriple.getPredicate(), dataTriple.getPredicate())
	...
	private static Node fixupNode(Node node)
	{
		...
		if (node.isLiteral())
		{
			...
			if (lang != null && !lang.equals(""))
				node = NodeFactory.createLiteral(node.getLiteralLexicalForm(), lang.toLowerCase(Locale.ROOT), node.getLiteralDatatype());
		}
		return node;
	}
	 ...
	 * @param triples
	 *            The collection of triples.
	 * @param iteratorDeleteAllowed
	 *            if true iterator on triple supports deletion and we want to enable iterator deletion.
	 */
	...
	protected ExtendedIterator<Triple> graphBaseFind(final TripleMatch m)
	{
		...
		if (iteratorDeleteAllowed)
		{
			...
			iter = SimpleEventManager.notifyingRemove(this, triples.iterator());
		}
		else
		{
