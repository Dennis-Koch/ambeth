...
/**
 * A rule preprocessor that scans the data looking for interesection definitions and augements the rule base by translations of the intersection statement.
 */
public class OWLRuleTranslationHook implements RulePreprocessHook
{
	...
	protected static void translateIntersectionList(Node node, Finder dataFind, List<Node> elements)
	{
		...
		/*
		 * - temp comment out during debugging if (dataFind.contains(new TriplePattern(description, RDF.type.asNode(), OWL.Restriction.asNode()))) { // Process
		 * a restriction element Node onprop = Util.getPropValue(description, OWL.onProperty.asNode(), dataFind); Node value; if ((value =
		 * Util.getPropValue(description, OWL.allValuesFrom.asNode(), dataFind)) != null) { elements.add(Functor.makeFunctorNode("all", new Node[] {onprop,
		 * value})); } else if ((value = Util.getPropValue(description, OWL.someValuesFrom.asNode(), dataFind)) != null) {
		 * elements.add(Functor.makeFunctorNode("some", new Node[] {onprop, value})); } else if ((value = Util.getPropValue(description, OWL.hasValue.asNode(),
		 * dataFind)) != null) { elements.add(Functor.makeFunctorNode("hasValue", new Node[] {onprop, value})); } else if ((value =
		 * Util.getPropValue(description, OWL.minCardinality.asNode(), dataFind)) != null) { elements.add(Functor.makeFunctorNode("min", new Node[] {onprop,
		 * value})); } else if ((value = Util.getPropValue(description, OWL.maxCardinality.asNode(), dataFind)) != null) {
		 * elements.add(Functor.makeFunctorNode("max", new Node[] {onprop, value})); } else if ((value = Util.getPropValue(description,
		 * OWL.cardinality.asNode(), dataFind)) != null) { elements.add(Functor.makeFunctorNode("max", new Node[] {onprop, value}));
		 * elements.add(Functor.makeFunctorNode("min", new Node[] {onprop, value})); } else { elements.add(description); } } else { // Assume its a class name
		 * elements.add(description); }
		 */
		// Above used to translated intersections into direct functor tests but
		// in fact the
		// references to the restriction bNode is sufficent and a better match
