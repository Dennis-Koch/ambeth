 ...
 * 
 * <p>
 * This program will read an RDF model, in a variety of languages, and copy it to the output stream in a possibly different language. Input can be read either
 * from a URL or from a file. The program writes its results to the standard output stream and sets its exit code to 0 if the program terminate normally, and to
...
public class rdfcopy extends java.lang.Object
{
	...
	protected static void read(Model model, String in, String lang) throws java.io.FileNotFoundException
	{
		try
		{
			URL url = new URL(in);
