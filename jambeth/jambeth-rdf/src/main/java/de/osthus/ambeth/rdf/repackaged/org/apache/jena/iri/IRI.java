...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.iri.impl.AbsIRIFactoryImpl;
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.iri.impl.Main;
 ...
 * <li>creating new IRI objects by resolving against this IRI as a base.
 * <li>
 * creating relative IRIs that when resolved against this IRI, would give a given absolute IRI.
 * </ul>
 * 
 * <p>
 * The standards, and other setttings, which are used in the methods are determined by the configuration of the {@link IRIFactory} used to create this IRI. To
 * check for conformance with a different standard, the IRI must be passed to {@link IRIFactoryI#create(IRI)} of a different appropriately configured
 * {@link IRIFactory}.
 * </p>
 */
abstract public class IRI extends AbsIRIFactoryImpl implements IRIFactoryI, IRIRelativize
{
	...
	/**
	 * Advanced usage only: all the violations found in analyzing this IRI. Some will be errors; others will be warnings; some will be violations of standards
	 * that are not being enforced, or are irrelevant (e.g. the http specification for an ftp URL); some will be simply internal workings of the IRI code.
	 * 
	 ...
	 * 
	 * @return The {@link Violation}s found which violate the factory's standards.
	 */
	...
	/**
	 * The authority component, found between the first "//" and the next "/". Any legal percent escape sequences are decoded. If the host name is an
	 * Internationalized Domain Name, then that is decoded too. This method may be more expensive than {@link #getRawAuthority()}.
	 * 
	 * @return The authority component, or null if none.
	 */
	...
	/**
	 * The fragment, found after a "#" at the end of the main URI (note a fragment may itself contain a "#"). Any legal percent escape sequences are decoded.
	...
	/**
	 * The host part of the authority. Found between an optional "@" and an optional ":" in the authority.
	 * 
	 ...
	 * The host part of the authority. Found between an optional "@" and an optional ":" in the authority.
	 * 
	 * Any legal percent escape sequences are decoded. Any legal punycode is decoded. This method may be more expensive than {@link #getRawHost()}.
	 * 
	 * @return The host, or null if none.
	 */
	 ...
	 *             An Internationalized Domain Name algorithm failed, there is no equivalent ascii string.
	 */
	abstract public String getASCIIHost() throws MalformedURLException;

	/**
	 * The path component of the IRI; always present, possibly the empty string. This includes any leading "/". Found after the authority, and before any "?" or
	 * "#". No decoding is done; this method is cheap.
	 * 
	 * @return The path.
	 */
	...
	/**
	 * The path component of the IRI; always present, possibly the empty string. This includes any leading "/". Found after the authority, and before any "?" or
	 * "#". Any legal percent escape sequences are decoded. This method may be more expensive than {@link #getRawPath()}.
	 * 
	 * @return The path.
	 */
	...
	/**
	 * Return code from {@link #getPort()}, indicating no port component found.
	 */
	final public static int NO_PORT = -1;
	/**
	 * Return code from {@link #getPort()}, indicating that a port component was found, but it is not a non-negative integer.
	 */
	...
	/**
	 * The query component of the IRI. Found after the "?" and before an optional "#". Any legal percent escape sequences are decoded. This method may be more
	 * expensive than {@link #getRawQuery()}.
	 * 
	 * @return The query component, or null if none.
	 */
	...
	/**
	 * The user information part of the authority component of the IRI. Found before the first "@" in the authority. May include a ":" separating the user name
	...
	/**
	 * The user information part of the authority component of the IRI. Found before the first "@" in the authority. May include a ":" separating the user name
	 * from a password, {@link ViolationCodes#HAS_PASSWORD}. Any legal percent escape sequences are decoded. This method may be more expensive than
	 * {@link #getRawUserinfo()}.
	 * 
	 * @return The user information, or null if none.
	 */
	...
	/**
	 * Are there any violations of the factory's specification settings. Quick check, equivalent to <code>violations(includeWarnings).hasNext()</code>, but
	 * faster.
	 * 
	 * @param includeWarnings
	...
	/**
	 * Are there any violations of the given specification. Quick check, equivalent to <code>exceptions(conformance).hasNext()</code>, but faster.
	 * 
	 * @param conformance
	...
	/**
	 * Is this an 'opaque' IRI. Is this IRI an absolute IRI with a path matching the path-rootless production: i.e. it has a scheme but no authority, and a path
	 * not starting in "/". Note: in the earlier RFC 2396 this concept was called being opaque.
	 * 
	...
	// boolean isXSanyURI();
	/**
	 * Returns an IRI that when resolved against this IRI would return <code>abs</code>. If possible, a relative IRI is formed, using any of the methods
	 * specified in flags, which is a bitwise or of values from {@link IRIRelativize}.
	 * <p>
	 * If <code>abs</code> contains a dot segment (either "/./" or "/../") then the contract cannot be satisfied and an incorrect answer is returned. This
	 * incorrect return value has an {@link ViolationCodes#NON_INITIAL_DOT_SEGMENT} violation associated with it.
	 * </p>
	 * 
	 * @param abs
	...
	/**
	 * Returns an IRI that when resolved against this IRI would return <code>abs</code>. If possible, a relative IRI is formed, using default methods.
	 * <p>
	 * If <code>abs</code> contains a dot segment (either "/./" or "/../") then the contract cannot be satisfied and an incorrect answer is returned. This
	 * incorrect return value has an {@link ViolationCodes#NON_INITIAL_DOT_SEGMENT} violation associated with it.
	 * </p>
	...
	/**
	 * Converts the IRI into ASCII. The hostname is converted into punycode; other components are converted using percent encoding. Even if the IRI is already
	 * ASCII, the hostname may be modified, if, for example, it (inappropriately) uses percent encoding. This may be impossible (even in the ASCII case), in
	...
	/**
	 * The logical IRI string as originally specified, use {@link #toDisplayString()} for display purposes such as error messages.
	 * 
	 ...
	 * @throws MalformedURLException
	 *             If IDNA conversion failed, or from java.net.URL
	 */
	abstract public URL toURL() throws MalformedURLException;

	/**
	 * Resolves an IRI against this one. This method is an alias for {@link IRIFactory#create(IRI)}.
	 * 
	...
	/**
	 * Resolves an IRI against this one. This method is an alias for {@link IRIFactory#create(String)}.
	 * 
	 ...
	 * </pre>
	 * 
	 * If no file or iris are specified on the command line, then standard input is used. In fact more than one spec can be used, in which case violations of
