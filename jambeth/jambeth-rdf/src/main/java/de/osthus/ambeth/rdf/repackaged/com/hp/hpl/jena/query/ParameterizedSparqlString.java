...
import de.osthus.ambeth.rdf.repackaged.org.apache.jena.atlas.lib.Pair;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.model.Literal;
...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.ARQException;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.serializer.SerializationContext;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.FmtUtils;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.sparql.util.NodeFactoryExtra;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.update.UpdateFactory;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.update.UpdateRequest;
 ...
 * <p>
 * Any variable in the command may have a value injected to it, injecting a value replaces all usages of that variable in the command i.e. substitutes the
 * variable for a constant, injection is done by textual substitution.
 * </p> <h4>Positional Parameters</h4>
 * <p>
 * You can use JDBC style positional parameters if you prefer, a JDBC style parameter is a single {@code ?} followed by whitespace or certain punctuation
 * characters (currently {@code ; , .}). Positional parameters have a unique index which reflects the order in which they appear in the string. Positional
 * parameters use a zero based index.
 * </p>
 * <h4>Buffer Usage</h3> </p> Additionally you may use this purely as a {@link StringBuffer} replacement for creating queries since it provides a large variety
 * of convenience methods for appending things either as-is or as nodes (which causes appropriate formatting to be applied). </p>
 ...
 * <li>Where you do not want to simply say some variable should have a certain value but rather wish to insert constants into the query/update in place of
 * variables</li>
 * <li>Defending against SPARQL injection when creating a query/update using some external input, see SPARQL Injection notes for limitations.</li>
 * <li>Provide a more convenient way to prepend common prefixes to your query</li>
 * </ul>
 * <p>
 * This class is useful for preparing both queries and updates hence the generic name as it provides programmatic ways to replace variables in the query with
 * constants and to add prefix and base declarations. A {@link Query} or {@link UpdateRequest} can be created using the {@link #asQuery()} and
 * {@link #asUpdate()} methods assuming the command an instance represents is actually valid as a query/update.
 * </p>
 * <h3>Warnings</h3>
 * <ol>
 * <li>Note that this class does not in any way check that your command is syntactically correct until such time as you try and parse it as a {@link Query} or
 * {@link UpdateRequest}.</li>
 * <li>Also note that injection is done purely based on textual replacement, it does not understand or respect variable scope in any way. For example if your
 * command text contains sub queries you should ensure that variables within the sub query which you don't want replaced have distinct names from those in the
 * outer query you do want replaced (or vice versa)</li>
 * </ol>
 * <h3>SPARQL Injection Notes</h3>
 * <p>
 * While this class was in part designed to prevent SPARQL injection it is by no means foolproof because it works purely at the textual level. The current
 * version of the code addresses some possible attack vectors that the developers have identified but we do not claim to be sufficiently devious to have thought
 * of and prevented every possible attack vector.
 * </p>
 * <p>
 * Therefore we <strong>strongly</strong> recommend that users concerned about SPARQL Injection attacks perform their own validation on provided parameters and
 * test their use of this class themselves prior to its use in any security conscious deployment. We also recommend that users do not use easily guess-able
 * variable names for their parameters as these can allow a chained injection attack though generally speaking the code should prevent these.
 * </p>
 */
public class ParameterizedSparqlString implements PrefixMapping
{
	...
	/**
	 * Sets the command text, overwriting any existing command text. If you want to append to the command text use one of the {@link #append(String)},
	 * {@link #appendIri(String)}, {@link #appendLiteral(String)} or {@link #appendNode(Node)} methods instead
	 * 
	 * @param command
	 *            Command Text
	 */
	...
	/**
	 * Appends a float as-is to the existing command text, to ensure correct formatting when used as a constant consider using the {@link #appendLiteral(float)}
	 * method
	 * 
	 * @param f
	 *            Float to append
	...
	/**
	 * Appends a long as-is to the existing command text, to ensure correct formatting when used as a constant consider using the {@link #appendLiteral(long)}
	 * method
	 * 
	 * @param l
	 *            Long to append
	 ...
	 * <p>
	 * Setting a parameter to null is equivalent to calling {@link #clearParam(int)} for the given variable
	 * </p>
	 * 
	 * @param index
	 *            Positional Index
	 ...
	 * <p>
	 * Setting a parameter to null is equivalent to calling {@link #clearParam(int)} for the given index
	 * </p>
	 * 
	 * @param index
	 *            Positional Index
	 ...
	 *            Value to inject
	 * @throws ARQException
	 *             Thrown if not safe to inject, error message will describe why it is unsafe to inject
	 */
	...
	/**
	 * Helper method which checks whether it is safe to inject to a positional parameter the given value
	 * 
	 * @param command
	 *            Current command string
	 * @param index
	 ...
	 * @param position
	 *            Position within the command string at which the positional parameter occurs
	 * @param n
	 *            Value to inject
	 * @throws ARQException
	 *             Thrown if not safe to inject, error message will describe why it is unsafe to inject
	...
	protected void validateSafeToInject(String command, int index, int position, Node n) throws ARQException
	{
		...
		if (n.isLiteral())
		{
			...
			{
				throw new ARQException("Command string is vunerable to injection attack, a positional paramter (index " + index
						+ ") appears inside of a literal and is bound to a literal which provides a SPARQL injection attack vector");
			}
		}
	}
	...
	/**
	 * Helper method which does light parsing on the command string to find the position of all relevant delimiters
	 * 
	 * @param command
	 *            Command String
	...
	/**
	 * <p>
	 * This method is where the actual work happens, the original command text is always preserved and we just generated a temporary command string by
	 * prepending the defined Base URI and namespace prefixes at the start of the command and injecting the set parameters into a copy of that base command
	 * string and return the resulting command.
	 * </p>
	 * <p>
	 * This class makes no guarantees about the validity of the returned string for use as a SPARQL Query or Update, for example if a variable parameter was
	 * injected which was mentioned in the SELECT variables list you'd have a syntax error when you try to parse the query. If you run into issues like this try
	 * using a mixture of variable and positional parameters.
	 * </p>
	 * 
	 * @throws ARQException
	 *             May be thrown if the code detects a SPARQL Injection vulnerability because of the interaction of the command string and the injected
	 *             variables
	 */
	@Override
	...
	{
		ParameterizedSparqlString copy = new ParameterizedSparqlString(this.cmd.toString(), null, (copyBase ? this.baseUri : null),
				(copyPrefixes ? this.prefixes : null));
		if (copyParams)
		{
