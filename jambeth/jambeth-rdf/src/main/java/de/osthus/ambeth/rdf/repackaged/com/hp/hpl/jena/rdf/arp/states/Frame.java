...
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.ElementLexer;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.Names;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.ParserSupport;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.QNameLexer;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.Taint;
import de.osthus.ambeth.rdf.repackaged.com.hp.hpl.jena.rdf.arp.impl.TaintImpl;
...
public abstract class Frame extends ParserSupport implements Names, FrameI, ARPErrorNumbers
{
	...
	public void processingInstruction(String target, String data) throws SAXParseException
	{
		...
		warning(null, WARN_PROCESSING_INSTRUCTION_IN_RDF, "A processing instruction is in RDF content. No processing was done. " + suggestParsetypeLiteral());
	}
	...
	void processPropertyAttributes(AttributeLexer ap, Attributes atts, AbsXMLContext x) throws SAXParseException
	{
		if (ap.type != null)
		{
			((HasSubjectFrameI) this).aPredAndObj(RDF_TYPE, URIReference.resolve(this, x, ap.type));
		}
		...
		if (ap.count != sz)
		{
			for (int i = 0; i < sz; i++)
			{
				if (!ap.done(i))
				{
					...
					{
						warning(pred, WARN_UNQUALIFIED_ATTRIBUTE, "Unqualified property attributes are not allowed. Property treated as a relative URI.");
					}
					if (rdfns.equals(uri) && !QNameLexer.isKnownRDFProperty(lName))
					{
						warning(pred, WARN_UNKNOWN_RDF_ATTRIBUTE, atts.getQName(i) + " is not a recognized RDF property or type.");
					}
					((HasSubjectFrameI) this).aPredAndObj(pred, new ARPString(this, atts.getValue(i), x));
				}
			}
		}
	}
	...
	{
		ElementLexer el = new ElementLexer(taint, this, uri, localName, rawName, E_RDF, 0, false);
		if (el.goodMatch)
		{
			AttributeLexer ap = new AttributeLexer(this, A_XMLBASE | A_XMLLANG | A_XML_OTHER, 0);
	...
	/**
	 * Additional message if mixed content is found in a syntactically disallowed place. Subclasses override to suppress message.
	 * 
	 */
	...
	public void characters(char[] ch, int start, int length) throws SAXParseException
	{
		if ((!nonWhiteMsgGiven) && !isWhite(ch, start, length))
		{
			...
			warning(ERR_NOT_WHITESPACE, "Expecting XML start or end element(s). String data \"" + new String(ch, start, length) + "\" not allowed."
					+ suggestParsetypeLiteral() + " Maybe a striping error.");
		}
	}
