...
/**
 * A generator represents a set of memoized results for a single tabled subgoal. The generator may be complete (in which case it just contains the complete
 * cached set of results for a goal), ready (not complete but likely to product more results if called) or blocked (not complete and awaiting results from a
 * dependent generator).
 * <p>
...
public class Generator implements LPAgendaEntry, LPInterpreterContext
{
	...
	// /**
	// * Return the estimated number of generators between the top level goal
	// and this one.
	// */
	// public int getDepth() {
	// return depth;
	...
	public void removeConsumer(ConsumerChoicePointFrame ccp)
	{
		...
		// However, a generator might be part of one query, incompletely
		// consumed
	...
	/**
	 * Check whether this generator is live (indirectly dependent on a ready generator), dead (complete) or in a deadlock loop which might or might not be live
	 * (unknown).
	 */
	...
	/**
	 * Scan the result of a (set of) completion check(s) to detect which of the unknowns are actually live and set the remaining (deadlocked) states to
	 * complete.
	 */
