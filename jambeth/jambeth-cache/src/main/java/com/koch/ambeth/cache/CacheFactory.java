...
public class CacheFactory implements ICacheFactory {
    ...
    @Override
    public IDisposableCache withParent(ICache parent, CheckedSupplier<IDisposableCache> runnable) {
        var oldParent = parentTL.get();
        parentTL.set(parent);
        try {
    ...
    protected IDisposableCache createIntern(CacheFactoryDirective cacheFactoryDirective, boolean privileged, boolean foreignThreadAware, Boolean useWeakEntries, String name) {
        var firstLevelCacheBC = beanContext.registerBean(ChildCache.class);
        ...
        var firstLevelCache = firstLevelCacheBC.finish();
