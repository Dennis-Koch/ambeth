...
public abstract class AbstractCache<V> implements ICache, IInitializingBean, IDisposable {
	...
	protected V existsValue(IObjRef ori) {
		...
		try {
			...
			Object requestedVersion =
					conversionHelper.convertValueToType(versionMember.getElementType(), ori.getVersion());

			if (requestedVersion == null || cacheVersion == null
					|| ((Comparable) cacheVersion).compareTo(requestedVersion) >= 0) {
				if (weakEntries) {
					addHardRefTL(cacheValue);
				}
	...
	protected void extractAlternateCacheKeys(IEntityMetaData metaData, Object obj,
		...
		for (int idIndex = metaData.getAlternateIdCount(); idIndex-- > 0;) {
			...
			if (obj instanceof Object[]) {
				...
				alternateId =
						compositeIdFactory.createIdFromPrimitives(metaData, idIndex, (AbstractCacheValue) obj);
			}
	...
	protected abstract CacheKey[] getAlternateCacheKeysFromCacheValue(IEntityMetaData metaData,
			V cacheValue);
	...
	protected abstract void setVersionOfCacheValue(IEntityMetaData metaData, V cacheValue,
			Object version);

	protected void setRelationsOfCacheValue(IEntityMetaData metaData, V cacheValue,
	...
	protected void removeCacheValueFromCacheSingle(IEntityMetaData metaData, byte idIndex,
			Object id) {
		Member idMember = metaData.getIdMemberByIdIndex(idIndex);
		id = conversionHelper.convertValueToType(idMember.getRealType(), id);
	...
	protected void removeAlternateCacheKeysFromCache(IEntityMetaData metaData,
			CacheKey[] alternateCacheKeys) {
	...
	protected abstract void putInternObjRelation(V cacheValue, IEntityMetaData metaData,
			IObjRelation objRelation, IObjRef[] relationsOfMember);

	protected void putIntern(Object objectToCache, ArrayList<Object> hardRefsToCacheValue,
			IdentityHashSet<Object> alreadyHandledSet, HashSet<IObjRef> cascadeNeededORIs) {
		...
		if (objectToCache.getClass().isArray()) {
			...
			for (int a = length; a-- > 0;) {
				putIntern(Array.get(objectToCache, a), hardRefsToCacheValue, alreadyHandledSet,
						cascadeNeededORIs);
			}
			return;
		}
		...
		if (key != null) {
			...
			if (!objectItselfIsUpToDate) {
				...
				Object hardRef = putIntern(metaData, objectToCache, key, version, alternateCacheKeys,
	...
	protected V putIntern(IEntityMetaData metaData, Object obj, Object id, Object version,
			CacheKey[] alternateCacheKeys, Object[] primitives, IObjRef[][] relations) {
	...
	protected void putAlternateCacheKeysToCache(IEntityMetaData metaData,
			CacheKey[] alternateCacheKeys, Object cacheValueR) {
		for (int a = alternateCacheKeys.length; a-- > 0;) {
			CacheKey alternateCacheKey = alternateCacheKeys[a];
			if (alternateCacheKey != null) {
				keyToCacheValueDict.put(alternateCacheKey.getEntityType(), alternateCacheKey.getIdIndex(),
						alternateCacheKey.getId(), cacheValueR);
			}
		}
	}
	...
	protected V getCacheValue(IEntityMetaData metaData, IObjRef objRef, boolean checkVersion) {
		...
		if (checkVersion && versionMember != null && objRef.getVersion() != null) {
			...
			// Compare operation only works on identical operand types
			Object requestedVersion =
					conversionHelper.convertValueToType(versionMember.getElementType(), objRef.getVersion());
	...
	public <E> E getObject(Class<E> type, Object... compositeIdParts) {
		IEntityMetaData metaData = entityMetaDataProvider.getMetaData(type);
		Object id =
				compositeIdFactory.createCompositeId(metaData, metaData.getIdMember(), compositeIdParts);
	...
	public <E> E getObject(Class<E> type, String idName, Object id,
		...
		IEntityMetaData metaData = entityMetaDataProvider.getMetaData(type);
		ObjRef objRef =
				new ObjRef(metaData.getEntityType(), metaData.getIdIndexByMemberName(idName), id, null);
		return (E) getObject(objRef, cacheDirective);
	}
	...
	@Override
	public abstract IList<Object> getObjects(List<IObjRef> orisToGet,
			Set<CacheDirective> cacheDirective);

	@Override
	public abstract IList<IObjRelationResult> getObjRelations(List<IObjRelation> objRels,
			Set<CacheDirective> cacheDirective);
