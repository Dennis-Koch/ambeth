...
public class CacheContextPostProcessor extends AbstractCascadePostProcessor {
    ...
    protected ICascadedInterceptor handleServiceIntern(IBeanContextFactory beanContextFactory, IServiceContext beanContext, IBeanConfiguration beanConfiguration, Class<?> type,
        ...
        var cacheContext = annotationCache.getAnnotation(type);
        if (cacheContext == null) {
            return null;
        }
        var cacheBehavior = cachePostProcessor.createInterceptorModeBehavior(type);
        ...
        if (beanContext.isRunning()) {
            interceptor = beanContext.registerWithLifecycle(interceptor)//
                                     ...
                                     .ignoreProperties(MergeInterceptor.PROCESS_SERVICE_PROP, MergeInterceptor.SERVICE_NAME_PROP)//
                                     .finish();
        } else {
        ...
        var cacheType = cacheContext.value();
        ...
        var ccInterceptor = new CacheContextInterceptor();
        if (beanContext.isRunning()) {
