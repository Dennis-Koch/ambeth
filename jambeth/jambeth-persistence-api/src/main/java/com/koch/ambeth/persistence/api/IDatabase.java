...
public interface IDatabase extends IDisposable, IDatabaseDisposeHookExtendable {
	...
	/**
	 * Allows to retrieve beans which are 1:1 related to the underlying database artifact. One example
	 * may be a java.sql.Connection bean for JDBC related database handles
	 *
	 * @param <T> May be anything
	 * @param autowiredType The type where a bean is supposed to be autowired to
	 ...
	 *
	 * @param <T> May be anything
	 * @param beanName The name where a bean is supposed to be uniquely bound to
	 * @param expectedType The expected type to skip later casts
	 * @return null, if no bean has been found
	 * @return Instance of T if bean has been found
	 * @throws ClassCastException if bean has been found but is not assignable to type <T>
	 */
	<T> T getNamedBeanInContext(String beanName, Class<T> expectedType);

	/**
	 * Getter for the random Ambeth database session ID. It identifies the instance in the
	 * DefaultDatabasePool.
	 *
	 * @return Session ID.
	 */
	...
	/**
	 * Setter for the random Ambeth database session ID. It identifies the instance in the
	 * DefaultDatabasePool.
	 *
	 * @param sessionId
	 */
	...
	/**
	 * Getter for the thread local current instance of IDatabase. (This differs from "this" in cases
	 * where the IDatabase handle is only a proxy to a thread local instance. Do not call getCurrent()
	 * and pass that result to foreign code if you do not know what you do.
	 *
	 ...
	 *
	 * @param savepoint savepoint that defines the changes to revert.
	 */
