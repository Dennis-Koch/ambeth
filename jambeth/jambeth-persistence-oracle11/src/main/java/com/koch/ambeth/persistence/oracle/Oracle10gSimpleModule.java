...
public class Oracle10gSimpleModule implements IInitializingModule {
	...
	public void afterPropertiesSet(IBeanContextFactory beanContextFactory) throws Throwable {
		...
		beanContextFactory.registerBean(Oracle10gExtendedDialect.class)
				.autowireable(IExtendedConnectionDialect.class);
		if (!externalTransactionManager && !databaseBehaviourStrict) {
			beanContextFactory.registerBean(Oracle10gThinDialect.class)
					.autowireable(IConnectionDialect.class);
		}
		else {
			beanContextFactory.registerBean(Oracle10gDialect.class)
					.autowireable(IConnectionDialect.class);
		...
		beanContextFactory.registerBean(Oracle10gSequencePrimaryKeyProvider.class)
				.autowireable(IPrimaryKeyProvider.class);

		IBeanConfiguration timestampConverter =
				beanContextFactory.registerBean(OracleTimestampConverter.class);
		beanContextFactory.link(timestampConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.TIMESTAMP.class, Long.class);
		beanContextFactory.link(timestampConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.TIMESTAMP.class, Long.TYPE);
		beanContextFactory.link(timestampConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.TIMESTAMP.class, Date.class);
		beanContextFactory.link(timestampConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.TIMESTAMP.class, Calendar.class);

		IBeanConfiguration arrayConverter = beanContextFactory.registerBean(OracleArrayConverter.class);
		beanContextFactory.link(arrayConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.ARRAY.class, Collection.class);
		beanContextFactory.link(arrayConverter).to(IDedicatedConverterExtendable.class)
				.with(oracle.sql.ARRAY.class, String.class);
	}
}
