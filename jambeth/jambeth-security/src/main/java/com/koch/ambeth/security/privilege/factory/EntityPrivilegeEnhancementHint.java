...
public class EntityPrivilegeEnhancementHint
	...
	public EntityPrivilegeEnhancementHint(Class<?> entityType, boolean create, boolean read,
	...
	public int hashCode() {
		return getClass().hashCode() ^ getEntityType().hashCode()
				^ AbstractPrivilege.calcIndex(create, read, update, delete, execute);
	}
	...
	public String getTargetName(Class<?> typeToEnhance) {
		return Type.getInternalName(entityType) + "$" + AbstractPrivilege.class.getSimpleName() + "_"
				+ AbstractPrivilege.upperOrLower(create, 'c') + AbstractPrivilege.upperOrLower(read, 'r')
				+ AbstractPrivilege.upperOrLower(update, 'u') + AbstractPrivilege.upperOrLower(delete, 'd')
