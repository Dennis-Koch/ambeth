...
public class PrivilegeProvider implements IPrivilegeProviderIntern, IInitializingBean, IDataChangeListener {
    ...
    public IPrivilege getPrivilege(Object entity, ISecurityScope[] securityScopes) {
        var objRefs = objRefHelper.extractObjRefList(entity, null);
        var result = getPrivileges(objRefs, securityScopes);
    ...
    public IPrivilege getPrivilegeByObjRef(IObjRef objRef, ISecurityScope[] securityScopes) {
        var result = getPrivilegesByObjRef(new ArrayList<>(new IObjRef[] { objRef }), securityScopes);
        return result.getPrivileges()[0];
    }
    ...
    public IPrivilegeResult getPrivilegesByObjRef(List<? extends IObjRef> objRefs, ISecurityScope[] securityScopes) {
        ...
        var authorization = context != null ? context.getAuthorization() : null;
        ...
        var missingObjRefs = new ArrayList<IObjRef>();
        var writeLock = this.writeLock;
        writeLock.lock();
        try {
            var result = createResult(objRefs, securityScopes, missingObjRefs, authorization, null);
        ...
        try {
            ...
            for (int a = 0, size = privilegeResults.size(); a < size; a++) {
                var privilegeResult = privilegeResults.get(a);
                var reference = privilegeResult.getReference();

                var privilegeKey = new PrivilegeKey(reference.getRealType(), reference.getIdNameIndex(), reference.getId(), userSID);
                var useCache = true;
                ...
                var privilege = createPrivilegeFromServiceResult(reference, privilegeResult);
                if (useCache) {
    ...
    protected IPrivilege createPrivilegeFromServiceResult(IObjRef objRef, IPrivilegeOfService privilegeOfService) {
        var propertyPrivilegesOfService = privilegeOfService.getPropertyPrivileges();
        ...
        var propertyPrivilegeNames = privilegeOfService.getPropertyPrivilegeNames();
        var metaData = entityMetaDataProvider.getMetaData(objRef.getRealType());
        var primitivePropertyPrivileges = new IPropertyPrivilege[metaData.getPrimitiveMembers().length];
        var relationPropertyPrivileges = new IPropertyPrivilege[metaData.getRelationMembers().length];
        var defaultPropertyPrivilege = PropertyPrivilegeImpl.createFrom(privilegeOfService);
        ...
        for (int b = propertyPrivilegesOfService.length; b-- > 0; ) {
            var propertyPrivilegeOfService = propertyPrivilegesOfService[b];
            var propertyName = interningFeature.intern(propertyPrivilegeNames[b]);
            var propertyPrivilege = PropertyPrivilegeImpl.create(propertyPrivilegeOfService.isCreateAllowed(), propertyPrivilegeOfService.isReadAllowed(), propertyPrivilegeOfService.isUpdateAllowed(),
        ...
        return entityPrivilegeFactoryProvider.getEntityPrivilegeFactory(metaData.getEntityType(), privilegeOfService.isCreateAllowed(), privilegeOfService.isReadAllowed(),
                                                     privilegeOfService.isUpdateAllowed(), privilegeOfService.isDeleteAllowed(), privilegeOfService.isExecuteAllowed())
                                             .createPrivilege(privilegeOfService.isCreateAllowed(), privilegeOfService.isReadAllowed(), privilegeOfService.isUpdateAllowed(),
                                                     privilegeOfService.isDeleteAllowed(), privilegeOfService.isExecuteAllowed(), primitivePropertyPrivileges, relationPropertyPrivileges);
    }
    ...
    protected ITypePrivilege createTypePrivilegeFromServiceResult(Class<?> entityType, ITypePrivilegeOfService privilegeOfService) {
        ...
        var defaultPropertyPrivilege = TypePropertyPrivilegeImpl.createFrom(privilegeOfService);
        if (propertyPrivilegesOfService == null || propertyPrivilegesOfService.length == 0) {
            return new SimpleTypePrivilegeImpl(privilegeOfService.isCreateAllowed(), privilegeOfService.isReadAllowed(), privilegeOfService.isUpdateAllowed(), privilegeOfService.isDeleteAllowed(),
                    privilegeOfService.isExecuteAllowed(), defaultPropertyPrivilege);
        }
        var propertyPrivilegeNames = privilegeOfService.getPropertyPrivilegeNames();
        var metaData = entityMetaDataProvider.getMetaData(entityType);
        var primitivePropertyPrivileges = new ITypePropertyPrivilege[metaData.getPrimitiveMembers().length];
        var relationPropertyPrivileges = new ITypePropertyPrivilege[metaData.getRelationMembers().length];
        Arrays.fill(primitivePropertyPrivileges, defaultPropertyPrivilege);
        Arrays.fill(relationPropertyPrivileges, defaultPropertyPrivilege);
        for (int b = propertyPrivilegesOfService.length; b-- > 0; ) {
            var propertyPrivilegeOfService = propertyPrivilegesOfService[b];
            ...
            if (propertyPrivilegeOfService != null) {
                propertyPrivilege =
                        TypePropertyPrivilegeImpl.create(propertyPrivilegeOfService.isCreateAllowed(), propertyPrivilegeOfService.isReadAllowed(), propertyPrivilegeOfService.isUpdateAllowed(),
                                propertyPrivilegeOfService.isDeleteAllowed());
            } else {
        ...
        return entityTypePrivilegeFactoryProvider.getEntityTypePrivilegeFactory(metaData.getEntityType(), privilegeOfService.isCreateAllowed(), privilegeOfService.isReadAllowed(),
                                                         privilegeOfService.isUpdateAllowed(), privilegeOfService.isDeleteAllowed(), privilegeOfService.isExecuteAllowed())
                                                 .createPrivilege(privilegeOfService.isCreateAllowed(), privilegeOfService.isReadAllowed(), privilegeOfService.isUpdateAllowed(),
                                                         privilegeOfService.isDeleteAllowed(), privilegeOfService.isExecuteAllowed(), primitivePropertyPrivileges, relationPropertyPrivileges);
    }
    ...
    public ITypePrivilege getPrivilegeByType(Class<?> entityType, ISecurityScope[] securityScopes) {
        var result = getPrivilegesByType(new ArrayList<Class<?>>(new Class<?>[] { entityType }), securityScopes);
    ...
    public ITypePrivilegeResult getPrivilegesByType(List<Class<?>> entityTypes, ISecurityScope[] securityScopes) {
        ...
        var missingEntityTypes = new ArrayList<Class<?>>();
        var writeLock = this.writeLock;
        writeLock.lock();
        try {
            var result = createResultByType(entityTypes, securityScopes, missingEntityTypes, authorization);
        ...
        try {
            for (int a = 0, size = privilegeResults.size(); a < size; a++) {
                var privilegeResult = privilegeResults.get(a);
                var entityType = privilegeResult.getEntityType();

                var securityScope = interningFeature.intern(privilegeResult.getSecurityScope().getName());

                var pi = createTypePrivilegeFromServiceResult(entityType, privilegeResult);
    ...
    protected IPrivilegeResult createResult(List<? extends IObjRef> objRefs, ISecurityScope[] securityScopes, List<IObjRef> missingObjRefs, IAuthorization authorization,
            IMap<PrivilegeKey, IPrivilege> privilegeResultOfNewEntities) {
        PrivilegeKey privilegeKey = null;

        var result = new IPrivilege[objRefs.size()];
        var userSID = authorization != null ? authorization.getSID() : null;

        for (int index = objRefs.size(); index-- > 0; ) {
            var objRef = objRefs.get(index);
            if (objRef == null) {
                continue;
            }
            ...
            for (int a = securityScopes.length; a-- > 0; ) {
                ...
                var existingPrivilegeItem = useCache ? privilegeCache.get(privilegeKey) : privilegeResultOfNewEntities != null ? privilegeResultOfNewEntities.get(privilegeKey) : null;
    ...
    protected ITypePrivilegeResult createResultByType(List<Class<?>> entityTypes, ISecurityScope[] securityScopes, List<Class<?>> missingEntityTypes, IAuthorization authorization) {
        var result = new ITypePrivilege[entityTypes.size()];
        var userSID = authorization != null ? authorization.getSID() : null;

        for (int index = entityTypes.size(); index-- > 0; ) {
            var entityType = entityTypes.get(index);
            if (entityType == null) {
                continue;
            }
            ITypePrivilege mergedTypePrivilege = null;
            for (int a = securityScopes.length; a-- > 0; ) {
                var existingTypePrivilege = entityTypePrivilegeCache.get(entityType, securityScopes[a].getName(), userSID);
    ...
    public static class PrivilegeKey {
        ...
        @Override
        public int hashCode() {
            return getClass().hashCode() ^ entityType.hashCode() ^ id.hashCode() ^ hashCode(userSID) ^ hashCode(securityScope);
        }
        ...
        public boolean equals(Object obj) {
            ...
            var other = (PrivilegeKey) obj;
            return Objects.equals(id, other.id) && Objects.equals(entityType, other.entityType) && idIndex == other.idIndex && Objects.equals(userSID, other.userSID) &&
