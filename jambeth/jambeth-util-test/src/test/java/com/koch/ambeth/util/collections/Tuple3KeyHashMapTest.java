...
public class Tuple3KeyHashMapTest {
	...
	protected Tuple3KeyHashMap<Integer, Long, Double, Object> fillForTest() {
		Tuple3KeyHashMap<Integer, Long, Double, Object> map =
	...
	public void init() {
		Tuple3KeyHashMap<Integer, Long, Double, Object> map =
				new Tuple3KeyHashMap<>(100, 0.75f);
		map.init();
	}
	...
	public void containsKey() {
		...
		Assert.assertFalse(
				map.containsKey(nonExistingKeys1[0], nonExistingKeys2[0], nonExistingKeys3[0]));
	}
	...
	public void get() {
		...
		for (int a = existingKeys1.length; a-- > 0;) {
			Assert.assertEquals(existingValues[a],
					map.get(existingKeys1[a], existingKeys2[a], existingKeys3[a]));
		}
	}
	...
	public void putIfNotExists() {
		...
		for (int a = nonExistingKeys1.length; a-- > 0;) {
			Assert.assertTrue(
					map.putIfNotExists(nonExistingKeys1[a], nonExistingKeys2[a], nonExistingKeys3[a], 1));
		}
		Assert.assertFalse(
				map.putIfNotExists(nonExistingKeys1[0], nonExistingKeys2[0], nonExistingKeys3[0], 2));
	...
	public void remove() {
		...
		for (int a = existingKeys1.length; a-- > 0;) {
			Assert.assertEquals(existingValues[a],
					map.remove(existingKeys1[a], existingKeys2[a], existingKeys3[a]));
		}
	...
	public void serialize() throws Throwable {
		...
		}
		{
			...
			Tuple3KeyHashMap<Integer, Long, Double, Object> cloneMap =
					(Tuple3KeyHashMap<Integer, Long, Double, Object>) is.readObject();
