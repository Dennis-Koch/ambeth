...
public class AnnotationUpdater extends AbstractLatexScanner implements IStartingBean {
	...
	protected void writeToAnnotationTexFile(AnnotationEntry annotationEntry, File targetFile)
			throws Exception {
	...
	protected StringBuilder listingsString(AnnotationEntry annotationEntry) {
		...
		if (annotationEntry.javaFile != null) {
			sb.append("\\inputjava{Annotation definition \\type{").append(annotationEntry.simpleName)
					.append("}}\n");
			sb.append("{").append(annotationEntry.javaFile).append("}\n");
		}
		if (annotationEntry.csharpFile != null) {
			sb.append("\\inputcsharp{Annotation definition \\type{").append(annotationEntry.simpleName)
					.append("}}\n");
	...
	protected boolean writeTableRow(AnnotationEntry annotationEntry, OutputStreamWriter fw)
			throws Exception {
		fw.append("\t\t");
	...
	protected void handleModel() throws Throwable {
		...
		if (!targetTexDirCP.startsWith(targetTexFileCP)) {
			throw new IllegalStateException(
					"Path '" + targetTexDirCP + "' must reside within '" + targetTexFileCP + "'");
		}
		...
		try {
			...
			fw.append("% Any changes have to be done to the java class "
					+ AnnotationUpdater.class.getName() + "\n");
			fw.append("%---------------------------------------------------------------\n");
