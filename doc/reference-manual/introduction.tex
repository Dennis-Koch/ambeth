...
just using a single module will not work in most cases. The dependencies between those modules are managed with \prettyref{stack:maven}. To support faster setting up your environment the
...
To use any feature of \AMBETH you need create a ``context'' in which the different modules (see \prettyref{ambeth:modules}) are available. A context can be created as described in \prettyref{module:IoC}. Alternatively the \type{de.osthus.ambeth.Ambeth} class can be used. 
