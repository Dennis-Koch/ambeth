...
\AvailableInJavaAndCsharp{Core functionality and intensively used by \AMBETH{} itself to provide a flexible, extensible but also high performance implementation to convert objects to other types. In the common scenario the \type{IConversionHelper}-bean expects two arguments to be able to work - the expected result type and the value (object instance) to be converted:}
