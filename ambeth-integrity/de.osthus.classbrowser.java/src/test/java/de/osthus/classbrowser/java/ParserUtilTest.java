...
public class ParserUtilTest
{
	...
	{
		List<ClassHolder> classes = Collections.singletonList(new ClassHolder("C:\\dev\\lib\\jambeth-ioc-2.2.321.jar", TestClass.class));
		Map<String, String> moduleMap = Collections.emptyMap();
		List<String> modulesToBeAnalyzed = Collections.emptyList();
		List<TypeDescription> typeDescriptions = ParserUtil.analyzeClasses(classes, moduleMap, modulesToBeAnalyzed);

		Assert.assertEquals(1, typeDescriptions.size());
		TypeDescription typeDescription = typeDescriptions.get(0);

		Assert.assertEquals("de.osthus.classbrowser.java.AbstractTestClass", typeDescription.getSuperType());
		List<String> interfaces = typeDescription.getInterfaces();
		Assert.assertEquals(2, interfaces.size());
		Assert.assertTrue(interfaces.contains("de.osthus.classbrowser.java.TestInterface1"));
		Assert.assertTrue(interfaces.contains("de.osthus.classbrowser.java.TestInterface2"));
	}
