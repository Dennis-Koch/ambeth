...
import de.osthus.classbrowser.java.ExitCode;
...
public class CompareApplication {
	...
	private static final List<CompareStatus> INFO_STATES = Arrays.asList(CompareStatus.NOT_COMPARED, CompareStatus.NO_MATCHING_CSHARP_CLASS_FOUND,
			CompareStatus.NO_MATCHING_JAVA_CLASS_FOUND, CompareStatus.MODULENAME_CASE, CompareStatus.INTERNAL_METHOD_NOT_FOUND);

	private static final List<CompareStatus> WARNING_STATES = Arrays.asList(CompareStatus.NO_MODULENAME_FOUND, CompareStatus.MODULENAME_DIFFERS,
			CompareStatus.PATTERN_VIOLATION, CompareStatus.METHOD_NAME_CASE, CompareStatus.PUBLIC_METHOD_NOT_FOUND);
	...
	private static void displayHelpAndWait() {
		List<String> messages = Arrays.asList( //
				...
				ARG_KEY_TARGETPATH + "={path} sets the path where the compare result file should be written to\n", //
				ARG_KEY_MISMATCHESONLY + "=true if only mismatches should be reported\n", //
				ARG_KEY_RESULTTYPE + "=" + EXPORT_TYPE_TEST + " (export as JUnit test results), =" + EXPORT_TYPE_CHECKSTYLE
						+ " (export as Checkstyle test results), =" + EXPORT_TYPE_COMPARE
						+ " (export as compare results) or any combination; export all is default if omitted\n", //
				"Example:\n", "java -D" + ARG_KEY_JAVAFILE + "=c:\\temp\\export_java.xml -D" + ARG_KEY_CSHARPFILE + "=c:\\temp\\export_csharp.xml -D"
	...
	/**
	 * Log out the given message. Convenience method to have a single point where to change the behavior.
	 * 
	 * @param message
	 *            Message to log
	 */
	...
	/**
	 * Export the compare results. If a target path is given a file is written, otherwise only the console is used.
	 * 
	 * @param results
	 *            List with CompareResult entities
	 * @param targetPath
	 *            Target path or null
	 * @param asXML
	 *            Flag if the output should be XML (only if target path is set and a file is written)
	 ...
	 * 
	 * @param results
	 *            Compare results; mandatory (but may be empty)
	 * @param fileName
	 ...
	 * 
	 * @param result
	 *            CompareResult used to fill the node
	 * @param compareError
	 *            Error or null (if types are equal)
	 * @param showStatus
